...
<form id="formList" action="" method="get" target="ifrmProcess">
    ...
    <table class="table table-rows">
        ...
        if (gd_isset($data)) {
            ...
            echo '<tr><td class="center" colspan="12">검색기능을 이용해주세요.</td></tr>';
        } else {
            echo '<tr><td class="center" colspan="12">검색된 정보가 없습니다.</td></tr>';
        }
        ?>
        </tbody>
    </table>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#btnSendPowerMail').click(function (e) {
            ...
            if ($sendTarget.val() == "select") { //회원선택 적용
                ...
                $.ajax({
                    type: "POST",
                    url: "../crm/mail_send_receive_refuse_count.php",
                    data: dataArray,
                    dataType: "json",
        ...
        $('#btnPointCharge', $formList).click(function (e) {
            e.preventDefault();
            power_mail_popup('../crm/mail_gate.php?charge=y');
        });
    ...
    function sendAmail(rejectMailingFl) {
        ...
        power_mail_popup('../crm/mail_gate.php?start=y');
