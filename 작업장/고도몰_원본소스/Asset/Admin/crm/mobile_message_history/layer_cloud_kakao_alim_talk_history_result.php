<section class="ncua-search-result js-cloud-kakao-alim-talk-history-result">
    ...
    <div class="ncua-search-result__content">
        ...
        <div class="ncua-table ncua-table--horizontal">
            <table class="js-result-table">
                ...
                <tbody>
                <?php if (!empty($cloudKakaoAlimTalkHistoryList)): ?>
                    <?php foreach ($cloudKakaoAlimTalkHistoryList as $cloudKakaoAlimTalkHistory): ?>
                        <tr>
                            <td>
                                <div>
                                    <label class="ncua-checkbox-field ncua-checkbox-field--xs has-text">
                                        <span class="ncua-checkbox-input ncua-checkbox-input--xs ncua-checkbox-field__input">
                                            <input type="checkbox" name="sendKeys[]" value="<?= $cloudKakaoAlimTalkHistory->getSendKey() ?>" />
                                        </span>
                                    </label>
                                </div>
                            </td>
                            <td>
                                <div><?= empty($cloudKakaoAlimTalkHistory->getUsedPoint()) ? '-' : $cloudKakaoAlimTalkHistory->getUsedPoint() ?></div>
                            </td>
                            <td>
                                <div class="ncua-flex-column">
                                    <?= $cloudKakaoAlimTalkHistory->getTriggerType()->value ?>
                                    <?php if ($cloudKakaoAlimTalkHistory->getTriggerType()->name === 'REPEATED'): ?>
                                        <span class="ncua-count"><?= $cloudKakaoAlimTalkHistory->getRepeatRound() ?: 1 ?>회차</span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td>
                                <div class="ncua-align-items-start"><button class="ncua-left-align mobile-history-list-message-content-btn js-show-message-contents" data-send-key="<?= $cloudKakaoAlimTalkHistory->getSendKey() ?>" type="button"><?= $cloudKakaoAlimTalkHistory->getContent() ?></button></div>
                            </td>
                            <td>
                                <div><?= $cloudKakaoAlimTalkHistory->getReservedAt() ?></div>
                            </td>
                            <td>
                                <div class="ncua-flex-column"><?= "{$cloudKakaoAlimTalkHistory->getTotalCount()} <span>({$cloudKakaoAlimTalkHistory->getSuccessCount()}/{$cloudKakaoAlimTalkHistory->getFailCount()})</span>" ?></div>
                            </td>
                            <td>
                                <div class="ncua-flex-column">
                                    <span><?= $cloudKakaoAlimTalkHistory->getSendStatus()->value ?></span>
                                    <button type="button" class="ncua-btn ncua-btn--xxs ncua-btn--secondary-gray js-show-history-detail" data-send-key="<?= $cloudKakaoAlimTalkHistory->getSendKey() ?>">상세</button>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<script type="text/javascript">
    ...
    function initializeEvents() {
        const container = document.querySelector('.js-cloud-kakao-alim-talk-history-result');
    ...
    function loadContentsByPage(page, sort = null, pageSize = null) {
        ...
        formData.append(key, value);
        formData.append('sender', 'kakaoAlrimCloud')
