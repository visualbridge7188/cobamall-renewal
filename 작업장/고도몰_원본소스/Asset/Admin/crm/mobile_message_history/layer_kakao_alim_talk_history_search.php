...
<script>
    ...
    function initializeEvents() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            const sender = form.querySelector('input[name="sender"]:checked')?.value;
            if (sender !== 'kakaoAlrim' && !formData.has('triggerTypes[]')) {
                NCDSAlert({message: '검색 조건을 선택해 주세요.', iconType: 'error'});
                return;
            }

            if (!formData.has('sendStatuses[]')) {
                NCDSAlert({message: '검색 조건을 선택해 주세요.', iconType: 'error'});
                return;
            }
        ...
        form.querySelector('button[type="reset"]')?.addEventListener('click', (e) => {
            setTimeout(() => {
                const isBizm = <?= json_encode($isSelectedBizm) ?>;
                form.querySelector('#triggerTypeAll').disabled = isBizm;
                form.querySelectorAll('input[name="triggerTypes[]"]').forEach(checkbox => {
                    checkbox.disabled = isBizm;
                    if (!isBizm) checkbox.checked = true;
                });
    ...
    function loadContents(formData, isSearch = false) {
        ...
        const urlMap = {
            kakaoAlrim: './mobile_message_history/layer_bizm_kakao_alim_talk_history_result.php',
            default: './mobile_message_history/layer_cloud_kakao_alim_talk_history_result.php',
        };
