                        ...
                        <tr>
                            <td>
                                <div>
                                    <label class="ncua-checkbox-field ncua-checkbox-field--xs has-text">
                                        <span class="ncua-checkbox-input ncua-checkbox-input--xs ncua-checkbox-field__input">
                                            <input type="checkbox" name="sendKeys[]" value="<?= $kakaoFriendTalkHistory->getSendGroupKey() ?>" />
                                        </span>
                                    </label>
                                </div>
                            </td>
                            <td>
                                <div><?= empty($kakaoFriendTalkHistory->getUsedPoint()) ? '-' : $kakaoFriendTalkHistory->getUsedPoint() ?></div>
                            </td>
                            <td>
                                <div><?= $kakaoFriendTalkHistory->getScheduleType()->value ?></div>
                                <?php if ($kakaoFriendTalkHistory->getScheduleType()->name === 'REPEAT'): ?>
                                    <span class="ncua-count"><?= $kakaoFriendTalkHistory->getIteration() ?: 1 ?>회차</span>
                                <?php endif; ?>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="ncua-align-items-start"><button class="ncua-left-align mobile-history-list-message-content-btn js-show-message-contents" data-send-history-no="<?= $kakaoFriendTalkHistory->getNo() ?>" type="button"><?= $kakaoFriendTalkHistory->getSnapshot() ?></button></div>
                            </td>
                            <td>
                                <div><?= $kakaoFriendTalkHistory->getSendDateTime() ?></div>
                            </td>
                            <td>
                                <div class="ncua-flex-column"><?= "{$kakaoFriendTalkHistory->getTotalCount()} <span>({$kakaoFriendTalkHistory->getSuccessCount()}/{$kakaoFriendTalkHistory->getFailCount()})</span>" ?></div>
                            </td>
                            <td>
                                <div class="ncua-flex-column">
                                    <span><?= $kakaoFriendTalkHistory->getSendResultStatus()->value ?></span>
                                    <button type="button" class="ncua-btn ncua-btn--xxs ncua-btn--secondary-gray js-show-history-detail" data-send-group-key="<?= $kakaoFriendTalkHistory->getSendGroupKey() ?>">상세</button>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<script type="text/javascript">
    ...
    function initializeEvents() {
        ...
        container.querySelector('button[data-click-target=excelDownload]').addEventListener('click', (e) => {
            const title = `카카오 친구톡 발송내역 다운로드<br/><span style="color: var(--gray-400); font-size: 13px; font-weight: 400;">발송이 완료된 내역만 다운로드됩니다.</span>`;
        ...
        container.addEventListener('click', (e) => {
            ...
            if (detailBtn) {
                showHistoryDetail(detailBtn.dataset.sendGroupKey);
            }
        });
    ...
    function showMessageContents(sendHistoryNo) {
        $.post('./mobile_message_history/layer_kakao_friend_talk_contents.php', {sendHistoryNo: sendHistoryNo}, function (data) {
    ...
    function showHistoryDetail(sendGroupKey) {
        $.post('./mobile_message_history/layer_message_history_detail.php', {sendGroupKey: sendGroupKey, type: 'FRIENDTALK'}, function (data) {
