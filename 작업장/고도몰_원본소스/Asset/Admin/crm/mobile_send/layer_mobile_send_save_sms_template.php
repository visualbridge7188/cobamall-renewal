...
<script type="text/javascript">
    ...
    document.getElementById('btnSaveTemplate').addEventListener('click', function() {
        ...
        const subject = document.querySelector('#templateSubject');
        const contents = document.querySelector('#templateContents');
        ...
        if (!subject.value.trim()) {
            NCDSValidator.highlight(subject);
            validate = false;
        } else {
            NCDSValidator.unhighlight(subject);
        }

        if (!validate) return;

        $.post('./sms/sms_template_ps.php', {
            mode: 'save',
