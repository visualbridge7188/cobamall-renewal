...
var gd_goods_view = function() {
    ...
    this.input_count_change = function(inputName, goodsFl) {
        ...
        let regularDeliveryGoodsFl = false;
        if (isRegularGoodsFl === 'y' && typeof getDeliveryType === 'function' && getDeliveryType() === 'regular') {
            regularDeliveryGoodsFl = true;
        }

        // 정기배송 상품일 경우 유효성 체크
        if (regularDeliveryGoodsFl && !validateRegularGoodsOptionsByOptionKey(optionDisplay)) {
            $(inputName).val('1');
            return false;
        }

        // 일반상품일 경우, 쿠폰 적용 여부 확인
        if (regularDeliveryGoodsFl === false && $("#option_display_item_" + optionDisplay + " input[name='couponApplyNo[]']").val()) {
    ...
    this.count_change = function(inputName, goodsFl) {
        ...
        // 정기배송 상품일 경우 유효성 체크
        if (regularDeliveryGoodsFl && !validateRegularGoodsOptionsByOptionKey(optionDisplay[0])) {
            return false;
        }

        // 일반상품일 경우, 쿠폰 적용 여부 확인
        if ($(regularDeliveryGoodsFl === false && "#option_display_item_" + optionDisplay[0] + " input[name='couponApplyNo[]']").val()) {
            alert(__('쿠폰이 적용된 상품은 수량변경을 할 수 없습니다.\n쿠폰취소 후 수량을 변경해주세요.'));
            return false;
        }
    ...
    this.option_price_display = function(inputName) {
        // 정기배송 상품일 경우 유효성 체크
        if (isRegularGoodsFl === 'y' &&
            typeof getDeliveryType === 'function' &&
            getDeliveryType() === 'regular' &&
            $('.item_regular_goods_box').is(':visible') &&
            !validateRegularGoodsAllOptions())
        {
            $('.item_add_option_box select').val('').trigger('chosen:updated');
            return false;
        }
        ...
        let optionPrice = arrTmp[1].trim() ?? '';
        let optionSalePrice = 0;
        let originOptionPrice = arrTmp[1].trim();

        // 마킹구분자가 포함되어 있을 경우
        if (optionPrice.includes(setMarkDivision)) {
            [optionSalePrice, originOptionPrice] = optionPrice.split(setMarkDivision);
            const isRegular = typeof getDeliveryType === 'function' && getDeliveryType() === 'regular';
            // 정기결제가일 경우, 옵션 원가를 베이스로 계산
            // 일반 상품일 경우, 옵션가(판매가)를 베이스로 계산
            optionPrice = isRegular ? originOptionPrice : optionSalePrice;
        }
        var optionStock = parseFloat(arrTmp[3].trim());
        var displayOptionkey = arrTmp[0]+'_'+$.now();

        // 정기배송 상품이며 할인율이 있는 경우 옵션에도 반영
        if (isRegularGoodsFl === 'y' && (typeof getDeliveryType === 'function' && getDeliveryType() === 'regular' || isForceCheckRegularGoodsDiscount()) && getRegularGoodsDiscountRate() > 0) {
            var regularGoodsDiscountRate = getRegularGoodsDiscountRate();
            optionPrice = optionPrice - (optionPrice * regularGoodsDiscountRate / 100);
        }
        ...
        if ($(frmId + " tr.optionKey"+setOptNo+"_" + arrTmp[0]).length && (setOptNo == "" ||  setOptNo == undefined) && setOptionTextFl !='y') {
            ...
            addHtml += complied({
                ...
                originOptionPrice:originOptionPrice,
            ...
            if (isRegularGoodsFl === 'y' && typeof getDeliveryType === 'function') {
                if (getDeliveryType() === 'normal') {
                    $(frmId + " div.end_price").show();
                } else {
                    setRegularGoodsOptions(displayOptionkey);
                    $(frmId + " div.regular_goods_total_price_box").show();
                }
            } else if (isRegularGoodsFl === 'n') {
                $(frmId + " div.end_price").show();
            }
        }
    };
    ...
    this.option_select = function(inputName, thisKey, nextVal, stockViewFl) {
        ...
        $.post('../goods/goods_ps.php', {'mode' : 'option_select', 'optionVal' : optionVal, 'optionKey' : thisKey, 'goodsNo' : setGoodsNo, 'mileageFl' : setMileageFl}, function(data) {
            if (typeof data.optionSno == 'string') {
                ...
                // 옵션원가에 대한 값이 존재할 경우, 구분자를 마크 구분자로 하여 할인된 옵션가와 옵션원가 모두 할당
                optionInfo[1] = data.originOptionPrice !== undefined ? data.optionPrice[0] + setMarkDivision + data.originOptionPrice[0] : data.optionPrice[0];
                            ...
                            if (data.optionViewFl[j] == 'y') {
                                ...
                                if ((parseInt(data.nextKey) + 1) == parseInt(optionCnt)) {
                                    ...
                                    if (parseInt(data.optionPrice[j]) != '0') {
                                        ...
                                        var setOptionPrice = data.optionPrice[j];

                                        const deliveryOptionType = document.querySelector('input[name="deliveryOptionType"]');
                                        // 정기결제(배송) 상품일 경우
                                        if (
                                            (deliveryOptionType != null && deliveryOptionType.value === 'regular' && isForceCheckRegularGoodsDiscount()) ||
                                            (isRegularGoodsFl === 'y' && (typeof getDeliveryType === 'function' && getDeliveryType() === 'regular' || isForceCheckRegularGoodsDiscount()))
                                        ) {
                                            // 옵션 원가가 존재할 경우 옵션원가를 기준으로 계산
                                            if (data.originOptionPrice !== undefined) {
                                                setOptionPrice = data.originOptionPrice[j];
                                            }

                                            // 정기결제(배송) 상품의 할인율이 존재할 경우, 해당 할인율에 맞추어 옵션가 계산
                                            if (getRegularGoodsDiscountRate() > 0) {
                                                var regularGoodsDiscountRate = getRegularGoodsDiscountRate();
                                                setOptionPrice = setOptionPrice - (setOptionPrice * regularGoodsDiscountRate / 100);
                                            }
                                        }

                                        if (optionPriceFl == 'y' && optionPriceDiffFl == 'y') {
                                            addSelectOption	+= ' : ' + addPlus + gd_money_format(setOptionPrice.toString()) + '';
                                        } else if (optionPriceFl == 'y' && optionPriceDiffFl == 'n' && defaultGoodsPrice != parseFloat(setOptionPrice)) {
                                            addSelectOption	+= ' (' + addPlus + gd_money_format(setOptionPrice.toString()) + ')';
                                        }
                                    }
    ...
    this.option_text_select = function(inputName) {
        // 정기배송 문구 옵션 상품 사전 검증
        if ($(inputName).length === 1 && setOptionFl == 'n' && isRegularGoodsFl === 'y' && typeof getDeliveryType === 'function' && getDeliveryType() === 'regular' && typeof validateRegularGoodsOptionsByOptionKey === 'function') {
            if (!validateRegularGoodsOptionsByOptionKey('0')) {
                $(inputName).val('');
                return false;
            }
        }
        ...
        let originOptionTextPrice = "";
        let originOptionTextTotalPrice = "";
        ...
        if (displayOptionDisplay) {
            ...
            if($(frmId+' .option_text_display_' + displayOptionkey).html() !== '' && setTemplate != 'Layer') {
                ...
                if (optionTextInputCnt <= emCnt) {
                    // 정기배송 상품일 경우 유효성 체크
                    if (isRegularGoodsFl === 'y' && typeof getDeliveryType === 'function' && getDeliveryType() === 'regular') {
                        if (!validateRegularGoodsAllOptions(true)) {
                            $(inputName).val('');
                            return false;
                        }
                    }
                    ...
                    let originOptionPrice = $lastDiv.find('input[name*=\'origin_option_price_\']').last().val();
                ...
                // 정기배송 상품일 경우 유효성 체크
                if (isRegularGoodsFl === 'y' && typeof getDeliveryType === 'function' && getDeliveryType() === 'regular' && displayOptionkey === '0') {
                    if (!validateRegularGoodsOptionsByOptionKey('0')) {
                        $(inputName).val('');
                        return false;
                    }
            ...
            $(frmId + ' input[name*=\'optionTextInput'+setOptNo+'\']').each(function (key) {
                if ($(this).val()) {
                    ...
                    originOptionTextPrice = parseFloat($(this).next('input').data('origin-add-price'));
                ...
                originOptionTextTotalPrice += originOptionTextPrice;
            ...
            if ($(frmId + ' tr.optionKey'+setOptNo+'_' + tmpStr[0] + optionTextKey).length) {
                ...
                optionText = optionTextSno + '<input type="hidden" value="' + optionTextTotalPrice + '" name="optionTextPriceSum'+setOptNo+'[]" /><input type="hidden" value="' + originOptionTextTotalPrice + '" name="originOptionTextPriceSum'+setOptNo+'[]" /><input type="hidden" value="' + optionTextTotalPrice + '" name="option_text_price_' + displayOptionkey + '" /><input type="hidden" value="' + originOptionTextTotalPrice + '" name="origin_option_text_price_' + displayOptionkey + '" /></div>';
                ...
                if (isRegularGoodsFl === 'y' && typeof getDeliveryType === 'function') {
                    const optionDisplayItem = $('#option_display_item_' + displayOptionkey);
                    if (optionDisplayItem.find('input[name="deliveryCycles[]"]').val() === '' && optionDisplayItem.find('input[name="deliveryCycleDays[]"]').val() === '') {
                        setRegularGoodsOptions(displayOptionkey);
                    }
                }

                // 문구 옵션을 모두 작성한 상태일 경우 모두 초기화 처리
                var textInputList = $(frmId + ' input[name*=\'optionTextInput' + setOptNo + '\']');
            ...
            // 정기배송 상품이고 배송주기 및 종료회차가 선택이 안 된 경우
            if (
                !['mypage', 'cart', 'regular_cart'].includes(type) &&
                isRegularGoodsFl === 'y' &&
                typeof getDeliveryType === 'function' &&
                getDeliveryType() === 'regular' &&
                !validateRegularGoodsAllOptions()
    ...
    this.goods_calculate = function(frmId, goodsFl, itemNo, goodsCnt) {
        ...
        // 정기배송 상품일 경우 정기배송가로 설정
        if (isRegularGoodsFl === 'y' && typeof getDeliveryType === 'function' && getDeliveryType() === 'regular') {
            goodsPrice = parseFloat($('.item_regular_goods_box input[name="regularPrice"]').val()) || 0;
        }
        ...
        if (goodsFl) {
            ...
            let originOptionPrice = parseFloat($(frmId + ' input[name="origin_option_price'+setOptNo+'_' + itemNo + '"]').val());
            let originOptionTextPrice = parseFloat($(frmId + ' input[name="origin_option_text_price'+setOptNo+'_' + itemNo + '"]').val());
            ...
            if(setOptNo == "" || setOptNo == undefined) {
                ...
                $('#option_display_item_' + itemNo + ' input[name="originOptionPriceSum[]"]').val((originOptionPrice * goodsCnt).toFixed(setDecimal));
                $('#option_display_item_' + itemNo + ' input[name="optionTextPriceSum[]"]').val((optionTextPrice * goodsCnt).toFixed(setDecimal));
                $('#option_display_item_' + itemNo + ' input[name="originOptionTextPriceSum[]"]').val((originOptionTextPrice * goodsCnt).toFixed(setDecimal));
                ...
                $('#option_display_item_' + setOptNo + '_' + itemNo + ' input[name="originOptionPriceSum' + setOptNo + '[]"]').val((originOptionPrice * goodsCnt).toFixed(setDecimal));
                ...
                $('#option_display_item_' + setOptNo + '_' + itemNo + ' input[name="originOptionTextPriceSum' + setOptNo + '[]"]').val((originOptionTextPrice * goodsCnt).toFixed(setDecimal));
    ...
    this.init = function(param) {
        ...
        setMarkDivision = param.setMarkDivision;
        ...
        isRegularGoodsFl = param.isRegularGoodsFl;
        type = param.type;
