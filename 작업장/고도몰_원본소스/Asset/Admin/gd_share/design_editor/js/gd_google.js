...
$(document).ready(function () {
    ...
    if ($googleLoginBtn.length > 0) {
        $googleLoginBtn.click(function () {
            if (navigator.userAgent.includes("MYAPP_WEBVIEW_APP")) {
                var baseUrl = $googleLoginBtn.data('return-url');
                var saveAutoLogin = $('input[name="saveAutoLogin"]').is(':checked') ? 'y' : 'n';
                var separator = baseUrl.indexOf('?') !== -1 ? '&' : '?';
                window.location.href = baseUrl + separator + 'saveAutoLogin=' + saveAutoLogin;
                return;
            }
