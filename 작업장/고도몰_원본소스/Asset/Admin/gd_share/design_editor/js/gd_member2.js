...
var gd_member2 = (function ($, _, window, document, undefined) {
    ...
    function validation_submit(func) {
        ...
        // 이미 등록된 사업자등록증이 있을 경우 필수항목 확인 패스
        var isBusiness = ($('input[name="memberFl"]').val() == 'business');
        if(is_my_page && isBusiness) {
            ...
            // 추가 첨부 항목도 row 별로 동일 우회 적용 (이미 등록된 파일이 있고 삭제하지 않으면 required 패스)
        ...
        if (validate.form()) {
            ...
            if (altchaWidget && typeof altchaWidget.getState === 'function' && altchaWidget.getState() !== 'verified') {
