...
var rFilter2 = /^(bmp|gif|jpg|jpeg|png|tiff)$/i;
var nTotalSize = 0;
    ...
    if (typeof ev.dataTransfer.files == 'undefined') {
        ...
        for (var i = 0, j = nImageFileCount; i < nCount; i++) {
            if (!rFilter.test(files[i].type)) {
                alert("이미지파일 (jpg,jpeg,gif,png,bmp,tiff)만 업로드 가능합니다.");
...
function html5Upload() {
    ...
    //sUploadURL= 'file_uploader_html5.php?editorUrl='+editorUrl; 	//upload URL
    sUploadURL = '/share/editor_file_uploader_html5_ps.php?editorUrl=' + editorUrl; 	//upload URL
    //파일을 하나씩 보내고, 결과를 받음.
...
function callFileUploader() {
    oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"), {
        ...
        sFiletype: "*.jpg;*.png;*.bmp;*.gif;*.jpeg;*.tiff",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)
        sMsgNotAllowedExt: 'JPG, GIF, PNG, BMP, TIFF 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
