...
$(function () {
	...
	catePanel.jsTree.init($('#categoryTree'),{
		...
		callback : {
            ...
            // 카테고리 생성 전 처리
            beforecreate	: function(NODE,TREE_OBJ) {
                if (catePanel.jsTree.lock_write_check() !== true) return false;
                return true;
            },
			// 카테고리 생성시
            ...
            // 카테고리 이름 변경 전 처리
            beforerename	: function(NODE,TREE_OBJ) {
                if (catePanel.jsTree.lock_write_check() !== true) return false;
                return true;
            },
			// 카테고리 이름 변경시
    ...
    // 쓰기권한 체크
    catePanel.jsTree.lock_write_check = function () {
    	if ($('#categoryTree').data('lock-write') == '1') {
    		alert(cateText+' 관리의 쓰기 권한이 없습니다. 권한은 대표운영자에게 문의하시기 바랍니다.');
    		return false;
		}
        return true;
    }
