 ...
 * NCDS 레이어 다이얼로그
 */
function ncds_layer_popup({message, title, size, dataSortID, callback, onshown, backdrop = true, closeByBackdrop = true}) {
    BootstrapDialog.show({
        title: title,
        size: get_layer_size(size),
        message: $(message),
...
function layer_add_info(fileCd, addParam) {
    ...
    switch (fileCd) {
        ...
        case 'naver_stats':
            ...
            break;
        case 'daum_stats':
            title = "카카오 쇼핑하우 노출상품 현황";
            addParam['size'] = "wide";
            break;
        case 'fbe_stats':
            title = "메타 FBE 노출상품 현황";
            addParam['size'] = "wide";
            break;
        case 'google_stats':
            title = "구글 쇼핑 노출상품 현황";
    ...
    const hasNcdsClass = document.body.classList.contains('ncds');
    const basePath = hasNcdsClass ? '/ncds' : '';
    const path = `/share${basePath}/layer_${fileCd}.php`;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // NCDS 확인 및 ncds-modal 클래스 추가
            if (hasNcdsClass) {
                configure.cssClass = 'ncds-modal';
            }
 ...
 * @param keyword
 */
function layer_member_search(keyword, key, searchKind) {
    var loadChk = $('div#layerSearchMember').length;
    if(_.isUndefined(key)) {
        key = 'memId';
    }
    var searchKindValue = searchKind;
    if ($('#formMemberSearch select[id="searchKind"]').prop('disabled') == true) {
        searchKindValue = 'fullLikeSearch';
    }
    $.get('../share/layer_member_search.php', {keyword: keyword, key: key, searchKind: searchKindValue}, function (data) {
        if (loadChk === 0) {
...
function layer_sms_send_list(sno) {
    ...
    $.ajax({
        url: '../share/layer_sms_send_list.php',
        type: 'get',
        data: addParam,
        async: false,
        success: function (data) {
...
function layer_kakao_send_list(sno, sendKey, status) {
    ...
    console.log(addParam);
    $.ajax({
        url: '../share/layer_kakao_send_list.php',
        type: 'get',
        data: addParam,
        async: false,
        success: function (data) {
...
/**
 * 개발소스관리 팝업 (레거시 - 하위호환 유지)
 *
 * @author Jong-tae Ahn <qnibus@godo.co.kr>
 * @deprecated dev_source_manager() 사용 권장 (새 탭 방식)
 */
function development_manager(page) {
    if (page === undefined || page === '') {
        page = 'index.php';
    }
    window.open('/development/' + page, 'development_manager', 'width=1100,height=800,scrollbars=yes');
}

/**
 * 개발소스관리 새 탭 열기 (팝업 -> 새 탭 전환)
 *
 * @param {string} page 페이지 파일명 (기본: index.php)
 */
function dev_source_manager(page) {
    if (page === undefined || page === '') {
        page = 'customizingInfo.php';
    }
    window.open('/development/' + page, '_blank');
}
...
function cache_manager() {
    window.open('/cache/mod_cache.php', 'cache_manager', 'width=1100,height=800,scrollbars=yes');
}
...
function member_mail(memNo) {
    window.open('../crm/mail_send.php?popupMode=yes&memNo=' + memNo, 'mail_form', 'width=1024,height=768,scrollbars=yes');
}
 ...
 */
function member_sms(memNo, memNm, cellPhone, smsFl) {
    ...
    win = popup({
        url: '../member/sms_send.php?receiverMemNo=' + memNo + '&receiverNm=' + memNm + '&receiverPhone=' + cellPhone + '&smsFl=' + smsFl,
        target: 'smsSendPopup',
        width: 1200,
        height: 600,
        scrollbars: 'yes',
...
function sms_auto_popup() {
    ...
    if ($js_link_sms_auto.length > 0) {
        $js_link_sms_auto.click(function (e) {
            ...
            window.open('../crm/auto_send.php?popupMode=yes' + hash, 'sms_auto', 'width=1400, height=700, scrollbars=no');
        });
    }
}
...
function mail_config_auto_popup() {
    ...
    if ($js_link_mail_auto.length > 0) {
        $js_link_mail_auto.click(function (e) {
            ...
            window.open('../crm/mail_config_auto.php?popupMode=yes' + hash, 'mail_auto', 'width=1400, height=700, scrollbars=no');
        });
    }
}
...
function regular_order_view_popup(applyNo, isProvider) {
    var path = get_script_dirpath('script/common.js'),
        url = '/order/regular_order_view.php?popupMode=yes&applyNo=' + applyNo;
    if (isProvider) {
        url = '/provider/order/regular_order_view.php?popupMode=yes&applyNo=' + applyNo;
    }
    win = popup({
        url: url,
        target: 'order_view',
 ...
 * @param formData ajax 호출 시 serialize 가 아닌 formData 객체를 사용하고자 할 때 사용
 */
function post_with_reload(url, data, reloadUrl, formData) {
...
$(function () {
    ...
    // 저장버튼 스크롤 — page-header affix 처리
    // .js-ncds-content-wrap 유무로 분기:
    //  - 있음 → NCDS 레이아웃: 본문 래퍼에 동적 paddingTop 보정
    //  - 없음 → 레거시 레이아웃(예: layout_crm): #content에 paddingTop 102px 보정
    const headerElement = document.querySelector('.page-header.js-affix');
    const fullWidthNotificationElement = document.querySelector('.ncua-full-width-notification');

    if (headerElement) {
        if (document.querySelector('.js-ncds-content-wrap')) {
            initAffixForNcds();
        } else {
            initAffixForLegacy();
        }
    ...
    function initAffixForNcds() {
        const OFFSET_TOP = 125;
        const contentWrapElement = document.querySelector('#content');

        const getHeaderWidth = () => {
            const contentElement = document.querySelector('.js-ncds-content-wrap');

            if (!contentElement) {
                return;
            }

            return contentElement.clientWidth;
        };

        const setAffixContentSpaceTop = (isAffixed = false) => {
            const innerContentElement = document.querySelector('.js-ncds-content-wrap');
            const getPaddingTop = () => {
                const headerSize = headerElement.clientHeight + parseInt(getComputedStyle(headerElement).marginBlockEnd, 10);

                return headerSize;
            };

            const setContentPaddingTop = (element, paddingTop) => {
                if (!element) return;
                isAffixed ? element.style.paddingTop = `${paddingTop}px` : element.removeAttribute('style');
            };

            setContentPaddingTop(innerContentElement, getPaddingTop());
        }


        headerElement.style.width = `${getHeaderWidth()}px`;

        $('.js-affix').affix({
            offset: {
                top: ($('#header').length > 0 ? OFFSET_TOP : 10),
                bottom: function () {
        ...
        $('.js-affix').bind('affix.bs.affix', function (e) {
            ...
            $('.page-header.js-affix').css({width: `${getHeaderWidth()}px`});

            setAffixContentSpaceTop(true);
        }).bind('affixed-top.bs.affix', function (e) {
            $('.page-header.js-affix').css({width: `${getHeaderWidth()}px`, left: 'auto'});
            setAffixContentSpaceTop();
        });
        ...
        const resizeObserver = new ResizeObserver((entries) => {
            requestAnimationFrame(() => {
                headerElement.style.width = `${getHeaderWidth()}px`;
                fullWidthNotificationElement && (fullWidthNotificationElement.style.width = `${getHeaderWidth()}px`);
            });
        });
        resizeObserver.observe(contentWrapElement);
    }

    function initAffixForLegacy() {
        // 레거시 레이아웃(예: layout_crm) 옛 affix 로직.
        // affixed-top 시 paddingTop만 비워서 #content의 인라인 style(min-width 등) 보존.
        let $content = $('#content .col-xs-12');
        if (!$content.length) {
    ...
    // 가로 스크롤 이벤트 처리 (throttle 적용)
    const handleHorizontalScroll = throttle(function() {
        if ($('.js-affix').hasClass('affix')) {
            const scrollLeft = $(window).scrollLeft();
            const baseLeft = $('body').hasClass('menu-no-border') ? 0 : 211;
            $('.page-header.js-affix').css({
                left: `${baseLeft - scrollLeft}px`
            });
        }
    }, 16); // 약 60fps
    ...
    $(document).on('click', '.js-layer-register', function (e) {
        ...
        params.notEmpty = (!_.isUndefined($(this).data('not-empty')) ? $(this).data('not-empty') : undefined);
    ...
    if ($js_link_sms_charge.length > 0) {
        $js_link_sms_charge.click(function (e) {
            window.open('../crm/popup_charge_message_points.php', 'sms_charge', 'width=1400, height=700, scrollbars=no');
        });
    }
...
/* 검색설정값 저장*/
function set_search_config(form, isOrderSearchMultiGrid) {
    var message = '현재 설정된 검색조건을 저장하시겠습니까? 저장 후 기본 검색설정으로 사용됩니다.<br> (직접 입력한 검색어와 조회기간은 저장되지 않습니다.)<br><br>검색결과의 <strong>"정렬조건"</strong>과 <strong>"페이지당 출력수 설정"</strong>(20개 보기 권장)도 같이 저장되며,<br><span style="color:red;">출력수가 많을 경우 리스트 접근 시, 부하문제로 페이지로딩이 오래 걸릴 수 있습니다.</span>';
    if(isOrderSearchMultiGrid == 'y') {
        message = '현재 설정된 검색조건을 저장하시겠습니까? 저장 후 기본 검색설정으로 사용됩니다.<br> 기간검색의 <strong>“기간검색 기준“</strong>과 <strong>“날짜범위“</strong>는 저장되나, 직접 입력한 검색어와 조회기간은 저장되지 않습니다.<br><br>검색결과의 <strong>"정렬조건"</strong>과 <strong>"페이지당 출력수 설정"</strong>(20개 보기 권장)도 같이 저장되며,<br><span style="color:red;">출력수가 많을 경우 리스트 접근 시, 부하문제로 페이지로딩이 오래 걸릴 수 있습니다.</span>';
    }
...
function set_is_order_search_multi_grid(isOrderSearchMultiGrid) {
    var typeMsg = isOrderSearchMultiGrid == 'y' ? '다중' : '단일';
    dialog_confirm('<span class="text-red">'+typeMsg+'</span> 검색조건으로 전환하시겠습니까?', function (result) {
        if (!result) {
            return false;
        }
        else {
            $.ajax({
                url: '/share/search_config_ps.php',
                type: 'GET',
                data: {'mode': 'changeSearchGrid', 'isOrderSearchMultiGrid': isOrderSearchMultiGrid},
                async: false,
...
function dateGetTime(date){
    ...
    var tempdt = new Date(tempDt1[0],tempDt1[1]-1,tempDt1[2],tempDt2[0],tempDt2[1]);
    ...
    var security_agreement = {
        show: function (options) {
            ...
            dialog.show_guide_end_dialog = function () {
                ...
                message += "<span class='mgt5 text-blue'>* 관리자 운영 보안 설정은 필요에 따라,<br/>[설정 > 관리 정책 > 운영 보안 설정]에서 사용함 설정이 가능합니다.</span></div>";
                var end_dialog = new BootstrapDialog({
                    message: message,
                    buttons: [{
                        label: "취소",
...
/**
 * ISMS 인증심사에 따른 검색어 조건 추가 > placeholder 문구
 * @param searchKey object
 * @param searchKind object
 * @param strSearchKey string
 * @param arrSearchKey array
 */
function setKeywordPlaceholder(searchKeyword, searchKind, strSearchKey, arrSearchKey) {
    var equalSearchPlaceHolder = '검색어 전체를 정확히 입력하세요.';
    var fullLikeSearchPlaceHolder = '검색어에 포함된 내용을 입력하세요.';
    var changedPlaceHolder = '';

    var searchKeyYn = 'Y';
    if (typeof strSearchKey != 'undefined' && typeof arrSearchKey != 'undefined') {
        if (arrSearchKey.indexOf(strSearchKey) === -1) {
            searchKeyYn = 'N';
        }
    }

    if (searchKind.val() == 'equalSearch' && searchKeyYn == 'Y') {
        changedPlaceHolder = equalSearchPlaceHolder;
    } else {
        changedPlaceHolder = fullLikeSearchPlaceHolder;
    }
    searchKeyword.attr("placeholder", changedPlaceHolder);

    if (searchKeyYn == 'Y') {
        searchKind.show();
        searchKind.prop("disabled", false);
    } else {
        searchKind.hide();
        searchKind.prop("disabled", true);
    }
}

/**
 * 고도몰 basic 요금제 변경 이동 처리
 * @param url string
 * @param isSuper string
 */
function upgrade_plan_godomall_basic(url, isSuper) {
    var message = "고도몰 pro부터 사용할 수 있는 기능입니다.<br />고도몰 pro로 요금제를 변경하시겠습니까?";
    dialog_confirm(message, function (result) {
        if (!result) {
            return false;
        } else {
            if (isSuper === 'n') {
                alert('상점 최고운영자만 요금제를 변경할 수 있습니다.');
            } else {
                window.open(url, 'upgrade_plan_godomall_basic', 'width=1100, height=850, scrollbars=no');
            }
        }
    });
}
...
const POPUP_HEIGHT = 725;
let layer;
let dimmedBackground;

/**
* 고도몰 운영 도우미 팝업창 노출
* @param {string} url
* @param {number} [width=POPUP_WIDTH]
* @param {number} [height=POPUP_HEIGHT]
* @returns {Window|null}
*/
function tutorial_layer(url, width = POPUP_WIDTH, height = POPUP_HEIGHT) {
    document.body.style.overflow = 'hidden';

    dimmedBackground = document.createElement('div');
    ...
    dimmedBackground.style.zIndex = '999';
    ...
    layer.style.boxShadow = '0 0 10px rgba(0, 0, 0, 0.5)';
...
function closePopup(event) {
    if (event.data === 'TUTORIAL_MODAL_CLOSE' && layer) {
        layer.style.display = 'none';
        dimmedBackground.style.display = 'none';
...
document.addEventListener('DOMContentLoaded', function () {
    const ncdsModal = document.querySelector('.ncua-modal-backdrop');
    const aggregatorIframe = document.querySelector('iframe[name="aggregator"]');

    if (!ncdsModal && !aggregatorIframe) return;

    function openAggregatorModal(payload) {
        if (!ncdsModal) return;
        const ncdsModalIframe = ncdsModal.querySelector('iframe');
        if (ncdsModalIframe) ncdsModalIframe.src = payload.iframeUrl;
        ncdsModal.style.display = '';
        document.body.style.overflow = 'hidden';
    }

    function setAggregatorIframeHeight(payload, category) {
        const iframeName = aggregatorHandlers[category]?.iframeName;
        if (!iframeName) {
            console.warn('Invalid category or missing iframeName:', category);
            return;
        }
        const frame = document.querySelector(`iframe[name="${iframeName}"]`);
        if (frame) {
    ...
    // 클릭 이벤트 - 배경 또는 닫기 버튼
    if (ncdsModal) {
        ncdsModal.addEventListener('click', function (e) {
            const clickedCloseButton = e.target.closest('.ncua-button-close-x');
            if (clickedCloseButton) {
                ncdsModal.style.display = 'none';
                document.body.style.overflow = '';
            }
        });
    }
    ...
    const aggregatorHandlers = {
        MARKETING_BOOSTER: {
            iframeName: 'marketingBooster',
            MODAL_OPEN: openAggregatorModal,
            IFRAME_HEIGHT : setAggregatorIframeHeight,
        },
        ANALYTICS: {
            iframeName: 'aggregator',
            IFRAME_HEIGHT : setAggregatorIframeHeight,
        },
    };

    // 에그리게이터 전용 메시지 핸들러
    function handleAggregatorMessage(event) {
        // 신뢰할 수 있는 origin인지 확인
        const trustedOrigins = [
            'https://aggregator.nhn-commerce.com',
            'https://apps-pc.nhn-commerce.com',
            'https://alpha-apps-pc.nhn-commerce.com'
        ];
        ...
        const categoryHandler = aggregatorHandlers[category];
        const handler = categoryHandler?.[type];

        if (typeof handler === 'function') {
            handler(payload, category);
        } else {
            console.warn('정의되지 않은 메시지:', category, type);
        }
    }

    window.addEventListener('message', handleAggregatorMessage);
});

function handleGodoImageUpload(images, editor) {
    if (!images.length) {
        return;
    }
    ...
    formData.append('editor', 'froala');
    formData.append('uploadType', 'admin');
    formData.append('editorUrl', location.origin + location.pathname);
    formData.append('Filedata', imageFile, imageFile.name);

    fetch('/share/editor_file_uploader_ps.php', {
        method: 'POST',
        body: formData,
    }).then(async response => {
        const json = await response.json().catch(() => ({}));
        if (!response.ok || (json && json.error)) {
            showImageUploadError(json.error);
            return;
        }
        if (!json.location || typeof json.location !== 'string' || !json.location.trim()) {
            showImageUploadError();
            return;
        }
        editor.image.insert(json.location.trim(), null, null, editor.image.get());
    }).catch(() => {
        showImageUploadError();
    });
}
...
/**
 * 관리자 클레임 접수 시 고객 클레임 신청 건과의 종류 일치 여부를 확인
 *
 * 체크된 상품주문번호들 중 고객 클레임 신청(미처리, 승인/거절 전) 건이 있고
 * 그 종류가 관리자 접수 종류와 다른 경우(자동 거절 케이스) 컨펌 모달 노출.
 * 모두 같은 종류이거나 신청 건이 없으면 즉시 진행(Case#1).
 *
 * @param {string} viewMode 관리자 접수 모드 ('back' / 'refund' / 'exchange')
 * @param {function} onConfirm 진행 시 실행할 콜백
 */
function confirmCustomerClaimReceipt(viewMode, onConfirm) {
    // 관리자 접수 모드 → userHandleMode 코드 매핑
    const adminClaimMode = {
        'back': 'b',
        'refund': 'r',
        'exchange': 'e'
    }[viewMode];
    ...
    // 클레임 종류 한글 라벨
    const customerClaimLabel = {
        'b': '반품',
        'r': '환불',
        'e': '교환'
    };

    // --- 1. 체크된 상품주문번호 중 고객 클레임 신청(미처리) 건만 추출
    const pendingClaims = []; // [{ mode: 'b'|'r'|'e' }, ...]
    const rowSelector = {
        'back':     '#backForm tr[id^="statusCheck_"]',
        'refund':   '#refundForm tr[id^="statusCheck_"]',
        'exchange': '#formOrderViewExchange ~ table tr[id^="statusCheck_"]'
    }[viewMode];
    $(rowSelector).each(function () {
        ...
        const userHandleMode = $row.data('user-handle-mode');
        const claimStatusName = $row.data('claim-status-name');

        // 미처리 신청만 카운트 (이미 승인/거절된 건은 자동 처리 대상 아님)
        const isPending = (
            (userHandleMode === 'r' && claimStatusName === '환불신청') ||
            (userHandleMode === 'b' && claimStatusName === '반품신청') ||
            (userHandleMode === 'e' && claimStatusName === '교환신청')
        );

        if (isPending) {
            pendingClaims.push({mode: userHandleMode});
        }
    });

    // --- 2. Case#1: 미처리 신청 건이 없으면 즉시 진행
    if (pendingClaims.length === 0) {
        onConfirm();
        return;
    }

    // --- 3. 종류가 다른 신청 건(자동 거절 대상) 분류
    const rejectClaims = pendingClaims.filter(function (c) {
        return c.mode !== adminClaimMode;
    });

    // --- 4. Case#1 (모두 같은 종류 → 자동 승인만 발생): 즉시 진행
    if (rejectClaims.length === 0) {
        onConfirm();
        return;
    }

    // --- 5. Case#2: 자동 거절될 건이 있음 → 컨펌 모달
    // 자동 거절될 종류 집계 (중복 제거, 교환/반품/환불 순으로 정렬)
    const claimOrder = {'e': 1, 'b': 2, 'r': 3};
    const rejectModes = [];
    rejectClaims.forEach(function (c) {
        if (rejectModes.indexOf(c.mode) === -1) {
            rejectModes.push(c.mode);
        }
    });
    rejectModes.sort(function (a, b) {
        return claimOrder[a] - claimOrder[b];
    });

    const rejectLabels = rejectModes.map(function (m) {
        return customerClaimLabel[m];
    }).join(', ');

    const message =
        '고객이 ' + rejectLabels + ' 신청한 상품입니다.<br>' +
        '고객 ' + rejectLabels + ' 신청 내역도 함께 거절 처리됩니다. ' + adminClaimLabel + ' 접수하시겠습니까?';
...
function setClaimSubmitDisabled(form, isDisabled) {
    $(form).find('button[type="submit"]').prop('disabled', isDisabled);
}
