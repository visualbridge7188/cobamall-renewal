		...
		$(this).change( function(event)
		{
			...
			postCallback	= function(data)
			{
				...
				if (emptyText !='' && emptyText != null)
				{
					...
					{
						tmpID	= selectID + i;
						firstOptionText = $(tmpID).find('option').first().text() || emptyText;
						$(tmpID).html('<option value="">'+firstOptionText+ '</option>');
					}
				}
				...
				if (data !='' && data != null) {
					firstOptionText = $(targetID).find('option').first().text() || emptyText;
					$(targetID).html('');
					var k = 1;
					for (var j = 0; j < data.length; j++)
					{
						if(j == 0)
						{
							$(targetID).get(0).add(new Option(firstOptionText,''), document.all ? j : null);
						}
						k	= k + j
						...
						//$(targetID).get(0).add(new Option(data[j]['optionText'],data[j]['optionValue'],data: {url: 'some_data',whatever: 1}), document.all ? k : null);
					}
				}
			}
