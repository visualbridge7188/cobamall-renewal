...
(function($) {
	...
	function tree_component () {
		return {
			...
			refresh : function (obj) {
				...
				if(this.container.children("ul").length == 0) {
				...
				if(this.opened && this.opened.length) {
					...
					for(var j = 0; this.opened && j < this.opened.length; j++) {
						if(this.settings.data.async) {
							...
							if(tmp.length && tmp.hasClass("closed") > 0) {
								opn = true;
			...
			off_height : function () {
				...
				if(!this.li_height) {
					...
					if(tmp.children("ul:eq(0)").length) this.li_height -= tmp.children("ul:eq(0)").height();
			...
			check_move : function (nod, ref_node, how) {
				if(this.locked) return false;
				if($(ref_node).closest("li.dragged").length) return false;
				...
				if(this.settings.rules.use_max_children) {
					if(typeof r.max_children != "undefined" && r.max_children != -1) {
						...
						if(tree_component.drag_drop.moving == true && tree_component.drag_drop.foreign == false) {
							c_count = tree_component.drag_drop.dragged.length;
							c_count = c_count - p.find('> ul > li.dragged').length;
						}
						if(r.max_children < p.find('> ul > li').length + c_count) return false;
					}
				}
			...
			hover_branch : function (obj) {
				...
				if(!obj.length) return this.error("HOVER: NOT A VALID NODE");
			...
			select_branch : function (obj, multiple) {
				...
				if(!obj.length) return this.error("SELECT: NOT A VALID NODE");
				obj.children("a").removeClass("hover");
				// CHECK AGAINST RULES FOR SELECTABLE NODES
				if(!_this.check("clickable", obj)) return this.error("SELECT: NODE NOT SELECTABLE");
				if(_this.callback("beforechange",[obj.get(0),_this]) === false) return this.error("SELECT: STOPPED BY USER");
				// IF multiple AND obj IS ALREADY SELECTED - DESELECT IT
				if(this.settings.rules.multiple != false && multiple && obj.children("a.clicked").length > 0) {
			...
			deselect_branch : function (obj) {
				...
				if(obj.children("a.clicked").length == 0) return this.error("DESELECT: NODE NOT SELECTED");
			...
			open_branch : function (obj, disable_animation, callback) {
				...
				if(this.locked) return this.error("LOCKED");
				var obj = this.get_node(obj);
				if(!obj.length) return this.error("OPEN: NO SUCH NODE");
				if(obj.hasClass("leaf")) return this.error("OPEN: OPENING LEAF NODE");
				if(this.settings.data.async && obj.find("li").length == 0) {
					...
					_datastore.load(this.callback("beforedata",[obj,this]),this,this.settings.data.opts,function(data){
						...
						_datastore.parse(data,_this,_this.settings.data.opts,function(str){
							str = _this.callback("onparse", [str, _this]);
							// if(obj.children('ul:eq(0)').children('li').length > 1) obj.children("ul").find('.loaading').parent().replaceWith(str); else
			...
			close_branch : function (obj, disable_animation) {
				...
				var _this = this;
				var obj = this.get_node(obj);
				if(!obj.length) return this.error("CLOSE: NO SUCH NODE");
				if(_this.callback("beforeclose",[obj.get(0),_this]) === false) return this.error("CLOSE: STOPPED BY USER");
				if(parseInt(this.settings.ui.animation) > 0 && !disable_animation && obj.children("ul:eq(0)").length == 1) {
				...
				if(this.selected && this.settings.ui.selected_parent_close !== false && obj.children("ul:eq(0)").find("a.clicked").length > 0) {
					obj.find("li:has(a.clicked)").each(function() {
						_this.deselect_branch(this);
					});
					if(this.settings.ui.selected_parent_close == "select_parent" && obj.children("a.clicked").length == 0) this.select_branch(obj, (this.settings.rules.multiple != false && this.selected_arr.length > 0) );
				}
			...
			open_all : function (obj, callback) {
				...
				var s = obj.find("li.closed").length;
			...
			create : function (obj, ref_node, position) {
				...
				if(!root && (!ref_node || !ref_node.length)) return this.error("CREATE: NO NODE SELECTED");
				...
				if(!root && ref_node.length == 0) { root = true; ref_node = this.container; }

				if(!root) {
					if(this.check("creatable", ref_node) !== true) return this.error("CREATE: CANNOT CREATE IN NODE"); // 20100916 sunny
					if(ref_node.hasClass("closed")) {
						if(this.settings.data.async && ref_node.children("ul").length == 0) {
							var _this = this;
				...
				if(this.settings.rules.use_max_children) {
					if(typeof r.max_children != "undefined" && r.max_children != -1 && r.max_children >= this.children(ref_node).length) return this.error("CREATE: MAX_CHILDREN REACHED");
				}
				...
				if((typeof position).toLowerCase() == "undefined" || position == "inside")
					position = (this.settings.rules.createat == "top") ? 0 : ref_node.children("ul:eq(0)").children("li").length;
				if(ref_node.children("ul").length == 0 || (root == true && ref_node.children("ul").children("li").length == 0) ) {
				...
				else if(pos == "before" && ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").length)
					var a = this.moved($li,ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").children("a:eq(0)"),"before", true);
				else if(pos == "after" &&  ref_node.children("ul:eq(0)").children("li:nth-child(" + (position) + ")").length)
					var a = this.moved($li,ref_node.children("ul:eq(0)").children("li:nth-child(" + (position) + ")").children("a:eq(0)"),"after", true);
				else if(ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").length)
					var a = this.moved($li,ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").children("a:eq(0)"),"before", true);
				else
			...
			rename : function (obj, new_name) {
				...
				obj = obj ? this.get_node(obj) : this.selected;
				var _this = this;
				if(!obj || !obj.length) return this.error("RENAME: NO NODE SELECTED");
				if(this.check("renameable", obj) !== true) return this.error("RENAME: NODE NOT RENAMABLE"); // 20100916 sunny
				...
				if((typeof new_name).toLowerCase() == "string") {
					...
					if (parseInt(_this.settings.rules.valLength) > 0) {
						_this.inp.prop('maxLength', parseInt(_this.settings.rules.valLength));
					}
			...
			remove : function(obj) {
				...
				if(obj && (!this.selected || this.get_node(obj).get(0) != this.selected.get(0) )) {
					obj = this.get_node(obj);
					if(obj.length) {
						if(this.check("deletable", obj) !== true) return this.error("DELETE: NODE NOT DELETABLE"); // 20100916 sunny
						if(!this.callback("beforedelete",[obj.get(0), _this])) return this.error("DELETE: STOPPED BY USER");
						$parent = obj.parent();
						if(obj.find("a.clicked").length) {
			...
			moved : function (what, where, how, is_new, is_copy, rb) {
				...
				if(!rb) {
					...
					rb[this.container.attr("id")] = this.get_rollback();
					if(!is_new) {
						var tmp = what.length > 1 ? what.eq(0).parents(".tree:eq(0)") : what.parents(".tree:eq(0)");
				...
				if(how == "inside" && this.settings.data.async) {
					...
					if(this.get_node($where).find("> ul > li > a.loading").length == 1) {
				...
				switch(how) {
					...
					case "after":
						...
						break;
					case "inside":
						if($where.parent().children("ul:first").length) {
			...
			cut : function (obj) {
				...
				if(!obj || !obj.length) return this.error("CUT: NO NODE SELECTED");
			...
			copy : function (obj) {
				if(this.locked) return this.error("LOCKED");
				obj = obj ? this.get_node(obj) : this.container.find("a.clicked").filter(":first-child").parent();
				if(!obj || !obj.length) return this.error("COPY: NO NODE SELECTED");
			...
			paste : function (obj, position) {
				...
				if(!root && (!obj || !obj.length)) return this.error("PASTE: NO NODE SELECTED");
				...
				if(position == "before") {
					...
					position = (this.settings.rules.createat == "top") ? 0 : obj.children("ul:eq(0)").children("li").length;
				}
				if(!root && obj.length == 0) { root = true; obj = this.container; }

				if(tree_component.cut_copy.copy_nodes && tree_component.cut_copy.copy_nodes.length) {
					var ok = true;
					if(!root && !this.check_move(tree_component.cut_copy.copy_nodes, obj.children("a:eq(0)"), "inside")) return false;

					if(obj.children("ul").length == 0 || (root == true && obj.children("ul").children("li").length == 0) ) {
					...
					else if(pos == "before" && obj.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").length)
					...
					else if(pos == "after" && obj.children("ul:eq(0)").children("li:nth-child(" + (position) + ")").length)
					...
					else if(obj.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").length)
						var a = this.moved(tree_component.cut_copy.copy_nodes,obj.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").children("a:eq(0)"),"before", false, true);
					else
				...
				if(tree_component.cut_copy.cut_nodes && tree_component.cut_copy.cut_nodes.length) {
					var ok = true;
					...
					if(!this.srch_opn) {
						...
						if(this.srch_opn && this.srch_opn.length) {
							...
							for(var j = 0; j < this.srch_opn.length; j++) {
								if(this.get_node("#" + this.srch_opn[j]).length > 0) {
									opn = true;
	...
	tree_component.mouseup = function(event) {
		...
		if(tmp.foreign === false && tmp.drag_node && tmp.drag_node.length) {
		...
		if(tmp.dragged && tmp.dragged.length) tmp.dragged.removeClass("dragged");
		tmp.dragged		= false;
		tmp.drag_help	= false;
		tmp.drag_node	= false;
		tmp.f_type		= false;
	...
	tree_component.mousemove = function(event) {
		...
		if(tmp.isdown) {
			...
			if(cnt.length == 0 || !tree_component.inst[cnt.attr("id")]) {
				if(tmp.scroll_time) clearTimeout(tmp.scroll_time);
			...
			if( (et.is(".tree") || et.is("ul") ) && et.find("li:eq(0)").length == 0) {
				var et_off = et.offset();
	...
	$(function () {
		...
		var css = '/* TREE LAYOUT */ .tree ul { margin:0 0 0 5px; padding:0 0 0 0; list-style-type:none; } .tree li { display:block; min-height:18px; line-height:18px; padding:0 0 0 15px; margin:0 0 0 0; /* Background fix */ clear:both; } .tree li ul { display:none; } .tree li a, .tree li span { display:inline-block;line-height:16px;height:16px;color:black;white-space:nowrap;text-decoration:none;padding:1px 4px 1px 4px;margin:0; } .tree li a:focus { outline: none; } .tree li a input, .tree li span input { margin:0;padding:0 0;display:inline-block;height:20px !important;border:1px solid white;background:white;font-size:10px;font-family:Verdana; } .tree li a input:not([class="xxx"]), .tree li span input:not([class="xxx"]) { padding:1px 0; } /* FOR DOTS */ .tree .ltr li.last { float:left; } .tree > ul li.last { overflow:visible; } /* OPEN OR CLOSE */ .tree li.open ul { display:block; } .tree li.closed ul { display:none !important; } /* FOR DRAGGING */ #jstree-dragged { position:absolute; top:-10px; left:-10px; margin:0; padding:0; } #jstree-dragged ul ul ul { display:none; } #jstree-marker { padding:0; margin:0; line-height:5px; font-size:1px; overflow:hidden; height:5px; position:absolute; left:-45px; top:-30px; z-index:1000; background-color:transparent; background-repeat:no-repeat; display:none; } #jstree-marker.marker { width:45px; background-position:-32px top; } #jstree-marker.marker_plus { width:5px; background-position:right top; } /* BACKGROUND DOTS */ .tree li li { overflow:hidden; } .tree > .ltr > li { display:table; } /* ICONS */ .tree ul ins { display:inline-block; text-decoration:none; width:16px; height:16px; } .tree .ltr ins { margin:0 4px 0 0px; } ';
		if(browser.msie) {
			if(browser.versionNumber == 6) css += '.tree li { height:18px; zoom:1; } .tree li li { overflow:visible; } .tree .ltr li.last { margin-top: expression( (this.previousSibling && /open/.test(this.previousSibling.className) ) ? "-2px" : "0"); } .marker { width:45px; background-position:-32px top; } .marker_plus { width:5px; background-position:right top; }';
			if(browser.versionNumber == 7) css += '.tree li li { overflow:visible; } .tree .ltr li.last { margin-top: expression( (this.previousSibling && /open/.test(this.previousSibling.className) ) ? "-2px" : "0"); }';
		}
	...
	$.extend($.tree.datastores, {
		"html" : function () {
			return {
				get		: function(obj, tree, opts) {
					return obj && $(obj).length ? $('<div>').append(tree.get_node(obj).clone()).html() : tree.container.children("ul:eq(0)").html();
				},
		...
		"json" : function () {
			return {
				get		: function(obj, tree, opts) {
					var _this = this;
					if(!obj || $(obj).length == 0) obj = tree.container.children("ul").children("li");
					else obj = $(obj);
					...
					if(tree.settings.languages.length) {
						...
						if(a.children("ins").length && a.children("ins").get(0).className.length) {
							json.data.icon = a.children("ins").get(0).className;
						}
						if(a.children("ins").length && a.children("ins").get(0).style.backgroundImage.length) {
