...
$(document).ready(function(){
                ...
                $ajax.done(function (responseData) {
                    if (responseData.resultList.count == 0) {
                        ...
                        $.each(responseData.resultList.list, function (sKey, sVal) {
                            ...
                            if(sVal.handleMode === 'z'){
                                html += '            <p style="background-color: #337ab7; color: #ffffff; text-align: center;">교환추가</p>';
                            }
                            html += '                <p class="goodsname">' + sVal.name + '</p>';
    ...
    // 송장스캔
    $('#btn_barcodescan').click(function() {
        function alertDismissed() {
            // do something
        }
        if ($('#applyDeliverySno').val() == 0 ) {
            navigator.notification.alert(
                '배송업체를 선택해주세요.',  // message
        ...
        var scan_obj = new Object();
        scan_obj.item = new Array();
        scan_obj.item[0] = new Object();
        scan_obj.item[0].idx = "1";
        scan_obj.item[0].ordno =  $('#orderNo').val();
        scan_obj.item[0].goodsnm = $('.goodsname').text();
        scan_obj.item[0].namereceiver = $('input[name="info[receiverName]"]').val();
        scan_obj.delivercomp = $('#applyDeliverySno option:selected').text();
        scan_obj.cnt = 1;

        var json_string = JSON.stringify(scan_obj);

        window.plugins.barcodeScanner.scan(function(result) {
            if (!result.cancelled) {
                successCBBarcodeScanner(result.text);
            } else {
                alert("바코드 모듈의 실행을 취소하셨습니다.");
            }
        }, function(error) {
            alert("API에 문제가 발생했습니다. 관리자에게 문의하세요. (Error Code : 1404)");
        }, json_string);

    });
    function successCBBarcodeScanner(result_txt) {
        console.log("f", arguments.callee.name.toString(), result_txt);
        var upload_data_idx = new Array();;
        var upload_data_ordno = new Array();
        var upload_data_scode = new Array();
        var upload_idx = 0;

        var data = jQuery.parseJSON(result_txt);

        upload_data_idx[0] = data.item[0].idx;
        upload_data_ordno[0] = data.item[0].ordno;
        upload_data_scode[0] = data.item[0].scancode;

        upload_data_cnt = parseInt(data.cnt);

        $('#invoiceNo').val(upload_data_scode[0]);
    }
