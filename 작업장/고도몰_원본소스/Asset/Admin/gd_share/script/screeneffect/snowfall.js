...
var snowFall = (function(){
    function jSnow(){
        ...
        var defaults = {
            ...
            // snowflake
            flake = function(_el, _size, _speed, _options)
            {
                // Flake properties
                ...
                this.opacity = _options.maxOpacity ? _options.maxOpacity : 1;
                this.opacityVector = -1;
                this.twinkleSpeed = 0.01 * _speed * defaults.maxOpacity;
                ...
                setStyle(flakeObj, {'opacity': this.opacity, 'width' : this.size, 'height' : this.size, 'position' : defaults.flakePosition, 'top' : 0, 'left' : 0, 'will-change': 'transform', 'fontSize' : 0, 'zIndex' : defaults.flakeIndex});
                ...
                this.update = function(){
                    ...
                    if (defaults.twinkle) {
                        if (this.opacity < 0.05) {
                            this.opacityVector = 1;
                        } else if (this.opacity > defaults.maxOpacity) {
                            this.opacityVector = -1;
                        }
                        this.opacity += this.twinkleSpeed * this.opacityVector;
        ...
        return{
            snow : function(_element, _options){
                ...
                for(i = 0; i < defaults.flakeCount; i+=1){
                    flakes.push(new flake(element, random((defaults.minSize * 100), (defaults.maxSize * 100)) / 100,
                        random(defaults.minSpeed, defaults.maxSpeed), _options));
                }
                // start the snow
                animateSnow();
            },
