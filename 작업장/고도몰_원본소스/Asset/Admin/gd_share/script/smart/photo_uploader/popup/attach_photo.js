...
var rFilter = /^(image\/bmp|image\/gif|image\/jpg|image\/jpeg|image\/png|image\/tiff|image\/svg\+xml)$/i;
var rFilter2 = /^(bmp|gif|jpg|jpeg|png|tiff|svg)$/i;
var nTotalSize = 0;
var nMaxImageMbSize = 5 * 1024 * 1024;
var nMaxImageSize = nMaxImageMbSize * 1024 * 1024;
var nMaxImageCount = 10;
var nMaxTotalImageSize = nMaxImageMbSize * nMaxImageCount * 1024 * 1024;
...
function checkDragAndDropAPI() {
    ...
    if (bSupportDragAndDropAPI == false) {    //드래그를 지원안하면 업로드 선택하는 영역 숨김
...
function goStartMode() {
    ...
    if (sSrc.indexOf("btn_confirm2.png") < 0) {
        welBtnConfirm.attr("src", "/admin/gd_share/script/smart/img/photoQuickPopup/btn_confirm2.png");
...
function goReadyMode() {
    ...
    if (sSrc.indexOf("btn_confirm2.png") >= 0) {
        ...
        welBtnConfirm.attr("src", "/admin/gd_share/script/smart/img/photoQuickPopup/btn_confirm.png");
    }
}
    ...
    if (typeof ev.dataTransfer.files == 'undefined') {
        ...
        for (var i = 0, j = nImageFileCount; i < nCount; i++) {
            if (!rFilter.test(files[i].type)) {
                alert("이미지파일 (jpg,jpeg,gif,png,bmp,tiff,svg)만 업로드 가능합니다.");
            } else if (files[i].size > nMaxImageSize) {
                alert("이미지 용량이 " + nMaxImageMbSize + "MB를 초과하여 등록할 수 없습니다.");
            } else {
                //제한된 수만 업로드 가능.
                if (j < nMaxImageCount) {
...
function addImage(ofile) {
    ...
    if (!!bExceedLimitTotalSize) {
        ...
        aFileList.push('	<a onclick="delImage(\'img' + nImageInfoCnt + '\')"><img class="del_button" src="/admin/gd_share/script/smart/img/photoQuickPopup/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
        aFileList.push('	</li> ');
...
function html5Upload() {
    ...
    if(opener.parent.parent.document.getElementById("imageStorage")) {
        var ftpStorage = opener.parent.parent.document.getElementById("imageStorage").value;
    } else {
        var ftpStorage = '';

    }
    sUploadURL = '/share/editor_file_uploader_html5_ps.php?ftpStorage='+ftpStorage +'&editorUrl='+editorUrl;	//upload URL
    //파일을 하나씩 보내고, 결과를 받음.
...
function callAjaxForHTML5(tempFile, sUploadURL) {
    //var bdId = getQueryVariable(sUploadURL, 'bdId');
    //if (typeof bdId == 'undefined') {
    //    bdId = '';
    //}

    let formData = new FormData();
    formData.append('uploadFile', tempFile, tempFile.name);

    var oAjax = jindo.$Ajax(sUploadURL, {
        type: 'xhr',
        method: "post",
        async: false,	//한꺼번에 처리하여 딜레이되는거 방지
        onload: function (res) { // 요청이 완료되면 실행될 콜백 함수
            ...
            if (res.readyState() == 4) {
                if (sResString.indexOf("NOTALLOW_") > -1) {
                    ...
                    alert("이미지 파일(jpg,gif,png,bmp,jpeg,tiff)만 업로드 하실 수 있습니다. (" + sFileName + ")");
                } else if (sResString.indexOf("ERRORMSG_") > -1) {
                    var msg = sResString.replace("ERRORMSG_", "");
                    alert(msg);
                } else if (sResString.indexOf("FILE_ERROR") > -1) {
...
function makeArrayFromString(sResString) {
    ...
    //opener.parent.setUploadImages('namxxxju');
...
function callFileUploader() {
    oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"), {
        //sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php?editorUrl='+editorUrl,	//샘플 URL입니다.
        sUrl: '/share/editor_file_uploader_ps.php?editorUrl=' + editorUrl,	//샘플 URL입니다.
        sCallback: '/admin/gd_share/script/smart/photo_uploader/popup/callback.html', //업로드 이후에 iframe이 redirect될 콜백페이지의 주소
        sFiletype: "*.jpg;*.png;*.bmp;*.gif;*.jpeg;*.tiff;*.svg",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)
        sMsgNotAllowedExt: 'JPG, GIF, PNG, BMP, TIFF, SVG 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
...
window.onload = function () {
    ...
    if (bSupportDragAndDropAPI) {
        ...
        document.getElementsByName('uploadType')[0].checked = true;
        document.getElementsByName('uploadType')[1].checked = false;

    } else {
        $Element("pop_container").hide();
        $Element("pop_container2").show();
        callFileUploader();
        document.getElementsByName('uploadType')[1].checked = true;
        document.getElementsByName('uploadType')[0].checked = false;
    }
...
jindo.$Ajax.prototype.request = function (oData) {
    ...
    if (opt.sendheader) {
        if (opt.contentType !== false && !this._headers["Content-Type"]) {
