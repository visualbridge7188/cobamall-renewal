...
gd_sms_send = (function (dialog) {
    ...
    function print_target_area(template_id, params) {
        ...
        // 대량발송 안내문구 출력
        if (params) {
            if (params['receiveTotal'] > 500 || params['receiverCnt'] > 500 || params['total'] > 500) {
                var large_caution = $('.sms-large-caution');
                large_caution.removeClass('display-none');
            }
        }
        return compiled;
    }
    ...
    function count_target_group() {
        // CRM 그룹 삭제 시 초기화
        if ($receiver_type.filter(':checked').val() === 'crmGroup') {
            $(".js-receive-total").data('receive-total', 0);
            $(".js-receive-total").html(0);
            
            $(".js-reject-count").data('reject-count', 0);
            $(".js-reject-count").html(`(수신거부 대상자 0 명 포함)`);
            return;
        }
    ...
    function count_target_popup() {
        ...
        $.ajax('../member/sms_send_ps.php', {
            ...
            success: function () {
                ...
                target_area.attr('data-total', params.receiveTotal);
                target_area.attr('data-reject-count', params.rejectCount);
                target_area.parent().append('<span class="text-red js-reject-count" data-reject-count="'+params.receiveTotal+'">(수신거부 대상자 '+params.rejectCount+' 명 포함)</span>');
    ...
    window.setSelectedCrmGroup = function (crmNo, groupName) {
        getCrmGroupInfo(crmNo).then(function (response) {
            $(".js-receive-total").data("receive-total", response.data.count);
            $(".js-receive-total").html(response.data.count);

            $(".js-reject-count").data("reject-count", response.data.smsOptOutCount);
            $(".js-reject-count").html("(수신거부 대상자 " + response.data.smsOptOutCount + " 명 포함)");

            $("#member_groupLayer").addClass("active");
            var compiledTemplate = _.template($("#templateCrmGroupItem").html());
            $("#member_groupLayer").html(compiledTemplate({name: groupName, targetNo: crmNo}));
        });
    };
    ...
    sms.set_click_excel_upload = function (selector) {
        $(selector).click(function () {
            ...
            if ($('#formSendSms').find(':file[name=excel]:eq(0)').length > 0) {
                $formExcel.find(':file').remove();
                var $file = $(':file[name=excel]:eq(0)');
                $formExcel.append($file);
            } else {
                var $file = $('#formExcel').find(':file[name=excel]:eq(0)');
            }
            ...
            dialog.show({message: '<span style="font-weight:bold;">발송인원 계산 중입니다.</span> <img src="/admin/gd_share/img/icon_loading.gif" alt="로딩중" width="20" class="middle"><br/>※ 페이지 이탈 시, 엑셀 업로드는 중단됩니다.'});
            $formExcel.submit();
        });
    };
    ...
    sms.set_click_receiver_type = function (selector) {
        ...
        $receiver_type.click(function (e) {
            ...
            let $crm_group_notice = $('.js-crm-group-notice');
            $sleep_notice.addClass('display-none');
            $target_area_excel.addClass('display-none');
            $crm_group_notice.addClass('display-none');
            ...
            } else if ($this.val() === 'crmGroup') {
                target_area.append(print_target_area(template_id));
                $crm_group_notice.removeClass('display-none');
            } else {
                alert('선택된 발송 대상이 없습니다.');
            }

            if ($("input[name='sms080Reject']").prop('checked')) {
                $('.js-cb-agree').prop('checked', true);
                $('.js-cb-agree').prop('disabled', true);
            }
        });
    };
    ...
    sms.set_click_sms080_contents = function (selector) {
        $(selector).click(function (e) {
            ...
            $(".js-cb-agree").prop('disabled', false);
                ...
                success: function () {
                    ...
                    $.each(response_json.lists, function (idx, item) {
                        ...
                        view.on('click', 'button.js-btn-save', function (e) {
                            if ($sms_contents_search_area.data('lock-write') == '1') {
                                alert('개별/전체 SMS 발송의 쓰기 권한이 없습니다. 권한은 대표운영자에게 문의하시기 바랍니다.');
                                return;
                            }
