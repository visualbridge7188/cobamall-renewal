        ...
        <table class="table table-cols">
            ...
            <?php if(gd_is_provider() === false) { ?>
                ...
                <td colspan="3">
                    ...
                    <label class="radio-inline">
                        <input type="radio" name="scmFl" value="a" <?=gd_isset($checked['scmFl']['a']); ?> onclick="$('#scmLayer').html('')" ;/>구분없음
                    </label>
            ...
            <tr>
                ...
                <td><div class="form-inline">
                    <input type="text" name="mustInfoNm" value="<?=$search['mustInfoNm']; ?>" class="form-control"/>
                    </div>
                </td>
            </tr>
            <tr>
    ...
    <form id="frmList" action="" method="get" target="ifrmProcess" >
        ...
        <table class="table table-rows">
            ...
            if (gd_isset($data)) {
                ...
                foreach ($data as $key => $val) {
                    if($val['scmNo'] == '0') $val['scmNm'] = "구분없음";
                    ?>

                    <tr>
                        <td class="center"><input type="checkbox" name="sno[<?=$val['sno']; ?>]" value="<?=$val['sno']; ?>"/></td>
                        <td class="center number"><?=number_format($page->idx--); ?></td>
                        <td onclick="show_popup('./goods_must_info_register.php?popupMode=yes&sno=<?=$val['sno']; ?>')" class="hand"><?=$val['mustInfoNm']; ?></td>
                        ...
                        <td class="center">
                            <a
                                href="./goods_must_info_register.php?sno=<?=$val['sno']; ?>" class="btn btn-white btn-xs"> 수정</a>
        ...
        <div class="table-action">
            ...
            <div class="pull-right">
                <button type="button" class="btn btn-white btn-icon-excel js-excel-download" data-target-form="frmSearchBase" data-search-count="<?=$page->recode['total']?>" data-total-count="<?=$page->recode['amount']?>" data-target-list-form="frmList" data-target-list-sno="sno">엑셀다운로드</button>
            </div>
        </div>
