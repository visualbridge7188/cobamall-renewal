    ...
    <div>
        <div class="table-title gd-help-manual">
            ...
            <div class="pull-right">
                <button type="button" class="btn btn-sm btn-black" onclick="set_search_config(this.form)">검색설정저장</button>
            </div>
        </div>
    </div>
    <div class="search-detail-box">
        ...
        <table class="table table-cols">
            ...
            <tr>
                ...
                <td>
                    <label class="radio-inline">
                        <input type="radio" name="deliveryType" value="all"
                            <?= empty($search['deliveryType']) || $search['deliveryType'] === 'all' ? 'checked="checked"' : ''; ?>/>전체(일반배송,
                        정기배송)
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="deliveryType" value="regular"
                            <?= $search['deliveryType'] === 'regular' ? 'checked="checked"' : ''; ?>/>정기배송
                    </label>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <div>
                        ...
                        <label class="radio-inline width-3xs mgl0">
                    ...
                    <div class="hidden" id="delivery_cycle_month">
                        ...
                        <?php for ($i = 1; $i <= 6; $i++): ?>
                            <label class="radio-inline width-3xs <?= $i === 1 ? 'mgl0' : '' ?>">
                                <input type="checkbox" name="deliveryCycleMonth[]" class="deliveryCycleMonth"
                                       value="<?= $i ?>" <?= in_array($i, $search['deliveryCycleMonth'] ?? []) ? 'checked' : '' ?>
                    ...
                    <div class="hidden" id="delivery_cycle_week">
                        <div>
                            <label class="radio-inline width-3xs">
                                <input type="checkbox" id="deliveryCycleWeekAll"<?= in_array($i, $search['deliveryCycleWeek'] ?? []) ? 'checked' : '' ?>
                            ...
                            for ($i = 1; $i <= 6; $i++): ?>
                                <label class="radio-inline width-3xs <?= $i === 1 ? 'mgl0' : '' ?>">
                                    <input type="checkbox" name="deliveryCycleWeek[]" class="deliveryCycleWeek"
                                           value="<?= $i ?>" <?= in_array($i, $search['deliveryCycleWeek'] ?? []) ? 'checked' : '' ?>
                        ...
                        <div class="hidden" id="max_delivery_rounds" style="display: inline-block;">
                            ...
                            <label class="radio-inline">
                                <input name="maxDeliveryRounds" class="form-control width-2xs"
                                       value="<?= $search['maxDeliveryRounds']; ?>" onblur="validateMaxDeliveryRounds(this)">
                            </label>
                            회 이하
                        </div>
                    </div>
            ...
            <tr>
                ...
                <td colspan="3">
                    <label class="radio-inline width-3xs">
                        <input type="radio" name="applyStatus" value="all" onclick="$('#scmLayer').html('');"
                            <?= empty($search['applyStatus']) || $search['applyStatus'] === 'all' ? 'checked="checked"' : ''; ?>/>전체
                    </label>
                    <label class="radio-inline mgl0">
                        <input type="radio" name="applyStatus" value="abled" onclick="$('#scmLayer').html('')" ;
                            <?= $search['applyStatus'] === 'abled' ? 'checked="checked"' : ''; ?>/>신청 가능
                    </label>
                    <label class="mgl5">
                        <input type="radio" name="applyStatus" value="disabled" onclick="$('#scmLayer').html('')" ;
                            <?= $search['applyStatus'] === 'disabled' ? 'checked="checked"' : ''; ?>/>신청 중지
                    </label>
                </td>
            </tr>
            </tbody>
...
<script type="text/javascript">
    ...
    /**
     * 상품 상세 검색 내 종료회차에 대해 설정 선택 시, 2이상 50이하의 값만 입력할 수 있도록 제한하는 함수
     */
    function validateMaxDeliveryRounds(input) {
        let value = parseInt(input.value, 10);
        if (value < 2) {
            input.value = ''; // 1 이하로 입력되지 않도록 설정
        } else if (value > 50) {
            input.value = ''; // 51 이상의 값이 입력되지 않도록 설정
        }
    }
