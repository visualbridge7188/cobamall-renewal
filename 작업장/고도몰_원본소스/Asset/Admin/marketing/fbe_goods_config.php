<div class="page-header js-affix">
    ...
    <div class="btn-group">
        <input type="button" value="노출상품 현황 확인" class="btn btn-white js-layer-fbe-stats"/>
...
<form id="frmSearchGoods" name="frmSearchGoods" method="get" class="js-form-enter-submit">
    ...
    <div class="search-detail-box">
        ...
        <table class="table table-cols">
            ...
            <tr>
                <th>메타 FBE <br/> 제품 피드 설정</th>
                <td>
                    <label class="radio-inline"><input type="radio" name="fbeFl" value="" <?=gd_isset($checked['fbeFl']['']); ?> />전체</label>
                    <label class="radio-inline"><input type="radio" name="fbeFl" value="y" <?=gd_isset($checked['fbeFl']['y']); ?> />설정함</label>
                    <label class="radio-inline"><input type="radio" name="fbeFl" value="n" <?=gd_isset($checked['fbeFl']['n']); ?> />설정안함</label>
                </td>
                <th>연동대상</th>
                <td>
                    <label class="radio-inline"><input type="radio" name="isLinked" value="" <?=gd_isset($checked['isLinked']['']); ?> />전체</label>
                    <label class="radio-inline"><input type="radio" name="isLinked" value="Y" <?=gd_isset($checked['isLinked']['Y']); ?> />Y</label>
                    <label class="radio-inline"><input type="radio" name="isLinked" value="N" <?=gd_isset($checked['isLinked']['N']); ?> />N</label>
                </td>
            </tr>
            </tbody>
        </table>
...
<form id="frmBatchFbe" name="frmBatchFbe" action="../goods/goods_ps.php"    target="ifrmProcess" method="post">
    <input type="hidden" name="mode" value="batch_fbe_config" />
    ...
    <div class="table-responsive">
        <table class="table table-rows table-fixed">
            ...
            <tr>
                ...
                <th class="width-xs center" rowspan="2">메타 FBE<br/>제품 피드 설정</th>
            ...
            if (gd_isset($data) && count($data) > 0 ) {
                ...
                foreach ($data as $key => $val) {
                    if ($val['goodsDiscountFl'] == 'y') {
                        if ($val['goodsDiscountUnit'] == 'price') {
                            $goodsDiscount = gd_currency_symbol() . $val['goodsDiscount'] . gd_currency_string();
                        } else {
                    ...
                    <tr>
                        ...
                        <td class="center"><?=gd_html_goods_image($val['goodsNo'], $val['imageName'], $val['imagePath'], $val['imageStorage'], 40, $val['goodsNm'], '_blank'); ?></td>
                        <td><a href="../goods/goods_register.php?goodsNo=<?=$val['goodsNo']; ?>" target="_blank"><span class="emphasis_text"><?=$val['goodsNm']; ?></span></a></td>
                        <td class="center"><?=$arrGoodsDisplay[$val['fbeFl']]; ?></td>
                        ...
                        <td class="center number"><div class="form-inline"><?=gd_currency_symbol(); ?><?=gd_money_format($val['goodsPrice']); ?><?=gd_currency_string(); ?></div></td>
                        <td class="center"><?=$goodsDiscount?></td>
                        <td class="center"><?=$val['isLinked']?></td>
                    </tr>
                    <?php
                }
            } else {
                ?>
                <tr><td class="no-data" colspan="14">검색된 정보가 없습니다.</td></tr>
            <?php } ?>
            </tbody>
        </table>
    </div>
    ...
    <div>
        <table class="table table-cols" id="setFbeConfig">
            <colgroup>
                <col class="width-md" />
                <col />
            </colgroup>
            <tr>
                <th class="center">메타 FBE <br/>제품 피드 설정</th>
                <td>
                    <label class="radio-inline"><input type="radio" name="fbeFl" value="y" checked="checked" />설정함</label>
                    <label class="radio-inline"><input type="radio" name="fbeFl" value="n" />설정안함</label>
                </td>
            </tr>
        </table>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $(".js-layer-fbe-stats").click(function(e){
            layer_add_info('fbe_stats');
        });
        $( "#batchSubmit" ).click(function() {
            ...
            msg += '메타 FBE 피드 설정 상태를 '+$('#setFbeConfig input[name="fbeFl"]:checked').closest('label').text()+ '으로 ';
            msg += '일괄 수정하시겠습니까?<br /><br />';
