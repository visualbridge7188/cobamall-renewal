    ...
    <div class="search-detail-box">
        ...
        <table class="table table-cols">
            ...
            <tr>
                ...
                <th>네이버쇼핑 도서 노출여부</th>
                <td>
                    <label class="radio-inline"><input type="radio" name="naverbookFlag" value="" <?=gd_isset($checked['naverbookFlag']['']); ?> />전체</label>
                    <label class="radio-inline"><input type="radio" name="naverbookFlag" value="y" <?=gd_isset($checked['naverbookFlag']['y']); ?> />노출함</label>
                    <label class="radio-inline"><input type="radio" name="naverbookFlag" value="n" <?=gd_isset($checked['naverbookFlag']['n']); ?> />노출안함</label>
                </td>
            </tr>
            <tr>
                <th>연동대상</th>
    ...
    <div class="table-responsive">
        <table class="table table-rows table-fixed">
            ...
            <tr>
                ...
                <th class="width-xs center" rowspan="2">네이버 쇼핑<br/>도서 노출여부</th>
            ...
            if (gd_isset($data) && gd_count($data) > 0 ) {
                ...
                foreach ($data as $key => $val) {
                    ...
                    <tr>
                        ...
                        <td class="center"><?=$arrGoodsDisplay[$val['naverbookFlag']]; ?></td>
    ...
    <div>
        <table class="table table-cols" id="setNaverConfig">
            ...
            <tr id="setNaverbook">
                ...
                <td>
                    <label class="radio-inline"><input type="radio" name="naverbookFlag" value="y"/>노출함</label>
                    <label class="radio-inline"><input type="radio" name="naverbookFlag" value="n" checked />노출안함</label>
                    <div class="notice-info">네이버쇼핑 노출여부 > 노출함인 경우에만 설정 가능합니다.</div>
                    <div class="notice-danger">네이버쇼핑 도서카테고리에 노출할 상품만 ‘노출함＇으로 체크하시기 바랍니다.</div>
                    <div class="notice-info">노출함 선택 시 네이버 도서상품 EP가 별도 생성됩니다.<br>('네이버쇼핑 노출여부 > 노출안함'인 상품은 '네이버 쇼핑 도서 노출여부'가 노출함이어도 도서상품 EP가 생성되지 않습니다.)</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#setNaverConfig input[name="naverFl"]').on('change', function(e) {
            var naverFl = $(this).val();
            if(naverFl == 'y') {
                $('#setNaverbook').show();
            } else {
                $('#setNaverbook').hide();
        ...
        $( "#batchSubmit" ).click(function() {
            ...
            msg += '네이버 쇼핑 노출 상태를 '+$('#setNaverConfig input[name="naverFl"]:checked').closest('label').text()+ '으로 \n';
