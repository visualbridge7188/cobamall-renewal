    ...
    <table class="table table-cols">
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                <div id="scmLayer" class="selected-btn-group <?=!empty($search['scmNo']) && $search['scmFl'] != 'all' ? 'active' : ''?>">
                    <h5>선택된 공급사 : </h5>
...
<form id="frmAddGroupDeliveryList" action="../policy/delivery_ps.php" method="post">
        ...
        if (empty($data) === false && is_array($data)) {
            ...
            foreach ($data as $row) {
                ...
                <tr class="text-center">
                    <td><input type="checkbox" name="deliverChk[]" value="<?=$row['sno']?>" <?=$row['defaultFl']=='y'?'disabled="disabled"':''?> required="required" minlength="1" /></td>
                    <td><?=$page->idx--?></td>
                    <td>
                    ...
                    <td><a href="../policy/delivery_area_regist.php?sno=<?=$row['sno']?>" class="btn btn-xs btn-white">수정</a></td>
                </tr>
            <?php } } else { ?>
            <tr>
...
//                }
        ...
        var strSearchKey = $('#frmSearchAddGroupDelivery #key').val();

        setKeywordPlaceholder(searchKeyword, searchKind, strSearchKey, arrSearchKey);

        searchKind.on('change', function(){
            setKeywordPlaceholder(searchKeyword, searchKind, $('#frmSearchAddGroupDelivery #key').val(), arrSearchKey);
        });

        $('#frmSearchAddGroupDelivery #key').on('change', function(){
