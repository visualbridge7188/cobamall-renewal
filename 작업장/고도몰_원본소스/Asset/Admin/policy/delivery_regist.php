    ...
    <table class="table table-cols">
            ...
            <tr>
                ...
                <td colspan="3" class="js-scm">
                    <?php if ($data['count'] != 1 && $data['basic']['defaultFl'] != 'y') { ?>
                        ...
                        <label class="radio-inline">
                            <input type="radio" name="scmFl" value="1" class="js-layer-register" <?php echo gd_isset($checked['scmFl'][1]); ?> data-type="scm" data-mode="radio" data-call-func="setScmNo"/>
                            공급사
                        </label>
                    ...
                    <?php } else {
                        $scmFlKey = gd_array_keys($checked['scmFl']);
                        ?>
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                <label class="form-inline">
                    <input type="text" name="basic[deliveryMethodEtc]" id="deliveryMethodEtc" value="<?=$deliveryMethodEtc?>" class="form-control width-md js-maxlength" maxlength="50" />
                </label>
                <div id="deliveryMethodFlhelp"></div>
            </td>
        </tr>
        <tr>
            <th>배송비 유형</th>
        ...
        <tr id="deliveryConfigTypeArea">
            ...
            <td colspan="3">
                ...
                <div id="deliveryConfigTypeAreaDiv"></div>
            </td>
        </tr>
        ...
        <tr id="basicDeliveryConfigFeeCount" class="display-none">
            ...
            <td colspan="3">
                ...
                <div class="notice-danger">네이버페이 서비스를 사용중인 경우, 설정과 상관없이 추가상품은 수량별 배송비 계산에 포함되지 않습니다.</div>
                <?php }?>
            </td>
        </tr>
        <tr>
...
<script type="text/javascript">
    ...
    function form_delivery_charge(fixMethod) {
        ...
        if (_.isEmpty(data.basic) === false) {
            ...
            if (fixMethod == 'price') {
                $('#repeatInfoMsg').empty();
                <?php if($useNaverPay == 'y') {?>
                if($("input[name='basic[rangeRepeat]']").prop("checked") === true && $('select[name="basic[fixFl]"]').val() === 'price') {
                    var msg = ' <div class="notice-danger">네이버페이 서비스를 사용중인 경우, "금액별 배송비 - 범위반복 설정"은 지원되지 않습니다. (설정 시, 배송비 금액이 비정상적 금액으로 청구될 수 있으므로 주의바랍니다.)</div>';
                    $('#repeatInfoMsg').html(msg);
                }
                <?php }?>
                ...
                <?php if($useNaverPay == 'y') {?>
                if($("input[name='basic[rangeRepeat]']").prop("checked") === true) {
                    var infoMsg = '<div class="notice-danger">네이버페이 서비스를 사용중인 경우, 범위 반복 설정시 1개부터 n개 마다 반복부과만 지원됩니다. (네이버페이 정책)<br>';
                     infoMsg+='첫 행은 "0개 이상 ~ 1개 미만일 때 : 0원" 으로 입력하셔야하며, 2번째 행에 반복 부과 수량과 금액만 입력하셔야 정상적인 네이버페이 서비스가 가능합니다.</div>';
                   $('#basicDeliveryConfigFeeForm').after(infoMsg);
                }
                <?php }?>
                $('#basicDeliveryConfigFeePrice').find('th, td').addClass('display-none');
    ...
    // 방문수령, 퀵배송 체크시
    function clickVisitQuick() {
        if ($("#deliveryMethodFlvisit").prop("checked") == true || $("#deliveryMethodFlquick").prop("checked") == true) {
            var msgTag = "<div class='notice-danger mgt10'>\"방문수령\" 또는 \"퀵배송\" 선택 시 설정에 따라 네이버페이 이용이 제한될 수 있습니다. 자세한 내용은 페이지 하단 도움말을 참고하세요.</div>";
            $("#deliveryMethodFlhelp").html(msgTag);
        } else if ($("#deliveryMethodFlvisit").prop("checked") == false && $("#deliveryMethodFlquick").prop("checked") == false) {
            $("#deliveryMethodFlhelp").html("");
        }
    }
    ...
    $(document).ready(function () {
        ...
        $("select[name='basic[fixFl]']").bind('change', function () {
            ...
            if (val == 'weight') {
                ...
                msg = '네이버페이 서비스를 사용중인 경우, 배송비 수량 범위 구간을 3구간 이내로 설정하시거나 \"범위 반복 설정 사용\"으로 설정하신 경우에만 네이버페이를 통하여 판매가 가능합니다. ';
                //범위 반복 설정 안내문 삭제
                $('#repeatInfoMsg').html("");
            }
            else if (val == 'price') {
        ...
        // 방문수령, 퀵배송 체크시
        $("#deliveryMethodFlvisit").on('click', clickVisitQuick);
        $("#deliveryMethodFlquick").on('click', clickVisitQuick);

        $("input[name='basic[deliveryConfigType]']").on('click', function () {
            if ($(this).val() == 'etc') {
                var msgTag = "<div class='notice-danger mgt10'>배송비 유형에 따라 네이버페이 이용이 제한될 수 있습니다. 이용에 참고 바랍니다.</div>";
                $("#deliveryConfigTypeAreaDiv").html(msgTag);
            } else {
                $("#deliveryConfigTypeAreaDiv").html("");
            }
        });
        ...
        <?php } else { ?>
        $("select[name='basic[fixFl]']").change(function(){
            <?php if (empty($overseasName) === false) { ?>
            if (this.value != 'weight') {
                alert('해외 상점(<?=$overseasName?>)에 설정된 배송비조건은 수정할 수 없습니다.<br />해외 배송조건에서 선택 해제 후 수정하세요.');
                $(this).val('weight');
        ...
        $("#frmDeliveryRegist").validate({
            ...
            rules: {
                ...
                'charge[unitStart][]': {
                    ...
                    digits : {
                        depends: function(){
                            if ($.inArray($('#basic_fixFl option:selected').val(), ['price', 'count']) != -1) {
                                return true;
                            } else {
                                return false;
                            }
                ...
                'charge[unitEnd][]': {
                    required: function() {
                        return $("input[name='basic[rangeRepeat]']").prop("checked");
                    },
                    number: true,
                    min: function() {
                        if($("input[name='basic[rangeRepeat]']").prop("checked") === true){
                            switch($('#basic_fixFl option:selected').val()){
                                case 'weight' :
                                    return 0.1;
                                    break;

                                case 'count' : case 'price' : default :
                                return 1;
                                break;
                            }
                        }
            ...
            messages: {
                ...
                'charge[price][]': {
                    ...
                    digits : "배송비는 정수형 숫자만 입력하실 수 있습니다.",
                },
                'charge[unitStart][]': {
                    required: "배송비 설정을 입력하세요.",
                    number: "숫자만 입력하실 수 있습니다.",
                    digits : "정수형 숫자만 입력하실 수 있습니다.",
                },
                'charge[unitEnd][]': {
                    required: "배송비 설정을 입력하세요.",
                    number: "숫자만 입력하실 수 있습니다.",
        ...
        $('.js-areaFl').change(function (e) {
            ...
            if ($(this).val() == 'y') {
                ...
                <?php if($useNaverPay == 'y') {?>
                var msg = "네이버페이로 결제 시 <a href='naver_pay_config.php' target='_blank'>[설정 > 결제정책 > 네이버페이설정]</a>의 지역별 배송비 설정에 따라 지역별 배송비가 책정됩니다.";
