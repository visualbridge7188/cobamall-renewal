...
<table class="table table-cols">
    ...
    <tr>
        <th>PG사 모듈 버전</th>
        <td>Nicepay WEB-API (V 1.0.6 – 20190227) / Nicepay smartphone WEB-API ( V 3.0.0 – 20181112)</td>
    </tr>
    <tr>
        <th>결제수단 설정</th>
        <td>
    ...
    <tr>
        ...
        <td>
            ...
            <?php } else { ?>
                <?php if ($data['pgApprovalSetting'] === 'y') { ?>
                    <input type="text" name="pgKey" value="<?php echo $data['pgKey'];?>" maxlength="100" class="form-control js-maxlength width-3xl" />
                    <?php if (empty($pgApprovalId) === false) { ?>
                    <div class="notice-info">개별 승인 요청이 승인 되었습니다. 반드시 정보를 확인 후 "PG 정보 저장"을 눌러 PG 설정을 완료 하셔야 합니다.</div>
                    <?php } ?>
                <?php } else { ?>
    ...
    <tr>
        <th>거래취소 비밀번호 설정</th>
        <td>
            <input type="password" name="pgCancelPassword" value="<?php echo $data['pgCancelPassword'];?>" maxlength="10" class="form-control width-lg">
            <div class="notice-info">
                숫자/영문 10자리 이하로 입력가능하며, 나이스페이 관리자 [가맹점정보 > 비밀번호관리]와 동일하게 등록해주세요.<br />
                - 나이스페이 관리자의 비밀번호 설정과 입력값이 다른 경우 취소처리가 불가합니다.
            </div>
            <div class="notice-info notice-danger">PG사 승인 완료 시 나이스페이 관리자에서 거래취소 비밀번호 설정 후, 고도몰 관리자에도 꼭! 반영해주셔야 취소 처리가 가능합니다.</div>
        </td>
    </tr>
    <tr>
        <th>일반 할부 사용 설정</th>
    ...
    <tr>
        <th>가상계좌 입금기한</th>
        <td class="form-inline">
            <input type="text" name="vBankDay" value="<?php echo $data['vBankDay'];?>" maxlength="2" class="form-control js-number width-3xs" data-number="2, 60, 3" /> 일
        </td>
    </tr>
    <tr>
        ...
        <td class="input_area snote">
            <?php
                ...
                echo '<div class="notice-info">' . $pgNm . ' 상점 관리자모드 [ 가맹점정보 > 기본정보 > 결제데이터통보 ]의 "가상계좌 항목"에 "URL/IP" 입력 후 저장합니다.</div>';
    ...
    <tr>
        ...
        <td>
            <a href="https://npg.nicepay.co.kr/logIn.do" target="_blank" class="btn btn-gray btn-sm"><?php echo $pgNm;?> 상점 관리자모드 바로가기</a>
            <a href="https://www.nicepay.co.kr/" target="_blank" class="btn btn-gray btn-sm"><?php echo $pgNm;?> 사이트 바로가기</a>
        </td>
    </tr>
</table>
...
<table class="table table-cols">
    ...
    <tr>
        <th>PG사 모듈 버전</th>
        <td>NICEPAY_ESCROW_API (V 1.0.0 – 20190227)</td>
    </tr>
    <tr>
        <th>사용 설정</th>
        <td>
...
<table class="table table-cols">
    ...
    <tr>
        <th>PG사 모듈 버전</th>
        <td>NICEPAY RECEIPT-API (V 1.0.0 – 20190325)</td>
    </tr>
    <tr>
        <th>사용 설정</th>
        <td>
