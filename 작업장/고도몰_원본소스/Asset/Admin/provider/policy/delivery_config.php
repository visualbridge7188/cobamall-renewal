            ...
            <tr>
                ...
                <td>
                    <label class="radio-inline">
                        <input type="radio" name="goodsDeliveryFl" value="" <?php echo gd_isset($checked['goodsDeliveryFl']['']); ?>/> 전체
                    </label>
                    <label class="radio-inline">
...
<form id="frmDeliveryList" action="../policy/delivery_ps.php" method="post">
    ...
    <table class="table table-rows">
        ...
        <tr class="text-center">
            <td><input type="checkbox" name="deliverChk[]" value="<?=$row['basicKey']?>" <?=$row['defaultFl']=='y' || (isset($row['goodsCount']) && $row['goodsCount'] > 0) ?'disabled="disabled"':''?>/></td>
            <td><?=$page->idx--?></td>
            <td><?=$row['basicKey']?></td>
            <td>
            ...
            <td><a href="../policy/delivery_regist.php?sno=<?=$row['basicKey']?>" class="btn btn-xs btn-white">수정</a></td>
        </tr>
        <?php $sortNo++; } } else { ?>
        <tr>
...
//            }
        ...
        var strSearchKey = $('#frmSearchDelivery select[name="key"]').val();

        setKeywordPlaceholder(searchKeyword, searchKind, strSearchKey, arrSearchKey);
        searchKind.change(function (e) {
            setKeywordPlaceholder(searchKeyword, searchKind, $('#frmSearchDelivery select[name="key"]').val(), arrSearchKey);
        });

        $('#frmSearchDelivery select[name="key"]').change(function (e) {
