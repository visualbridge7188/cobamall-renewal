...
<form id="frmScmAdjust" action="./scm_adjust_ps.php" method="post">
    <input type="hidden" name="mode" value="insertScmAdjustAfterOrder"/>
        ...
        if (empty($data) === false && is_array($data)) {
            foreach ($data as $key => $val) {
                ...
                <tr class="text-center">
                    ...
                    <td class="text-left">
                        <div class="goods_name one_line hand" title="주문 상품명" onclick="goods_register_popup('<?= $val['goodsNo']; ?>', 'true');"><?= gd_html_cut(gd_remove_tag($val['goodsNm']), 46, '..'); ?></div>
                        <?php
                        // 옵션 처리
                    ...
                    foreach ($val['addGoods'] as $aVal) {
                        ...
                        <tr class="text-center add-goods">
                            <td class="text-left">
                                ...
                                <span class="goods_name one_line hand" title="추가 상품명" onclick="addgoods_register_popup('<?= $aVal['addGoodsNo']; ?>', 'true');"><?= gd_html_cut(gd_remove_tag($aVal['goodsNm']), 46, '..'); ?>
