...
<form id="frmScmAdjust" action="./scm_adjust_ps.php" method="post">
        ...
        if (empty($data) === false && is_array($data)) {
            foreach ($data as $key => $val) {
                ...
                <tr class="text-center">
                    ...
                    <td class="text-left">
                        <div class="goods_name one_line hand" title="주문 상품명" onclick="goods_register_popup('<?= $val['goodsNo']; ?>');"><?= gd_html_cut(gd_remove_only_tag(stripslashes($val['goodsNm'])), 46, '..'); ?></div>
                        <?php
                        // 옵션 처리
    ...
    <div class="table-action">
        ...
        <div class="notice-danger" style="clear: both; margin-left: 13px;">
            여러 공급사의 주문상품을 선택하여 정산요청 시 가장 상단의 공급사 주문상품만 정산요청됩니다.
        </div>
    </div>
</form>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#frmSearchGoods #key').change(function (e){
