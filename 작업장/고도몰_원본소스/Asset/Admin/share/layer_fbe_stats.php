...
<div>
    ...
    <div>
        <table class="table table-cols no-title-line">
            ...
            <tr>
                <th>메타 FBE<br/>상품노출 현황</th>
                <td><span class="js-stats-fbe"><?=number_format($statsData['fbe'])?></span>개의 상품을 메타 FBE에 노출 중입니다. (총 상품 수 : <span class="js-stats-total"><?=number_format($statsData['total'])?></span>개)</td>
                <td><button type="button" class="btn btn-sm btn-gray js-fbe-stats-load">새로고침</button></td>
            </tr>
        </table>
    </div>
</div>
...
<div>
    <p class="notice-info">메타 FBE에 노출되는 상품수를 카테고리별로 확인하실 수 있습니다.</p>
    <table class="table table-rows table-fixed">
        ...
        <tr>
            ...
            <?php } ?>
            <th>카테고리명</th>
            <th class="width10p">메타 FBE<br/>노출 상품 수</th>
        ...
        if ((gd_isset($data) && is_array($data)) || ($search['categoryNoneFl'] =='y' && empty($search['cateNm']) === true && empty($search['cateGoods'][0]) === true)) {
            if ($search['categoryNoneFl'] =='y' && empty($search['cateNm']) === true && empty($search['cateGoods'][0]) === true) { ?>
                <tr>
                    ...
                    <td class="center"><?= number_format($goodsFbeStats['fbe']['']); ?></td>
                    <td class="center"><?= number_format($goodsFbeStats['total']['']); ?></td>
                    <td class="center">-</td>
                </tr>
            <?php } else {
                foreach ($data as $key => $val) { ?>
                    <tr>
                        ...
                        <td class="center"><?= number_format($goodsFbeStats['fbe'][$val['cateCd']]); ?></td>
                        <td class="center"><?= number_format($goodsFbeStats['total'][$val['cateCd']]); ?></td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(".js-fbe-stats-load").click(function(e){
            get_fbe_stats();
        });
    });

    function get_fbe_stats() {
        $(".loading").show();
        $.post('../goods/goods_ps.php', {'mode': 'get_fbe_stats'}, function (data) {
            $(".loading").hide();
            var getData = $.parseJSON(data);
            if (getData.fbe && getData.total) {
                $(".js-stats-fbe").html(numeral(getData.fbe).format('0,0'));
    ...
    function layer_list_search(pagelink) {
        var cateNm = $('.js-form-fbe-stats input[name="cateNm"]').val();
        var categoryNoneFl = $('.js-form-fbe-stats input[name="categoryNoneFl"]:checked').val();
        var cateGoods = '';
        ...
        $(".loading").show();
        $.get('../share/layer_fbe_stats.php', parameters, function (data) {
