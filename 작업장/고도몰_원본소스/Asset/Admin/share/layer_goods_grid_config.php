...
<div class="goods-grid-area">
	<form name="goodsGridForm" id="goodsGridForm" action="./goods_ps.php" method="post" target="ifrmProcess">
		<input type="hidden" name="mode" value="get_goods_admin_grid_list" />
		<input type="hidden" name="goodsGridMode" value="<?=$goodsGridMode?>" />
		...
		<div class="goods-grie-bottom-info-area">
			...
			<?php if ($goodsGridMode != 'regular_goods_list') : ?>
				<table class="table-cols">
					<tbody>
					<tr>
						<th class="add-display-th">노출항목 추가 설정</th>
						<td class="add-display-td">
							<?php
							if(gd_is_provider() === false) {
							?>
							<label>
								<input type='checkbox' value='y' data-kind="best" name="listDisplay[best]"/>
								<img src='/admin/gd_share/img/bl_required.png' style="padding-right:2px;">인기상품 진열
							</label>
							<label>
								<input type='checkbox' value='y' data-kind="main" name="listDisplay[main]"/>
								<img src='/admin/gd_share/img/bl_required.png' style="padding-right:2px;">메인분류
							</label>
							<?php
							}
							?>
							<label>
								<input type='checkbox' value='y' data-kind ="cate" name="listDisplay[cate]"/>
								<img src='/admin/gd_share/img/bl_required.png' style="padding-right:2px;">카테고리
							</label>
							<label><input type='checkbox' value='y' data-kind ="icon" name="listDisplay[icon]"/>아이콘</label>
							<label><input type='checkbox' value='y' data-kind ="color" name="listDisplay[color]"/>대표색상</label>
						</td>
					</tr>
					</tbody>
				</table>

				<div class="notice-danger">"<img src='/admin/gd_share/img/bl_required.png'>" 표기된 항목이 노출되거나, 노출 조회항목이 많은 경우 상품 검색 속도가 느려질 수 있습니다.</div>
			<?php endif; ?>
		</div>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$(".js-save").click(function(e){
			$("input[name='mode']").val('save_goods_admin_grid_list');
			$("#goodsGridForm").submit();
		});
		...
		$(".js-add").click(function(e){
			...
			$(".js-field-default tr.default_select").each(function () {
				...
				if(check == true){
					...
					newClone.append("<input type='hidden' name='goodsGridList[]' value='"+$(this).data("field-key")+"' />");
		...
		$(".js-goods-grid-sort").change(function(){
			...
			$.post('goods_ps.php', $("#goodsGridForm").serialize(), function (data) {
			...
			if(data){
				...
				if(goods_grid_data_select){
					...
					$.each(goods_grid_data_select, function (key, val) {
						...
						contentsHtml+= val;
						contentsHtml+= "<input type='hidden' name='goodsGridList[]' value='"+key+"' />";
						contentsHtml+= "</td></tr>";
					});
