<div class="box option-layer">
    <form name="frmViewLayer" id="frmViewLayer" method="post">
        ...
        <input type="hidden" name="scmNm" value="<?= $goodsView['scmNm'] ?>" />
        ...
        <input type="hidden" name="regularPrice" id="regularPrice" value="<?= $regularGoodsData['regularPrice'] ?>" />
        <div class="view">
            <div class="scroll-box">
                <div class="regular_goods_layout">
                    <div class="img_section">
                        <?= $goodsView['image']['detail']['thumb'][0] ?>
                    </div>
                    <div class="content_section">
                        <div class="goods_name">
                            <h3 class="goods_name_title">
                            ...
                            <div class="benefits" style="display:none">
                                <strong>구매혜택</strong>
                                <div>
                                    <p class="sale">할인 : <strong class="total_benefit_price"></strong>
                                        <span class="benefit_price"></span></p>
                                    <p class="mileage">적립 <?= gd_display_mileage_name() ?> :
                                        <strong class="total_benefit_mileage"></strong>
                                        <span class="benefit_mileage"></span></p>
                                </div>
                            </div>
                            <div>
                                <div class="form-inline">
                                    ...
                                    <?php if ($goodsView['multipleDeliveryFl'] === true) : ?>
                                        ...
                                        <?php elseif (gd_isset($goodsView['delivery']['basic']['fixFl']) == 'fixed') : ?>
                                            <span><?= gd_currency_symbol() ?><?= gd_money_format($goodsView['delivery']['firstCharge']['0']['price']) ?><?= gd_currency_string() ?></span>
                                    ...
                                    <?php else : ?>
                                        <?php if (gd_isset($goodsView['delivery']['basic']['fixFl']) == 'free') : ?>
                                            ...
                                            <input type="hidden" name="deliveryPrice" value="0">
                                        <?php elseif (gd_isset($goodsView['delivery']['basic']['fixFl']) == 'fixed') : ?>
                                            <span><?= gd_currency_symbol() ?><?= gd_money_format($goodsView['delivery']['charge']['0']['price']) ?><?= gd_currency_string() ?></span>
                            ...
                            <!-- 배송방식 -->
                            <?php if (gd_isset($goodsView['delivery']['basic']['fixFl']) != 'free' && count($goodsView['delivery']['basic']['deliveryMethodFlData']) > 0) : ?>
                                <div style="margin-top: 5px;">
                                    <div class="form-inline">
                                        <strong>배송방식</strong>
                                        <?php if (count($goodsView['delivery']['basic']['deliveryMethodFlData']) == 1) : ?>
                        ...
                        <?php if ($goodsView['optionFl'] == 'y') : ?>
                            <?php if ($goodsView['optionDisplayFl'] == 's') : ?>
                                <div class="add_goods_choice choice">
                                    <div class="list">
                                        <strong>
                                            <?php if ($goodsView['optionEachCntFl'] == 'one' && empty($goodsView['optionName']) === false) : ?>
                                            ...
                                            <select name="optionSnoInput" class="form-inline" style="width:100%" onchange="goodsViewLayerController.option_price_display(this);">
                                                ...
                                                <?php foreach ($goodsView['option'] as $val) : ?>
                                                    <?php if ($val["optionViewFl"] == 'y') : ?>
                                                        <option value="<?= $val["sno"] ?><?= INT_DIVISION ?><?= gd_money_format($val["optionPrice"], false) ?><?= INT_DIVISION ?><?= $val["mileage"] ?><?= INT_DIVISION ?><?= $val["stockCnt"] ?><?= STR_DIVISION ?><?= $val["optionValue"] ?><?= STR_DIVISION ?><?php if ($goodsView['stockFl'] == 'y' && $val['optionSellFl'] == 't') { ?><?= $optionSoldOutCode[$val['optionSellCode']] ?><?php } ?><?= STR_DIVISION ?><?php if ($val['optionDeliveryFl'] == 't' && !empty($optionDeliveryDelayCode[$val['optionDeliveryCode']])) { ?><?= $optionDeliveryDelayCode[$val['optionDeliveryCode']] ?><?php } ?>" <?php if (($goodsView['stockFl'] == 'y' && $goodsView['stockCnt'] < $goodsView['minOrderCnt']) || ($goodsView['stockFl'] == 'y' && $goodsView['fixedOrderCnt'] == 'option' && $val['stockCnt'] < $goodsView['minOrderCnt']) || ($goodsView['stockFl'] == 'y' && $val['stockCnt'] == '0') || $val["optionSellFl"] == 'n' || $val["optionSellFl"] == 't'){ ?>disabled="disabled"<?php } ?> <?php if (gd_isset($TPL_VAR["optionInfo"]['optionSno']) && $TPL_VAR["optionInfo"]['optionSno'] == $val["sno"]) { ?> selected='selected' <?php } ?>>
                                                            <?= $val["optionValue"] ?>
                                                            : <?= gd_currency_symbol() ?><?= gd_money_format($val["optionPrice"]) ?><?= gd_currency_string() ?>
                                                            <?php if ($val["optionSellFl"] == 't') : ?>
                                                                [<?= $optionSoldOutCode[$val["optionSellCode"]] ?>]
                                                            <?php elseif (($goodsView['stockFl'] == 'y' && $val["stockCnt"] < 1) || $val["optionSellFl"] == 'n') : ?>
                                                                [<?= $optionSoldOutCode['n'] ?>]
                                                            <?php else : ?>
                                                                <?php if ($goodsView['stockFl'] == 'y') : ?>
                                                                    : <?= number_format($val["stockCnt"]) ?>개
                                                                <?php endif; ?>
                                                            <?php endif; ?>
                                                            <?php if ($val['optionDeliveryFl'] == 't' && !empty($val['optionDeliveryCode'])) : ?>
                                                                [<?= $optionDeliveryDelayCode[$val['optionDeliveryCode']] ?>]
                                                            <?php endif; ?>
                                                        </option>
                                                    <?php endif; ?>
                                                <?php endforeach; ?>
                            ...
                            <?php elseif ($goodsView['optionDisplayFl'] == 'd') : ?>
                                <?php foreach ($goodsView['optionName'] as $k => $val) : ?>
                                    <?php if ($k == 0) : ?>
                                        <input type="hidden" name="optionSnoInput" value="<?= gd_isset($TPL_VAR["optionInfo"]['optionSnoText']) ?>" />
                                    ...
                                    <div class="add_goods_choice choice">
                                        <div class="list">
                                            <strong><?= $val ?></strong>
                                            <div>
                                                <select name="optionNo_<?= $k ?>" class="tune" style="width:100%;" onchange="goodsViewLayerController.option_select(this,'<?= $k ?>', '<?= gd_isset($goodsView['optionName'][($k + 1)]) ?>');"<?php if ($k > 0) : ?> disabled="disabled"<?php endif; ?>>
                                                    <option value="">
                                                        =
                                                        <?php if ($k == 0) : ?>
                                                            <?= $val ?> 선택
                                                        <?php else : ?>
                                                            <?= $goodsView['optionName'][($k - 1)] ?>을 먼저 선택해 주세요
                                                        <?php endif; ?> =
                                                    </option>
                                                    <?php if ($k == 0) : ?>
                                                        <?php foreach ($goodsView['optionDivision'] as $key => $TPL_V2) : ?>
                                                            <option value="<?= $TPL_V2 ?>" <?php if (($goodsView['stockFl'] == 'y' && $goodsView['stockCnt'] < $goodsView['minOrderCnt']) || ($goodsView['stockFl'] == 'y' && $goodsView['fixedOrderCnt'] == 'option' && isset($goodsView['optionDivisionStock']) && $goodsView['optionDivisionStock'][$key]['stockCnt'] < $goodsView['minOrderCnt']) || ($goodsView['stockFl'] == 'y' && $goodsView['optionDivisionStock'][$key]['stockCnt'] == '0') || $goodsView['optionDivisionStock'][$key]['optionSellFl'] == 'n' || $goodsView['optionDivisionStock'][$key]['optionSellFl'] == 't') { ?> disabled="disabled"<?php } ?> >
                                                                <?= $TPL_V2 ?>
                                                                <?php if ($goodsView['optionDivisionStock'][$key]['optionSellFl'] == 't') : ?>
                                                                    [<?= $optionSoldOutCode[$goodsView['optionDivisionStock'][$key]['optionSellCode']] ?>]
                                                                <?php elseif (($goodsView['stockFl'] == 'y' && $goodsView['optionDivisionStock'][$key]['stockCnt'] < 1) || $goodsView['optionDivisionStock'][$key]['optionSellFl'] == 'n') : ?>
                                                                    [<?= $optionSoldOutCode['n'] ?>]
                                                                <?php endif; ?>
                                                            </option>
                                                        <?php endforeach; ?>
                                    ...
                                    <div id="iconImage_<?= $k ?>" class="option_icon"></div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        <?php endif; ?>
                        <!--  옵션 끝 -->
                            ...
                            <?php foreach ($goodsView['optionText'] as $k => $val) : ?>
                                <div class="add_goods_choice choice">
                                    <div class="list">
                                        ...
                                        <?php endif; ?>
                                        <strong>
                                            <span class="optionTextNm_<?= $k ?>"><?= $val["optionName"] ?><?php if ($val["mustFl"] == 'y') : ?><em>(필수)</em><?php endif; ?></span>
                                            <input type="hidden" name="optionTextMust_<?= $k ?>" value="<?= $val["mustFl"] ?>" />
                                            <input type="hidden" name="optionTextLimit_<?= $k ?>" value="<?= $val["inputLimit"] ?>" />
                                        </strong>
                                        <div class="option_input optionTextDisplay<?= $val["sno"] ?>">
                                            <span class="txt-field">
                                                <input type="hidden" name="optionTextSno_<?= $k ?>" value="<?= $val["sno"] ?>" />
                                                <input type="text" name="optionTextInput_<?= $k ?>" value="" size="25" class="text" data-sno="<?= $val["sno"] ?>" onkeydown="goodsViewLayerController.enterKey(this)" onchange="goodsViewLayerController.option_text_select(this)" placeholder="<?= $val["inputLimit"] ?>글자를 입력하세요." />
                                                <input type="hidden" value="<?= $val["addPrice"] ?>" />
                                            </span>
                                            <?php if ($val["addPrice"] != 0) : ?>
                                                <span class="msg">※ 작성시 <?= gd_currency_symbol() ?><?= gd_money_format($val["addPrice"]) ?><?= gd_currency_string() ?> 추가</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <?php if ($goodsView['addGoods']) : ?>
                            <?php foreach ($goodsView['addGoods'] as $k => $val) : ?>
                                <div class="add_goods_choice choice <?php if ($k == '0') : ?>add <?php endif; ?>">
                                    <div class="list">
                                        <strong>
                                            <?= $val["title"] ?>
                                            <?php if ($val["mustFl"] == 'y') : ?>
                                                <em>(필수)</em>
                                                <input type="hidden" name="addGoodsInputMustFl[]" value="<?= $k ?>">
                                            <?php endif; ?>
                                        </strong>
                                        <div>
                                            <select name="addGoodsInput<?= $k ?>" data-key="<?= $k ?>" class="tune" style="width:100%;" onchange="goodsViewLayerController.add_goods_select(this)">
                                                <option value="">추가상품</option>
                                                <?php foreach ($val["addGoodsList"] as $TPL_V2) : ?>
                                                    <option value="<?= $TPL_V2["addGoodsNo"] ?><?= INT_DIVISION ?><?= $TPL_V2["goodsPrice"] ?><?= STR_DIVISION ?><?= $TPL_V2["goodsNm"] . '(' . $TPL_V2["optionNm"] . ')' ?><?= STR_DIVISION ?><?= rawurlencode(gd_html_add_goods_image($TPL_V2["addGoodsNo"], $TPL_V2["imageNm"], $TPL_V2["imagePath"], $TPL_V2["imageStorage"], 30, $TPL_V2["goodsNm"], '_blank')) ?><?= STR_DIVISION ?><?= $k ?><?= STR_DIVISION ?><?= $TPL_V2["stockUseFl"] ?><?= STR_DIVISION ?><?= $TPL_V2["stockCnt"] ?>" <?php if ($TPL_V2["soldOutFl"] == 'y' || ($TPL_V2["stockUseFl"] == '1' && $TPL_V2["stockCnt"] == '0')) : ?> disabled="disabled" <?php endif; ?>>
                                                        <?= $TPL_V2["goodsNm"] ?> (<?= $TPL_V2["optionNm"] ?> / <?= gd_currency_symbol() ?><?= gd_money_format($TPL_V2["goodsPrice"]) ?><?= gd_currency_string() ?> <?php if ($TPL_V2["soldOutFl"] == 'y' || ($TPL_V2["stockUseFl"] == '1' && $TPL_V2["stockCnt"] == '0')) : ?> / 품절 <?php endif; ?>)
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                    ...
                    <?php if ($goodsView['optionFl'] == 'y') : ?>
                        <div class="option_total_display_area" style="display:none">
                            <div class="regular_order_goods option_display_area"></div>
                        </div>
                    <?php else : ?>
                        <div class="regular_order_goods option_display_area">
                            <div class="option_display_item_0" id="option_display_item_0">
                                <div class="check optionKey_0">
                                    ...
                                    <input type="hidden" name="deliveryCycleTypes[]" value="" />
                                    <input type="hidden" name="deliveryCycles[]" value="" />
                                    <input type="hidden" name="deliveryCycleDays[]" value="" />
                                    <input type="hidden" name="maxDeliveryRounds[]" value="" />
                                    <span class="name">
                                        <strong><?= gd_isset($goodsView['goodsNmDetail']) ?></strong>
                                        <span id="option_regular_display_0"></span>
                                        <span class="option_text_display_0"></span>
                                    </span>
                                    <div class="price">
                                        <span class="count">
                                            <input type="text" class="text goodsCnt_0" title="수량" name="goodsCnt[]" value="<?= $goodsView['defaultGoodsCnt']; ?>" data-value="<?= $goodsView['defaultGoodsCnt']; ?>" data-stock="<?= $goodsView['totalStock'] ?>" data-key="0" onchange="goodsViewLayerController.input_count_change(this,'1');return false;">
                                            <span>
                                        ...
                                        <em><input type="hidden" value="<?= gd_isset($goodsView['option'][0]['optionPrice'], 0) ?>" name="optionPriceSum[]">
                                            <input type="hidden" value="<?= gd_isset($goodsView['option'][0]['optionPrice']) ?>" name="option_price_0"><?= gd_currency_symbol() ?>
                                            <strong class="option_price_display_0"><?= gd_money_format(gd_isset($goodsView['option'][0]['optionPrice'], 0), false) ?></strong><?= gd_currency_string() ?>
                                        </em>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="end-price" style="display:none">
                        ...
                        <?php if ($regularGoodsData['applyStatus'] === 'disabled') : ?>
                            <input type="button" value="판매 중지" class="btn btn-lg btn-gray disabled-goods-btn" />
                        <?php else : ?>
...
<style>
.add_goods_choice.add  {border-top: 1px solid #e9e9e9;}
.add_goods_choice .list strong  {display: table-cell; width: 81px; color: #555; padding: 0 10px 0; vertical-align: top;}
.add_goods_choice .list > div  {display: table-cell; width: 330px;}

.regular_goods_layout .add_goods_choice {width: 100%; padding-top: 15px;}
.regular_goods_layout .add_goods_choice > strong {white-space: nowrap;}
.regular_goods_layout .goods_name_title {font-size: 14px; font-weight: bold; color: #000;}
.regular_goods_layout .goods_name {border-bottom: 1px solid #e6e6e6; padding: 0 0 15px;}
.regular_goods_layout .img_section {border: 1px solid #dbdbdb; float: left; width: 70px; height: 70px; margin: 0 20px 0 5px;}
.regular_goods_layout .content_section {float: left; /*overflow: hidden; */ padding-right:0; margin: 0 0 15px 0;}
.regular_goods_layout .user_input {margin: 0 0 9px;}
.regular_goods_layout .regular_order_goods {clear: both;}

.regular_order_goods {background: #f5f5f5; border-top: none; border-bottom: 2px solid #ff4c2e;}
.regular_order_goods .check {display: table; padding: 13px 14px 13px 19px;}
.regular_order_goods > .check {border-top: 1px solid #dbdbdb;}
.regular_order_goods .add {padding: 10px 0 10px; border-top: 1px solid #dbdbdb;}
.regular_order_goods .add .check {padding: 0 14px 10px 19px;}
.regular_order_goods .add .check .name {padding: 0 0 0 28px; background: url('<?=PATH_ADMIN_GD_SHARE?>/img/self_order_member_cart/ap.png') no-repeat 7px 11px;}
.regular_order_goods .check .name > span {display: block; padding: 5px 0 8px;}
.regular_order_goods .check .name > span em {display: block; color: #777;}
.regular_order_goods .check > * {vertical-align: middle;}
.regular_order_goods .check .name {display: table-cell; width: 271px; padding: 0 10px 0 0; color: #555;}
.regular_order_goods .check .name > strong {color: #111; font-size: 13px;}
.regular_order_goods .check .name > strong.btm-txt {display: inline-block; padding-top: 7px;}
.regular_order_goods .check .name > a img {margin: 0 0 0 2px; vertical-align: -4px;}
.regular_order_goods .add .check .name img {width: 31px; height: 31px; margin: 0 4px 0 0; border: 1px solid #ccc; vertical-align: middle;}
.regular_order_goods .check .price {display: table-cell; width: 210px; padding: 0 13px; text-align: right;}
.regular_order_goods .check .price .count {display: inline-block; vertical-align: middle;}
.regular_order_goods .check .price .count .text {float: left; width: 43px; height: 33px; border: 1px solid #ccc; color: #3f3f3f; line-height: 31px; text-align: center;}
.regular_order_goods .check .price .count span {float: left; margin: 0 0 0 -1px;}
.regular_order_goods .check .price .count span button {display: block; width: 23px; height: 17px; text-indent: -9999px;}
.regular_order_goods .check .price .count span button.up {border:0; background: url('<?=PATH_ADMIN_GD_SHARE?>/img/count-up.png') no-repeat left top;}
.regular_order_goods .check .price .count span button.down { margin: -1px 0 0; background: url('<?=PATH_ADMIN_GD_SHARE?>/img/count-down.png') no-repeat left top; border:0;}
.regular_order_goods .check .price em {display: inline-block; min-width: 87px; padding: 0 0 0 15px; color: #333; font-size: 13px; text-align: right; vertical-align: middle;}
.regular_order_goods .check .price em strong {font-family: arial;}
.regular_order_goods .check .del {display: table-cell;}
.regular_order_goods .check .del button {width: 16px; height: 16px; background: url('<?=PATH_ADMIN_GD_SHARE?>/img/del.png') no-repeat left top; text-indent: -9999px; border:0;}
.scroll-box {overflow-x: hidden; overflow-y: auto; height: 480px; padding:0; margin:0;}
.end-price {clear: both;}
</style>
...
<script type="text/javascript">
    var gd_goods_view = function () {
        ...
        this.option_text_select = function (inputName) {
            ...
            if (displayOptionDisplay) {
                ...
                if ($(frmId + ' div.optionKey_' + tmpStr[0] + optionTextKey).length) {
                    ...
                    // 문구 옵션을 모두 작성한 상태일 경우 모두 초기화 처리
                    var textInputList = $(frmId + ' input[name*=\'optionTextInput\']');
                    if (textInputList.filter((_, el) => $(el).val().trim() === '').length === 0) {
                        textInputList.val('');
                    }
                }
            } else {
        ...
        this.goods_calculate = function (frmId, goodsFl, itemNo, goodsCnt) {
            // 정기배송 상품일 경우 정기배송가로 계산
            goodsPrice = parseFloat($('#frmViewLayer input[name="regularPrice"]').val()) || 0;
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        var parameters = {
            ...
            'setAddGoodsFl': '<?= $goodsView['addGoods'] ? 'y' : 'n' ?>',
        ...
        $(".select-goods-btn").on('click', function (e) {
            ...
            if (typeof prepareRegularGoodsPayload === 'function') {
                prepareRegularGoodsPayload();
            }
        });

        $('.disabled-goods-btn').on('click', function () {
            alert("판매 중지 상태인 정기배송 상품입니다.");
        });
        ...
        var optionText = $(".optionTextDisplay<?= $k?>").find("input[name*='optionTextInput_']");
        $(optionText).val("<?= $val?>");
        <?php endforeach; ?>
        ...
        $("#frmViewLayer select[name*='addGoodsInput']").each(function (key) {
            ...
            if ($(this).find("option[value*='<?= $val?>']").length) {
                ...
                $("#frmViewLayer .addGoodsCnt_" + optionKey + '_<?= $val?>').val('<?= $optionInfo['addGoodsCnt'][$k]?>');
                goodsViewLayerController.goods_calculate("#frmViewLayer", '', optionKey + '<?= INT_DIVISION ?><?= $val?>', '<?= $optionInfo['addGoodsCnt'][$k]?>');
            }
    ...
    function benefit_calculation() {
        ...
        if ($("#frmViewLayer input[name*='goodsNo']").length == 0) {
            parameters += "&goodsNo%5B%5D=<?= $goodsView['goodsNo'] ?>&goodsCnt%5B%5D=1&couponBenefitExcept=1";
        }
        ...
        $.post('../order/order_ps.php', parameters, function (data) {
            ...
            if (getData.totalDcPrice > 0 || getData.totalMileage > 0) {
                ...
                if (getData.totalDcPrice > 0) {
                    ...
                    if (getData.goodsDcPrice) {
                        tmp.push("상품 : " + " <?= gd_currency_symbol() ?>" + numeral(getData.goodsDcPrice).format() + "<?= gd_currency_string() ?>");
                        $("#set_dc_goods_price").val(getData.goodsDcPrice);
                    }
                    if (getData.memberDcPrice) {
                        tmp.push("회원 : " + " <?= gd_currency_symbol() ?>" + numeral(getData.memberDcPrice).format() + "<?= gd_currency_string() ?>");
                        $("#set_dc_member_price").val(getData.memberDcPrice);
                    }
                    if (getData.myappDcPrice) {
                        tmp.push(" 모바일앱 : " + " <?= gd_currency_symbol() ?>" + numeral(getData.myappDcPrice).format() + "<?= gd_currency_string() ?>");
                        $("#set_dc_myapp_price").val(getData.myappDcPrice);
                    }

                    $(".benefit_price").html("(" + tmp.join() + ")");

                    $(".total_benefit_price").html("-<?= gd_currency_symbol() ?>" + numeral(getData.totalDcPrice).format() + "<b><?= gd_currency_string() ?></b>");
                ...
                if (getData.totalMileage > 0) {
                    ...
                    var tmp = new Array();
                    if (getData.goodsMileage) tmp.push("상품 : " + numeral(getData.goodsMileage).format() + "<?= $mileageData['unit'] ?>");
                    if (getData.memberMileage) tmp.push("회원 : " + numeral(getData.memberMileage).format() + "<?= $mileageData['unit'] ?>");

                    $(".benefit_mileage").html("(" + tmp.join() + ")");

                    $(".total_benefit_mileage").html("+" + getData.totalMileage + "<?= $TPL_VAR["mileageData"]['unit'] ?>");
                } else {
                    $(".benefits p.mileage").hide();
                }
            ...
            $(".total_price").html(" <?= gd_currency_symbol() ?> " + numeral(totalPrice).format() + "<b><?= gd_currency_string() ?></b>");
    ...
    // 정기배송 배송정보 변경 관련
    function prepareRegularGoodsPayload () {
        const regularGoodsPayload = {};

        // 신청 정보 설정
        regularGoodsPayload.mode = 'update_regular_goods';
        regularGoodsPayload.applyNo = parseInt($('#frmViewLayer input[name="applyNo"]').val());
        regularGoodsPayload.goodsNo = parseInt('<?= $goodsView['goodsNo'] ?>');
        regularGoodsPayload.goodsCnt = parseInt($('#frmViewLayer input[name="goodsCnt[]"]').val());
        regularGoodsPayload.regularGoodsNo = parseInt('<?= $regularGoodsData['regularGoodsNo'] ?>');

        // 선택 옵션 설정
        regularGoodsPayload.optionSno = $('#frmViewLayer input[name="optionSno[]"]').val();

        // 문구 옵션 설정
        const optionTextSnoList = [];
        const optionTextValueList = [];
        if ($('#frmViewLayer input[name^="optionText[0]["]').length > 0) {
            const optionText = JSON.parse('<?= json_encode($goodsView["optionText"]); ?>');
            $('#frmViewLayer input[name^="optionText[0]["]').each(function() {
                const inputOptionText = $(this).attr('name');
                const optionTextSno = inputOptionText.match(/\[0\]\[(\d+)\]/)[1];
                optionTextSnoList.push(parseInt(optionTextSno));
                optionTextValueList.push($(this).val());
            });
        }
        regularGoodsPayload.optionTextSno = optionTextSnoList;
        regularGoodsPayload.optionTextValue = optionTextValueList;

        // 추가 상품 설정
        const addGoodsNoList = []
        const addGoodsCntList = [];
        if ($('#frmViewLayer input[name^="addGoodsNo[0][]"]').length > 0) {
            $('#frmViewLayer input[name^="addGoodsNo[0][]"]').each(function(index) {
                const addGoodsNo = $(this).val();
                const addGoodsCnt = $('#frmViewLayer input[name^="addGoodsCnt[0][]"]').eq(index).val();
                addGoodsNoList.push(addGoodsNo);
                addGoodsCntList.push(addGoodsCnt);
            });
        }
        regularGoodsPayload.addGoodsNo = addGoodsNoList;
        regularGoodsPayload.addGoodsCnt = addGoodsCntList;

        // 변경할 정기배송 상품 선택 처리
        selectRegularGoodsOption(regularGoodsPayload);
    }

    // 정기배송 상품변경 처리
    function selectRegularGoodsOption(regularGoodsPayload) {
        $.post('../order/layer_regular_order_ps.php', regularGoodsPayload, function (data) {
            if (data.result === 'success') {
                dialog_alert(data.message, '상품변경', {
                    callback: function() {
                ...
                dialog_alert(data.message, '경고', {location: window.location.href});
            }
        });
    }
</script>
...
<script type="text/html" id="optionTemplate">
    <div id="option_display_item_<%=displayOptionkey%>" style="border-top:1px solid #dbdbdb;">
        <div class="check optionKey_<%=optionSno%>">
            ...
            <div class="price">
                <span class="count">
                    <input type="text" class="text goodsCnt_<%=displayOptionkey%>" title="수량" name="goodsCnt[]" value="<?= $goodsView['defaultGoodsCnt']; ?>" data-value="<?= $goodsView['defaultGoodsCnt']; ?>" data-stock="<%=optionStock%>"   data-key="<%=displayOptionkey%>" onchange="goodsViewLayerController.input_count_change(this,'1');return false;">
                    <span>
