...
class Cart
{
    ...
    public $totalMemberBrandDcPrice;
    public $totalDcPrice;
    public $checkUseBundle;
    public $useBundleCnt;
    ...
    public function getCartInfo($cartSno = null, $cartField = null, $arrBind = null, $dataArray = false, $stripSlashesFl = true)
    {
        ...
        if ($cartSno) {
            ...
            if (is_array($cartSno) === true) {
                ...
                if ($this->db->strWhere) {
                    $this->db->strWhere = " (" . @gd_implode(' OR ', $arrWhere) . ") AND " . $this->db->strWhere;
                } else {
    ...
    public function saveInfoCart($arrData, $tempCartPolicyDirectOrder = 'n', $channel = '')
    {
        ...
        if($this->isWrite !== true && $this->isWriteMemberCartAdd !== true) { //수기 주문의 경우 체크하지 않음
            if (empty($arrData['couponApplyNo']) === false && gd_count($arrData['couponApplyNo']) > 0) {
    ...
    public function setInflowGoods($goodsNo)
    {
        ...
        if (empty($parse['inflow']) === false && $parse['goodsNo'] == $goodsNo) {
            ...
            $inflowGoods[] = $goodsNo;
            \Cookie::set('inflow_goods', json_encode(gd_array_unique($inflowGoods)));
            \Cookie::set('inflow', $parse['inflow']);
        }
    }
    ...
    public function saveGoodsToCart($arrData)
    {
        ...
        } elseif ($arrData['cartMode'] == 'present') {
            // 선물하기 설정
            $arrData['directCart'] = 'present';
            $check['cnt'] = 0;

            // 선물하기 쿠키 생성
            Cookie::set('isPresentCart', true, 0, '/');
        } else {
    ...
    public function updateInfoCart($arrData)
    {
        ...
        foreach ($arrData['goodsNo'] as $goodsIdx => $goodsNo) {
            foreach ($fieldData as $field) {
                if (gd_in_array($field, ['deliveryCollectFl', 'deliveryMethodFl']) === true) {
    ...
    public function getCartMemberCoupon($arrBind)
    {
        if (is_array($arrBind['param'])) {
            $addWhere = gd_implode(' AND ', $arrBind['param']);
        } else {
    ...
    protected function checkDuplicationGoods($arrData)
    {
        ...
        $strSQL = "SELECT count(goodsNo) as cnt, sno, goodsCnt, addGoodsNo, addGoodsCnt FROM " . $this->tableName . " WHERE " . gd_implode(' AND ', $arrBind['param']) . $strWhere . " ORDER BY sno ASC";
        $getData = $this->db->query_fetch($strSQL, $arrBind['bind'], false);
    ...
    protected function setInsetCart($arrData)
    {
        ...
        // 장바구니 통계 저장 (바로구매, 선물하기, 수기주문은 제외)
        if (!in_array($arrData['directCart'], ['y', 'present']) && $this->isWrite !== true) {
    ...
    protected function getBuyableStock($goodsNo, $goodsCnt, $useBundleGoods = '')
    {
        ...
        if ($this->checkUseBundle !== true) {
            $strCntSQL = "SELECT COUNT(*) as cnt FROM " . DB_CART . " WHERE useBundleGoods = 1";
    ...
    public function setMemberCouponApplyOrderWrite($cartSno, $memberCouponNo, $memberCartAddTypeCouponNo='')
    {
        ...
        if ($memberCouponNo){
            ...
            if(trim($memberCartAddTypeCouponNo) !== ''){
                ...
                $newMemberCouponNo = gd_implode(INT_DIVISION, $newMemberCouponNoArr);
            ...
            if ($memberCouponUsable) {
                ...
                $returnCouponNo = gd_implode(INT_DIVISION, gd_array_intersect($memberCartAddTypeCouponNoArr, $memberCouponNoArr));
            } else {
                throw new AlertBackException(__('사용 할 수 없는 쿠폰 입니다.'));
            }
        }
    ...
    public function setMemberCouponDelete($cartSno, $deleteMemberCouponNo = null)
    {
        ...
        if ($memberCouponNo['memberCouponNo'] > 0) {
            ...
            if($this->isWrite === true && $this->useRealCart !== true){
                ...
                $memberCouponNoList = explode(INT_DIVISION, $memberCouponNo['memberCouponNo']);
                $placeholders = implode(',', array_fill(0, count($memberCouponNoList), '?'));

                array_walk($memberCouponNoList, function ($couponNo) use (&$arrBind) {
                    $this->db->bind_param_push($arrBind, 'i', $couponNo);
                });

                $validateSql = "SELECT memberCouponNo, memberCouponState FROM ". DB_MEMBER_COUPON ." WHERE memberCouponNo IN ($placeholders)";
                $validateData = $this->db->query_fetch($validateSql, $arrBind);
                ...
                if (empty($arrInitMemberCouponNo) === false) {
                    $initMemberCouponNo = gd_implode(INT_DIVISION, $arrInitMemberCouponNo);
    ...
    public function setCartDelete($getData)
    {
        ...
        // 회원 로그인 체크
        if ($this->isLogin === true) {
            $arrBind['param'] = 'sno IN (' . gd_implode(' , ', $param) . ') AND memNo = ?';
            $this->db->bind_param_push($arrBind['bind'], 'i', $this->members['memNo']);
        } else {
            $arrBind['param'] = 'sno IN (' . gd_implode(' , ', $param) . ') AND  siteKey = ?';
            $this->db->bind_param_push($arrBind['bind'], 's', $this->siteKey);
        }
    ...
    protected function setDeleteDirectCart()
    {
        ...
        if($session->get('related_goods_order') == 'y'){
            ...
            // 바로구매와 선물하기 삭제
            if ($this->isLogin === true) {
                $arrBind['param'] = 'memNo = ? AND directCart IN (\'y\', \'present\')';
                $this->db->bind_param_push($arrBind['bind'], 'i', $this->members['memNo']);
            } else {
                $arrBind['param'] = 'siteKey = ? AND directCart IN (\'y\', \'present\')';
                $this->db->bind_param_push($arrBind['bind'], 's', $this->siteKey);
            }
            // 장바구니 삭제 전 쿠폰 장바구니사용 상태 되돌리기
    ...
    public function setCartToWish($getData)
    {
        ...
        if ($this->isLogin === true) {
            ...
            $strWhere = 'sno IN (' . gd_implode(' , ', $param) . ') AND  siteKey = ?';
        ...
        $strSQL = 'INSERT INTO ' . DB_WISH . ' (' . gd_implode(', ', $insertFieldData) . ', regDt) SELECT ' . gd_implode(', ', $selectFieldData) . ', now() FROM ' . $this->tableName . ' WHERE ' . $strWhere;
    ...
    public function getCartGoodsData($cartIdx = null, $address = null, $tmpOrderNo = null, $isAddGoodsDivision = false, $isCouponCheck = false, $postValue = [], $setGoodsCnt = [], $setAddGoodsCnt = [], $setDeliveryMethodFl = [], $setDeliveryCollectFl = [], $deliveryBasicInfoFl = false)
    {
        ...
        if (empty($cartIdx) === false) {
            if (is_array($cartIdx)) {
                ...
                if (empty($tmpWhere) === false) {
                    ...
                    $arrWhere[] = 'c.sno IN (' . gd_implode(' , ', $tmpAddWhere) . ')';
                }
                unset($tmpWhere);
        ...
        // 바로 구매 설정
        if (Request::getFileUri() != 'cart.php' 
            && (Request::getFileUri() != 'order_ps.php' || (Request::getFileUri() == 'order_ps.php' && gd_in_array(Request::post()->get('mode'), ['set_recalculation']) === true))
        ) {
            // 바로구매 조건
            if (Cookie::has('isDirectCart') 
                && $this->cartPolicy['directOrderFl'] == 'y' 
            ...
            // 선물하기 조건
            } elseif (Cookie::has('isPresentCart')) {
                $arrWhere[] = 'c.directCart = ?';
                $this->db->bind_param_push($arrBind, 's', 'present');
            }
        } else {
            // 바로 구매 쿠키 삭제
            if (Cookie::has('isDirectCart')) {
        ...
        if($mallBySession) {
            ...
            $strSQLGlobal = "SELECT gg." . gd_implode(', gg.', $arrFieldGoodsGlobal) . " FROM ".$this->tableName." as c INNER JOIN ".DB_GOODS_GLOBAL." as gg ON  c.goodsNo = gg.goodsNo AND gg.mallSno = '".$mallBySession['sno']."'  WHERE " . gd_implode(' AND ', $arrWhere) ;
        ...
        while ($data = $this->db->fetch($result)) {
            ...
            if ((\Component\Order\OrderMultiShipping::isUseMultiShipping() == true || $postValue['isAdminMultiShippingFl'] === 'y') && empty($setAddGoodsCnt) === false) {
                ...
                foreach ($setAddGoodsCnt[$data['sno']] as $aKey => $aVal) {
                    $tmpAddGoodsNoKey = gd_array_search($aKey, $addGoodsNo);
            ...
            //매입처 관련 정보
            if(gd_is_plus_shop(PLUSSHOP_CODE_PURCHASE) === false || (gd_is_plus_shop(PLUSSHOP_CODE_PURCHASE) === true && !gd_in_array($data['purchaseNo'],gd_array_keys($purchaseData))))  {
            ...
            //구매불가 대체 문구 관련
            if($data['goodsPermissionPriceStringFl'] =='y' && $data['goodsPermission'] !='all' && (($data['goodsPermission'] =='member'  && $this->isLogin === false) || ($data['goodsPermission'] =='group'  && !gd_in_array($this->_memInfo['groupSno'],explode(INT_DIVISION,$data['goodsPermissionGroup']))))) {
            ...
            $soldOutStateCodeArray = [self::POSSIBLE_SOLD_OUT, self::POSSIBLE_OPTION_SELL_NO, self::POSSIBLE_STOCK_OVER];
            if ($this->cartPolicy['soldOutFl'] == 'n' && in_array($data['orderPossibleCode'], $soldOutStateCodeArray)) {
                $_delCartSno[] = $data['sno'];
                unset($data);
                continue;
            }
            ...
            if (gd_in_array('coupon', $exceptBenefit) === true && ($data['exceptBenefitGroup'] == 'all' || ($data['exceptBenefitGroup'] == 'group' && gd_in_array($this->_memInfo['groupSno'], $exceptBenefitGroupInfo) === true))) {
                if (empty($data['memberCouponNo']) === false) {
                    $exceptCouponNo[$data['sno']] = $data['memberCouponNo'];
                }
                $data['couponBenefitExcept'] = "y";
            ...
            if (\Component\Order\OrderMultiShipping::isUseMultiShipping() === true && $deliveryBasicInfoFl === true && empty($this->deliveryBasicInfo[$data['deliverySno']]) === true) {
                ...
                $this->deliveryBasicInfo[$data['deliverySno']]['deliveryMethodFl'] = gd_array_filter(explode(STR_DIVISION, $this->deliveryBasicInfo[$data['deliverySno']]['deliveryMethodFl']));
            }
        ...
        if (!gd_in_array('false', $this->payLimit)) { // 상품별 결제수단 체크에서 결제가능한 결제수단이 없는것으로 나왔을때는 처리필요없음
            if($this->isWrite === true){
                if (empty($this->_memInfo) === false && !gd_in_array('gb', $this->_memInfo['settleGb'])) {
                    if (empty($this->payLimit)) {
                        ...
                        if($this->_memInfo['settleGb'] != 'all') {
                            ...
                            $payLimit = gd_array_intersect($settleGb, $this->payLimit);
                            if (gd_count($this->payLimit) > 0 && !gd_in_array('false', $this->payLimit) && gd_count($payLimit) == 0) {
                                $this->payLimit = ['false'];
                            } else {
                                $this->payLimit = $payLimit;
                            }
                ...
                if (empty($this->_memInfo) === false) {
                    if (empty($this->payLimit)) {
                        ...
                        $payLimit = gd_array_intersect($this->_memInfo['settleGb'], $this->payLimit);
                        if (gd_count($this->payLimit) > 0 && !gd_in_array('false', $this->payLimit) && gd_count($payLimit) == 0) {
                            $this->payLimit = ['false'];
                        } else {
                            $this->payLimit = $payLimit;
        ...
        if (Globals::get('gGlobal.isFront') || $this->isAdminGlobal === true) {
            if ($address !== null) {
                $getCart = $this->getOverseasDeliveryDataInfo($getCart, gd_array_column($getData, 'deliverySno'), $address);
            }
        } else {
            // 복수배송지 사용시 배송정보 재설정
...
//                                $tmpAddGoods['price']['goodsPriceSubtotal'] = ($tmpAddGoods['price']['goodsPriceSum'] - $aVal['addGoodsMemberDcPrice'] - $aVal['addGoodsMemberOverlapDcPrice']);
    ...
    protected function getCartDataInfo($getData, $postValue = [])
    {
        ...
        $isAllFl = (gd_count($getData) > 1) ? 'T' : 'F';
        ...
        if($couponPolicy['productCouponChangeLimitType'] == 'n' && $this->isWrite === false) {
            ...
            if((Request::getFileUri() == 'cart_ps.php') && gd_in_array($postValue['mode'], $this->cartPsPassModeArray) == true) {
        ...
        $optionSellCode = gd_array_merge($deliverySellNew, $deliverySell);
        ...
        $optionDeliveryReasonCode = gd_array_merge($deliveryReasonNew, $deliveryReason);
        ...
        foreach ($getData as $dataKey => $dataVal) {
            ...
            if ($dataVal['exceptBenefitGroup'] == 'all' || ($dataVal['exceptBenefitGroup'] == 'group' && gd_in_array($this->_memInfo['groupSno'], $exceptBenefitGroupInfo) === true)) {
                $exceptBenefitFl = true;
                //회원 추가할인 제외
                if (gd_in_array('add', $exceptBenefit) === true) $getData[$dataKey]['addDcFl'] = false;
                //회원 중복할인 제외
                if (gd_in_array('overlap', $exceptBenefit) === true) $getData[$dataKey]['addDcFl'] = false;
            }
            ...
            if ($dataVal['addGoodsFl'] === 'y' && empty($dataVal['addGoodsNo']) === false) {
                foreach ($dataVal['addGoodsNo'] as $key => $val) {
                    ...
                    if ($this->_memInfo['fixedOrderTypeDc'] == 'brand') {
                        if (gd_in_array($tmp['brandCd'], $this->_memInfo['dcBrandInfo']->cateCd)) {
                    ...
                    foreach ($getData[$dataKey]['addGoodsMustFl'] as $aVal) {
                        if (gd_in_array($val, $aVal['addGoods']) === true) {
                    ...
                    $getData[$dataKey]['orderPossibleMessageList'] = gd_array_unique($getData[$dataKey]['orderPossibleMessageList']);
            ...
            if ($dataVal['couponBenefitExcept'] == 'n') {
                ...
                if ($dataVal['memberCouponNo']) {
                    ...
                    if (gd_array_search('LIMIT_MIN_PRICE', $tmp['memberCouponAlertMsg']) === false) {
                        if (is_array($tmp['memberCouponSalePrice'])) {
                            $goodsOptCouponSalePriceSum = gd_array_sum($tmp['memberCouponSalePrice']);
                        }
                        if (is_array($tmp['memberCouponAddMileage'])) {
                            $goodsOptCouponAddMileageSum = gd_array_sum($tmp['memberCouponAddMileage']);
                        }
                    } else {
            ...
            if ($dataVal['memberBenefitExcept'] == 'n' && $memberDcFl == true) {
                // 회원 추가 마일리지 적립 적용 제외
                if (gd_in_array('mileage', $exceptBenefit) === true && $exceptBenefitFl === true) {
                } else {
                ...
                if ($this->getChannel() != 'naverpay') {
                    if (empty($tmpMemberDcInfo[$goodsNo][$optionSno]) === false) {
                        ...
                        // 브랜드 할인율
                        if ($this->_memInfo['fixedOrderTypeDc'] == 'brand') {
                            if (gd_in_array($getData[$dataKey]['brandCd'], $this->_memInfo['dcBrandInfo']->cateCd)) {
                        ...
                        $getData[$dataKey]['memberDcInfo'] = json_encode(gd_array_merge($tmp, ['goodsCnt' => $dataVal['goodsCnt']]));
                    }
                    // 회원 추가 할인혜택 적용 제외
                    if (gd_in_array('add', $exceptBenefit) === true && $exceptBenefitFl === true) {
                    } else {
                        $getData[$dataKey]['addDcFl'] = $tmp['addDcFl'];
                        $getData[$dataKey]['price']['memberDcPrice'] = $tmp['memberDcPrice'];
                    }
                    // 회원 중복 할인혜택 적용 제외
                    if (gd_in_array('overlap', $exceptBenefit) === true && $exceptBenefitFl === true) {
                    } else {
            ...
            //회원등급 > 브랜드별 추가할인 상품 브랜드 정보
            if ($this->_memInfo['fixedOrderTypeDc'] == 'brand') {
                if (gd_in_array($dataVal['brandCd'], $this->_memInfo['dcBrandInfo']->cateCd)) {
        ...
        foreach ($goodsPriceInfo as $key => $val) {
            ...
            // 상품 전체 주문금액
            $tmpOrderPrice['memberDcByPrice'] += gd_array_sum($tmpPrice[$key]['all']['memberDcByPrice']);
            $tmpOrderPrice['couponDcPrice'][$key] += gd_array_sum($val['couponDcPrice']);

            // 추가할인 가능시 상품전체금액 계산
            if ($val['addDcFl'] === true) {
                $tmpOrderPrice['addDcTotal']['memberDcByPrice'] += gd_array_sum($tmpPrice[$key]['all']['memberDcByPrice']);
            }
            // 중복할인 가능시 상품전체금액 계산
            if ($val['overlapDcFl'] === true) {
                $tmpOrderPrice['overlapDcTotal']['memberDcByPrice'] += gd_array_sum($tmpPrice[$key]['all']['memberDcByPrice']);
            }
            if (empty($tmpPrice[$key]['all']['memberDcByAddPrice']) === false) {
                foreach ($tmpPrice[$key]['all']['memberDcByAddPrice'] as $k => $v) {
                    // 추가상품 전체 주문금액
                    $tmpOrderPrice['memberDcByAddPrice'] += gd_array_sum($v);
                    // 추가할인 가능시 추가상품전체금액 계산
                    if ($val['addDcFl'] === true) {
                        $tmpOrderPrice['addDcTotal']['memberDcByAddPrice'] += gd_array_sum($v);
                    }
                    // 중복할인 가능시 추가상품전체금액 계산
                    if ($val['overlapDcFl'] === true) {
                        $tmpOrderPrice['overlapDcTotal']['memberDcByAddPrice'] += gd_array_sum($v);
                    }
                }
            }
        }
        ...
        if ($this->getChannel() != 'naverpay') {
            ...
            foreach ($goodsPriceInfo as $key => $val) {
                if ($val['couponBenefitExcept'] == 'n') {
                    if (gd_in_array($this->_memInfo['fixedOrderTypeDc'], ['goods', 'order', 'brand']) === true) {
                        // 회원 추가 할인혜택 적용 제외
                        if (gd_in_array('add', $val['exceptBenefit']) === true && $val['exceptBenefitFl'] === true) {
                        } else {
                            ...
                            foreach ($val['goodsCnt'] as $k => $v) {
                                ...
                                if (gd_in_array($this->_memInfo['fixedOrderTypeDc'], ['brand']) === true) {
                                    // 추가 상품 브랜드 할인율
                                    ...
                                    $getData[$k]['price']['addGoodsMemberDcPrice'] = gd_isset(gd_array_sum($addDcPrice['info']['addGoods'][$k]), 0);
                    ...
                    if (gd_in_array($this->_memInfo['fixedOrderTypeOverlapDc'], ['goods', 'order']) === true) {
                        // 회원 중복 할인혜택 적용 제외
                        if (gd_in_array('overlap', $val['exceptBenefit']) === true && $val['exceptBenefitFl'] === true) {
                        } else {
                            ...
                            foreach ($val['goodsCnt'] as $k => $v) {
                                ...
                                $getData[$k]['price']['addGoodsMemberOverlapDcPrice'] = gd_isset(gd_array_sum($overlapDcPrice['info']['addGoods'][$k]), 0);
                    ...
                    if (gd_in_array($this->_memInfo['fixedOrderTypeMileage'], ['goods', 'order']) === true) {
                        // 회원 추가 마일리지 적립 적용 제외
                        if (gd_in_array('mileage', $val['exceptBenefit']) === true && $val['exceptBenefitFl'] === true) {
                        } else {
                            ...
                            foreach ($val['goodsCnt'] as $k => $v) {
                                ...
                                $getData[$k]['mileage']['addGoodsMemberMileage'] = gd_isset(gd_array_sum($memberMileage['addGoods'][$k]), 0);
...
//                unset($getData[$dataKey]['mileageFl'], $getData[$dataKey]['mileageGoods'], $getData[$dataKey]['mileageGoodsUnit']);
        ...
        if ($this->totalUseMileage > 0) {
            if ($this->mileageGiveInfo['give']['excludeFl'] == 'r' || $this->mileageGiveInfo['give']['excludeFl'] == 'm'  || $this->mileageGiveInfo['give']['excludeFl'] == 'p') {
                ...
                $totalGoodsPrice = gd_array_sum($standardMileageGoodsPrice);
                $totalGoodsCount = gd_count($standardMileageGoodsPrice);
        ...
        foreach ($getData as $dataKey => $dataVal) {
            ...
            $this->multiShippingOrderInfo[$getData[$dataKey]['sno']] = [
                ...
                'getKey' => gd_count($getCart[$scmNo][$deliverySno]),
            ];
        ...
        if (empty($sameDelivery['setKey']) === false) {
            ...
            if (gd_count($getCart[$scmNo][$deliverySno]) == gd_count($setSameCart[$scmNo][$deliverySno])) {
    ...
    public function getDivisionOrderCoupon($goodsPrice)
    {
        ...
        if ($totalCouponOrderDcPrice > 0) {
            ...
            $lastGoodsNo = end(gd_array_keys($goodsPrice));
            foreach ($goodsPrice as $goodsNo => $val) {
                $price = $goodsPrice[$goodsNo];

                $lastKey = end(gd_array_keys($val['goodsCnt']));
                foreach ($val['goodsCnt'] as $k => $v) {
                        ...
                        switch ($goodsDeliveryFixFl) {
                            ...
                            case 'price':
                                ...
                                if (empty($getDeliveryInfo[$deliverySno]['pricePlusStandard']) === false) {
                                    if (gd_in_array('option', $getDeliveryInfo[$deliverySno]['pricePlusStandard']) === true) {
                                        $tmp['deliveryByStandard'] += $gVal['price']['optionPriceSum'];
                                    }
                                    if (gd_in_array('add', $getDeliveryInfo[$deliverySno]['pricePlusStandard']) === true) {
                                        $tmp['deliveryByStandard'] += $gVal['price']['addGoodsPriceSum'];
                                    }
                                    if (gd_in_array('text', $getDeliveryInfo[$deliverySno]['pricePlusStandard']) === true) {
                                ...
                                if (empty($getDeliveryInfo[$deliverySno]['priceMinusStandard']) === false) {
                                    if (gd_in_array('goods', $getDeliveryInfo[$deliverySno]['priceMinusStandard']) === true) {
                                    ...
                                    if (gd_in_array('coupon', $getDeliveryInfo[$deliverySno]['priceMinusStandard']) === true) {
                            ...
                            case 'count':
                                ...
                                if($getDeliveryInfo[$deliverySno]['addGoodsCountInclude'] === 'y'){
                                    $deliveryGoodsCnt += (int)gd_array_sum(gd_array_column($gVal['addGoods'], 'addGoodsCnt'));
                                }
...
//                                    $areaDelivery['addArea'] = implode(' ', $rejoinAddress);
                        ...
                        if ($goodsDeliveryFl != 'y' && $sameGoodsDeliveryFl == 'y') {
                            ...
                            $setDeliveryCond[$deliverySno][$gVal['goodsNo']]['addGoodsLineCnt'] = $setDeliveryCond[$deliverySno][$gVal['goodsNo']]['addGoodsLineCnt'] + gd_count($gVal['addGoods']);
                            ...
                            $setDeliveryCond[$deliverySno]['addGoodsLineCnt'] = $setDeliveryCond[$deliverySno]['addGoodsLineCnt'] + gd_count($gVal['addGoods']);
...
//                                    $areaDelivery['addArea'] = implode(' ', $rejoinAddress);
            ...
            foreach ($setDeliveryCond as $dKey => $dVal) {
                ...
                if ($dVal['deliveryByCalculate'] === 'y') {
                    ...
                    if ($this->overseasDeliveryPolicy['data']['standardFl'] === 'ems') {
                        ...
                        // 전체 무게
                        $totalDeliveryByStandard = gd_array_sum(gd_array_column($setDeliveryCond, 'deliveryByStandard'));
    ...
    public function checkOrderPossible($data, $whsiFl = false)
    {
        ...
        switch ($data['goodsPermission']) {
            ...
            case 'group':
                ...
                if (!gd_in_array($this->_memInfo['groupSno'], $group)) {
                    $data['orderPossible'] = 'n';
        ...
        if($this->isWrite === true){
            if($this->isLogin === false){
                ...
                if($data['payLimit'] && $data['payLimitFl'] == 'y'){
                    if($this->_memInfo['settleGb'] != 'all') { // 회원 등급 결제수단 제한이 있을 때
                        ...
                        $payLimit = explode(STR_DIVISION, $data['payLimit']);
                        $payLimit = gd_array_intersect($settleGb, $payLimit);
                        ...
                        $payLimit = explode(STR_DIVISION, $data['payLimit']);
                        if (!gd_in_array('gb', $payLimit) && !gd_in_array('gm', $payLimit) && !gd_in_array('gd', $payLimit)) {
                            $data['orderPossible'] = 'n';
                            $data['orderPossibleCode'] = $data['orderPossibleCodeArr'][] = self::POSSIBLE_NOT_FIND_PAYMENT;
                            $data['orderPossibleMessage'] = $orderPossibleMessage[] = __('사용가능한 결제수단 없음');
                        }
            ...
            if (empty($this->_memInfo) === false && $data['payLimit'] && $data['payLimitFl'] == 'y') {
                $payLimit = explode(STR_DIVISION, $data['payLimit']);
                $payLimit = gd_array_intersect($this->_memInfo['settleGb'], $payLimit);
                if (gd_count($payLimit) == 0 && !Globals::get('gGlobal.isFront')) {
                    $data['orderPossible'] = 'n';
                    $data['orderPossibleCode'] = $data['orderPossibleCodeArr'][] = self::POSSIBLE_NOT_FIND_PAYMENT;
                    $data['orderPossibleMessage'] = $orderPossibleMessage[] = __('사용가능한 결제수단 없음');
                }
        ...
        if($this->isWrite !== true){
            ...
            // 네이버페이 결제형 [결제 제한 조건 사유]
            $naverpayConfig = gd_policy('naverEasyPay.config');
            if ($naverpayConfig['useYn'] == 'y' && !(new \Bundle\Component\Payment\Naverpay\NaverpayHelper($naverpayConfig))->isBuyable(
                $data['goodsNo'],
                $data['brandCd'],
                $data['cateCd'])
            ) {
                $data['orderPossibleMessage'] = $orderPossibleMessage[] = '네이버페이 결제형 구매 불가 상품';
            }
            ...
            // 토스페이 제한 조건 체크
            $policyTosspay = gd_policy('pg.tosspay') ?? [];
            $tosspayConfig = new TosspayConfig($policyTosspay);
            $isUse = $tosspayConfig->isTosspayViewSettleGeneral(Request::isMobile(), gd_is_admin());
            if ($isUse) {
                if ($tosspayConfig->isBuyable($data['goodsNo'], $data['brandCd'], $data['cateCd'])) {
                    $this->orderCheckoutDataPossible['tosspay'] = true;
                } else {
                    $this->orderCheckoutDataPossible['tosspay'] = false;
                    $data['orderPossibleMessage'] = $orderPossibleMessage[] = '토스페이 구매 불가 상품';
                }
            }
        }

        $data['orderPossibleMessageList'] = gd_array_unique($orderPossibleMessage);
        // 구매 가능여부 체크
    ...
    public function getMemberDcFlInfo($data)
    {
        ...
        if (empty($this->_memInfo) === false) {
            ...
            if ($this->_memInfo['dc' . ucwords($this->_memInfo['dcType'])] > 0 || $this->memberDc['totalDcBrandBankPercent'] > 0) {
                ...
                // 추가 할인 적용제외 - SCM
                if ($data['addDcFl'] === true && empty($this->_memInfo['dcExOption']) === false && gd_in_array('scm', $this->_memInfo['dcExOption']) === true) {
                    if (empty($this->_memInfo['dcExScm']) === false && gd_in_array($data['scmNo'], $this->_memInfo['dcExScm']) === true) {
                        $data['addDcFl'] = false;
                    }
                }

                // 추가 할인 적용제외 - 카테고리 (대표 카테고리만 확인후에 아래에서 현재 카테고리 전부를 확인 처리 함)
                if ($data['addDcFl'] === true && empty($this->_memInfo['dcExOption']) === false && gd_in_array('category', $this->_memInfo['dcExOption']) === true && empty($this->_memInfo['dcExCategory']) === false) {
                    $goodsCateCd = \App::load('\\Component\\Goods\\Goods')->getGoodsNoToCateCd($data['goodsNo'])['cateCd'];
                    foreach ($goodsCateCd as $key=>$val){
                        if (gd_in_array($val, $this->_memInfo['dcExCategory']) === true) {
                            $data['addDcFl'] = false;
                        }
                    }
                    if($data['addDcFl'] !== false){
                        $this->memberDc['dc_category'][] = $data['goodsNo'];
                        $this->memberDc['dc_category'] = gd_array_unique($this->memberDc['dc_category']);
                    }
                }

                // 추가 할인 적용제외 - 브랜드
                if ($data['addDcFl'] === true && empty($this->_memInfo['dcExOption']) === false && gd_in_array('brand', $this->_memInfo['dcExOption']) === true) {
                    if (empty($this->_memInfo['dcExBrand']) === false && gd_in_array($data['brandCd'], $this->_memInfo['dcExBrand']) === true) {
                        $data['addDcFl'] = false;
                    }
                }

                // 추가 할인 적용제외 - 상품
                if ($data['addDcFl'] === true && empty($this->_memInfo['dcExOption']) === false && gd_in_array('goods', $this->_memInfo['dcExOption']) === true) {
                    if (empty($this->_memInfo['dcExGoods']) === false && gd_in_array($data['goodsNo'], $this->_memInfo['dcExGoods']) === true) {
                        $data['addDcFl'] = false;
                    }
                }
            ...
            if ($this->_memInfo['overlapDc' . ucwords($this->_memInfo['overlapDcType'])] > 0 && empty($this->_memInfo['overlapDcOption']) === false) {
                ...
                // 중복 할인 적용제외 - SCM
                if ($data['overlapDcFl'] === false && gd_in_array('scm', $this->_memInfo['overlapDcOption']) === true) {
                    if (empty($this->_memInfo['overlapDcScm']) === false && gd_in_array($data['scmNo'], $this->_memInfo['overlapDcScm']) === true) {
                        $data['overlapDcFl'] = true;
                    }
                }

                // 중복 할인 적용제외 - 카테고리 (대표 카테고리만 확인후에 아래에서 현재 카테고리 전부를 확인 처리 함)
                if ($data['overlapDcFl'] === false && gd_in_array('category', $this->_memInfo['overlapDcOption']) === true && empty($this->_memInfo['overlapDcCategory']) === false) {
                    if (gd_in_array($data['cateCd'], $this->_memInfo['overlapDcCategory']) === true) {
                        $data['overlapDcFl'] = true;
                    } else {
                        $this->memberDc['overlap_category'][] = $data['goodsNo'];
                        $this->memberDc['overlap_category'] = gd_array_unique($this->memberDc['overlap_category']);
                    }
                }

                // 중복 할인 적용제외 - 브랜드
                if ($data['overlapDcFl'] === false && gd_in_array('brand', $this->_memInfo['overlapDcOption']) === true) {
                    if (empty($this->_memInfo['overlapDcBrand']) === false && gd_in_array($data['brandCd'], $this->_memInfo['overlapDcBrand']) === true) {
                        $data['overlapDcFl'] = true;
                    }
                }

                // 중복 할인 적용제외 - 상품
                if ($data['overlapDcFl'] === false && gd_in_array('goods', $this->_memInfo['overlapDcOption']) === true) {
                    if (empty($this->_memInfo['overlapDcGoods']) === false && gd_in_array($data['goodsNo'], $this->_memInfo['overlapDcGoods']) === true) {
                        $data['overlapDcFl'] = true;
                    }
                }
    ...
    protected function getMemberDcForCateCd()
    {
        ...
        if (($this->memberDc['dc'] === true && empty($this->memberDc['dc_category']) === false) || ($this->memberDc['overlap'] === false && empty($this->memberDc['overlap_category']) === false)) {
            $arrGoodsNo = gd_array_merge((array)$this->memberDc['dc_category'], (array)$this->memberDc['overlap_category']);
            $arrGoodsNo = gd_array_unique($arrGoodsNo);
            $strSQL = "SELECT goodsNo, cateCd FROM " . DB_GOODS_LINK_CATEGORY . " WHERE goodsNo IN (" . gd_implode(', ', $arrGoodsNo) . ")";
    ...
    protected function getAddGoodsInfo($arrAddGoodsNo)
    {
        ...
        if (empty($arrAddGoodsNo) === false) {
            $arrAddGoodsNo = gd_array_values(gd_array_unique($arrAddGoodsNo, SORT_NUMERIC));

            $strSQL = "SELECT " . gd_implode(', ', $arrFieldAddGoods) . " FROM " . DB_ADD_GOODS . " as ag  WHERE ag.addGoodsNo IN (" . gd_implode(', ', $arrAddGoodsNo) . ")";

            /**해외몰 관련 **/
            if($mallBySession) {
                $arrFieldAddGoodsGlobal = DBTableField::setTableField('tableAddGoodsGlobal',null,['mallSno']);
                $strSQLGlobal = "SELECT " . gd_implode(', ', $arrFieldAddGoodsGlobal) . " FROM ".DB_ADD_GOODS_GLOBAL." WHERE addGoodsNo IN (" . gd_implode(', ', $arrAddGoodsNo) . ") AND mallSno = '".$mallBySession['sno']."'";

                $tmpData = $this->db->query_fetch($strSQLGlobal);
                $globalData = array_combine (gd_array_column($tmpData, 'addGoodsNo'), $tmpData);
            }
            ...
            while ($data = $this->db->fetch($result)) {
                ...
                if($mallBySession && $globalData[$data['addGoodsNo']]) {
                    ...
                    $data = array_replace_recursive($data, gd_array_filter(array_map('trim',$globalData[$data['addGoodsNo']])));
                }
    ...
    protected function getOptionTextInfo($arrOptionTextSno)
    {
        ...
        if (empty($arrOptionTextSno) === false) {
            $arrOptionTextSno = gd_array_values(gd_array_unique($arrOptionTextSno, SORT_NUMERIC));
            $strSQL = "SELECT sno, " . gd_implode(', ', $arrFieldOptionText) . " FROM " . DB_GOODS_OPTION_TEXT . " WHERE sno IN (" . gd_implode(', ', $arrOptionTextSno) . ")";
            $result = $this->db->query($strSQL);
            while ($data = $this->db->fetch($result)) {
    ...
    protected function getGoodsDcData($goodsDiscountFl, $goodsDiscount, $goodsDiscountUnit, $goodsCnt, $goodsPrice, $fixedGoodsDiscount = null, $goodsDiscountGroup = null, $goodsDiscountGroupMemberInfo = null)
    {
        ...
        if (gd_in_array('option', $fixedGoodsDiscountData) === true) $goodsPriceTmp +=$goodsPrice['optionPriceSum'];
        if (gd_in_array('text', $fixedGoodsDiscountData) === true) $goodsPriceTmp +=$goodsPrice['optionTextPriceSum'];
        ...
        if ($goodsDiscountFl === 'y') {
            switch ($goodsDiscountGroup) {
                case 'group':
                    ...
                    $discountKey = gd_array_flip($goodsDiscountGroupMemberInfoData['groupSno'])[$this->_memInfo['groupSno']];
    ...
    protected function getMemberMileageData($memInfo, $goodsPrice)
    {
        ...
        if ($this->mileageGiveInfo['info']['useFl'] === 'y') {
            ...
            if (empty($memInfo['fixedRateOption']) === false) {
                if (gd_in_array('option', $memInfo['fixedRateOption']) == '1') {
                    $tmp['memberDcByPrice'] = $tmp['memberDcByPrice'] + $goodsPrice['optionPriceSum'];
                }
                if (gd_in_array('goods', $memInfo['fixedRateOption']) == '1') {
                    $tmp['memberDcByPrice'] = $tmp['memberDcByPrice'] + $goodsPrice['addGoodsPriceSum'];
                }
                if (gd_in_array('text', $memInfo['fixedRateOption']) == '1') {
    ...
    protected function getMemberDcPriceData($goodsNo, $memInfo, $goodsPrice, $arrCateCd, $addDcFl, $overlapDcFl, $arrBrandCd = null)
    {
        ...
        if ($addDcFl === true && empty($arrCateCd[$goodsNo]) === false) {
            ...
            foreach ($arrCateCd[$goodsNo] as $gVal) {
                if (isset($memInfo['dcExCategory']) && gd_in_array($gVal, $memInfo['dcExCategory'])) {
                    $addDcFl = false;
                }
            }
        }
        ...
        if ($overlapDcFl === false && empty($arrCateCd[$goodsNo]) === false) {
            // 해당 상품이 연결된 카테고리 체크
            foreach ($arrCateCd[$goodsNo] as $gVal) {
                if (isset($memInfo['overlapDcCategory']) && gd_in_array($gVal, $memInfo['overlapDcCategory'])) {
                    $overlapDcFl = true;
                }
            }
        }
        ...
        if ($this->mileageGiveInfo['info']['useFl'] === 'y') {
            ...
            if ($mileageFl == 'g') {
                if ($mileageGroup == 'group') { //마일리지 지급대상(특정회원등급)
                    ...
                    $mileageKey = gd_array_flip($mileageGroupMemberInfoData['groupSno'])[$this->_memInfo['groupSno']];
                    if ($mileageKey >= 0) {
    ...
    public function setOrderSettleCalculation($requestData)
    {
        ...
        if (gd_count($couponNos) > 0) {
            if ($requestData['couponApplyOrderNo'] != '') {
                ...
                $totalCouponOrderDcPrice = gd_array_sum($orderCouponPrice['memberCouponSalePrice']);
                $totalCouponDeliveryDcPrice = gd_array_sum($orderCouponPrice['memberCouponDeliveryPrice']);
                $totalCouponOrderMileage = gd_array_sum($orderCouponPrice['memberCouponAddMileage']);
            ...
            $this->totalSettlePrice -= (int)$this->totalCouponOrderDcPrice + (int)$this->totalCouponDeliveryDcPrice;
            $this->totalDcPrice += (int)$this->totalCouponOrderDcPrice + (int)$this->totalCouponDeliveryDcPrice;
            $this->totalMileage += (int)$this->totalCouponOrderMileage;
        }

        // 배송비 혜택이 무료이고 스킨패치가 적용되있을 경우 해당 주문에 적용된 배송비 0원처리
        if (empty($this->deliveryFree) === false && $this->_memInfo['deliveryFree'] == 'y') {
            $this->totalDeliveryFreeCharge = $this->totalDeliveryCharge - gd_array_sum($this->totalGoodsDeliveryAreaPrice);
        ...
        // 실결제금액 마이너스인 경우
        if ((is_numeric($requestSettlePrice) && $requestSettlePrice < 0) || $this->totalSettlePrice < 0) {
    ...
    public function setMileageUseLimitPrice($postData=[])
    {
        ...
        // 지역별 배송 금액
        $mileagePrice['deliveryAreaCharge'] = ($postData['totalGoodsDeliveryAreaPrice']) ? $postData['totalGoodsDeliveryAreaPrice'] : gd_array_sum($this->totalGoodsDeliveryAreaPrice);
...
//            }
    ...
    public function getOrderGoodsWithOtherUser($cartGoods = null)
    {
        ...
        $goodsData = $goods->goodsDataDisplay('goods', gd_implode(INT_DIVISION, $goodsList), 12, 'sort asc', 'list', false, true, false, false, 166);
    ...
    public function goodsViewBenefit($getData)
    {
        ...
        // 제외 혜택 대상 여부
        $exceptBenefitFl = false;
        if ($goodsData['exceptBenefitGroup'] == 'all' || ($goodsData['exceptBenefitGroup'] == 'group' && gd_in_array($this->_memInfo['groupSno'], $exceptBenefitGroupInfo) === true)) {
            $exceptBenefitFl = true;
        }
        ...
        foreach ($getData['goodsCnt'] as $k => $v) {
            ...
            if (empty($getData['addGoodsNo'][$k]) === false) {
                ...
                if ($this->_memInfo['fixedOrderTypeDc'] == 'brand') {
                    ...
                    foreach ($getData['addGoodsBrandCd'][0] as $addGoodsKey => $addGoodsBrandCd) {
                        if (gd_in_array($addGoodsBrandCd, $this->_memInfo['dcBrandInfo']->cateCd)) {
            ...
            if ($this->useMyapp) {
                ...
                $myappBenefitParams['addGoodsPrice'] = $price['addGoodsPriceSum'];
                $myappBenefit = $myapp->getOrderAdditionalBenefit($myappBenefitParams);
            ...
            if ($getData['couponBenefitExcept'] == 'n') {
                ...
                if ($getData['couponApplyNo'][$k] > 0) {
                    ...
                    if (is_array($tmpCouponPrice['memberCouponAlertMsg']) && (gd_array_search('LIMIT_MIN_PRICE', $tmpCouponPrice['memberCouponAlertMsg']) === true)) {
                        // 'LIMIT_MIN_PRICE' 일때 구매금액 제한에 걸려 사용 못하는 쿠폰 처리
                        // 수량 변경 시 구매금액 제한에 걸림
                        // 적용된 쿠폰 모두 제거
                    ...
                    if (is_array($tmpCouponPrice['memberCouponSalePrice'])) {
                        $goodsOptCouponSalePriceSum = gd_array_sum($tmpCouponPrice['memberCouponSalePrice']);
                    }
                    if (is_array($tmpCouponPrice['memberCouponAddMileage'])) {
                        $goodsOptCouponAddMileageSum = gd_array_sum($tmpCouponPrice['memberCouponAddMileage']);
                    }
            ...
            if ($getData['memberBenefitExcept'] == 'n' && $memberDcFl == true) {
                ...
                // 회원 추가 할인혜택 적용 제외
                if (gd_in_array('add', $exceptBenefit) === true && $exceptBenefitFl === true) {} else {
                    $data['memberDcPrice'] += $tmp['memberDcPrice'];
                    $data['tmpMemberDcPrice'] += $tmp['memberDcPrice'];
                }
                // 회원 중복 할인혜택 적용 제외
                if (gd_in_array('overlap', $exceptBenefit) === true && $exceptBenefitFl === true) {} else {
                    $data['memberDcPrice'] += $tmp['memberOverlapDcPrice'];
                    $data['tmpMemberOverlapDcPrice'] += $tmp['memberOverlapDcPrice'];
                }
                // 회원 추가 마일리지 적립 적용 제외
                if (gd_in_array('mileage', $exceptBenefit) === true && $exceptBenefitFl === true) {} else {
        ...
        if ($getData['memberBenefitExcept'] == 'n' && $memberDcFl === true) {
            ...
            if (gd_in_array($this->_memInfo['fixedOrderTypeDc'], ['goods', 'order', 'brand']) === true) {
                if (gd_in_array('add', $exceptBenefit) === true && $exceptBenefitFl === true) {} else {
                    //회원등급 > 브랜드별 추가할인 상품 브랜드 정보
                    if ($this->_memInfo['fixedOrderTypeDc'] == 'brand') {
                        if (gd_in_array($getData['brandCd'], $this->_memInfo['dcBrandInfo']->cateCd)) {
        ...
        if ($getData['goodsMileageExcept'] == 'n') {
            $goodsCnt = gd_array_sum($getData['goodsCnt']);
            $price['goodsPriceSum'] = gd_array_sum($getData['goodsPriceSum']);
            $price['optionPriceSum'] = gd_array_sum($getData['optionPriceSum']);
            $price['optionTextPriceSum'] = gd_array_sum($getData['optionTextPriceSum']);
            $price['addGoodsPriceSum'] = gd_array_sum($getData['addGoodsPriceSum']);
     ...
     */
    public function setMergeCart($memNo = null)
    {
        $session = \App::getInstance('session');
        if($session->get('related_goods_order') ==  'y'){
            return;
        ...
        if (!$memNo) {
            ...
            foreach ($cartData as $key => $val) {
                if ($val['cnt'] > 1) {
                    ...
                    if (gd_count($tempArrayAdd) > 0) {
                        $tempAddNo = json_encode(ArrayUtils::removeEmpty(gd_array_keys($tempArrayAdd)));
                        $tempAddCnt = json_encode(ArrayUtils::removeEmpty(gd_array_values($tempArrayAdd)));
                    }
                ...
                if ($val['cnt'] > 1) {
                    ...
                    foreach($mergeList as $k => $v) {
                        ...
                        if ($v['memberCouponNo'] != '') {
                            ...
                            if(gd_count($memberCouponArray) > 0){
                            ...
                            if($this->isWrite === true){
                                ...
                                if(trim($owMemberCartSnoData) !== ''){
                                    ...
                                    if(gd_count($owMemberCartSnoDataArr) > 0){
                                        $cartSnoIndex = gd_array_search($v['sno'], $owMemberCartSnoDataArr);
                                    ...
                                    $owMemberCartSnoDataArrNew = gd_implode(",", $owMemberCartSnoDataArr);
                                    $owMemberRealCartSnoDataArrNew = gd_implode(",", $owMemberRealCartSnoDataArr);
                                    $owMemberCartCouponNoDataArrNew = gd_implode(",", $owMemberCartCouponNoDataArr);
    ...
    public function getMemberGoodsMileageData($tmp, $tmpOrder, $memInfo)
    {
        ...
        if ($memInfo['fixedOrderTypeMileage'] == 'order' && empty($tmpOrder) === false) {
            ...
            if (gd_in_array('goods', $memInfo['fixedRateOption']) === true && empty($tmpOrder['memberDcByAddPrice']) === false) {
                $memberDcByPrice += $tmpOrder['memberDcByAddPrice'];
            }
        } else {
            $memberDcByPrice = gd_array_sum($tmp['all']['memberDcByPrice']);
            if (gd_in_array('goods', $memInfo['fixedRateOption']) === true && empty($tmp['all']['memberDcByAddPrice']) === false) {
                foreach ($tmp['all']['memberDcByAddPrice'] as $v) {
                    $memberDcByPrice += gd_array_sum($v);
                }
            }
        }
    ...
    public function getMemberGoodsAddDcPriceData($tmp, $tmpOrder, $goodsNo, $memInfo, $goodsPrice, $arrCateCd, $addDcFl, $couponDcPrice = [], $arrBrandCd = null)
    {
        if ($memInfo['fixedOrderTypeDc'] == 'order' && empty($tmpOrder) === false) {
            ...
            if ($memInfo['fixedRatePrice'] == 'settle') {
                $memberDcByPrice -= gd_array_sum($couponDcPrice);
            }
        } else {
            $memberDcByPrice = gd_array_sum($tmp['all']['memberDcByPrice']);
            if (gd_in_array('goods', $memInfo['fixedRateOption']) === true && empty($tmp['all']['memberDcByAddPrice']) === false) {
            ...
            if (empty($memInfo['fixedRateOption']) === false) {
                ...
                if (gd_in_array('goods', $memInfo['fixedRateOption']) == '1') {
                    if (empty($goodsPrice['addGoodsCnt'][$k]) === false) {
                        ...
                        $lastKey = end(gd_array_keys($goodsPrice['addGoodsCnt'][$k]));
                ...
                foreach ($tmpPrice['unit']['memberDcByPrice'] as $k => $v) {
                    ...
                    if (empty($tmpPrice['unit']['memberDcByAddPrice']) === false) {
                        foreach ($tmpPrice['unit']['memberDcByAddPrice'][$k] as $key => $val) {
                            if ($this->_memInfo['fixedOrderTypeDc'] == 'brand' && gd_in_array('goods', $this->_memInfo['fixedRateOption']) == '1') {
    ...
    {
        $cartSno = gd_array_keys($exceptCouponNo);
        $memberCouponSno = gd_array_values($exceptCouponNo);
        ...
        $arrBind = $this->db->get_binding(DBTableField::tableCart(), $arrData, 'update', gd_array_keys($arrData));
        $this->db->bind_param_push($arrBind['bind'], 'i', @gd_implode(',', $cartSno));
        $this->db->bind_param_push($arrBind['bind'], 'i', $this->members['memNo']);
        $this->db->set_update_db($tableCart, $arrBind['param'], 'sno IN (?) AND memNo = ?', $arrBind['bind']);
        unset($arrData, $arrBind);

        // 회원쿠폰사용여부 초기화
        $arrBind = $this->db->get_binding(DBTableField::tableMemberCoupon(), $arrCouponData, 'update', gd_array_keys($arrCouponData));
        $this->db->bind_param_push($arrBind['bind'], 'i', @gd_implode(',', $memberCouponSno));
        $this->db->bind_param_push($arrBind['bind'], 'i', $this->members['memNo']);
    ...
    public function setCartGoodsCnt($memNo, $cartIdx = null)
    {
        ...
        if (empty($cartIdx) === false) {
            if (is_array($cartIdx)) {
                ...
                if (empty($tmpWhere) === false) {
                    ...
                    $arrWhere[] = 'sno IN (' . gd_implode(' , ', $tmpAddWhere) . ')';
                }
                unset($tmpWhere);
            } elseif (is_numeric($cartIdx)) {
                $arrWhere[] = 'sno = ?';
        ...
        foreach ($data as $val) {
            ...
            if ($tmpGoodsCnt != $val['goodsCnt'] && $tmpGoodsCnt > 0) {
                ...
                $subBind = $this->db->get_binding(DBTableField::tableCart(), $arrData, 'update', gd_array_keys($arrData));
        ...
        foreach ($tmpData as $goodsNo => $val) {
            ...
            if ($tmpGoodsData['maxOrderCnt'] > 0 && $tmpGoodsData['maxOrderCnt'] < $tmpGoodsCnt) {
                $tmpGoodsCnt = $tmpGoodsData['maxOrderCnt'];
                if ($tmpGoodsData['fixedOrderCnt'] == 'option') $tmpGoodsCnt *= gd_count($val['goodsCntBySno']);
            }
    ...
    {
        $cartAllIdx = gd_array_unique($postValue['cartAllSno']); // 중복제거
        $returnData = []; // 리턴배열
    ...
    public function setProductCouponGoodsAllPrice($getData, $couponSno=null, $isFront = 'system')
    {
        ...
        foreach ($getData as $dataKey => $dataVal) {
            ...
            $exceptBenefit = explode(STR_DIVISION, $dataVal['exceptBenefit']);
            $exceptBenefitGroupInfo = explode(INT_DIVISION, $dataVal['exceptBenefitGroupInfo']);
            if(gd_in_array('coupon', $exceptBenefit) === false || ($dataVal['exceptBenefitGroup'] == 'group' && gd_in_array($this->_memInfo['groupSno'], $exceptBenefitGroupInfo) === false)) {
                ...
                if($memberCouponData['couponExceptProviderType'] == 'y') {
                    ...
                    if (gd_in_array($dataVal['scmNo'], $exceptProviderGroupArr) == true) {//공급사 존재
                        continue;
                    }
                }
                // 제외 카테고리
                if($memberCouponData['couponExceptCategoryType'] == 'y') {
                    ...
                    foreach ($cateCdArr as $cateKey => $cateVal) {
                        if(gd_in_array($cateVal, $exceptCategoryGroupArr) == true) {//카테고리 존재
                            $matchCateData++;
                        }
                    }
                ...
                if($memberCouponData['couponExceptBrandType'] == 'y') {
                    ...
                    if(gd_in_array($dataVal['brandCd'], $exceptBrandGroupArr) == true) {//브랜드 존재
                        continue;
                    }
                }
                // 제외 상품
                if($memberCouponData['couponExceptGoodsType'] == 'y') {
                    ...
                    if (gd_in_array($dataVal['goodsNo'], $exceptGoodsGroupArr) == true) {//상품 존재
                        continue;
                    }
                }

                // 쿠폰 사용 가능범위
                if ($memberCouponData['couponApplyProductType'] !== 'all') {
                    // 카테고리
                    if ($memberCouponData['couponApplyProductType'] == 'category'){
                        $cateArr = $goods->getGoodsLinkCategory($dataVal['goodsNo']);
                        $cateCdArr = [];
                        if (is_array($cateArr)) {
                            $cateCdArr = gd_array_column($cateArr, 'cateCd');
                        }
                        $applyCategoryGroupArr = explode(INT_DIVISION, $memberCouponData['couponApplyCategory']);
                        $crossCategory = gd_array_intersect($applyCategoryGroupArr, $cateCdArr);
                        if (!$crossCategory) {
                            continue;
                        }
                    }
                    // 브랜드
                    if ($memberCouponData['couponApplyProductType'] == 'brand') {
                        $applyBrandGroupArr = explode(INT_DIVISION, $memberCouponData['couponApplyBrand']);
                        if(gd_in_array($dataVal['brandCd'], $applyBrandGroupArr) == false) {
                            continue;
                        }
                    }
                    // 상품
                    if ($memberCouponData['couponApplyProductType'] == 'goods') {
                        $applyGoodsGroupArr = explode(INT_DIVISION, $memberCouponData['couponApplyGoods']);
                        if (gd_in_array($dataVal['goodsNo'], $applyGoodsGroupArr) == false) {
                            continue;
                        }
                    }
                    // 공급사
                    if($memberCouponData['couponApplyProductType'] == 'provider') {
                        $applyProviderGroupArr = explode(INT_DIVISION, $memberCouponData['couponApplyProvider']);
                        if (gd_in_array($dataVal['scmNo'], $applyProviderGroupArr) == false) {
                            continue;
                        }
                    }
                }
                ...
                if(gd_is_plus_shop(PLUSSHOP_CODE_TIMESALE) === true) {
                    if($dataVal['timeSaleFl'] == 'y') {
                        $strScmSQL = 'SELECT ts.couponFl as timeSaleCouponFl,ts.sno as timeSaleSno,ts.goodsNo as goodsNo FROM ' . DB_TIME_SALE . ' as ts WHERE FIND_IN_SET(' . $dataVal['goodsNo'] . ', REPLACE(ts.goodsNo,"' . INT_DIVISION . '",",")) AND ts.startDt < NOW() AND  ts.endDt > NOW() AND ts.pcDisplayFl="y"';
    ...
    public function getCartDataByExtraData($extra = null, $arrField = '*')
    {
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . $this->tableName . ' AS c ' . gd_implode(' ', $query);
        $getData = $this->db->query_fetch($strSQL, $arrBind);
        ...
        try {
            ...
            foreach ($couponApplyNoList as $memCouponNoList) {
                ...
                foreach ($memCouponNoArr as $memCouponNo) {
                    ...
                    $strSQL = 'SELECT memberCouponNo, memberCouponState FROM ' . DB_MEMBER_COUPON . ' as mc WHERE ' . gd_implode(' AND ', $arrWhere);
    ...
    private function getMemberGroupBenefit($orderData, $orderCoupon) {
        ...
        foreach ($orderData as $orderDataKey => $orderDataValue) {
            ...
            if (gd_in_array($orderDataValue['sno'], $goodsCouponGoods)) continue;
            ...
            if (gd_in_array('option',  $this->_memInfo['fixedRateOption'])) $basePrice += $orderDataValue['price']['optionPriceSum']; //상품가
            if (gd_in_array('goods',   $this->_memInfo['fixedRateOption'])) $basePrice += $orderDataValue['price']['addGoodsPriceSum']; //추가상품가
            if (gd_in_array('text',    $this->_memInfo['fixedRateOption'])) $basePrice += $orderDataValue['price']['optionTextPriceSum']; //텍스트옵션가
            ...
            {
                //특정 공급사
                if (gd_in_array('scm', $this->_memInfo['dcExOption'])) {
                    if (gd_in_array($orderDataValue['scmNo'], $this->_memInfo['dcExScm'])) {
                ...
                //특정 카테고리
                if (gd_in_array('category', $this->_memInfo['dcExOption'])) {
                    foreach ($orderDataValue['cateAllCd'] as $orderDataValueCateAllCdValue) {
                        if (gd_in_array($orderDataValueCateAllCdValue['cateCd'], $this->_memInfo['dcExCategory'])) {
                            $exceptGoodsDc[$orderDataValue['sno']]['price'] = $basePrice;
                            $exceptGoodsDc[$orderDataValue['sno']]['goodsNo'] = $orderDataValue['goodsNo'];
                            $exceptGoodsDc[$orderDataValue['sno']]['cateAllCd'] = $orderDataValue['cateAllCd'];
                            $exceptGoodsDc[$orderDataValue['sno']]['brandCd'] = $orderDataValue['brandCd'];
                ...
                //특정 브랜드
                if (gd_in_array('brand', $this->_memInfo['dcExOption'])) {
                    if (gd_in_array($orderDataValue['brandCd'], $this->_memInfo['dcExBrand'])) {
                        $exceptGoodsDc[$orderDataValue['sno']]['price'] = $basePrice;
                        $exceptGoodsDc[$orderDataValue['sno']]['goodsNo'] = $orderDataValue['goodsNo'];
                        $exceptGoodsDc[$orderDataValue['sno']]['cateAllCd'] = $orderDataValue['cateAllCd'];
                        $exceptGoodsDc[$orderDataValue['sno']]['brandCd'] = $orderDataValue['brandCd'];
                ...
                //특정 상품
                if (gd_in_array('goods', $this->_memInfo['dcExOption'])) {
                    if (gd_in_array($orderDataValue['goodsNo'], $this->_memInfo['dcExGoods'])) {
                        $exceptGoodsDc[$orderDataValue['sno']]['price'] = $basePrice;
                        $exceptGoodsDc[$orderDataValue['sno']]['goodsNo'] = $orderDataValue['goodsNo'];
                        $exceptGoodsDc[$orderDataValue['sno']]['cateAllCd'] = $orderDataValue['cateAllCd'];
                        $exceptGoodsDc[$orderDataValue['sno']]['brandCd'] = $orderDataValue['brandCd'];
                ...
                //상품별 그룹 혜택 적용 제외 여부
                if (gd_in_array('add', $tmpExceptBenefit[$orderDataKey]) && empty($exceptGoodsDc[$orderDataValue['sno']])) {
                    if ($orderDataValue['exceptBenefitGroup'] == 'all') {
                        ...
                        if (gd_in_array($this->_memInfo['groupSno'], $tmpExceptBenefitGroupInfo)) {
                            $exceptGoodsDc[$orderDataValue['sno']]['price'] = $basePrice;
                            $exceptGoodsDc[$orderDataValue['sno']]['goodsNo'] = $orderDataValue['goodsNo'];
                            $exceptGoodsDc[$orderDataValue['sno']]['cateAllCd'] = $orderDataValue['cateAllCd'];
                            $exceptGoodsDc[$orderDataValue['sno']]['brandCd'] = $orderDataValue['brandCd'];
            ...
            //중복 할인 적용 할 상품
            {
                ...
                //상품별 그룹 혜택 적용 제외 여부
                $tmpExcept = false;
                if (gd_in_array('overlap', $tmpExceptBenefit[$orderDataKey])) {
                    if ($orderDataValue['exceptBenefitGroup'] == 'all') {
                        $tmpExcept = true;
                    } else if ($orderDataValue['exceptBenefitGroup'] == 'group') {
                        $tmpExceptBenefitGroupInfo = explode(INT_DIVISION, $orderDataValue['exceptBenefitGroupInfo']);
                ...
                if ($tmpExcept === false) {
                    //특정 공급사
                    if (gd_in_array('scm', $this->_memInfo['overlapDcOption'])) {
                        if (gd_in_array($orderDataValue['scmNo'], $this->_memInfo['overlapDcScm'])) {
                    ...
                    //특정 카테고리
                    if (gd_in_array('category', $this->_memInfo['overlapDcOption'])) {
                        foreach ($orderDataValue['cateAllCd'] as $orderDataValueCateAllCdValue) {
                            if (gd_in_array($orderDataValueCateAllCdValue['cateCd'], $this->_memInfo['overlapDcCategory'])) {
                                $overlapDcGoodsDc[$orderDataValue['sno']]['price'] = $basePrice;
                                $overlapDcGoodsDc[$orderDataValue['sno']]['goodsNo'] = $orderDataValue['goodsNo'];
                                $overlapDcGoodsDc[$orderDataValue['sno']]['cateAllCd'] = $orderDataValue['cateAllCd'];
                                $overlapDcGoodsDc[$orderDataValue['sno']]['brandCd'] = $orderDataValue['brandCd'];
                    ...
                    //특정 브랜드
                    if (gd_in_array('brand', $this->_memInfo['overlapDcOption'])) {
                        if (gd_in_array($orderDataValue['brandCd'], $this->_memInfo['overlapDcBrand'])) {
                            $overlapDcGoodsDc[$orderDataValue['sno']]['price'] = $basePrice;
                            $overlapDcGoodsDc[$orderDataValue['sno']]['goodsNo'] = $orderDataValue['goodsNo'];
                            $overlapDcGoodsDc[$orderDataValue['sno']]['cateAllCd'] = $orderDataValue['cateAllCd'];
                            $overlapDcGoodsDc[$orderDataValue['sno']]['brandCd'] = $orderDataValue['brandCd'];
                    ...
                    //특정 상품
                    if (gd_in_array('goods', $this->_memInfo['overlapDcOption'])) {
                        if (gd_in_array($orderDataValue['goodsNo'], $this->_memInfo['overlapDcGoods'])) {
                            $overlapDcGoodsDc[$orderDataValue['sno']]['price'] = $basePrice;
                            $overlapDcGoodsDc[$orderDataValue['sno']]['goodsNo'] = $orderDataValue['goodsNo'];
                            $overlapDcGoodsDc[$orderDataValue['sno']]['cateAllCd'] = $orderDataValue['cateAllCd'];
                            $overlapDcGoodsDc[$orderDataValue['sno']]['brandCd'] = $orderDataValue['brandCd'];
            ...
            //마일리지 적용 제외 상품
            {
                ...
                $tmpExceptBenefit[$orderDataKey] = explode(STR_DIVISION, $orderDataValue['exceptBenefit']);
                //상품별 그룹 혜택 적용 제외 여부
                if (gd_in_array('mileage', $tmpExceptBenefit[$orderDataKey])) {
                    if ($orderDataValue['exceptBenefitGroup'] == 'all') {
                ...
                case 'brand':
                    ...
                    foreach ($this->_memInfo['dcBrandInfo']->cateCd as $dcBrandInfoKey => $dcBrandInfoVal) {
                        if ($orderDataValue['brandCd'] == $dcBrandInfoVal) {
                            ...
                            $percent = ($this->_memInfo['dcBrandInfo']->goodsDiscount[$dcBrandInfoKey]);
                        } else if ($dcBrandInfoVal == 'allBrand' && !gd_in_array($orderDataValue['brandCd'], $this->_memInfo['dcBrandInfo']->cateCd) && $orderDataValue['brandCd'] != '') {
                            $percent = ($this->_memInfo['dcBrandInfo']->goodsDiscount[$dcBrandInfoKey]);
                        }
                    }
                    foreach ($this->_memInfo['dcBrandInfo']->cateCd as $dcBrandInfoKey => $dcBrandInfoVal) {
