...
class ExcelRequest
{
    ...
    /** @var array 카카오페이 결제수단 */
    const KAKAOPAY_PAY_METHOD = [
        'CARD' => '(신용카드)',
        'MONEY' => '(카카오머니)'
    ];
    ...
    public function reserveExcelFile()
    {
        ...
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_EXCEL_REQUEST . ' as er' . gd_implode(' ', $query);
    ...
    public function saveInfoExcelRequest($arrData)
    {
        ...
        foreach ($arrReplyStatus as $key => $val) {
            ...
            $reData[] = gd_implode('=', $newArrReplyStatus);
        }

        // replyStatus 상태 문자열로 변경후 whereDetail 재정의
        $arrData['whereDetail'] = gd_implode('&', $reData);
        ...
        // 5년 경과 주문 삭제 대상 리스트 (고유번호 저장 - 엑셀 다운로드시 사용)
        if ($arrData['mode'] === 'lapse_order_delete_excel_download') {
            $arrData['whereDetail']['lapseOrderDeleteSno'] = $arrData['sno'];
        }
    ...
    public function makeExcelFile(&$arrData)
    {
        ...
        if ($formData['menu'] == 'order' || $arrData['menu'] === 'orderDraft') {
            if ($formData['location'] == 'tax_invoice_request' || $formData['location'] == 'tax_invoice_list') {
                ...
                } elseif ($formData['location'] === 'regular_order_list') {
                    // 정기결제
                    $regularOrderAdmin = \App::getInstance(\Component\RegularDelivery\RegularOrderAdmin\RegularOrderAdminListExcel::class);
                    $rows = $regularOrderAdmin->generateExcelForm(unserialize($arrData['whereCondition']), $formData['excelField'], $defaultField);
                    $getData = $this->getRegularOrderList($rows);
                } else {
            ...
            if ($formData['location'] == 'service_privacy_down') {
                ...
                $tmpGetData = $this->getServicePrivacyHistory($arrData, $formData['excelField'], $excelFieldName);
            } else {
                $tmpGetData = $this->getMemberMemberList(unserialize($arrData['whereCondition']), $formData['excelField'], $excelFieldName);
            }
            $getData = $tmpGetData['getData'];
            $arrData['totalCount'] = $tmpGetData['totalCount'];
        } else if ($formData['menu'] == 'promotion' && $formData['location'] == 'coupon_manage') {
        ...
        if ($getData) {
            ...
            $arrData['fileName'] = gd_implode(STR_DIVISION, $this->fileConfig['fileName']);
        ...
        foreach ($data as $k => $v) {
            ...
            foreach ($v as $key => $val) {
                ...
                if (($key % 20 == 0 && $key != '0') || $totalNum - 1 == $key) {
                    ...
                    echo "<script> parent.progressExcel('" . $progressPercent . "'); </script>";
                }
            ...
            $setData[] = gd_array_merge($setHedData, $tmpData);
        }
                ...
                foreach ($excelField as $excelKey => $excelValue) {
                    ...
                    switch ($excelValue) {
                        ...
                        case 'presentKindCd':
                            ...
                            if ($presentKindCd) {
                                $tmpData[] = "<td>" . gd_implode("<br>", $presentKindCd) . "</td>";
                            } else {
                                $tmpData[] = "<td></td>";
                            }
                            unset($presentKindCd);
                        ...
                        case 'conditionInfo':
                            ...
                            foreach ($_tmpTermsData as $k1 => $v1) {
                                ...
                                $_tmpTermsName[] = gd_implode("/", $textValue);
                            }

                            $conditionInfo = gd_implode("<br>", $_tmpTermsName);
                        ...
                        case 'exceptFl':
                            ...
                            if ($_tmpData) $tmpData[] = "<td>" . gd_implode("<br>", $_tmpData) . "</td>";
                            else $tmpData[] = "<td></td>";
                        ...
                        case 'exceptInfo':
                            if ($val['exceptGoodsNo']) {
                                ...
                                if ($_tmpGoodsNm) {
                                    $exceptInfo[] = __("예외상품");
                                    $exceptInfo[] = gd_implode("<br>", $_tmpGoodsNm);
                                };
                            }
                            ...
                            if ($val['exceptCateCd']) {
                                ...
                                if ($_tmpTermsData) {
                                    ...
                                    $exceptInfo[] = gd_implode("<br>", $_tmpTermsData);
                                };
                            ...
                            if ($exceptInfo) $tmpData[] = "<td>" . gd_implode("<br>", $exceptInfo) . "</td>";
                            else $tmpData[] = "<td></td>";
    ...
    public function getGoodsGoodsList($whereCondition, $excelField, $excelFieldName)
    {
        ...
        for ($i = 0; $i <= $pageNum; $i++) {
            ...
            $fileName = $this->fileConfig['location'] . gd_array_sum(explode(' ', microtime()));
            ...
            foreach ($data as $key => $val) {
                $progress = ($totalNum - 1) > 0 ? round((100 / ($totalNum - 1)) * ($key + ($i * $this->excelPageNum))) : 100;
                ...
                if ($isGlobal) {
                    ...
                    if ($globalData) {
                        $globalField = gd_array_keys($globalData[0]);
                ...
                $isNaverRental = $val['naverProductFlag'] == 'r';

                $imageStorage = '';
                $goodsImageList = $goodsAdmin->getGoodsImage($val['goodsNo']);
                ...
                foreach ($excelField as $excelKey => $excelValue) {
                    ...
                    if (gd_in_array($excelValue, $arrTag)) {
                    ...
                    switch ($excelValue) {
                        ...
                        case 'payLimit':
                            ...
                            $tmpData[] = "<td>" . gd_implode('<br>', $paylimit) . "</td>";
                            unset($paylimit);
                        ...
                        case 'goodsColor':
                            if ($val['goodsColor']) {
                                ...
                                $tmpData[] = "<td>" . gd_implode(STR_DIVISION, $goodsColor) . "</td>";
                                unset($goodsColor);
                            } else {
                                $tmpData[] = "<td></td>";
                            }
                        ...
                        case 'category' :
                            ...
                            if ($categoryList) {
                                ...
                                $tmpData[] = "<td>" . gd_implode("<br>", $_tmpCategoryData) . "</td>";
                            } else {
                                $tmpData[] = "<td></td>";
                            }
                            unset($categoryList);
                        ...
                        case 'goodsPermission':
                            ...
                            if ($val['goodsPermission'] == 'all') {
                                ...
                                if ($val['goodsPermission'] == 'member') {
                                    ...
                                    $memberGroupName = GroupUtil::getGroupName("sno IN ('" . gd_implode("','", $val['goodsPermissionGroup']) . "')");
                                    $goodsPermissionStr = __('특정회원') . "<br>(" . gd_implode(STR_DIVISION, $memberGroupName) . ")";
                                }
                        ...
                        case 'goodsAccess':
                            ...
                            if ($val['goodsAccess'] == 'all') {
                                ...
                                if ($val['goodsAccess'] == 'member') {
                                    ...
                                    $memberGroupName = GroupUtil::getGroupName("sno IN ('" . gd_implode("','", $val['goodsAccessGroup']) . "')");
                                    $goodsAccessStr = __('특정회원') . "<br>(" . gd_implode(STR_DIVISION, $memberGroupName) . ")";
                                }
                        ...
                        case 'imageList':
                            ...
                            if ($imageList) {
                                foreach ($imageList as $k1 => $v1) {
                                    $_tmpImageList[] = $v1['goodsImageStorage'] == 'obs' ? $v1['imageUrl'] : $v1['imageName'];
                                }
                                $tmpData[] = "<td>" . gd_implode("<br>", $_tmpImageList) . "</td>";
                                unset($_tmpImageList);
                            } else {
                                $tmpData[] = "<td></td>";
                            }
                        ...
                        case 'goodsIconEndYmd':
                            ...
                            if ($iconList) {
                                ...
                                if ($excelValue == 'goodsIconCd' || $excelValue == 'goodsIconCdPeriod') {
                                    $tmpData[] = "<td>" . gd_implode("||", $_tmpIconList) . "</td>";
                                    unset($_tmpIconList);
                            ...
                            if ($val['optionFl'] == 'y') {
                                ...
                                if ($optionData) {
                                    ...
                                    foreach ($optionData as $k1 => $v1) {
                                        ...
                                        $_tmpOption[] = gd_implode("/", $_tmpOptionInfo);
                                        unset($_tmpOptionInfo);
                                    }

                                    $tmpData[] = "<td>" . gd_implode("<br>", $_tmpOption) . "</td>";
                                    unset($_tmpOption);
                            ...
                            if ($val['addGoodsFl'] == 'y') {
                                ...
                                if ($addGoods) {
                                    ...
                                    foreach ($addGoods as $k1 => $v1) {
                                        ...
                                        $_tmpAddGoodsInfo[] = gd_implode(",", $v1['addGoods']);
                                        $_tmpAddGoods[] = gd_implode("/", $_tmpAddGoodsInfo);
                                        unset($_tmpAddGoodsInfo);
                                    }

                                    $tmpData[] = "<td>" . gd_implode("<br>", $_tmpAddGoods) . "</td>";
                                    unset($_tmpAddGoods);
                        ...
                        case 'hscode':
                            ...
                            if ($val['hscode']) {
                                ...
                                if ($hscode) {
                                    ...
                                    $tmpData[] = "<td>" . gd_implode("<br>", $_tmpHsCode) . "</td>";
                                    unset($_tmpHsCode);
                            ...
                            if ($val['optionTextFl'] == 'y') {
                                ...
                                if ($optionText) {
                                    foreach ($optionText as $k1 => $v1) {
                                        ...
                                        $_tmpOptionText[] = gd_implode("/", $_tmpOptionTextInfo);
                                        unset($_tmpOptionTextInfo);
                                    }
                                    $tmpData[] = "<td>" . gd_implode("<br>", $_tmpOptionText) . "</td>";
                        ...
                        case 'relationCnt' :
                            ...
                            if ($val['relationFl'] != 'n') {
                                if ($val['relationGoodsNo']) {
                                    $tmpData[] = "<td>" . gd_count(explode(INT_DIVISION, $val['relationGoodsNo'])) . "</td>";
                                } else {
                                    $tmpData[] = "<td>0</td>";
                                }
                            } else {
                        ...
                        case 'goodsAddInfo' :
                            ...
                            if ($addInfo) {
                                ...
                                $tmpData[] = "<td>" . gd_implode("<br>", $_tmpAddInfo) . "</td>";
                                unset($_tmpAddInfo);
                            } else {
                                $tmpData[] = "<td></td>";
                            }
                        ...
                        case 'goodsMustInfo' :
                            ...
                            if ($goodsMustInfo) {
                                ...
                                $tmpData[] = "<td>" . gd_implode("<br>", $_tmpAddMustInfo) . "</td>";
                        ...
                        case 'naverProductMonthlyRentalPay':
                            $naverProductMonthlyRentalPay = $isNaverRental ? $goodsNaver['shoppingPcRentalPay'] : "";
                            $tmpData[] = "<td style='mso-number-format:\"\@\";'>" . $naverProductMonthlyRentalPay . "</td>";
                            break;
                        case 'naverProductTotalRentalPay':
                            $naverProductTotalRentalPay = $isNaverRental ? $goodsNaver['shoppingTotalRentalPay'] : "";
                            $tmpData[] = "<td style='mso-number-format:\"\@\";'>" . $naverProductTotalRentalPay . "</td>";
                            break;
                        case 'naverProductFlagRentalPeriod':
                            $naverProductFlagRentalPeriod = $isNaverRental ? $goodsNaver[$excelValue] : "";
                            $tmpData[] = "<td style='mso-number-format:\"\@\";'>" . $naverProductFlagRentalPeriod . "</td>";
                            break;
                        case 'naverAgeGroup' :
                        ...
                        case 'kcmarkNo' :
                            ...
                            $tmpData[] = '<td>' . gd_implode(STR_DIVISION, $tmpKcmMarkInfoArr) . '</td>';
                            break;
                        case 'kcmarkDivFl' :
                            $tmpKcmMarkInfoArr = [];
                            foreach ($kcmarkInfoArr as $kcmMarkInfoValue) {
                        ...
                        case 'sellStopStock':
                            ...
                            $tmpData[] = '<td>' . gd_implode("<br />", $tmpString) . '</td>';
                        ...
                        case 'optionSellFl':
                            ...
                            foreach ($optionData as $key_inner => $value_inner) {
                                if ($value_inner['optionSellFl'] == 'y') $tmpString[] = '정상';
                                    ...
                                    $deliverySell = gd_array_merge($deliverySellNew, $deliverySell);
                        ...
                        case 'googleFl' :
                            $arrBind = [];
                            $googleQuery = "SELECT useFl FROM " . DB_GOOGLE_GOODS_FEED . " WHERE goodsNo = ?";
                            $this->db->bind_param_push($arrBind, 'i', $val['goodsNo']);
                            $googleData = $this->db->query_fetch($googleQuery, $arrBind, false);
                            $googleData['useFl'] = isset($googleData['useFl']) ? $googleData['useFl'] : 'y';
                            $tmpData[] = "<td>" . $googleData['useFl'] . "</td>";
                            unset($googleData, $googleQuery);
                            break;
                        default  :
                ...
                if ($key == '0') {
                    fwrite($fh, gd_implode(chr(13) . chr(10), $setHedData));
                    unset($setHedData);
                }
                fwrite($fh, gd_implode(chr(13) . chr(10), $tmpData));
                unset($tmpData);
            }
    ...
    /**
     * 조회된 정기결제(배송) 상품의 총 갯수를 바탕으로 페이지 수 계산
     *
     * @param int $totalNum : 조회된 정기결제(배송) 상품의 총 갯수
     * @return int
     */
    private function calculatePageNum(int $totalNum): int
    {
        // 페이지 수 계산 (excelPageNum 개수 기준으로 페이징 처리)
        if ($this->excelPageNum >= $totalNum) {
            return 0;
        } else {
            return ceil($totalNum / $this->excelPageNum) - 1;
        }
    }
     ...
     * @return string
     */
    private function getExcelFileName(): string
    {
        // 고유한 파일명을 생성 (마이크로타임을 숫자로 변환하여 중복 방지)
        $timestampParts = explode(' ', microtime());
        $timestampSum = array_sum($timestampParts);

        return $this->fileConfig['location'] . $timestampSum;
    }

    /**
     * 진행률 계산 및 진행률 50% 단위 또는 마지막 항목일 경우 JS로 전달
     *
     * @param int $rowIndex : 현재 처리 중인 데이터의 index 번호
     * @param int $totalNum : 전체 정기결제(배송) 상품 수
     * @return void
     */
    private function sendProgressScript(int $rowIndex, int $totalNum)
    {
        // 전체 진행률 계산 (0 ~ 100%)
        $progressPercent = ($totalNum > 1)
            ? round((100 / ($totalNum - 1)) * $rowIndex)
            : 100;

        // 진행률 50% 단위 또는 마지막 항목일 경우에만 JS로 전달
        $shouldReportProgress = (
            $rowIndex % round($this->excelPageNum * 0.5) === 0 ||
            $progressPercent === 100
        );

        if ($shouldReportProgress) {
            echo "<script>parent.progressExcel('" . gd_isset($progressPercent, 0) . "');</script>";
        }
    }
     ...
     *
     * @param $fileHandle
     * @param array $excelField : 엑셀 필드
     * @param array $excelFieldName : 엑셀 필드명
     * @return void
     */
    private function writeExcelTableHeader($fileHandle, array $excelField, array $excelFieldName)
    {
        fwrite($fileHandle, "<tr>");

        foreach ($excelField as $excelValue) {
            if (!isset($excelFieldName[$excelValue])) {
                continue;
            }
            fwrite($fileHandle, "<td class='title'>" . htmlspecialchars($excelFieldName[$excelValue]['name'], ENT_NOQUOTES) . "</td>");
        }
    ...
    /**
     * 엑셀에 입력할 정기 상품 리스트 데이터 행 반환
     *
     * @param string $field : 필드 key 값
     * @param $value : 해당 필드의 값
     * @param array $row : 전체 행 데이터
     * @param array $regularGoodsDeliveryCycleList : 정기결제(배송)상품 배송 주기 리스트
     * @param array $regularGiftPresentInfoList : 정기결제(배송) 상품 사은품 상세 정보 리스트
     * @param array $giftNmList : 사은품 명 리스트
     * @return string : HTML 테이블 셀(<td>) 형태로 포맷된 문자열
     */
    private function getFormattedCellValue(string $field, $value, array $row, array $regularGoodsDeliveryCycleList, array $regularGiftPresentInfoList, array $giftNmList): string
    {
        switch ($field) {
            case 'discountUseFl':
            case 'giftPresentUseFl':
                $useFl = $value === 'y' ? '사용함' : '사용안함';
                return "<td>" . htmlspecialchars($useFl, ENT_NOQUOTES) . "</td>";

            case 'giftPresentInfo' :
                return $this->getFormattedGiftPresentInfoCell($row, $regularGiftPresentInfoList, $giftNmList);

            case 'deliveryType':
                $deliveryType = $value === 'all' ? '전체(일반배송, 정기배송)' : '정기배송';
                return "<td>" . htmlspecialchars($deliveryType, ENT_NOQUOTES) . "</td>";

            case 'deliveryCycleType':
                return $this->getFormattedDeliveryIntervalCell($value, $row, $regularGoodsDeliveryCycleList);

            case 'deliveryRoundInfo':
                $display = '전체';
                if ($row['deliveryRoundsDisplayType'] === 'disabled') {
                    $display = '미설정';
                } elseif ($row['deliveryRoundsDisplayType'] === 'abled') {
                    $display = $row['maxDeliveryRounds'] . '회';
                }
                return "<td>" . htmlspecialchars($display, ENT_NOQUOTES) . "</td>";

            case 'applyStatus':
                $applyStatus = $value === 'abled' ? '신청 가능' : '신청 중지';
                return "<td>" . htmlspecialchars($applyStatus, ENT_NOQUOTES) . "</td>";

            case 'conditionType' :
                if (!empty($value)) {
                    $conditionType = $value === 'unconditional' ? '무조건' : '수량별';
                } else {
                    $conditionType = '';
                }
                return "<td>" . htmlspecialchars($conditionType, ENT_NOQUOTES) . "</td>";
            default:
                return "<td>" . htmlspecialchars($value, ENT_NOQUOTES) . "</td>";
        }
    }

    /**
     * 정기결제(배송) 상품 상세 정보에 따른 포맷된 셀 HTML 반환
     *
     * @param array $row : 전체 행 데이터
     * @param array $regularGiftPresentInfoList : 정기결제(배송) 상품 상세 정보리스트
     * @param array $giftNmList : 사은품명 리스트
     * @return string : HTML 테이블 셀(<td>) 형태로 포맷된 문자열
     */
    private function getFormattedGiftPresentInfoCell(array $row, array $regularGiftPresentInfoList, array $giftNmList): string
    {
        $giftPresentInfoCell = "<td style='mso-wrap-text:true; white-space:pre-wrap;'>";

        if (empty($regularGiftPresentInfoList[$row['regularGiftPresentSno']])) {
            return $giftPresentInfoCell . "</td>";
        }

        $lastIndex = count($regularGiftPresentInfoList[$row['regularGiftPresentSno']]) - 1;

        foreach ($regularGiftPresentInfoList[$row['regularGiftPresentSno']] as $index => $info) {

            // 사은품 증정 수량 구간
            $giftPresentInfoCell .= $info['conditionStart'] . ' ~ ' . $info['conditionEnd'] . '/';

            // 사은품 명
            $multiGiftNos = json_decode($info['multiGiftNo'], true);

            foreach ($multiGiftNos as $giftNo) {
                $giftPresentInfoCell .= ($giftNmList[$giftNo] ?? '이름 없음') . ', ';
            }

            // 마지막 쉼표와 공백 제거
            $giftPresentInfoCell = rtrim($giftPresentInfoCell, ', ');

            // 선택 수량
            if ($info['selectCnt'] === 0) {
                $giftPresentInfoCell .= ' / 선택수량 : 전체지급 / ';
            } else {
                $giftPresentInfoCell .= ' / 선택수량 : ' . $info['selectCnt'] . '개씩 지급 / ';
            }
            // 지급 수량
            $giftPresentInfoCell .= '지급수량 : ' . $info['giveCnt'] . '개씩 지급 /';

            // 줄 바꿈
            if ($index !== $lastIndex) {
                $giftPresentInfoCell .= "<br>";
            }
        }
    ...
    /**
     * 배송 간격 타입에 따른 포맷된 셀 HTML 반환
     *
     * @param string $type : 배송 간격 타입 (month 또는 week)
     * @param array $row : 전체 행 데이터
     * @param array $regularGoodsDeliveryCycleList : 정기결제(배송)상품 배송 주기 리스트
     * @return string : HTML 테이블 셀(<td>) 형태로 포맷된 문자열
     */
    private function getFormattedDeliveryIntervalCell(string $type, array $row, array $regularGoodsDeliveryCycleList): string
    {
        // 월 단위 배송 주기 출력
        if ($type === 'month') {
            // regularGoodsDeliveryCycle에서 유효한 monthCycle 값만 매핑하여 출력
            $months = array_map(function ($cycle) {
                return isset(DeliveryCycle::DELIVERY_CYCLE_MONTH[$cycle['monthCycle']])
                    ? DeliveryCycle::DELIVERY_CYCLE_MONTH[$cycle['monthCycle']] : null;
            }, $regularGoodsDeliveryCycleList[$row['sno']]);

            // null인 항목 제거하고 출력
            $months = array_filter($months);

            return "<td style='mso-wrap-text:true; white-space:pre-wrap;'>" . implode(', ', $months) . "</td>";
        }

        // 주 단위 배송 주기 출력
        if ($type === 'week') {
            // 주기 (n주) 텍스트 구성
            $weeks = array_filter(array_map(function ($cycle) {
                $week = $cycle['weekCycle'];

                // 매핑된 주 라벨이 있으면 사용, 없으면 숫자 + "주"로 출력
                return isset(DeliveryCycle::DELIVERY_CYCLE_WEEK[$week])
                    ? DeliveryCycle::DELIVERY_CYCLE_WEEK[$week] : null;

            }, $regularGoodsDeliveryCycleList[$row['sno']]));

            // 요일 (월~금) 텍스트 구성
            $days = array_filter(array_map(function ($cycle) {
                $day = $cycle['weekDayCycle'];

                // 매핑된 요일 라벨이 있으면 사용, 없으면 null 처리
                return isset(DeliveryCycle::DELIVERY_CYCLE_WEEK_DAY[$day])
                    ? DeliveryCycle::DELIVERY_CYCLE_WEEK_DAY[$day] . '요일' : null;

            }, $regularGoodsDeliveryCycleList[$row['sno']]));

            // 주기 + 요일 목록을 줄바꿈 포함하여 출력
            return "<td style='mso-wrap-text:true; white-space:pre-wrap;'>" . implode(', ', $weeks) . "<br>" . implode(', ', $days) . "</td>";
        }
     ...
     * 정기결제(배송) 상품 엑셀 파일
     *
     * @param array $whereCondition : 상품 조회 조건
     * @param string $excelField : 엑셀 필드
     * @param array $excelFieldName : 엑셀 필드명
     * @return bool : 엑셀 작성 성공 여부
     */
    public function getGoodsRegularGoodsList(array $whereCondition, string $excelField, array $excelFieldName): bool
    {
        ...
        // 조건에 맞는 엑셀 데이터 조회
        $regularGoodsData = $regularGoods->getRegularGoodsListToExcel($whereCondition);
        $totalNum = $regularGoodsData['totalCount'];
        $regularGoodsList = $regularGoodsData['regularGoodsList'];
        $regularGoodsDeliveryCycleList = $regularGoodsData['regularGoodsDeliveryCycleList'];
        $regularGiftPresentInfoList = $regularGoodsData['regularGiftPresentInfoList'];
        $giftNmList = $regularGoodsData['giftNmList'];

        $pageNum = $this->calculatePageNum($totalNum);

        // 페이지별로 엑셀 파일 생성
        for ($currentPage = 0; $currentPage <= $pageNum; $currentPage++) {
            ...
            // 파일 핸들 오픈 (쓰기 모드)
            $fileHandle = fopen($tmpFilePath, 'a+');

            // 엑셀 헤더 및 테이블 시작 태그 작성
            fwrite($fileHandle, $this->excelHeader . "<table border='1'>");
            ...
            // 헤더 행 출력
            $this->writeExcelTableHeader($fileHandle, $excelField, $excelFieldName);

            // 시작 인덱스
            $start = $currentPage * $this->excelPageNum;

            // 끝 인덱스
            $end = $start + $this->excelPageNum;
            $end = min($end, count($regularGoodsList));

            // 정기 상품 리스트 데이터 행 작성
            for ($rowIndex = $start; $rowIndex < $end; $rowIndex++) {

                $val = $regularGoodsList[$rowIndex];

                // 진행률 계산 및 진행률 50% 단위 또는 마지막 항목일 경우 JS로 전달
                $this->sendProgressScript($rowIndex, $totalNum);
                ...
                foreach ($excelField as $excelValue) {
                    ...
                    if ($excelValue === 'goodsNm' && $whereCondition['goodsNameTagFl'] == 'y') {
                        // 상품명 HTML 태그 제외
                        $value = isset($val[$excelValue]) ? StringUtils::removeTag($val[$excelValue]) : '';
                    } else {
                        $value = isset($val[$excelValue]) ? $val[$excelValue] : '';
                    }

                    $cellValue = $this->getFormattedCellValue($excelValue, $value, $val, $regularGoodsDeliveryCycleList, $regularGiftPresentInfoList, $giftNmList);
                    fwrite($fileHandle, $cellValue);
                }
                fwrite($fileHandle, "</tr>");
            }
                        ...
                        case 'mustInfo':
                            ...
                            if (is_array($_tmpArray)) {
                                ...
                                foreach ($_tmpArray['infoTitle'] as $k1 => $mustInfoData) {
                                    ...
                                    $_tmpMustInfo[] = gd_implode(" / ", $_tmpMustInfoData);
                                    unset($_tmpMustInfoData);
                                }
                                $tmpData[] = "<td>" . gd_implode("<br>", $_tmpMustInfo) . "</td>";
                                unset($_tmpMustInfo);
                            } else {
                                $tmpData[] = "<td></td>";
                            }
   ...
   */
    ...
    public function getMemberMemberList($whereCondition, $excelField, $excelFieldName)
    {
        ...
        if (gd_in_array('privateApprovalOption', $excelField) || gd_in_array('privateOffer', $excelField) || gd_in_array('privateConsign', $excelField)) {
        ...
        if (gd_in_array('mailAgreementDt', $excelField) || gd_in_array('smsAgreementDt', $excelField)) {
        ...
        foreach ($data as $k => $v) {
            ...
            foreach ($v as $key => $val) {
                ...
                foreach ($excelField as $excelKey => $excelValue) {
                    ...
                    switch ($excelValue) {
                        ...
                        case 'privateOffer':
                            ...
                            foreach ($privateAgreeChoice as $privateAgreeChoiceValue) {
                                if (gd_array_key_exists($privateAgreeChoiceValue['sno'], $val[$excelValue . 'Fl']) && $val['mallSno'] == DEFAULT_MALL_NUMBER) {
                ...
                $tmpDataRow[] = '</tr>';
                $tmpData[] = gd_implode('', $tmpDataRow);
            }

            $setHedData[] = '</tr>';
            $setData[] = gd_array_merge([gd_implode('', $setHedData)], $tmpData);
        }
    ...
    */
    ...
    public function getPlusreviewPlusreviewBoard($whereCondition, $excelField, $excelFieldName)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $setHeadData[] = "</tr>";
            $setData[] = gd_array_merge($setHeadData, $tmpData);
        }
   ...
   */
    public function getScmScmList($whereCondition, $excelField, $excelFieldName)
    {
        ...
        $count['staff'] = 0;
        $count['account'] = 0;
        if (gd_in_array('staff', $excelField)) {
            $count['staff'] = 1;
            $department = gd_code('02001'); // 부서
        }
        if (gd_in_array('account', $excelField)) {
            $count['account'] = 1;
        ...
        if ($count['staff'] > 0 || $count['account'] > 0) {
            foreach ($data as $k => $v) {
                if ($count['staff'] > 0 && empty($v['staff']) == false) {
                    ...
                    if (gd_count($_staff) > $count['staff']) $count['staff'] = gd_count($_staff);
                }
                if ($count['account'] > 0 && empty($v['account']) == false) {
                    $_account = json_decode($v['account']);
                    if (gd_count($_account) > $count['account']) $count['account'] = gd_count($_account);
                }
            }
        }
    ...
    */
    public function getScmScmAdjustList($whereCondition, $excelField, $excelFieldName)
    {
        ...
        if ($this->excelPageNum) {
            ...
            $convertData = array_chunk($data['convertData'], gd_count($data['convertData']), true);
                        ...
                        foreach ($excelField as $excelKey => $excelValue) {
                            ...
                            switch ($excelValue) {
                                ...
                                case 'optionInfo':
                                    ...
                                    if ($tmpOptionInfo) {
                                        $tmpData[] = "<td>" . gd_implode("<br/>", $tmpOptionInfo) . "</td>";
                                    } else {
                                        $tmpData[] = "<td></td>";
                                    }
                                    unset($tmpOptionInfo);
                                ...
                                case 'optionTextInfo':
                                    ...
                                    if ($tmpOptionTextInfo) {
                                        $tmpData[] = "<td>" . gd_implode("<br/>", $tmpOptionTextInfo) . "</td>";
                                    } else {
                                        $tmpData[] = "<td></td>";
                                    }
    ...
    */
    public function getOrderList($whereCondition, $excelField, $defaultField, $excelFieldName)
    {
        ...
        if (gd_in_array($this->fileConfig['location'], $userHandleMode)) {
        ...
        if (empty($orderData['totalCount']) === true && empty($orderData['orderList']) === true) {
            ...
            $isGenerator = true;
            $totalNum = gd_count($orderData['totalCount']);
        ...
        for ($i = 0; $i <= $pageNum; $i++) {
            ...
            foreach ($data as $key => $val) {
                $progress = ($totalNum == 1) ? 100 : round((100 / ($totalNum - 1)) * ($key + ($i * $this->excelPageNum)));
                ...
                if (empty($_tmpRefundAccount) == false) {
                    $val['refundAccountNumber'] = gd_implode(STR_DIVISION, $_tmpRefundAccount);
                    unset($_tmpRefundAccount);
                }
                if (empty($_tmpRefundAccountNumber) == false) {
                    $val['userRefundAccountNumber'] = gd_implode(STR_DIVISION, $_tmpRefundAccountNumber);
                ...
                if (gd_is_provider()) {
                    if ($orderType == 'goods') {
                        ...
                        $val['scmDeliveryCharge'] = gd_array_sum(array_combine($tmpScmDeliverySno, $tmpScmDeliveryCharge));
                    } else {
                ...
                foreach ($excelField as $excelKey => $excelValue) {
                    ...
                    switch ($excelValue) {
                        ...
                        case 'optionCode':
                            if ($tmpOptionCode) {
                                $tmpData[] = "<td class='xl24'>" . gd_implode("<br/>", $tmpOptionCode) . "</td>";
                            } else {
                                $tmpData[] = "<td></td>";
                            }
                        ...
                        case 'orderStatus':
                            ...
                            break;
                        case 'settleKind':
                            $settleKind = $orderAdmin->printSettleKind($val[$excelValue]);
                            switch ($val['settleKind']) {
                                case 'pk' : // 카카오페이 결제수단 구분 추가
                                    $settleKind .= '<br />' . self::KAKAOPAY_PAY_METHOD[$val['pgSettleNm']];
                                    break;
                            }
                            $tmpData[] = "<td>" . $settleKind . "</td>";
                            break;
                        ...
                        case 'ogi.giftNo':
                            ...
                            if ($gift) {
                                ...
                                if ($excelValue == 'ogi.presentSno') {
                                    $tmpData[] = "<td>" . gd_implode("<br>", $presentTitle) . "</td>";
                                } else if ($excelValue == 'ogi.giftNo') {
                                    $tmpData[] = "<td>" . gd_implode("<br>", $giftInfo) . "</td>";
                                } else if ($excelValue == 'totalGift') {
                                    $tmpData[] = "<td>" . gd_implode("<br>", $totalGift) . "</td>";
                                }
                            } else {
                                $tmpData[] = "<td></td>";
                            }
                        ...
                        case 'receiverAddressTotal':
                            ...
                            if ($val['mallSno'] != $this->gGlobal['defaultMallSno']) {
                                ...
                                $countriesCode = array_combine(gd_array_column($countriesCode, 'code'), gd_array_column($countriesCode, 'countryNameKor'));
                        ...
                        case 'addGoodsNo':
                            ...
                            if ($addGoods) {
                                ...
                                $tmpData[] = "<td>" . gd_implode("<br>", $addGoodsInfo) . "</td>";
                        ...
                        case 'useCouponNm': // 사용된 쿠폰명
                            ...
                            $tmpCouponNmText = gd_array_merge($tmpGoodsCouponNmText, gd_array_merge($tmpOrderCouponNmText, $tmpDeliveryCouponNmText));
                            $tmpCouponNm = gd_implode('<br/>', $tmpCouponNmText);
                            $tmpData[] = '<td>' . $tmpCouponNm . '</td>';
                            break;
                        case 'goodsDiscountInfo':
                            ...
                            $tmpGoodsDiscountInfo = gd_implode('<br/>', $tmpGoodsDiscountInfoText);
                        ...
                        case 'memberPolicy':
                            ...
                            $tmpMemberPolicy = gd_implode('<br/>', $tmpMemberPolicyText);
                ...
                if ($key == '0') {
                    fwrite($fh, gd_implode(chr(10), $setHedData));
                    unset($setHedData);
                }
                ...
                if ($whereCondition['optionCountFl'] === 'per') {
                    ...
                    for ($tmpDataCnt = 1; $tmpDataCnt <= $tmpGoodsCnt; $tmpDataCnt++) {
                        fwrite($fh, gd_implode(chr(10), $tmpData));
                    }
                } else {
                    fwrite($fh, gd_implode(chr(10), $tmpData));
                }
     ...
     * @return bool
     */
    public function getRegularOrderList(array $rows): bool
    {
        $fileName = 'regular_order_list' . array_sum(explode(' ', microtime()));
        $tmpFilePath = \UserFilePath::data('excel', 'order', $fileName . ".xls")->getRealPath();

        $fh = fopen($tmpFilePath, 'a+');
        fwrite($fh, $this->excelHeader . "<table border='1'>");

        fwrite($fh, implode(chr(10), $rows));
        fwrite($fh, "</table>");
        fwrite($fh, $this->excelFooter);
        fclose($fh);
    ...
    */
    public function getOrderTaxInvoiceRequest($whereCondition, $excelField, $excelFieldName)
    {
        ...
        if (empty(gd_array_intersect($rowspanField, $excelField)) === true) {
            $rowspanFix = 1;
        }

        if (empty($whereCondition['orderNo']) === false) {
            foreach ($whereCondition as $key => $value) {
                if (gd_in_array($key, ['orderNo', 'pageNum'])) continue;
        ...
        foreach ($data as $k => $v) {
            ...
            $tmpData = [];
            foreach ($v as $key => $val) {
                $rowspan = $rowspanFix ?? gd_count($val['taxInvoiceInfo']);
                if (($key % 20 == 0 && $key != '0') || $totalNum - 1 == $key) {
                    $progressPercent = ($totalNum - 1) > 0 ? round((100 / ($totalNum - 1)) * $key) : 100;
                    echo "<script> parent.progressExcel('" . $progressPercent . "'); </script>";
                }
    ...
    */
    ...
    public function getInfoExcelRequest($sno = null, $goodsField = null, $arrBind = null, $dataArray = false)
    {
        ...
        if ($sno) {
            ...
            if (is_array($sno) === true) {
                $arrWhere[] = "er.sno IN ('" . gd_implode("','", $sno) . "')";
                // 상품 코드가 하나인경우
            } else {
                $arrWhere[] = 'er.sno = ?';
     ...
     * @return mixed
     */
    public function getVisitIpExcel($data, $dataType, $searchData, $excelField, $excelFieldName)
    {
        $useCellChar = ['A', 'B', 'C'];
        if ($dataType == 'list') {
            array_push($useCellChar, 'D');
        }
        $lastCellChar = $useCellChar[gd_count($useCellChar) - 1];
        ...
        foreach ($data as $k => $v) {
            ...
            $tmpData[$k] = gd_array_merge($tmpHeader, $tmpData[$k]);
            unset($tmpHeader);
        }
    ...
    public function setStyleExcelTitle($obj, $startChar, $cellNum, $useCellChar)
    {
    ...
    public function getDeleteLapseOrderList($sno, $excelField, $defaultField, $excelFieldName)
    {
        ...
        for ($i = 0; $i <= $pageNum; $i++) {
            ...
            $fileName = $this->fileConfig['location'] . '_' . gd_array_sum(explode(' ', microtime()));
            $tmpFilePath = UserFilePath::data('excel', $this->fileConfig['menu'], $fileName . ".xls")->getRealPath();
                    ...
                    switch ($excelValue) {
                        case 'detail':
                            ...
                            if ($result['viewCnt'] > 500) {
                                ...
                                $tmpSearchData = array();
                                for ($i = 0; $i < gd_count($searchData); $i++) {
                                    if ($i % 500 == 0) {
                                        $tmpSearchData[] = $searchData[$i];
                                        if ($i == 0) {
                                            $searchTargetMsg = '처리대상 : ' . gd_implode(',', $tmpSearchData);
                                        } else {
                                            $searchTargetMsg = gd_implode(',', $tmpSearchData);
                                        }
