...
class Webftp
{
    ...
    public function __construct()
    {
        ...
        $this->_appURL = Request::getDomainUrl() . Request::getPhpSelf();

        // Load the configuration file
        $this->_config = App::getConfig('host.webftp')->toArray();

        // Set the file types array to a global variable
        $this->_fileTypes = App::getConfig('host.filetype')->toArray();
    }
    ...
    public function zipDirectory($directory)
    {
        ...
        if ($this->_config['zip_dirs']) {
            ...
            header("Content-Disposition: attachment; filename=\"$filenameNoExt.zip\"");
     ...
     *
     * @param mixed $directory Path to be breadcrumbified
     *
    ...
    {
        $permission = [
            'is_readable'   => is_readable($filePath) ? 'read' : null,
            'is_writable'   => is_writable($filePath) ? 'write' : null,
            'is_executable' => is_executable($filePath) ? 'execute' : null,
        ];

        return ArrayUtils::removeEmpty($permission);
    }
     ...
     * @access protected
     */
    private function _setDirectoryPath($dir)
    {
    ...
    private function _readDirectory($directory, $sort = 'natcase')
    {
        ...
        // Sort the array
        $reverseSort = gd_in_array($this->_directory, $this->_config['reverse_sort']);
    ...
    private function _arraySort($array, $sortMethod, $reverse = false)
    {
        ...
        if ($this->_config['list_folders_first']) {
            ...
            if ($reverse) {
                $sortedArray[1] = gd_array_reverse($sortedArray[1]);
                $sortedArray[2] = gd_array_reverse($sortedArray[2]);
            }
    ...
    private function _isHidden($filePath)
    {
        ...
        if ($this->_config['hide_dot_files']) {
            $this->_config['hidden_files'] = gd_array_merge(
    ...
    private function _getRelativePath($fromPath, $toPath)
    {
        ...
        $arrayMax = max(gd_count($fromPathArray), gd_count($toPathArray));
        ...
        $relativePath = gd_implode('/', $diffArray);
     ...
     * @author Jong-tae Ahn <qnibus@godo.co.kr>
     */
    public function isRecursivelyDeleteable($directory)
    {
        $stack = [$directory];
