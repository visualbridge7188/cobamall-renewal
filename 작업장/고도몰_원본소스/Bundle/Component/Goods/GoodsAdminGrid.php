...
class GoodsAdminGrid
{
        ...
        */
        ...
        'regular_goods_list' => [
            'defaultList' => [ // 디폴트 리스트
                ...
                'regularPrice', // 정기결제가
                'scmNo', // 공급사
                'regularDeliveryCycleType', // 배송 주기
                'regularDeliveryRoundsDisplayType', // 종료회차 설정
                'applyStatus', // 신청상태
                'regDt' // 등록일/수정일
            ],
            'list' => [ // 전체리스트
                ...
                'goodsPrice', // 판매가
                'regularPrice', // 정기결제가
                'regularDiscountUseFl', // 정기할인
                'scmNo', // 공급사
                'regularDeliveryType', //배송방법 노출여부
                'regularDeliveryCycleType', // 배송 주기
                'regularDeliveryRoundsDisplayType', // 종료회차 설정
                'regularGiftPresentUseFl', //사은품 사용
                'applyStatus', // 신청상태
                'regDt', // 등록일/수정일
                'memo' //관리자 메모
            ],
	...
	public $goodsKeyNameList = [
		...
		'memo' => '관리자메모', // 관리자메모,
        'regularDiscountUseFl' => '정기할인', // 정기할인
		'regularPrice' => '정기결제가', // 정기결제가
        'regularDeliveryType' => '배송방법 노출여부', // 배송방법 노출여부
		'regularDeliveryCycleType' => '배송 주기', // 배송 주기
		'regularDeliveryRoundsDisplayType' => '종료회차', // 종료회차
        'regularGiftPresentUseFl' => '사은품 사용', // 사은품 사용
		'applyStatus' => '신청상태', // 신청 상태
		'regDt' => '등록일/수정일', // 등록일, 수정일 - regDt, modDt
		'delDt' => '등록일/삭제일', // 등록일, 삭제일 - regDt, delDt
	];
	...
	public function getGoodsGridConfigList($goodsGridMode)
	{
		...
		// 교집합 리스트
		$goodsGridConfigList['intersect'] = gd_array_intersect(gd_array_flip($goodsGridConfigList['all']), gd_array_flip($goodsGridConfigList['select']));
		// 노출항목 리스트 (추가기능)
	...
	private function setListNameApply($goodsGridlist)
	{
		...
		//키, 배열 전환
		$goodsGridlistReverse = gd_array_flip($goodsGridlist ?? []);
		//공통 값 체크
		$listTmp_stdKey = gd_array_intersect_key($goodsGridlistReverse, $this->goodsKeyNameList);
		$listTmp_stdValue = gd_array_intersect_key($this->goodsKeyNameList, $goodsGridlistReverse);
    ...
    private function setListOptionNameApply($goodsGridlist)
    {
        ...
        $goodsGridlistReverse = gd_array_flip($goodsGridlist);
        //공통 값 체크
        $listTmp_stdKey = gd_array_intersect_key($goodsGridlistReverse, $this->goodsKeyOptionNameList);
        $listTmp_stdValue = gd_array_intersect_key($this->goodsKeyOptionNameList, $goodsGridlistReverse);
    ...
    private function setListBatchStockNameApply($goodsGridlist)
    {
        ...
        $goodsGridlistReverse = gd_array_flip($goodsGridlist);
        //공통 값 체크
        $listTmp_stdKey = gd_array_intersect_key($goodsGridlistReverse, $this->goodsKeyBatchStockNameList);
        $listTmp_stdValue = gd_array_intersect_key($this->goodsKeyBatchStockNameList, $goodsGridlistReverse);
	 ...
	 */
	public function getGoodsAdminGridMode()
	{
		$goodsGridMode = '';
	...
	public function getGoodsAdminAddField($selectData)
	{
		$setData = [];
		if($selectData) {
			$addData = gd_array_flip((array)$this->goodsFieldAddList);
			$selectData = gd_array_flip((array)$selectData);
			foreach ( $selectData as $key => $value) {
				if(gd_in_array($value, $addData)) {
    ...
    /**
     * GoodsScmMainList 변수값 내 키값으로 defaultList 반환
     *
     * @param string $key : 조회의 기준이 될 키값
     * @return array : 조회된 defaultList
     */
    public function getGoodsScmMainListDefaultListByKey(string $key): array
    {
        return $this->goodsScmMainList[$key]['defaultList'];
    }
}
