...
class OivsObject
{
    ...
    /**
     * des Triple-DES Symmetric Key Algorithm
     *
     * @param $key
     * @param $message
     * @param $encrypt
    ...
    function des($key, $message, $encrypt, $mode, $iv)
    {
        ...
        $iterations = ((gd_count($keys) == 32) ? 3 : 9); //single or triple des
        if ($iterations == 3) {
    ...
    /**
     * des_createKeys Triple-DES Key Generating
     *
     * @param $key
     * @return array
     */
    function des_createKeys($key)
    {
        ...
        for ($j = 0; $j < $iterations; $j++) {
            ...
            $left ^= $temp;
            $right ^= ($temp >> 16 & 0x0000ffff) + (($temp & 0x0000ffff) << 16);
            ...
            for ($i = 0; $i < gd_count($shifts); $i++) {
                ...
                $keys[$n++] = $righttemp ^ (($temp & 0x0000ffff) << 16);
            }
        } //	for each iterations
    ...
    /**
     * getRandomKey Random Key Generate
     *
     * @param $digits
     * @return string
     */
    ...
    function resolveClientData($sendInfo)
    {
        ...
        if (gd_count($aryValues) < 10)
            ...
            $this->message = urldecode($aryValues[5]);
            $this->paKey = $aryValues[6];
            $this->niceNm = urldecode($aryValues[7]);
