...
use Framework\Helper\LogMessageHelper;
...
class Member extends \Component\AbstractComponent
{
    ...
    public function getMemberInfo($memNo = null)
    {
        ...
        $this->db->strField = gd_implode(', ', $arrField['member']) . ', ' . gd_implode(', ', $arrField['memberGroup']);
        ...
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_MEMBER . ' m ' . gd_implode(' ', $query);
    ...
    public function getMemberId($memNo)
    {
        ...
        $this->db->strField = gd_implode(', ', $arrField[0]) . ', ' . gd_implode(', ', $arrField[1]);
        $this->db->strJoin = 'INNER JOIN ' . DB_MEMBER_GROUP . ' mg ON m.groupSno = mg.sno ';
    ...
    public function checkSleepMember($sleepFl, $memId, $memPw)
    {
        ...
        if ($sleepFl === 'y') {
            ...
            $returnUrl = (Request::isFromMyapp() === true || Request::isMyapp() === true || Request::isByapps() === true) ? '../../member/wake.php' : '../member/wake.php';
    ...
    public function login($memId, $memPw)
    {
        ...
        $logger->channel('userLogin')->info('로그인 시도', [$this->getRequestData()]);

        // 동일 IP 접근제한 로그 (로그인 시도) - 마이앱의 경우에는 사용하지 않도록 추가
        if (Request::post()->get('myappApi') != true && Request::isByapps() != true) {
            $loginIp = Request::getRemoteAddress();
            $this->setLogIpUserLoginTry($loginIp);
        }

        $session = App::getInstance('session');
        if ($session->has(Member::SESSION_MEMBER_LOGIN) && (Request::isMyapp() != true && Request::isByapps() != true)) {
            throw new AlertRedirectException(__('회원으로 로그인 된 상태입니다.'), 300, null, '/');
        }

        if (Validator::userid($memId, true, false) === false) {
            throw new Exception(sprintf(__('입력된 %s은(는) 잘못된 형식입니다.'), __('아이디')), 500);
        }

        if (Validator::required($memPw) === false) {
            throw new Exception(sprintf(__('입력된 %s은(는) 잘못된 형식입니다.'), __('정보')), 500);
        }

        // 동일 IP 로그인 연속 시도에 의한 접속제한 처리
        $logIpLoginTryInfo = $this->selectLogIpUserLoginTry($loginIp, 'Y');
        if ($logIpLoginTryInfo['limitFlag'] === 'Y') {
            if ($this->isCheckLoginTimeout($logIpLoginTryInfo['onLimitDt'])) {
                $logger->channel('userLogin')->warning('로그인이 제한된 회원입니다.', [$this->getRequestData(['userLoginTryLimit' => $logIpLoginTryInfo,]),]);
                throw new Exception(__('존재하지 않거나 잘못된 정보로 잦은 로그인 시도하였습니다. 정보보호를 위해 15분간 접속이 차단됩니다.'), 500);
            }
        }

        $memberWithGroup = $this->getMemberByLogin($memId);
        if ($memberWithGroup['loginLimit']['limitFlag'] == 'y') {
            if ($this->isGreaterThanLimitLoginTime($memberWithGroup['loginLimit']['onLimitDt'])) {
                $logger->channel('userLogin')->warning('로그인이 제한된 회원입니다.', [$this->getRequestData(['loginLimit' => $memberWithGroup['loginLimit'],]),]);
                throw new \Exception(__('로그인이 제한되었습니다. 10분 후에 시도해 주세요.'), 500);
            } else {
                $initLimitLoginLog = $this->initLimitLoginLog($memberWithGroup);
                $memberWithGroup = $initLimitLoginLog['member'];
                unset($initLimitLoginLog);
            }
        }
        if ($session->has(SESSION_GLOBAL_MALL)) {
            $mallBySession = $session->get(SESSION_GLOBAL_MALL);
            if ($memberWithGroup['mallSno'] != $mallBySession['sno']) {
                $logger->channel('userLogin')->warning(
                    '가입된 상점이 다릅니다.', [
                        $this->getRequestData(
                            [
                                'memberMallSno'  => $memberWithGroup['mallSno'],
                                'sessionMallSno' => $mallBySession['sno'],
                            ]
                        ),
                    ]
                );
                throw new LoginException(null, '회원정보를 찾을 수 없습니다.', LoginException::CODE_NOT_FOUND_MEMBER);
            }
        } elseif ($memberWithGroup['mallSno'] != DEFAULT_MALL_NUMBER) {
            $logger->channel('userLogin')->warning(
                '가입된 상점이 다릅니다.', [
                    $this->getRequestData(
                        [
        ...
        if (!is_array($memberWithGroup) && $memberWithGroup['memId'] != $memId) {
            $logger->channel('userLogin')->warning(
                '로그인 아이디가 다릅니다.', [
                    $this->getRequestData(
                        [
                            'memberMallId' => $memberWithGroup['memId'],
                        ]
                    ),
                ]
            );
        ...
        $memberWithGroup = $this->credentialMemberPassword($memPw, $memberWithGroup);
        $hackOutParams = [
            'memNo' => $memberWithGroup['memNo'],
            'memId' => $memberWithGroup['memId'],
        ];
        $this->_validateHackOuMember($hackOutParams);

        if (Request::post()->get('myappApi') != true) {
            $this->checkSleepMember($memberWithGroup['sleepFl'], $memId, $memPw);
            $this->_validateApplyFlag(gd_isset($memberWithGroup['appFl'], 'n'));
            $this->_checkAdultFlagAndUpdate($memberWithGroup);
        }
    ...
    public function credentialMemberPassword($loginPwd, $data)
    {
        ...
        $migrationType = gd_policy('migration.info');
        if ($migrationType['previousService'] == 'enamoo') {
            $enamooToGodomall = \App::load('\\Component\\Migration\\EnamooToGodomall');
            $enamooPassword = $enamooToGodomall->fetchEnamooPassword($loginPwd, $data);
            if (!empty($enamooPassword)) {
                $data['memPw'] = $enamooPassword;
                return $data;
            }
        }
        ...
        if (isset($data) && is_array($data)) {
            ...
            foreach ($data as $dKey => $dVal) {
                if (gd_in_array($dKey, $encryptKey)) {
        ...
        throw new LoginException($data['memId'], '회원정보를 찾을 수 없습니다.', LoginException::CODE_NOT_FOUND_MEMBER);
    }
    ...
    public function saveLoginLog($memNo)
    {
        ...
        if (empty($tmp['sno']) === true) {
            ...
            $arrBind = $this->db->get_binding(DBTableField::tableMemberLoginlog(), $arrData, 'insert', gd_array_keys($arrData));
...
//            $db->strLimit = '1';
        ...
        $data = gd_array_merge($memberData, $sleepMemberData);
    ...
    public function updatePassword($memId, $memPw)
    {
        $arrUpdate[] = 'memPw = ?';
        $arrUpdate[] = 'modDt = now()';
        $arrUpdate[] = 'changePasswordDt = now()';
     ...
     * @param $memPw
     */
    public function checkPassword($memberNo, $memPw)
    {
        $arrBind = [];
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT '. array_shift($query). ' FROM '.  DB_MEMBER. gd_implode(' ', $query);
        $oldPw = $this->db->query_fetch($strSQL, $arrBind, false)['memPw'];
        return Digester::isValid($oldPw, $memPw);
    }
    ...
    public function updateMemberByMembersNo(array $membersNo, array $field, array $data)
    {
        ...
        $strWhere = 'memNo IN(' . gd_implode(',', array_fill(0, gd_count($membersNo), '?')) . ')';
    ...
    {
        $this->recodeMailRejectionLog($rejectEmail);
    ...
    /**
     * 이메일 수신 거부 시, 경로 관련 로그 기록
     *
     * @return void
     */
    private function recodeMailRejectionLog($rejectEmail) {
        $flowArr = LogMessageHelper::getTracePathArray();

        $server = \Request::server();
        $referrer = $server->get('HTTP_REFERER');
        $httpUserAgent = $server->get('HTTP_USER_AGENT');
        $remoteAddr = $server->get('REMOTE_ADDR');

        \Logger::channel('mail')->info(get_class($this) . ' ' . $rejectEmail .' {REFERRER: [' . $referrer . '], HTTP_USER_AGENT: [' . $httpUserAgent . '], REMOTE_ADDR: [' . $remoteAddr . ']}, FLOW: ', $flowArr);
    }
    ...
    public function join($params)
    {
        ...
        if ($joinField['captcha']['use'] === 'y') {
            ...
            $altchaPayload = gd_isset($params['altcha']);
            if (!empty($altchaPayload)) {
                $rst = \App::getInstance(AltchaService::class)->verify((string) $altchaPayload);
            } else {
                $captcha = \App::load(Captcha::class);
        ...
        $hasGoogleUserProfile = $session->has(GoogleLoginService::SESSION_USER_PROFILE);
        $passValidation = $hasPaycoUserProfile  ||
                          $hasNaverUserProfile  ||
        ...
        if (
            $hasWonderUserProfile === false
            && !($hasKakaoUserProfile && \App::getInstance(KakaoLoginPolicy::class)->useKakaoSync()) // (카카오 로그인 & 카카오 싱크)가 아니면 전화번호 인증해야 함
            && $authCellPhonePolicy->get('useFl', 'n') === 'y' 
            && $ipinPolicy->get('useFl', 'n') === 'n'
            && !$session->has('simpleJoin')
        ) {
        ...
        $callbackFunction = 'insertMember';
        if ($passValidation && !$hasAppleUserProfile) {
            $callbackFunction = 'insertMemberByThirdParty';
        }
        $memNo = SimpleCache::init(SimpleCache::REDIS)
            ->executeWithLock($member['memId'], 'memberGroup', [$this->memberDAO, $callbackFunction], $member);
        $member['memNo'] = $memNo;
    ...
    public function register(\Component\Member\MemberVO $vo)
    {
        ...
        \DB::transaction(
            function () use ($vo, $insertMemberData) {
        ...
        } else if ($kakaoCloudAutoConfig['useFlag'] == 'y' && $kakaoCloudAutoConfig['memberUseFlag'] == 'y') {
    ...
    public function getDataByCrm($memberNo)
    {
        ...
        if (ArrayUtils::isEmpty($member) === true) {
            throw new WarningException(__('회원을 찾을 수 없습니다.'));
        }
     ...
     * @throws Exception
     */
    public function getMemberDataWithChecked($memNo, &$memberData, &$checked = null)
    {
        if (Validator::number($memNo, null, null, true) === false) {
            throw new WarningException(__('유효하지 않은 회원번호 입니다.'));
        }
    ...
    public function getMemberDataOrderWrite($memNo)
    {
        ...
        $strSQL = 'SELECT ' . @gd_implode(',', $strField) . ' FROM ' . DB_MEMBER_INVOICE_INFO . ' WHERE memNo = ?';
        $_arrBind = ['i', $memNo];
    ...
    {
        $tmpData = $this->getDataByTable(DB_MEMBER_HACKOUT, gd_array_values($hackOutParams), gd_array_keys($hackOutParams));
    ...
    public function setSimpleJoinLog($memNo, $memInfo, $coupon, $eventType)
    {
        ...
        $data['memberCouponNo'] = gd_implode(INT_DIVISION, gd_array_column($coupon, 'memberCouponNo'));
        $data['couponNm'] = gd_implode(STR_DIVISION, gd_array_column($coupon, 'couponNm'));
        $data['eventType'] = $eventType;
        $arrBind = $this->db->get_binding(DBTableField::tableMemberSimpleJoinLog(), $data, 'insert', gd_array_keys($data));
     ...
     * @return array
     */
    public function getSimpleJoinLog($searchData, $searchPeriod, $eventType)
    {
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_MEMBER_SIMPLE_JOIN_LOG . ' msl ' . gd_implode(' ', $query);
    ...
    {
        $data['eventType'] = $eventType;
        $arrBind = $this->db->get_binding(DBTableField::tableMemberSimpleJoinPushLog(), $data, 'insert', gd_array_keys($data));
    ...
    public function getSimpleJoinPushLog($searchData, $searchPeriod = 6) {
        ...
        $query = $this->db->query_complete();
        $strSQL = ' SELECT '. array_shift($query) .' FROM ' . DB_MEMBER_SIMPLE_JOIN_PUSH_LOG . gd_implode(' ', $query);
