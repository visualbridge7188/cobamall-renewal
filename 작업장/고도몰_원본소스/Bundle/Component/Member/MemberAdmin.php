...
class MemberAdmin extends \Component\Member\Member
{
    ...
    {
        parent::__construct();
        $this->fieldTypes = gd_array_merge(DBTableField::getFieldTypes('tableMember'), DBTableField::getFieldTypes('tableMemberMileage'));
    ...
    public function getMemberList(array $arrWhere = null, $sort = null, array $arrBind = null, $start = null, $limit = null, array $arrIncludeField = null, array $arrExcludeField = null)
    {
        ...
        $this->db->strField = gd_implode(', ', DBTableField::setTableField('tableMember', $arrIncludeField, $arrExcludeField, 'm'));
    ...
    public function getMemberMileagePageList($arrData)
    {
        ...
        $page->recode['total'] = $this->mileageDAO->countMemberBatchMileageList($arrData['listType']); // 검색 레코드 수
        $page->setPage();
        $page->setUrl(Request::getQueryString());
    ...
    public function modifyMemberData($arrData, $from = null)
    {
        ...
        $historyFilter = gd_array_keys($arrData);

        // 회원 등급 변경 또는 회원 정보 변경 시 웹훅 발송용 데이터 세팅
        ...
        if (isset($arrData['memPw'])) {
            ...
            if(Digester::isValid($before['memPw'], $memPw)){
        ...
        if ($result == 1) {
            // 사업자 등록증 등록
            if ($arrData['memberFl'] == 'business' && !empty($arrData['companyCertification'])) {
                $companyCertificationService = \App::getInstance(\Component\Member\Company\CompanyCertification::class);
                $companyCertificationService->upload($arrData['companyCertification'], $arrData['memNo']);
                $arrData['companyCertification'] = $arrData['companyCertification']['name'][0]; // 회원변경 로그를 위해 업로드된 이미지명 할당
            }

            // 사업자 등록증 삭제
            if ($arrData['memberFl'] == 'business' && !empty($arrData['certificationDeleteSno'])) {
                $companyCertificationService = \App::getInstance(\Component\Member\Company\CompanyCertification::class);
                if (empty($arrData['companyCertification'])) {
                    $arrData['companyCertification'] = '삭제'; // 회원변경 로그 (덮어 쓰기가 아닌 경우만 삭제 로그 추가)
                }
            ...
            $historyService->addBeforeAndAfter($arrData);
            ...
            if (isset($arrData['memPw'])) {
                ...
                // 마이앱 로그아웃 처리를 위한 토픽 발행
                MemberUtil::notifyPasswordChanged($arrData['memNo']);
            }
            ...
            if ($before['groupSno'] !== $after['groupSno'] || $from == 'excel') {
                ...
                if ($from != 'excel') {
                    ...
                    // 회원 등급 변경 시 웹훅 발송
                    $memberWebHookService->sendGradeChangedWebHook($after);
                }
            ...
            // 회원 정보 변경 시 웹훅 발송
            $memberWebHookService->sendInfoChangedWebHook($after);
        } else {
    ...
    public function applyGroupGradeByMemberNo($groupSno, $memberNo)
    {
        ...
        $this->db->bind_param_push($arrBind, 's', gd_implode(',', $memberNo));
    ...
    {
        $count = gd_count($arrWhere);
        $where = $count > 0 ? ' WHERE ' . gd_implode(' and ', $arrWhere) : '';
        $strSQL = 'SELECT * FROM ' . DB_MEMBER . $where;
        $data = $this->db->query_fetch($strSQL, (gd_count($arrWhere) > 0 ? $arrBind : null));
    ...
    public function getRecommIdList($memNo)
    {
        ...
        $funcLists = function ($arrBind, $arrWhere) use ($page) {
            ...
            $strSQL = 'SELECT /* 추천인 조회 */' . array_shift($query) . ' FROM ' . DB_MEMBER . ' ' . gd_implode(' ', $query);
    ...
    public function getNotificationReplaceCodes(array $memberNos, ?array $replaceKeys = null): array
    {
        ...
        // 휴먼 예정 정보 지정
        /** @var \Bundle\Component\Member\Sleep\SleepService $sleepService*/
        $sleepService = App::load(SleepService::class);
        $memberSleepConfig = gd_policy('member.sleep');
    ...
    protected function calculatePassJoinDays(?string $entryDt): string
    {
        $start = $entryDt && $entryDt != '0000-00-00 00:00:00'
            ? Carbon::parse($entryDt)
            : Carbon::now();
        $now = Carbon::now();
        $diffDays = $start->diffInDays($now);
        return number_format($diffDays);
    }

    protected function calculateSleepScheduleDate(?string $expirationFl, ?string $lastLoginDt): string
    {
        $expirationFl = $expirationFl ?? "1";
        $expirationDays = (int)$expirationFl * 365; // 개인정보 유효기간 일수

        $lastLoginDt = $lastLoginDt ?? "";

        $sleepScheduleDt = DateTimeUtils::dateFormatByParameter(
            'Y-m-d',
            $member['lastLoginDt'],
            '+' . $expirationDays . ' day'
        );
