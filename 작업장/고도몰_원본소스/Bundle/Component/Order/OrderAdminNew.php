 ...
 */
class OrderAdminNew extends \Component\Order\OrderNew
{
    ...
    protected function _setSearch($searchData, $searchPeriod = 7, $isUserHandle = false)
    {
        ...
        if ($this->search['exceptOrderStatus']) { //예외처리할 주문상태 쿼리
            $exceptStatusQuery = gd_implode("','", $this->search['exceptOrderStatus']);
    ...
    public function getOrderListForAdmin($searchData, $searchPeriod, $isUserHandle = false)
    {
        ...
        if($page->hasRecodeCache('total') === false) {
            if (Manager::isProvider()) {
                // 검색된 주문의 개수
                $total = $this->db->query_fetch('SELECT COUNT(distinct(og.sno)) AS cnt FROM ' . DB_ORDER_GOODS . ' og ' . gd_implode(' ', str_ireplace('limit ' . $page->recode['start'] . ',' . $searchData['pageNum'], '', $query)), $this->arrBind, true);
                ...
                $total = $this->db->query_fetch('SELECT (o.realTaxSupplyPrice + o.realTaxFreePrice + o.realTaxVatPrice) AS price, COUNT(distinct(og.sno)) AS cnt FROM ' . DB_ORDER_GOODS . ' og ' . gd_implode(' ', str_ireplace('limit ' . $page->recode['start'] . ',' . $searchData['pageNum'], '', $query)), $this->arrBind, true);
            }
    ...
    public function getOrderListForAdminMobileapp($searchData, $searchPeriod, $isUserHandle = false)
    {
        ...
        if ($this->search['statusMode'] != 'all' && $this->search['statusMode'] != '') {
            ...
            array_push($this->arrWhere, '(' . gd_implode(' OR ', $tmpWhere) . ')');
    ...
    public function getOrderView($orderNo, $orderGoodsNo = null, $handleSno = null, $statusMode = null, $excludeStatus = null, $orderStatusMode = null)
    {
        ...
        if (empty($getData) === true) {
            throw new WarningException(sprintf(__('[%s] 주문 정보가 존재하지 않습니다.'), $orderNo));
        }
...
//                $getData['cnt']['delivery'][$val['deliverySno']] += 1 + $addGoodsCnt;
        ...
        $getData['naverpay']['totalDiscount'] = $totalProductDiscountAmount + $totalDeliveryDiscountAmount + $getData['naverpay']['NaverMileagePaymentAmount'] + $getData['naverpay']['ChargeAmountPaymentAmount'];
        ...
        if(gd_count($updateData) > 0){
            ...
            if ($presentOrder->isPresentOrder((string) $arrData['orderNo'])) {
                ...
                if ($orderGoodsNo === null || $orderGoodsNo <= 0) {
                    \Logger::channel('order')->error(__METHOD__ . ' 선물하기 수령자 정보 업데이트 실패: orderGoodsNo가 없거나 유효하지 않음', ['orderNo' => $arrData['orderNo'] ?? '', 'orderGoodsNo' => $orderGoodsNo, 'bundle' => $arrData['bundle'] ?? []]);
...
//                        $fieldData['update']['userHandleSno'] = $bundleData['sno'];
     ...
     * @throws Exception
     */
    public function updateStatusPreprocessFront($orderNo, $goodsData, $statusMode, $changeStatus, $reason = false, $bundleFl = false, $mode = null, $useVisit = null, $autoProcess = false, $userHandleFl = false)
    {
        $isFront = true;
        $result = $this->updateStatusUnconditionalPreprocess($orderNo, $goodsData, $statusMode, $changeStatus, $reason, $bundleFl, $mode, $useVisit, $autoProcess, $isFront);
    ...
    public function updateStatusUnconditionalPreprocess($orderNo, $goodsData, $statusMode, $changeStatus, $reason = false, $bundleFl = false, $mode = null, $useVisit = null, $autoProcess = false, $isFront = false)
    {
        ...
        $changeCheck = false;
        $userHandleFl = $userHandleFl ?? null;
        if (gd_in_array(substr($changeStatus, 0, 1), $this->statusStandardCode[$statusMode]) || $isFront == true) {
            ...
            foreach ($goodsData as $key => $val) {
                ...
                if($channel != 'naverpay') {
                    if (substr($val['orderStatus'], 0, 1) == 'e' && substr($changeStatus, 0, 1) != 'e') {
                        $changeStatusCheck = false;
                    }
                    if (substr($val['orderStatus'], 0, 1) == 'z' && substr($changeStatus, 0, 1) != 'z') {
                        $changeStatusCheck = false;
                    }
                }
                ...
                if ($val['orderStatus'] == 'r3' || $val['orderStatus'] == 'c' || $val['orderStatus'] == 'f') {
                    if($channel != 'naverpay') {
                        ...
                        if ($changeStatus === 'r3') {
                            $this->setCommonLog(
                                'order',
                                __METHOD__ . '[' . __LINE__ . '], ' . ' 환불완료/취소/실패인 경우 무조건 상태변경 금지'
                            );
                        }
                    }
                }
        ...
        if ($changeCheck === true || $channel == 'naverpay') {
            ...
            switch ($functionName) {
                case 'statusChangeCodeP':
                    $resultValue = \DB::transaction(function () use ($functionName, $orderNo, $bundleData) {
                        return $this->$functionName($orderNo, $bundleData, true);
                    });
    ...
    /**
     * PG 결제 확인 처리(미사용-어드민쪽에서 쓰게될지도 몰라 남겨둠)
     *
     * @param string $orderNo 주문 번호
     ...
     * @param string $orderStatus 처리할 주문 상태
     */
    public function setStatusChangePgFront($orderNo, $orderStatus)
    {
        return $this->updateStatusPreprocessFront($orderNo, $this->getOrderGoods($orderNo), 'f', $orderStatus, __('PG 결제 완료'), true);
    }
    ...
    public function cancelOrderAutomaticallySearch()
    {
        ...
        $limit = $this->cancelOrderlLimit; // 처리 범위
        $this->countByTotalCancelOrder = $this->cancelOrderAutomaticallyLimit($total, $limit); // 조회할 횟수
        ...
        if (gd_count($this->targetOrderGoodsNoArr) > 0) {
            try {
                ...
                \Logger::channel('scheduler')->error('[autoCancelJob] 주문 자동취소 중 예외발생', $e->getMessage());
            }
        }
    }
     ...
     * @return string
     */
    public function getOrderInvoiceInfo($groupCd)
    {
        // 추출 필드
        $arrIncludeOi = [
            'groupCd',
    ...
    {
        // 일괄등록 정보 보기
        $getData['info'] = $this->getOrderInvoiceInfo($groupCd);
    ...
    public function saveOrderInfo($cartInfo, $orderInfo, $orderPrice, $checkSumData = true )
    {
        ...
        // 공통 주문저장 로직 실행
        $this->saveOrder($cartInfo, $orderInfo, $orderPrice, $checkSumData, null, null, null, null);
    ...
    private function _getOrderAddFieldConvertData($orderAddFieldData)
    {
        ...
        if ($orderAddFieldData['orderAddFieldType']) {
            if ($orderAddFieldData['orderAddFieldType'] == 'text') {
                ...
                $convertData['orderAddFieldOption'] = gd_implode('<br/>', $convertData['orderAddFieldOption']);
                ...
                $convertData['orderAddFieldOption'] = sprintf('%s : ' . gd_implode(', ', $convertData['orderAddFieldOption']), __('입력값'));
