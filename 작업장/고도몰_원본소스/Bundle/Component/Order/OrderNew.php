...
class OrderNew
{
    ...
    /**
     * @var array 재고 차감이후 이슈발생시 재고 롤백쿼리
     */
    public $aRollbackQuery = '';
    ...
    public function generateOrderNo()
    {
        ...
        try {
            $this->db->query('INSERT INTO es_orderNoSequence VALUES ()');
        } catch (\Exception $e) {
            throw new Exception($e->getMessage());
        }
        $orderNoSequence = $this->db->insert_id() % 10; // 시퀀스 1의 자리 추출 
        
        $microtime = microtime(true); // 년월일시분초밀리초 15자리
        $orderDate = date('ymdHis', $microtime) . sprintf('%03d', ($microtime * 1000) % 1000);

        return $orderDate . $orderNoSequence;
    }
    ...
    public function saveOrder($orderInfo, $order, $memberData, $deliveryInfo, $aCartData, $history, $couponInfo, $taxPrice)
    {
        ...
        // $orderStatusPre 값 처리
        $orderStatusPre = $order['orderStatus'];
        ...
        $orderInfoSno = $this->db->insert_id();

        // 선물하기 주문 데이터 준비
        $presentOrder = \App::getInstance(PresentOrder::class);
        $presentOrderData = $presentOrder->preparePresentOrder($aCartData, $orderInfo, $this->orderNo);
        $isPresentOrder = $presentOrderData['isPresentOrder'];
        $receiverInfo = $presentOrderData['receiverInfo'];
        $aCartData = $presentOrderData['preparedCartData'];
        ...
        // 로그 저장
        \Logger::channel('order')->info('INSERT orderDelivery : ' . $this->orderNo, $deliveryInfo);

        // 정책별 배송 정보 저장
        foreach ($deliveryInfo as $k => $v) {
            $v['orderInfoSno'] = $orderInfoSno;
            $arrBind = $this->db->get_binding(DBTableField::tableOrderDelivery(), $v, 'insert');
            $this->db->set_insert_db(DB_ORDER_DELIVERY, $arrBind['param'], $arrBind['bind'], 'y', false);
            $deliveryInfoSno[$k]['orderDeliverySno'] = $this->db->insert_id();
            unset($arrBind);
        }

        // 로그 저장
        \Logger::channel('order')->info('INSERT orderGoods : ' . $this->orderNo, $aCartData);

        $receiverIndex = 0;
        foreach ($aCartData as $gVal) {
            // deliverySno 저장
            $gVal['orderDeliverySno'] = $deliveryInfoSno[$gVal['deliveryInfoKey']]['orderDeliverySno'];
            ...
            $orderGoodsNo = $tmpDbInsertId;

            // 선물하기 주문인 경우 수령자 정보 저장 (1:1 매칭)
            $receiverIndex = $presentOrder->processReceiver(
                $isPresentOrder,
                $receiverInfo,
                $receiverIndex,
                $this->orderNo,
        ...
        if (MemberUtil::checkLogin() == 'member') {
            // 최초 주문여부 확인
            if ($this->isFirstSale($order['memNo'])) {
                $order['firstSaleFl'] = 'y';
            }

            $memData = array();
            if ($orderInfo['simpleJoin'] === 'y') {
                $memData['memNo'] = $order['memNo'];
                $memData['memNm'] = $orderInfo['orderName'];
                $memData['phone'] = $orderInfo['orderPhone'];
                $memData['cellPhone'] = $orderInfo['orderCellPhone'];
            ...
            if ($orderInfo['reflectApplyMember'] === 'y' || $orderInfo['reflectApplyDirectMember'] === 'y' || $orderInfo['reflectApplyShippingMember'] === 'y') {
                $memData['memNo'] = $order['memNo'];
                $memData['zipcode'] = $orderInfo['receiverZipcode'];
                $memData['zonecode'] = $orderInfo['receiverZonecode'];
                $memData['address'] = $orderInfo['receiverAddress'];
                $memData['addressSub'] = $orderInfo['receiverAddressSub'];
                $memData['phone'] = $orderInfo['receiverPhone'];
                $memData['cellPhone'] = $orderInfo['receiverCellPhone'];
                $memData['lastSaleDt'] = date('Y-m-d G:i:s');
        ...
        if(\Cookie::has('_fbp') === true || \Cookie::has('_fbc') === true){
            ...
            $order['fbPixelKey'] = json_encode($fbPixelCookie, JSON_UNESCAPED_UNICODE);
        }
        ...
        // 로그 저장
        \Logger::channel('order')->info('INSERT order : ' . $this->orderNo, $order);
        ...
        if (gd_in_array($orderInfo['settleKind'], $this->settleKindReceiptPossible) === true && $orderInfo['receiptFl'] == 'r') {
            ...
            $receipt['supplyPrice'] = $taxPrice['taxSupplyPrice'];
            $receipt['taxPrice'] = $taxPrice['taxVatPrice'];
            $receipt['freePrice'] = $taxPrice['taxFreePrice'];

            $receipt['firstRegDt'] = date('Y-m-d H:i:s');
        ...
        if ($orderInfo['settleKind'] == 'gb') {
            ...
            $resultValue = $this->statusChangeCodeP($this->orderNo, $arrOrderGoodsSno);

            // 전액 할인 - 결제 내역 존재 여부에 따른 SMS 발송 (T : 가능 / F : 불가)
            if ($resultValue['smsFl'] === 'T') {
                // 안심 번호 신청
                $safeNumber = \App::load('Component\\Service\\SafeNumber');
                $safeNumber->setUseSafeNumber($this->orderNo);

                // 메일 / SMS 전송
                $this->sendOrderInfo(Code::ORDER, 'mail', $this->orderNo);
                $this->sendOrderInfo(Code::INCASH, 'all', $this->orderNo);
                $presentNotification = \App::getInstance(PresentNotification::class);
                if ($presentOrder->isPresentOrder((string) $this->orderNo)) {
                    $presentNotification->sendPresentInfo(Code::PRESENT, (string) $this->orderNo);
                }
            }

            // 주문번호 및 회원번호가 존재할 경우, 결제 완료 웹훅 발송
        ...
        try {
            if ($orderInfo['settleKind'] == 'gb' || $orderInfo['settleKind'] == self::SETTLE_KIND_ZERO) {
                ...
                $orderAnalyticsData = [
                    ...
                    'order' => array_merge($order, $taxPrice, $orderInfo),
                    'orderGoods' => $aCartData,
                    'orderDeliveries' => $deliveryInfo,
                ];
    ...
    public function saveOrderInfo($cartInfo, $orderInfo, $order, $checkSumData = true)
    {
        // 동일 주문 체크
        if ($checkSumData) {
            $checkOrder = $this->setOrderCheckByChecksumData($cartInfo, $orderInfo, $order);
        }
        // 체크섬 데이터
        $order['checksumData'] = $checkOrder['data'];

        // 주문서 기본 저장 변수
        $order['memNo'] = Session::get('member.memNo');

        // 모바일 주문 체크
        if (Request::isMobile() && Request::isMobileDevice()) {
            $order['orderTypeFl'] = 'mobile';
            // 마이앱 주문시 OS 체크
            ...
            if ($this->useMyapp && Request::isMyapp()) {
                ...
                $order['appOs'] = $myappConnectOs;
                $order['pushCode'] = $pushCode;
            }
        } else {
            $order['orderTypeFl'] = 'pc';
        }
        ...
        if (Request::isByapps() && Request::isMobileDevice()) {
            ...
            if (empty($matches) === false) {
                $order['appOs'] = (gd_in_array(current($matches), ['iPhone','iPad','iPod'])) ? 'ios' : 'android';
            } else {
                $order['appOs'] = "etc"; // 반응형 웹
            }
        }
        ...
        // 주문로그 저장
        \Logger::channel('order')->info(__METHOD__ . 'ORDER NO : ' . $this->orderNo, $orderInfo);
        ...
        foreach ($cartInfo as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                ...
                foreach ($dVal as $gKey => $gVal) {
                    ...
                    $originGoodsPrice = $gVal['price']['goodsPriceSubtotal'];
                    $deliveryForGoodsDcRate = NumberUtils::divisionExceptZero(($totalOriginGoodsPrice[$dKey] == 0 ? 0 : $originGoodsPrice), $totalOriginGoodsPrice[$dKey]);
        ...
        if (MemberUtil::checkLogin() == 'member') {
            ...
            $memberData = $member->getMember($order['memNo'], 'memNo', '*');
        }
                ...
                foreach ($dVal as $gKey => $gVal) {
                    ...
                    if ($onlyOneDelivery === true) {
                        ...
                        if (Globals::get('gGlobal.isFront') && $onlyOneOverseasDelivery === true) {
                            $deliveryInfo[$deliveryInfoNum]['deliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['taxSupplyDeliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['taxVatDeliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['taxFreeDeliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['realTaxSupplyDeliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['realTaxVatDeliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['realTaxFreeDeliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['deliveryPolicyCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['deliveryAreaCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['divisionDeliveryUseDeposit'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['divisionDeliveryUseMileage'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['divisionDeliveryCharge'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['deliveryInsuranceFee'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['deliveryCollectPrice'] = 0;
                            $deliveryInfo[$deliveryInfoNum]['deliveryWholeFreePrice'] = 0;
                        }
                    ...
                    $gVal['deliveryInfoKey'] = $deliveryInfoNum;
        ...
        $order['memberPolicy'] = json_encode($memberInfo, JSON_UNESCAPED_UNICODE);
        ...
        $taxPrice['taxSupplyPrice'] = $taxSupplyPrice;
        $taxPrice['taxVatPrice'] = $taxVatPrice;
        $taxPrice['taxFreePrice'] = $taxFreePrice;

        if (empty($couponInfo) === true) {
            $couponInfo = 'empty';
        }
     ...
     */
    public function getOrderGoodsByGoodsNo($orderNo = null, $goodsNo = null, $handleSno = null, $arrInclude = null, $arrExclude = ['orderNo'], $memNo = null, $orderStatus = null)
    {
        // 출력 필드
        $arrField = DBTableField::setTableField('tableOrderGoods', $arrInclude, $arrExclude, 'og');
    ...
    protected function setStatusChange($orderNo, $arrData, $autoProcess = false)
    {
        ...
        foreach ($arrData['sno'] as $key => $val) {
            ...
            $this->orderLog($orderNo, $val, $logCode01, $logCode02, gd_implode(' | ', $reason), false, $autoProcess);
        }
        unset($arrWhere);
     ...
     * @param string $orderNo 주문 번호
     * @param array  $arrData 상태 및 주문상품NO 정보
     * @param array  $frontFlag 프론트에서 주문하면서 호출한것인지 확인
     */
    public function statusChangeCodeP($orderNo, $arrData, $frontFlag = false)
    {
        ...
        // 결제 내역 존재 여부에 따른 SMS 발송 플래그 (T : 가능 / F : 불가)
        return ['smsFl' => $orderFunctionProcess ? 'T' : 'F'];
    }
...
// $arrData['sno']의 카운트와 해당주문번호의 품목카운트가 다르면 REPAYPART 동일하면 REPAY로 보내도록 처리
    ...
    public function setMinusDepositVariation($orderNo)
    {
        ...
        if ($totalUseDeposit > 0) {
            ...
            $redisKey = sprintf('%s_%s', 'deposit', $memNo);
            $variationDepositResult = SimpleCache::init(SimpleCache::REDIS)
                ->executeWithLock($redisKey, 'memberDeposit', [$deposit, 'setMemberDeposit'], $memNo, ($totalUseDeposit * -1), Deposit::REASON_CODE_GROUP . Deposit::REASON_CODE_GOODS_BUY, Deposit::MODE_ORDER, $orderNo);

            if ($variationDepositResult) {
    ...
    protected function setMinusDepositRestore($orderNo, $targetOrderGoodsNoArr = null)
    {
        ...
        if ($totalUseDeposit > 0) {
            ...
            $redisKey = sprintf('%s_%s', 'deposit', $memNo);
            $restoreDepositResult = SimpleCache::init(SimpleCache::REDIS)
                ->executeWithLock($redisKey, 'memberDeposit', [$deposit, 'setMemberDeposit'], $memNo, $totalUseDeposit, Deposit::REASON_CODE_GROUP . Deposit::REASON_CODE_ORDER_CANCEL, Deposit::MODE_ORDER, $orderNo);

            if ($restoreDepositResult) {
    ...
    public function setGoodsStockCutback($orderNo, $arrGoodsSno, $pgFlag = false)
    {
        ...
        if (empty($getData) === false) {
            foreach ($getData as $key => $val) {
                if (empty($val['giftNo']) === false && $val['stockFl'] == 'y') {
                    $strWhere = 'giftNo = \'' . $val['giftNo'] . '\' AND stockFl = \'y\' AND stockCnt - ' . $val['giveCnt'] . ' >= 0';
                    $affectedRow = $this->db->set_update_db(DB_GIFT, 'stockCnt = stockCnt - ' . $val['giveCnt'], $strWhere);
                    if ($affectedRow > 0) {
                        // 롤백쿼리 저장
                ...
                if (empty($addGoodsData) === false) {
                    ...
                    if ($addGoodsData['stockUseFl'] == '1') {
                        if ($addGoodsData['stockCnt'] > 0 && $addGoodsData['stockCnt'] - $val['goodsCnt'] >= 0) {
                            // 추가상품 재고차감
                            $strWhere = 'addGoodsNo = \'' . $val['goodsNo'] . '\' AND stockUseFl = \'1\' AND stockCnt - ' . $val['goodsCnt'] . ' >= 0';
                            $updateResult = $this->db->set_update_db(DB_ADD_GOODS, 'stockCnt = stockCnt - ' . $val['goodsCnt'], $strWhere);
                            if (!$updateResult) {
                                //재고 차감 처리 후 문제 발생시 재고 차감 롤백 쿼리 실행
                                $this->failDoRollbackQuery();
                    ...
                    $this->orderLog($orderNo, $val['sno'], '재고차감', $logCode, $logDesc);
                    if ($pgFlag) {
                        throw new \Exception('PG_NO_ORDERGOODS_DATA');
                    }
                }
                unset($arrWhere, $arrBind);
                ...
                if (empty($optionData) === false) {
                    if ($optionData['stockFl'] == 'y') {
                        ...
                        if ($optionData['stockCnt'] > 0 && $optionData['stockCnt'] - $val['goodsCnt'] >= 0) {
                            $arrWhere[] = 'go.stockCnt - ' . $val['goodsCnt'] . ' >= 0';
                            // 상품 재고 수정
                            $updateResult = $this->db->set_update_db(DB_GOODS_OPTION, 'stockCnt = stockCnt - ' . $val['goodsCnt'], str_replace('go.', '', gd_implode(' AND ', $arrWhere)), $arrBind);
    ...
    public function getOrderTaxPrice($orderNo)
    {
        ...
        if ($settleDelivery > 0) {
            $tmpTax = round(NumberUtils::divisionExceptZero($settleDelivery, (1 + (10 / 100))));
    ...
    public function getLastOrderInfo($memNo)
    {
        ...
        $addField = "o.orderNo, og.orderStatus, FLOOR(o.settlePrice) AS settlePrice, o.orderGoodsNm, o.orderGoodsNmStandard, og.goodsNo, g.imagePath, g.imageStorage, IF(gi.goodsImageStorage = 'obs', gi.imageUrl, gi.imageName) as imageName, goodsImageStorage";
