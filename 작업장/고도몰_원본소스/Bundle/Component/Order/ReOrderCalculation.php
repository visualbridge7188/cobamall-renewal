...
class ReOrderCalculation
{
    ...
    /** @var \Origin\Service\WebHook\OrderWebHookService $orderWebHookService 주문 웹훅 서비스 */
    ...
    public function __construct()
    {
        ...
        $this->orderWebHookService = \App::getInstance(OrderWebHookService::class);
    }
    ...
    public function getOrderOriginalDepositMileageData($orderNo, $claimStatus)
    {
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_ORDER_ORIGINAL . ' as oo ' . gd_implode(' ', $query);
        $getData = $this->db->query_fetch($strSQL, $arrBind, false);
        unset($arrBind);
    ...
    public function getOrderGoodsStatusOriginalData($orderNo)
    {
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_ORDER_GOODS_ORIGINAL . ' as ogo ' . gd_implode(' ', $query);
        $getData = $this->db->query_fetch($strSQL, $arrBind);
        unset($arrBind);
    ...
    public function getOrderCouponData($orderNo, $statusMode = '')
    {
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_ORDER_COUPON . ' as oc ' . gd_implode(' ', $query);
        $getData = $this->db->query_fetch($strSQL, $arrBind);
        unset($arrBind);
    ...
    public function getCancelOrderGoodsData($orderNo, $arrOrderGoods)
    {
        ...
        $this->db->bind_param_push($arrBind, $this->fieldTypes['orderGoods']['orderNo'], $orderNo);
        $this->db->bind_param_push($arrBind, 's', gd_implode(',', $arrOrderGoods));
    ...
    public function getSelectOrderGoodsCancelData($orderNo, $arrOrderGoods)
    {
        // 반환할 주문 상품 정보
        $cancelData = [];
        $cancelOrderGoodsSno = gd_array_keys($arrOrderGoods);
        ...
        $cancelData['cancelGoodsPriceSum'] = gd_array_sum($cancelData['cancelGoodsPrice']);//취소 상품 금액 합
        $cancelData['cancelGoodsDcPriceSum'] = gd_array_sum($cancelData['cancelGoodsDcPrice']);//취소 상품 할인 합
        $cancelData['cancelGoodsEnuriPriceSum'] = gd_array_sum($cancelData['cancelGoodsEnuriPrice']);//취소 상품 에누리 합
        $cancelData['cancelMemberDcPriceSum'] = gd_array_sum($cancelData['cancelMemberDcPrice']);//취소 회원 추가 할인 합
        $cancelData['cancelMemberOverlapDcPriceSum'] = gd_array_sum($cancelData['cancelMemberOverlapDcPrice']);//취소 회원 중복 할인 합
        $cancelData['cancelGoodsCouponDcPriceSum'] = gd_array_sum($cancelData['cancelGoodsCouponDcPrice']);//취소 상품 쿠폰 할인 합
        if ($this->myappUseFl) {
            $cancelData['cancelMyappDcPriceSum'] = gd_array_sum($cancelData['cancelMyappDcPrice']);//취소 상품 마이앱 할인 합
        }
        ...
        $cancelData['totalCancelDeliveryMemberDcPrice'] = gd_isset(gd_array_sum($cancelData['totalCancelDeliveryMemberDcPrice']), 0);// 취소 회원 배송비 무료 가능 금액

        // 조정 가능 배송비 합
        $cancelData['totalCancelDeliveryPriceFirst'] = gd_array_sum($cancelData['totalCancelDeliveryPriceSum']);
    ...
    public function getSelectOrderGoodsRefundData($orderNo, $arrRefundOrderGoods, $arrEtcOrderGoods)
    {
        ...
        foreach ($arrEtcOrderGoods as $scmNo => $deliveryData) {
            foreach ($deliveryData as $deliveryNo => $goodsData) {
                foreach ($goodsData as $key => $val) {
                    ...
                    if(!gd_in_array($val['sno'], $refundGoodsSno)) {
                        $dcSum = [
                            $val['goodsDcPrice'],
                            $val['memberDcPrice'],
                        ...
                        $etcSettlePrice -= (gd_array_sum($dcSum));
                    }
        ...
        $refundData['refundGoodsPriceSum'] = gd_array_sum($refundData['refundGoodsPrice']);//환불 상품 금액 합
        $refundData['refundGoodsDcPriceSum'] = gd_array_sum($refundData['refundGoodsDcPrice']);//환불 상품 할인 합
        $refundData['refundMemberDcPriceSum'] = gd_array_sum($refundData['refundMemberDcPrice']);//환불 회원 추가 할인 합
        $refundData['refundMemberOverlapDcPriceSum'] = gd_array_sum($refundData['refundMemberOverlapDcPrice']);//환불 회원 중복 할인 합
        $refundData['refundGoodsCouponDcPriceSum'] = gd_array_sum($refundData['refundGoodsCouponDcPrice']);//환불 상품 쿠폰 할인 합
        $refundData['refundGoodsEnuriDcPriceSum'] = gd_array_sum($refundData['refundGoodsEnuriDcPrice']);//환불 운영자 할인 합
        $refundData['refundOrderCouponDcPriceSum'] = gd_array_sum($refundData['refundOrderCouponDcPrice']);//환불 주문 쿠폰 할인 합
        if ($this->myappUseFl) {
            $refundData['refundMyappDcPriceSum'] = gd_array_sum($refundData['refundMyappDcPrice']);//환불 마이앱 할인 합
        }
        $refundData['refundGoodsDepositSum'] = gd_array_sum($refundData['refundGoodsDeposit']);//환불 상품에 적용된 예치금 금액 합
        $refundData['refundGoodsMileageSum'] = gd_array_sum($refundData['refundGoodsMileage']);//환불 상품에 적용된 마일리지 금액 합
        $refundData['refundGoodsDeliveryDepositSum'] = gd_array_sum($refundData['refundGoodsDeliveryDeposit']);//환불 배송비에 적용된 예치금 금액을 상품에 안분한 금액 합
        $refundData['refundGoodsDeliveryMileageSum'] = gd_array_sum($refundData['refundGoodsDeliveryMileage']);//환불 배송비에 적용된 마일리지 금액을 상품에 안분한 금액 합
        // 환불 상품혜택 할인 합
        ...
        foreach ($orderDeliveryData as $val) {
            ...
            if (gd_in_array($val['sno'], $refundData['totalRefundDelivery'])) {
        ...
        $refundData['refundPolicyDeliveryPriceSum'] = gd_array_sum($refundData['refundPolicyDeliveryPrice']);//환불 배송비 금액 합
        $refundData['refundAreaDeliveryPriceSum'] = gd_array_sum($refundData['refundAreaDeliveryPrice']);//환불 지역별 배송비 합
        $refundData['refundOverseaDeliveryPriceSum'] = gd_array_sum($refundData['refundOverseaDeliveryPrice']);//환불 해외 배송비 합
        $refundData['refundDeliveryDepositSum'] = gd_array_sum($refundData['refundDeliveryDeposit']);//환불 배송비에 적용된 예치금 금액 합
        $refundData['refundDeliveryMileageSum'] = gd_array_sum($refundData['refundDeliveryMileage']);//환불 배송비에 적용된 마일리지 금액 합
        $refundData['refundDeliveryCouponSum'] = gd_array_sum($refundData['refundDeliveryCoupon']);//환불 배송비에 적용된 배송비쿠폰 합
        $refundData['refundDeliveryMemberDcSum'] = gd_array_sum($refundData['refundDeliveryMemberDc']);//환불 배송비에 적용된 회원 배송비무료 금액
        ...
        $refundData['totalRefundDeliveryPrice'] = gd_array_sum($refundData['refundDeliveryPrice']);

        // 주문 정보의 데이터
        $orderData = $this->getOrderData($orderNo);
        $refundData['totalRefundGoodsUseDepositPrice'] = gd_array_sum($totalAddPaymentPrice['goods']['deposit']); // 환불 가능 상품 예치금 금액
        $refundData['totalRefundGoodsUseMileagePrice'] = gd_array_sum($totalAddPaymentPrice['goods']['mileage']); // 환불 가능 상품 마일리지 금액
        $refundData['totalRefundDeliveryUseDepositPrice'] = gd_array_sum($totalAddPaymentPrice['delivery']['deposit']); // 환불 가능 배송비 예치금 금액
        $refundData['totalRefundDeliveryUseMileagePrice'] = gd_array_sum($totalAddPaymentPrice['delivery']['mileage']); // 환불 가능 배송비 마일리지 금액
    ...
    public function setRefundCompleteOrderGoods($getData, $autoProcess = false)
    {
        ...
        foreach ($orderInfo['goods'] as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                foreach ($dVal as $gKey => $gVal) {
                    ...
                    if (gd_count($orderRefundInfo['refundOrderGoodsSnos']) === $refundGoodsIndex) {
                        ...
                        $refundGoodsDeposit = NumberUtils::getNumberFigure((1 / gd_count($orderRefundInfo['refundOrderGoodsSnos'])) * $getData['info']['refundGoodsUseDeposit'], '0.1', 'round');
                        $refundGoodsMileage = NumberUtils::getNumberFigure((1 / gd_count($orderRefundInfo['refundOrderGoodsSnos'])) * $getData['info']['refundGoodsUseMileage'], '0.1', 'round');
                        $refundDeliveryDeposit = NumberUtils::getNumberFigure((1 / gd_count($orderRefundInfo['refundOrderGoodsSnos'])) * $getData['info']['refundDeliveryUseDeposit'], '0.1', 'round');
                        $refundDeliveryMileage = NumberUtils::getNumberFigure((1 / gd_count($orderRefundInfo['refundOrderGoodsSnos'])) * $getData['info']['refundDeliveryUseMileage'], '0.1', 'round');
        ...
        if (gd_count($orderEtcInfo['goods']) > 0) {
            ...
            foreach ($orderEtcInfo['goods'] as $key => $value) {
                ...
                if (gd_count($orderEtcInfo['goods']) === $index) {
                    //차감되지 않은 realtax 차감
        ...
        foreach ($orderInfo['goods'] as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                foreach ($dVal as $gKey => $gVal) {
                    ...
                    if (!gd_in_array($gVal['orderDeliverySno'], $processEnd)) {
                        ...
                        if ($orderInfo['mallSno'] == DEFAULT_MALL_NUMBER || ($orderInfo['mallSno'] > DEFAULT_MALL_NUMBER && $onlyOverseasDelivery === false)) {
                            ...
                            if (gd_array_sum($realTaxDeliveryRate) === 0) {
        ...
        // *** 6. 남은배송비에 분배처리
        if (gd_count($orderEtcInfo['delivery']) > 0) {
            // 남은 상품이 나눠가져야 할 realtax 금액
            $plusRealTaxSupplyPrice = $moveRealTaxData['plus']['realTaxSupplyPrice'];
            $plusRealTaxVatPrice = $moveRealTaxData['plus']['realTaxVatPrice'];
            $plusRealTaxFreePrice = $moveRealTaxData['plus']['realTaxFreePrice'];
            ...
            foreach ($orderEtcInfo['delivery'] as $key => $value) {
                $divisionMinusSupplyPrice = $divisionMinusVatPrice = $divisionMinusFreePrice = 0;
                if (gd_count($orderEtcInfo['delivery']) === $index) {
                    //차감되지 않은 realtax 차감
        ...
        if (gd_count($orderEtcInfo['groupDeliveryGoods']) > 0) {
            ...
            foreach ($orderEtcInfo['groupDeliveryGoods'] as $dKey => $dVal) {
                ...
                foreach ($dVal as $orderGoodsSno => $gVal) {
                    if (gd_count($orderEtcInfo['groupDeliveryGoods'][$dKey]) === $index) {
        ...
        $cntTmpUpdateOrderHandleData = gd_count($tmpUpdateOrderHandleData);
        if ($cntTmpUpdateOrderHandleData > 0) {
            ...
            foreach ($tmpUpdateOrderHandleData as $orderHandleSno => $handleData) {
                ...
                $refundData['originGiveMileage'] = gd_array_sum([
        ...
        if ((gd_array_sum($refundRealTaxData) + $totalRefundDeliveryInsuranceFee) != $refundCashSum) {
        ...
        if (trim($returnError) !== '') {
            $this->setCommonLog(
                'order',
                __METHOD__ . '[' . __LINE__ . '], ' . ' 환불 처리 실패 메시지 & 데이터 : ',
                [
                    'returnError' => $returnError,
                    'orderHandleData' => $orderRefundInfo['orderHandleData']
                ]
            );

            // PG 취소처리에서 실패하면 f4 상태로 업데이트한다.
            foreach ($orderRefundInfo['orderHandleData'] as $goodsData) {
                $orderAdmin->updateStatusPreprocess($getData['orderNo'], $goodsData, 'f', 'f4', $returnError, false, null, null, false, true);
            }
            // transaction rollback 처리를 위한 exception
    ...
    public function setRefundCompleteOrderGoodsNew($getData, $autoProcess = false, $userHandleFl = false)
    {
        ...
        if ($getData['info']['refundMethod'] == '현금환불') {
            if ($getData['info']['completePgPrice'] > 0 || $getData['info']['completeDepositPrice'] > 0 || $getData['info']['completeMileagePrice'] > 0) {
                throw new Exception(__('현금환불이 아닌 환불 금액정보가 있습니다. 다시 확인해주세요.'));
            }
        }
        if ($getData['info']['refundMethod'] == 'PG환불') {
            if ($getData['info']['completeCashPrice'] > 0 || $getData['info']['completeDepositPrice'] > 0 || $getData['info']['completeMileagePrice'] > 0) {
                throw new Exception(__('PG환불이 아닌 환불 금액정보가 있습니다. 다시 확인해주세요.'));
            }
        }
        if ($getData['info']['refundMethod'] == '예치금환불') {
            if ($getData['info']['completeCashPrice'] > 0 || $getData['info']['completePgPrice'] > 0 || $getData['info']['completeMileagePrice'] > 0) {
                throw new Exception(__('예치금환불이 아닌 환불 금액정보가 있습니다. 다시 확인해주세요.'));
            }
        }
        if ($getData['info']['refundMethod'] == '기타환불') {
            if ($getData['info']['completeCashPrice'] > 0 || $getData['info']['completePgPrice'] > 0 || $getData['info']['completeDepositPrice'] > 0) {
                throw new Exception(__('기타환불이 아닌 환불 금액정보가 있습니다. 다시 확인해주세요.'));
            }
        }
        ...
        foreach ($getData['aAliveDeliverySno'] as $val) {
            ...
            } elseif (gd_isset($getData['refundDeliveryCharge_' . $val . 'Max'], 0) == 0 && gd_isset($getData['refundDeliveryCharge_' . $val . 'Coupon'], 0) > 0) { // 배송비 전체가 쿠폰사용일때
                $refundData['aRefundDelivery'][$val]['iPrice'] = 0;
                $refundData['aRefundDelivery'][$val]['iCoupon'] = $getData['refundDeliveryCharge_' . $val . 'Coupon'];
                $refundData['refundDeliverySum'] = $getData['refundDeliveryCouponDcPrice'];
        ...
        foreach ($getData['aAliveDeliverySno'] as $val) {
            ...
            } elseif ($refundData['aRefundDelivery'][$val]['iPrice'] == 0 && gd_isset($getData['refundDeliveryCharge_' . $val . 'Coupon'], 0) > 0) { // 배송비 전체가 쿠폰사용일때
                // 아무것도 안함 위에서 iCoupon값 넣어줌
            } else {
        ...
        foreach ($getData['aAliveDeliverySno'] as $val) {
            if (gd_isset($getData['refundDeliveryCharge_' . $val . 'Max'], 0) > 0) { // 환불대상
                ...
                $updateData['aRefundDelivery'][$val]['iPrice'] = 0;
        ...
        foreach ($getData['aAliveDeliverySno'] as $val) {
            if ($updateData['aRefundDelivery'][$val]['iPrice'] > 0) {
                $tempPerDeliveryCoupon = 0;
                $tempPerDeliveryCoupon = NumberUtils::divisionExceptZero($updateData['aRefundDelivery'][$val]['iPrice'], $updateData['refundDeliverySum']);
            ...
            } elseif ($updateData['refundDeliverySum'] == 0 && $updateData['refundDeliveryCouponDcPrice'] > 0) { // 배송비 전체가 쿠폰사용일때
                $tempDeliveryInfo = $this->getOrderDeliveryData($getData['orderNo'], $val);
                $updateData['aRefundDelivery'][$val]['iCoupon'] = $tempDeliveryInfo[0]['divisionDeliveryCharge'];
            } else {
                $updateData['aRefundDelivery'][$val]['iCoupon'] = 0;
            }
        ...
        if ($updateData['refundDeliverySum'] == 0 && $updateData['refundDeliveryCouponDcPrice'] > 0) { // 배송비 전체가 쿠폰사용일때
        ...
        // 마일리지 배송비 패치여부 및 설정값
        $mileageUseDeliveryFl = $orderAdmin->getOrderCurrentMileagePolicy($getData['orderNo'])['use']['mileageUseDeliveryFl'];

        // 이미 환불 완료인 경우 제외 (중복 환불 방지)
        if (empty($getData['refund']) === false) {
            $confirmRefundCompletion = $this->confirmRefundCompletion($getData['orderNo'], gd_array_keys($getData['refund']));
        }

        foreach ($getData['refund'] as $handleSno => $aVal) {
            if (empty($confirmRefundCompletion) === false) {
                if (gd_in_array($handleSno, $confirmRefundCompletion)) {
                    \Logger::channel('order')->info(__METHOD__ . '[' . __LINE__ . '], ' . ' Refund already processed : orderNo[' . $getData['orderNo'] . '], handleSno[' . $handleSno . ']');
                    unset($getData['refund'][$handleSno]);
                    continue;
                }
            }
            ...
            $updateHandleData[$handleSno]['refundAccountNumber'] = $getData['info']['refundAccountNumber'];
        ...
        foreach ($getData['aAliveDeliverySno'] as $val) {
            ...
            $tempPerPartOrder = NumberUtils::divisionExceptZero($tempThisPrice, ($updateData['goodsPrice'] - $updateData['refundDcSum']) + $updateData['refundDeliverySum']);

            if ($mileageUseDeliveryFl === 'n') {
                $updateDeliveryData[$val]['divisionDeliveryUseDeposit'] = 0; // 배송비 안분 예치금
                $updateDeliveryData[$val]['divisionDeliveryUseMileage'] = 0; // 배송비 안분 적립금
            } else {
            ...
            if ($orderInfo['deliveryTaxInfo'][$val][0] == 't') { // 배송비 세율적용이면
                $tmpUpdateDeliveryTaxPrice = NumberUtils::taxAll($tempTaxPrice, $orderInfo['deliveryTaxInfo'][$val][1], $orderInfo['deliveryTaxInfo'][$val][0]); // 배송비 부가세 계산
                $updateDeliveryData[$val]['realTaxSupplyDeliveryCharge'] = $tmpUpdateDeliveryTaxPrice['supply']; // 실 배송비 공급가
                $updateDeliveryData[$val]['realTaxVatDeliveryCharge'] = $tmpUpdateDeliveryTaxPrice['tax']; // 실 배송비 부가세
            ...
            $tempPerPartOrder = NumberUtils::divisionExceptZero($tempThisPrice, ($refundData['goodsPrice'] - $refundData['refundDcSum']) + $refundData['refundDeliverySum']);
            ...
            if ($orderInfo['deliveryTaxInfo'][$val][0] == 't') { // 배송비 세율적용이면
                $tmpRefundDeliveryTaxPrice = NumberUtils::taxAll($tempTaxPrice, $orderInfo['deliveryTaxInfo'][$val][1], $orderInfo['deliveryTaxInfo'][$val][0]); // 환불 배송비 부가세 계산
                $refundDeliveryData[$val]['realTaxSupplyDeliveryCharge'] = $tmpRefundDeliveryTaxPrice['supply']; // 실 배송비 공급가
                $refundDeliveryData[$val]['realTaxVatDeliveryCharge'] = $tmpRefundDeliveryTaxPrice['tax']; // 실 배송비 부가세
        ...
        // 배송비가 0원인경우 태울필요가없다
        if ($orgTotalData['totalDeliveryPrice'] > 0) {
        ...
        $smsCnt = 0;    // 부분 환불 시 sms 일괄 전송을 위한 cnt(sms 개선)
        $refundData['goodsMileageTotalPrice'] = 0; // goodsMileage가있는 상품의 금액 합계
        $reason = ($getData['reason'] ? $getData['reason'] : __('일괄')); // 변경사유
        foreach ($orderInfo['goods'] as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                foreach ($dVal as $gKey => $gVal) {
                    if (gd_in_array($gVal['sno'], $refundTarget)) { // 환불 대상처리
                        $smsCnt++;
                        // sms 개선(환불 시 금액 전달을 위한 변수, 부분 환불 시 sms 일괄 전송을 위한 cnt)
                        ...
                        // 주문 상태 변경 처리 및 재고 복원여부 처리
                        $isReturnStock = ($getData['returnStockFl'] === 'y') ? true : false;  // 환불상세에서 재고환원 여부에 동의시 처리여부
                        $orderAdmin->updateStatusPreprocess($getData['orderNo'], $orderRefundInfo['orderHandleData'][$gVal['handleSno']], 'r', 'r3', $reason, $isReturnStock, null, null, $autoProcess, $userHandleFl);
                        ...
                        if ($gVal['memberMileage'] > 0 && $gVal['plusMileageFl'] == 'n') {
                            $refundData['memberMileageTotalPrice'] += (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']);
                        }
                        if ($gVal['couponGoodsMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $refundData['couponGoodsMileageTotalPrice'] += (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']);
                        }
                        if ($gVal['divisionCouponOrderMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $refundData['divisionCouponOrderMileageTotalPrice'] += (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']);
                        }
                    } else {
                        $tempOrderHandle[$gVal['orderDeliverySno']]['iUpdateCount'] += 1;
                        if ($gVal['memberMileage'] > 0 && $gVal['plusMileageFl'] == 'n') {
                            $updateData['memberMileageTotalPrice'] += (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']);
                        }
                        if ($gVal['couponGoodsMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $updateData['couponGoodsMileageTotalPrice'] += (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']);
                        }
                        if ($gVal['divisionCouponOrderMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $updateData['divisionCouponOrderMileageTotalPrice'] += (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']);
                        }
                    }
                }
            }
        ...
        foreach ($orderInfo['goods'] as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                foreach ($dVal as $gKey => $gVal) {
                    ...
                    if (gd_in_array($gVal['sno'], $refundTarget)) { // 환불 대상처리
                        $order = new Order();
                        if ($order->isLoggedMemberOrderGoodsCountData($getData['orderNo'], $gVal['sno'])) {
                            try {
                                $this->db->begin_tran();
                                $aGoodsSno['sno'][0] = $aGoodsSno['GoodsSno'][0] = $gVal['sno'];
                                $aGoodsSno['changeStatus'] = '';
                                $orderKind['kind'] = '';
                                $orderKind['channel'] = '';
                                $claimInfo['orderCount'] = $gVal['goodsCnt'];
                                $order->setCountOrderGoods($getData['orderNo'], $aGoodsSno, $orderKind, true, $claimInfo);
                                $this->db->commit();
                                \Logger::channel('order')->info(__METHOD__ . '[' . __LINE__ . '], ' . ' 아이디 기준 구매수량 카운트 처리 성공 : ', ["orderNo: " . $getData['orderNo'], "orderGoodsNo: " . $gVal['sno']]);
                                unset($aGoodsSno, $claimInfo);
                            } catch (\Exception $e) {
                                $exceptionMessage = __METHOD__ . ', ' . $e->getFile() . '[' . $e->getLine() . '], ' . $e->getMessage();
                                \Logger::channel('order')->error($exceptionMessage, $e->getTrace());
                                $this->db->rollback();
                            }
                        }
                        ...
                        if (gd_isset($refundData['goodsPrice'], 0) > 0) {
                        ...
                        if ($gVal['memberMileage'] > 0 && $gVal['plusMileageFl'] == 'n') {
                            $tempMemberMileagePerPart = (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']) / $refundData['memberMileageTotalPrice'];
                        }
                        if ($gVal['couponGoodsMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $tempCouponGoodsMileagePerPart = (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']) / $refundData['couponGoodsMileageTotalPrice'];
                        }
                        if ($gVal['divisionCouponOrderMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $tempDivisionCouponOrderMileagePerPart = (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']) / $refundData['divisionCouponOrderMileageTotalPrice'];
                        }
                        ...
                        $refundGoodsData[$gVal['sno']]['goodsDcPrice'] = NumberUtils::getNumberFigure($gVal['goodsDcPrice'], 0, "");
                        $refundGoodsData[$gVal['sno']]['memberDcPrice'] = NumberUtils::getNumberFigure($gVal['memberDcPrice'], 0, "");
                        $refundGoodsData[$gVal['sno']]['memberOverlapDcPrice'] = NumberUtils::getNumberFigure($gVal['memberOverlapDcPrice'], 0, "");
                        ...
                        $refundGoodsData[$gVal['sno']]['couponGoodsDcPrice'] = NumberUtils::getNumberFigure($gVal['couponGoodsDcPrice'], 0, "");
                        ...
                        if ($gVal['memberMileage'] > 0 && $gVal['plusMileageFl'] == 'n') {
                            $refundGoodsData[$gVal['sno']]['memberMileage'] = NumberUtils::getNumberFigure($refundData['refundGroupMileage'] * $tempMemberMileagePerPart, '0.1', 'round');
                        } else {
                            $refundGoodsData[$gVal['sno']]['memberMileage'] = $gVal['memberMileage'];
                        }
                        if ($gVal['couponGoodsMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $refundGoodsData[$gVal['sno']]['couponGoodsMileage'] = NumberUtils::getNumberFigure($refundData['refundGoodsCouponMileage'] * $tempCouponGoodsMileagePerPart, '0.1', 'round');
                        } else {
                            $refundGoodsData[$gVal['sno']]['couponGoodsMileage'] = $gVal['couponGoodsMileage'];
                        }
                        if ($gVal['divisionCouponOrderMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $refundGoodsData[$gVal['sno']]['divisionCouponOrderMileage'] = NumberUtils::getNumberFigure($refundData['refundOrderCouponMileage'] * $tempDivisionCouponOrderMileagePerPart, '0.1', 'round');
                        } else {
                            $refundGoodsData[$gVal['sno']]['divisionCouponOrderMileage'] = $gVal['divisionCouponOrderMileage'];
                        }
                        ...
                        if (gd_isset($updateData['goodsPrice'], 0) > 0) {
                            $tempPerPart = (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']) / $updateData['goodsPrice'];
                        } else {
                            $tempPerPart = 0;
                        }
                        $tempPerPartWithDelivery = NumberUtils::divisionExceptZero(
                            (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']),
                            ($updateData['goodsPrice'] + $updateData['refundDeliverySum'])
                        );
                        if ($gVal['memberMileage'] > 0 && $gVal['plusMileageFl'] == 'n') {
                            $tempMemberMileagePerPart = NumberUtils::divisionExceptZero(
                                (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']),
                                $updateData['memberMileageTotalPrice']
                            );
                        }
                        if ($gVal['couponGoodsMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $tempCouponGoodsMileagePerPart = NumberUtils::divisionExceptZero(
                                (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']),
                                $updateData['couponGoodsMileageTotalPrice']
                            );
                        }
                        if ($gVal['divisionCouponOrderMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            $tempDivisionCouponOrderMileagePerPart = NumberUtils::divisionExceptZero(
                                (($gVal['goodsPrice'] + $gVal['optionPrice'] + $gVal['optionTextPrice']) * $gVal['goodsCnt']),
                                $updateData['divisionCouponOrderMileageTotalPrice']
                            );
                        }
                        ...
                        $updateGoodsData[$gVal['sno']]['goodsDcPrice'] = NumberUtils::getNumberFigure($gVal['goodsDcPrice'], 0, "");
                        $updateGoodsData[$gVal['sno']]['memberDcPrice'] = NumberUtils::getNumberFigure($gVal['memberDcPrice'], 0, "");
                        $updateGoodsData[$gVal['sno']]['memberOverlapDcPrice'] = NumberUtils::getNumberFigure($gVal['memberOverlapDcPrice'], 0, "");
                        ...
                        $updateGoodsData[$gVal['sno']]['couponGoodsDcPrice'] = NumberUtils::getNumberFigure($gVal['couponGoodsDcPrice'], 0, "");
                        ...
                        if ($gVal['couponGoodsMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            if ($getData['refundGoodsCouponMileageFlag'] == 'F') {
                                $updateGoodsData[$gVal['sno']]['couponGoodsMileage'] = NumberUtils::getNumberFigure($gVal['couponGoodsMileage'], null, null);
                            } else {
                                $updateGoodsData[$gVal['sno']]['couponGoodsMileage'] = NumberUtils::getNumberFigure($updateData['refundGoodsCouponMileage'] * $tempCouponGoodsMileagePerPart, '0.1', 'round');
                            }
                        } else {
                            $updateGoodsData[$gVal['sno']]['couponGoodsMileage'] = $gVal['couponGoodsMileage'];
                        }
                        if ($gVal['divisionCouponOrderMileage'] > 0 && $gVal['couponMileageFl'] == 'n') {
                            if ($getData['refundOrderCouponMileageFlag'] == 'F') {
                                $updateGoodsData[$gVal['sno']]['divisionCouponOrderMileage'] = NumberUtils::getNumberFigure($gVal['divisionCouponOrderMileage'], null, null);
                            } else {
                                $updateGoodsData[$gVal['sno']]['divisionCouponOrderMileage'] = NumberUtils::getNumberFigure($updateData['refundOrderCouponMileage'] * $tempDivisionCouponOrderMileagePerPart, '0.1', 'round');
                            }
                        } else {
                            $updateGoodsData[$gVal['sno']]['divisionCouponOrderMileage'] = $gVal['divisionCouponOrderMileage'];
                        }
                        if ($gVal['memberMileage'] > 0 && $gVal['plusMileageFl'] == 'n') {
                            if ($getData['refundGroupMileageFlag'] == 'F') {
                                $updateGoodsData[$gVal['sno']]['memberMileage'] = NumberUtils::getNumberFigure($gVal['memberMileage'], null, null);
                            } else {
                                $updateGoodsData[$gVal['sno']]['memberMileage'] = NumberUtils::getNumberFigure($updateData['refundGroupMileage'] * $tempMemberMileagePerPart, '0.1', 'round');
                            }
                        } else {
                            $updateGoodsData[$gVal['sno']]['memberMileage'] = $gVal['memberMileage'];
                        }
        ...
        \Logger::channel('order')->info(__METHOD__ . '[' . __LINE__ . '], ' . ' 환불 금액 안분 처리: ', [
            'refundData' => $refundData, 
            'refundGoodsData' => $refundGoodsData, 
            ...
            foreach ($sVal as $dKey => $dVal) {
                foreach ($dVal as $gKey => $gVal) {
                    if (gd_in_array($gVal['sno'], $refundTarget)) { // 환불 대상처리
                        ...
                        if ($gVal['goodsTaxInfo'][0] == 't') {
                            $tmpRefundGoodsTaxPrice = NumberUtils::taxAll($tempPrice, $gVal['goodsTaxInfo'][1], $gVal['goodsTaxInfo'][0]); // 환불 주문상품 부가세 계산
                            $refundGoodsData[$gVal['sno']]['realTaxSupplyGoodsPrice'] = $tmpRefundGoodsTaxPrice['supply'];
                            $refundGoodsData[$gVal['sno']]['realTaxVatGoodsPrice'] = $tmpRefundGoodsTaxPrice['tax'];
                            ...
                            $tmpTotalRefundChargeTaxPrice = NumberUtils::taxAll($getData['refundCharge' . $gVal['handleSno']], $gVal['goodsTaxInfo'][1], $gVal['goodsTaxInfo'][0]);
                            $orgTotalData['totalRefundChargeTaxSupply'] += $tmpTotalRefundChargeTaxPrice['supply'];
                            $orgTotalData['totalRefundChargeTaxVat'] += $tmpTotalRefundChargeTaxPrice['tax'];
                        } else {
                        ...
                        if ($gVal['goodsTaxInfo'][0] == 't') {
                            $tmpUpdateGoodsTaxPrice = NumberUtils::taxAll($tempPrice, $gVal['goodsTaxInfo'][1], $gVal['goodsTaxInfo'][0]); // 주문상품 부가세 계산
                            $updateGoodsData[$gVal['sno']]['realTaxSupplyGoodsPrice'] = $tmpUpdateGoodsTaxPrice['supply'];
                            $updateGoodsData[$gVal['sno']]['realTaxVatGoodsPrice'] = $tmpUpdateGoodsTaxPrice['tax'];
        ...
        \Logger::channel('order')->info(__METHOD__ . '[' . __LINE__ . '], ' . ' 환불 금액 업데이트 처리 : ', [
            'refundGoodsData' => $refundGoodsData, 
            'updateGoodsData' => $updateGoodsData,
        ]);
        ...
        if ($presentOrder->isPresentOrder($getData['orderNo'])) {
    ...
    public function getRefundEtcData($getData, $handleSno, $isAll, $orderRefundInfo)
    {
        ...
        foreach ($totalOrderInfo['goods'] as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                foreach ($dVal as $gKey => $gVal) {
                    //현재 취소하려는 주문상품 제외
                    if((int)$isAll === 1){
                        if(gd_in_array(substr($gVal['orderStatus'], 0, 1), ['r'])){
                            continue;
                        }
                    }
                    else {
                    ...
                    // 취소, 교환취소 주문상품 제외
                    if(gd_in_array(substr($gVal['orderStatus'], 0, 1), ['e', 'c'])){
                        continue;
                    }
                    // 환불완료 주문상품 제외
                    if(gd_in_array($gVal['orderStatus'], ['r3'])){
                        continue;
                    }

                    if (gd_count($tmpTotalOrderGoodsRateData['delivery'][$gVal['orderDeliverySno']]) < 1 && gd_in_array($gVal['orderDeliverySno'], $orderRefundInfo['refundOrderDeliverySno']) === false) {
        ...
        if(gd_count($tmpTotalOrderGoodsRateData['groupDeliveryGoods']) > 0){
        ...
        if(gd_count($tmpTotalOrderGoodsRateData['goods']) > 0){
            $totalRate = 100;
            $index = 1;

            foreach($tmpTotalOrderGoodsRateData['goods'] as $key => $value){
                if($index === gd_count($tmpTotalOrderGoodsRateData['goods'])){
                ...
                $realTaxRateSum = gd_array_sum($totalOrderGoodsRateData['goods'][$key]['rateTax']);
        ...
        if(gd_count($tmpTotalOrderGoodsRateData['delivery']) > 0){
            $totalRate = 100;
            $index = 1;

            foreach($tmpTotalOrderGoodsRateData['delivery'] as $key => $value){
                if($index === gd_count($tmpTotalOrderGoodsRateData['delivery'])){
                ...
                $realTaxRateSum = gd_array_sum($totalOrderGoodsRateData['delivery'][$key]['rateTax']);
                if($realTaxRateSum !== 100){
        ...
        unset($tmpTotalOrderGoodsRateData, $index, $totalRate, $totalOrderInfo);
    ...
    public function restoreRefundCoupon($getData)
    {
        ...
        if (empty($getData['tmp']['memberCouponNo']) === false) {
            ...
            foreach ($getData['tmp']['memberCouponNo'] as $memberCouponNo) {
                ...
                $this->db->bind_param_push($arrBind['bind'], $this->fieldTypes['orderCoupon']['orderNo'], $getData['orderNo']);
    ...
    public function restoreRefundUseDeposit($getData, $orderInfo, $orderRefundInfo)
    {
        ...
        if ($totalRefundUseDeposit > 0) {
            ...
            if ($deposit->setMemberDeposit($orderInfo['memNo'], $totalRefundUseDeposit, Deposit::REASON_CODE_GROUP . Deposit::REASON_CODE_ORDER_CANCEL, 'o', $orderInfo['orderNo'])) {
                ...
                $this->db->set_update_db(DB_ORDER_GOODS, $arrBind['param'], 'sno IN (\'' . gd_implode('\',\'', $orderRefundInfo['restoreDepositSnos']) . '\')', $arrBind['bind']);
    ...
    public function restoreRefundUseMileage($getData, $orderInfo, $orderRefundInfo)
    {
        ...
        if ($totalRefundUseMileage > 0) {
            ...
            if ($mileage->setMemberMileage($orderInfo['memNo'], $totalRefundUseMileage, Mileage::REASON_CODE_GROUP . Mileage::REASON_CODE_USE_GOODS_BUY_RESTORE, 'o', $orderInfo['orderNo'])) {
                ...
                $compareField = gd_array_keys($orderGoodsData);
                $arrBind = $this->db->get_binding(DBTableField::tableOrderGoods(), $orderGoodsData, 'update', $compareField);
                $this->db->set_update_db(DB_ORDER_GOODS, $arrBind['param'], 'sno IN (\'' . gd_implode('\',\'', $orderRefundInfo['restoreMileageSnos']) . '\')', $arrBind['bind']);
                unset($arrBind, $orderGoodsData);
            }
        }
    }
    ...
    public function minusRefundGiveMileage($getData ,$orderInfo, $orderRefundInfo)
    {
        ...
        if ($orderRefundInfo['totalRefundGiveMileage'] > 0) {
            ...
            if ($mileage->setMemberMileage($orderInfo['memNo'], ($orderRefundInfo['totalRefundGiveMileage'] * -1), Mileage::REASON_CODE_GROUP . Mileage::REASON_CODE_ADD_GOODS_BUY_RESTORE, 'o', $orderInfo['orderNo'])) {
                ...
                $compareField = gd_array_keys($orderGoodsData);
                $arrBind = $this->db->get_binding(DBTableField::tableOrderGoods(), $orderGoodsData, 'update', $compareField);
                $this->db->set_update_db(DB_ORDER_GOODS, $arrBind['param'], 'sno IN (\'' . gd_implode('\',\'', $orderRefundInfo['giveMileage']) . '\')', $arrBind['bind']);
                unset($arrBind, $orderGoodsData);
            }
        }
     ...
     * 환불 완료 시 회원 구매금액/구매건수 차감
     *
     * setOrderPriceMember() 에서 배송비를 saleAmt 에 포함시킨 기준과 동일하게:
     *   - orderDeliverySno 단위로, 해당 배송비 그룹 내 구매확정 이력이 없는 경우에만 배송비 포함
     *   - 같은 배송비 그룹의 첫 번째 상품 처리 시에만 배송비를 합산
     *
     * @param array $getData         환불 요청 데이터 (레거시 유지)
     * @param array $orderInfo       주문 정보
     * @param array $orderRefundInfo 환불 대상 주문 상품 정보
     *
     * @return void
     */
    public function updateRefundMemberPrice($getData, $orderInfo, $orderRefundInfo)
    {
        ...
        // 환불 대상 상품 sno 중 구매확정 추출
        $confirmedSnoMap = [];
        foreach ($orderRefundInfo['refundOrderGoodsSnos'] as $sno) {
            if ($orderAdmin->isSetOrderStatus($orderInfo['orderNo'], $sno) !== false) {
                $confirmedSnoMap[$sno] = true;
            }
        }

        \Logger::channel('order')->info(__METHOD__ . '[' . __LINE__ . '], confirmedSnoMap : ', [$confirmedSnoMap]);

        // 구매확정 상품이 없으면 처리 불필요
        if (empty($confirmedSnoMap)) {
            return;
        }

        // 환불 대상 sno
        $refundSnoSet = array_flip($orderRefundInfo['refundOrderGoodsSnos']);

        $deliveryIncludeMap = []; // [orderDeliverySno => bool] true=배송비 차감 대상
        foreach ($orderInfo['goods'] as $sVal) {
            foreach ($sVal as $dVal) {
                foreach ($dVal as $gVal) {
                    $dSno = $gVal['orderDeliverySno'];
                    if (!isset($deliveryIncludeMap[$dSno])) {
                        $deliveryIncludeMap[$dSno] = true; // 초기값: 배송비 포함
                    }
                    if ($deliveryIncludeMap[$dSno] === false) {
                        continue; // 이미 제외로 결정된 그룹은 스킵
                    }
                    // 환불 대상이 아닌 상품 중 구매확정 이력 체크
                    if (!isset($refundSnoSet[$gVal['sno']])
                        && $orderAdmin->isSetConfirmOrderGoodsStatus($orderInfo['orderNo'], $gVal['sno']) === true
                    ) {
                        $deliveryIncludeMap[$dSno] = false;
                    }
                }
            }
        }

        // 환불 대상 상품의 건수 및 금액 집계 (r1: 환불요청, r2: 환불거절)
        $refundCnt          = 0;
        $refundPrice        = 0;
        $deliveryCountMap   = []; // [orderDeliverySno => int] 배송비 그룹별 처리된 상품 수
        $refundableStatuses = ['r1' => true, 'r2' => true];

        foreach ($orderInfo['goods'] as $sVal) {
            foreach ($sVal as $dVal) {
                foreach ($dVal as $gVal) {
                    if (!isset($confirmedSnoMap[$gVal['sno']], $refundableStatuses[$gVal['orderStatus']])) {
                        continue;
                    }
                    ...
                    // 상품 금액 누적 합산
                    $refundPrice += $gVal['realTaxSupplyGoodsPrice'] + $gVal['realTaxVatGoodsPrice'] + $gVal['realTaxFreeGoodsPrice'];

                    // 배송비 합산: orderDeliverySno 그룹 기준으로 첫 번째 상품 처리 시에만
                    $dSno = $gVal['orderDeliverySno'];
                    if (($deliveryIncludeMap[$dSno] ?? false) === true && (int)($deliveryCountMap[$dSno] ?? 0) < 1) {
                        $deliveryCharge = $gVal['realTaxSupplyDeliveryCharge'] + $gVal['realTaxVatDeliveryCharge'] + $gVal['realTaxFreeDeliveryCharge'];
                        \Logger::channel('order')->info(__METHOD__ . ' deliveryCharge(dSno=' . $dSno . '): ' . $deliveryCharge);
                        $refundPrice += $deliveryCharge;
                    }
                    $deliveryCountMap[$dSno] = (int)($deliveryCountMap[$dSno] ?? 0) + 1;
                }
            }
        }
        ...
        \Logger::channel('order')->info(__METHOD__ . '[' . __LINE__ . '], refundCnt: ' . $refundCnt . ', refundPrice: ' . $refundPrice);

        // 회원정보 구매금액/구매건수 차감
        $where     = 'memNo = \'' . $this->db->escape($orderInfo['memNo']) . '\' AND saleCnt > 0 AND saleAmt > 0';
        $updateSet = 'saleAmt = IF(saleAmt >= ' . $refundPrice . ', saleAmt - ' . $refundPrice . ', 0)'
                   . ', saleCnt = IF(saleCnt >= ' . $refundCnt  . ', saleCnt - ' . $refundCnt  . ', 0)';

        $this->db->set_update_db_query(DB_MEMBER, $updateSet, $where);
    }
    ...
    public function completeRefundPrice($getData, $orderInfo, $refundRealTaxData)
    {
        try {
            ...
            if ($getData['info']['completeMileagePrice'] > 0) {
                ...
                // 페이코일때 상태변경 모듈 실행
                if ($orderInfo['pgName'] == 'payco' && (gd_in_array($orderInfo['settleKind'], ['fv']) === true && $orderInfo['paycoData']['refundUseYn'] != 'Y') && gd_count($getData['refund']) == $orderInfo['orderGoodsCnt']) {
                    $pgConf = gd_pgs('payco');
            ...
            // 전액할인일 경우 return
            if($orderInfo['mallSno'] == DEFAULT_MALL_NUMBER && gd_in_array($orderInfo['settleKind'], ['gz']) === true) {
                return '';
            }

            // pg환불금액이 0일 경우 환불 불가
            if ($getData['info']['refundMethod'] == 'PG환불' && $getData['info']['completePgPrice'] <= 0) {
                throw new Exception('PG환불 금액이 0인 경우 PG환불이 불가합니다.');
            }
            ...
            if ($getData['info']['completePgPrice'] > 0) {
                ...
                // 환불 금액 (PG 취소 금액 이외의 다른 금액 포함) // jaewon noh - 해당부분에서 새액비율재계산을하면서 1원씩 비율이 틀어지는 케이스가 발생해 해당부분에선 넘겨받은값만으로 처리하도록 변경
//                $realTaxPgRate = $this->getRealTaxRate($refundRealTaxData['realTaxSupplyPrice'], $refundRealTaxData['realTaxVatPrice'], $refundRealTaxData['realTaxFreePrice']);
                ...
                $realTaxRefundPgSupplyPrice = $refundRealTaxData['realTaxSupplyPrice'];
                $realTaxRefundPgVatPrice = $refundRealTaxData['realTaxVatPrice'];
                $realTaxRefundPgFreePrice = $refundRealTaxData['realTaxFreePrice'];
...
//                list($realTaxRefundPgSupplyPrice, $realTaxRefundPgVatPrice, $realTaxRefundPgFreePrice) = $this->getRealTaxBalance($getData['info']['completePgPrice'], $realTaxRefundPgSupplyPrice, $realTaxRefundPgVatPrice, $realTaxRefundPgFreePrice);
                ...
                // 해외PG의 경우 해외PG금액도 셋팅
                if ($orderInfo['mallSno'] > DEFAULT_MALL_NUMBER || $orderInfo['overseasSettlePrice'] > 0) {
                    $tmp['overseasSettlePrice'] = $orderInfo['overseasSettlePrice'];
                    $tmp['overseasSettleCurrency'] = $orderInfo['overseasSettleCurrency'];
                }
                ...
                // 에스크로 관련 데이터
                $tmp['escrowDenyFl'] = $orderInfo['escrowDenyFl'];
                $tmp['escrowConfirmFl'] = $orderInfo['escrowConfirmFl'];
                $tmp['escrowDeliveryFl'] = $orderInfo['escrowDeliveryFl'];
                ...
                if ($result !== true) {
                    ...
                    if(($orderInfo['pgRealTaxSupplyPrice']+$orderInfo['pgRealTaxVatPrice']+$orderInfo['pgRealTaxFreePrice']) > 1){
                        ...
                        $arrBind = $this->db->get_binding(DBTableField::tableOrder(), $orderUpdateData, 'update', gd_array_keys($orderUpdateData));
                        $arrWhere = 'orderNo = ?';
    ...
    public function setCancelOrderGoods($cancel, $cancelMsg, $cancelPrice, $cancelReturn, $restoreCouponFl = true)
    {
        ...
        $iTotalCancelGoodsCount = gd_count($cancel['orderGoods']);
        $iCancelCount = 0;
        $iDivisionCouponOrderDcPrice = 0;
        foreach ($originOrderGoodsData as $key => $val) {
            $singleOrderStatus = substr($val['orderStatus'], 0, 1);
            if (!gd_in_array($singleOrderStatus, ['c', 'e', 'r', 'f'])) {
                if (gd_array_key_exists($val['sno'], $cancel['orderGoods'])) {
                    ...
                    if ($goodsCnt == 0) {// 남은 수량이 0 이면 주문 상품 상태를 취소로 변경
                        ...
                        // 입금대기 상태에서 취소 시 환불배송비는 0원으로 예외처리
                        $updateCancelGoodsData[$val['sno']]['handleCancelDeliveryPrice'] = ($singleOrderStatus == 'o') ? 0 : ($cancelPrice['deliveryCancel'][$val['orderDeliverySno']] + $cancelPrice['areaDeliveryCancel'][$val['orderDeliverySno']] + $cancelPrice['overseaDeliveryCancel'][$val['orderDeliverySno']]);
...
//            if ($val['deliveryCollectFl'] == 'pre') { // 선불만 처리
            if (gd_array_key_exists($val['sno'], $cancelData['totalCancelDeliveryPrice'])) { // 취소 상품의 배송비 조건 만
                ...
                // 추가 배송비
                $cancelPriceAddDelivery = ($cancelPrice['addDelivery'] == 0) ? 0 : $cancelPrice['addDelivery'][$val['sno']];

                $updateCancelDeliveryData[$val['sno']]['deliveryPolicyCharge'] = $updateCancelDeliveryData[$val['sno']]['deliveryPolicyCharge'] + $cancelPriceAddDelivery;
...
//            }
        ...
        $countGoods = gd_count($goodsPrice);
        $i = 1;
        $sumGoods = 0;
        ...
        $countDelivery = gd_count($deliveryPrice);
        $i = 1;
        $sumDelivery = 0;
        foreach ($deliveryPrice as $deliverySno => $price) {
            if ($i == $countDelivery) {
                ...
                $divisionDeliveryCharge = $totalDeliveryPrice == 0 ? 0 : $updateCancelOrderData['totalCouponDeliveryDcPrice'] * ($price / $totalDeliveryPrice);
                $updateCancelDeliveryData[$deliverySno]['divisionDeliveryCharge'] = gd_number_figure($divisionDeliveryCharge, $this->couponTruncPolicy['unitPrecision'], $this->couponTruncPolicy['unitRound']);
        ...
        if(gd_count($originOrderDeliveryData) > 0){
            foreach($originOrderDeliveryData as $key => $value){
                // 회원배송비 무료 취소시
                if (gd_array_key_exists($value['sno'], $cancelData['totalCancelDeliveryPrice']) && $cancelPrice['deliveryMemberDcCancelFl'] === 'a') {
        ...
        $totalGoodsSalePrice = gd_array_sum($goodsPrice) - gd_array_sum($goodsDcPrice);
        ...
        foreach ($goodsDelivery as $orderDeliveryNo => $key) {
            $countDeliveryByGoods = gd_count($goodsDelivery[$orderDeliveryNo]);
            $i = 1;
            $sumDeposit = 0;
            $sumMileage = 0;
        ...
        if ($originOrderData['receiptFl'] == 'r') {
            ...
            if (empty($orderCashReceiptData) === false) {
                ...
                $compareField = gd_array_keys($receipt);
                $arrBind = $this->db->get_binding(DBTableField::tableOrderCashReceipt(), $receipt, 'update', $compareField);
                $arrWhere = 'sno = ?';
        ...
        $compareField = gd_array_keys($updateCancelOrderData);
        ...
        foreach ($updateCancelGoodsData as $orderGoodsSno => $val) {
            if (gd_array_key_exists($orderGoodsSno, $cancel['orderGoods'])) {
                //환불 부가결제 금액이 있을때
            ...
            //상품 구매 개수 복원
            $order = new Order();
            if ($order->isLoggedMemberOrderGoodsCountData($cancel['orderNo'], $orderGoodsSno)) {
                $aGoodsSno['sno'][0] = $aGoodsSno['GoodsSno'][0] = $orderGoodsSno;
                $aGoodsSno['changeStatus'] = '';
                $orderKind['kind'] = '';
                $orderKind['channel'] = '';
                $claimInfo['orderCount'] = $cancel['orderGoods'][$orderGoodsSno];
                $order->setCountOrderGoods($cancel['orderNo'], $aGoodsSno, $orderKind, true, $claimInfo);
            }
        }
        ...
        foreach ($cancel['orderGoods'] as $cancelOrderGoodsSno => $cancelGoodsCnt) {
            ...
            if (gd_array_key_exists($cancelOrderGoodsSno, $insertCancelGoodsData)) {
        ...
        foreach ($updateCancelGoodsData as $orderGoodsSno => $val) {
            ...
            if (gd_in_array($orderGoodsSno, $insertCancelOrderGoodsNo)) {
        ...
        if ($cancelReturn['couponFl'] == 'y') {
            $this->setReturnCoupon($cancel['orderNo'], $cancelReturn['coupon'], $claimStatus, $restoreCouponFl);
        }
        ...
        // 주문 상품 상태 변경 WebHook 발송
        if (is_array($cancel['orderGoods'])) {
            $webhookOrderGoodsSnos = array_merge(array_keys($cancel['orderGoods']), $insertCancelOrderGoodsNo ?? []);
            $this->orderWebHookService->sendOrderGoodsWebHook($webhookOrderGoodsSnos, $claimStatus, $cancelMsg['orderStatus']);
        }
        ...
        if ($statusMode == 'a') { // @todo 상품 추가 시 사은품 추가선택 기능 제공
            ...
            if($statusMode === 'e'){
                ...
                if($exchangeMode === 'anotherExchange'){
                    ...
                    if(gd_count($orderGoodsSnoArray) > 0){
                        $plusMileageArray = [];
                        foreach($orderGoodsSnoArray as $key => $orderGoodsSno){
                            $orderGoodsData = $order->getOrderGoodsData($orderNo, $orderGoodsSno);
                            foreach ($orderGoodsData as $scmNo => $dataVal) {
                                $plusMileageArray[] = gd_array_sum(gd_array_column($dataVal, 'goodsMileage'));
                                $plusMileageArray[] = gd_array_sum(gd_array_column($dataVal, 'memberMileage'));
                                $plusMileageArray[] = gd_array_sum(gd_array_column($dataVal, 'couponGoodsMileage'));
                            }
                        }
                        if(gd_array_sum($plusMileageArray) < 1){
                            $returnData['exchangeMileageApplyFl']  = 'n';
                        }
                    }
                }
     ...
     * @param boolean $restoreCouponFl 쿠폰 복원 여부
     */
    public function setReturnCoupon($orderNo, $memberCoupon, $claimStatus, $restoreCouponFl = true)
    {
        foreach ($memberCoupon as $memberCouponNo) {
            if ($restoreCouponFl === true) {
                // 주문번호에 따른 회원 쿠폰 번호를 복원 체크 업데이트
                $updateCouponReturn['minusRestoreCouponFl'] = 'y';
                $updateCouponReturn['plusRestoreCouponFl'] = 'y';
                $compareField = gd_array_keys($updateCouponReturn);
                ...
                unset($getData['regDt']);
                unset($getData['modDt']);
                unset($getData['memberCouponCartDate']);
                unset($getData['memberCouponUseDate']);
                $getData['memberCouponState'] = 'y';
                $getData['orderWriteCouponState'] = 'y';
                $compareField = gd_array_keys($getData);
    ...
    public function setOrderRestore($orderNo, $claimStatus)
    {
        ...
        foreach ($orderGoodsData as $val) {
            ...
            $order = new Order();
            if ($order->isLoggedMemberOrderGoodsCountData($orderNo, $val['sno'])) {
                $aGoodsSno['sno'][0] = $aGoodsSno['GoodsSno'][0] = $val['sno'];
                $aGoodsSno['changeStatus'] = '';
                $orderKind['kind'] = '';
                $orderKind['channel'] = '';
                $claimInfo['orderCount'] = $val['goodsCnt'];
                $order->setCountOrderGoods($orderNo, $aGoodsSno, $orderKind, false);
            }
        }
        $orderGoodsOriginalData = $this->getOrderGoodsStatusOriginalData($orderNo);

        // 복원된 재고를 다시 차감(취소 했던 주문 정보로 복원 진행)
        if (gd_count($stockOrderGoodsSno) > 0) {
            $order = App::load(\Component\Order\Order::class);
            $stockResult = $order->setGoodsStockCutback($orderNo, $stockOrderGoodsSno);
        }
        if (gd_in_array('오류', $stockResult['code']) === true) {
            $getKey = gd_array_search('오류', $stockResult['code']);
        ...
        $cashFl = false; // 현금영수증
        if ($claimStatus == 'c') {
            if (is_object($this->member) === false) {
            ...
            if (empty($orderOriginalData['memNo']) === false) {
                $memInfo = $this->member->getMemberInfo($orderOriginalData['memNo']);
            }
            ...
            $tmpDiffDeposit = $diffDeposit > 0 ? $diffDeposit : -$diffDeposit;
            $tmpDiffMileage = $diffMileage > 0 ? $diffMileage : -$diffMileage;

            if (empty($memInfo) === false && $memInfo['deposit'] - $tmpDiffDeposit >= 0) {
                if ($diffDeposit == 0) {}
                else {
                    // 가감 처리
                    $deposit = App::load(\Component\Deposit\Deposit::class);
                ...
                if ($diffDeposit == 0) {}
                    ...
                    $exceptMsg[] = '고객이 보유한 예치금 부족';
                }
            }

            if (empty($memInfo) === false && $memInfo['mileage'] - $tmpDiffMileage >= 0) {
                if ($diffMileage == 0) {
                } else {
                    // 가감 처리
                    $mileage = App::load(\Component\Mileage\Mileage::class);
                ...
                if ($diffMileage == 0) {
                    ...
                    $exceptMsg[] = '고객이 보유한 마일리지 부족';
                }
            }
        }

        if (empty($exceptMsg) === false) {
            $message = '<b>취소복원이 불가합니다. 복원 불가 사유를 확인해주세요.</b><br /><br />';
            $message .= @gd_implode('<br />', $exceptMsg);
            throw new Exception($message);
        }
    ...
    public function getSelectOrderGoodsAddData($postValue)
    {
        ...
        foreach ($orderGoodsData as $scmNo => $orderGoodsData) {
            foreach ($orderGoodsData as $key => $val) {
                ...
                if ($settleStatus === false && gd_in_array(substr($val['orderStatus'], 0, 1), $settleAfterStatus)) { // 입금 전 후 체크
                    $settleStatus = true;
                }
            }
        }
        ...
        if($this->multiShippingOrderFl === true){
            if(gd_count($originOrderInfoData) > 0){
            ...
            $postValue = gd_array_merge((array)$postValue, (array)$setMultiShippingAddPostData);
        ...
        $cartAdmin->orderGoodsChange = true;
        ...
        foreach ($cartAddData as $scmNo => $deliveryVal) {
            foreach ($deliveryVal as $deliverySno => $cartGoodsVal) {
                foreach ($cartGoodsVal as $cartGoodsData) {
                    if (gd_array_key_exists($deliverySno, $orderGoodsDelivery)) {
        ...
        $addData['enuriSumPrice'] = (int)gd_array_sum(explode(INT_DIVISION, $postValue['enuri']));
     ...
     * @param boolean $orderGoodsChange 주문상품교환여부
     *
     * @throws
     *
     * @return boolean true
    ...
    public function setAddOrderGoods($orderNo, $addData, $orderGoodsChange = false)
    {
        ...
        if($originOrderData['multiShippingFl'] === 'y'){
            ...
            if(gd_count($originMultiOrderInfoData) > 0){
            ...
            $addData = gd_array_merge((array)$addData, (array)$setMultiShippingAddPostData);
        ...
        // 추가한 장바구니 불러오기
        $cartAdmin = new CartAdmin(0, false, $originOrderData['mallSno']);
        $cartAdmin->orderGoodsChange = $orderGoodsChange;
        ...
        foreach ($cartAddData as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                ...
                foreach ($dVal as $gKey => $gVal) {
                    ...
                    // 상품에 연결된 전체 카테고리 코드
                    $gVal['cateAllCd'] = is_array($gVal['cateAllCd']) ? json_encode($gVal['cateAllCd']) : $gVal['cateAllCd'];
        ...
        $updateAddOrderData['orderGoodsCnt'] = $originOrderData['orderGoodsCnt'] + gd_count($arrOrderGoodsSno['sno']);
        ...
        $updateAddOrderData['totalEnuriDcPrice'] = $originOrderData['totalEnuriDcPrice'] + (int)gd_array_sum($addData['enuri']); //총 운영자 추가 할인 금액
        ...
        $compareField = gd_array_keys($updateAddOrderData);
        ...
        // 주문 상품 상태 변경 WebHook 발송
        if (is_array($arrOrderGoodsSno['sno'])){
            $this->orderWebHookService->sendOrderGoodsWebHook($arrOrderGoodsSno['sno'], $orderStatus);
        }
    ...
    public function setBackRefundOrderGoods($arrData, $changeStatusName, $isChargeBack = false)
    {
        ...
        // 차지백 서비스로 환불접수하는 경우 현재 상태에 대한 변경 가능 상태 기준표에 환불 추가
        if ($isChargeBack === true) {
            // 현재가 배송 인경우 => 배송, 상품, 반품, 교환
            if (gd_in_array('r', $orderAdmin->statusStandardCode['d']) === false) {
                $orderAdmin->statusStandardCode['d'][] = 'r';
            }

            // 현재가 구매확정 인경우 => 배송, 상품, 반품, 교환
            if (gd_in_array('r', $orderAdmin->statusStandardCode['s']) === false) {
                $orderAdmin->statusStandardCode['s'][] = 'r';
            }

            // 현재가 교환 인경우 => 교환
            if (gd_in_array('r', $orderAdmin->statusStandardCode['e']) === false) {
                $orderAdmin->statusStandardCode['e'][] = 'r';
            }

            // 현재가 교환추가 인경우 => 교환추가
            if (gd_in_array('r', $orderAdmin->statusStandardCode['z']) === false) {
                $orderAdmin->statusStandardCode['z'][] = 'r';
            }
        }
        ...
        if (!empty($bundleData)) {
            ...
            if ($methodType != 'cancel') {
                ...
                foreach ($bundleData['sno'] as $orderGoodsSno) {
                    ...
                    if (!empty($newOrderGoodsData['userHandleGoodsNo'])) {
                        $webhookOrderGoodsSnos[] = $newOrderGoodsData['userHandleGoodsNo'];
                        $webhookOrderGoodsSnos[] = $orderGoodsSno;
                    }
        ...
        // 주문 상품 상태 변경 WebHook 발송
        if (is_array($webhookOrderGoodsSnos)){
            $this->orderWebHookService->sendOrderGoodsWebHook($webhookOrderGoodsSnos, 'restore');
        }
    }
    ...
    public function setSameExchangeOrderGoods($postData)
    {
        ...
        if(gd_count($tmpChangeOptionArr) > 0){
        ...
        if (gd_count($tmpOrderGoodsSnoArr) > 0) {
        ...
        if (gd_count($orderGoodsSnoArr) < 1) {
            ...
            if ($differentGoodsCnt > 0) {
                ...
                if($postData['supplyCouponMileage'] === 'n'){
                    ...
                    if((int)$orderGoodsData[$orderGoodsSno]['couponGoodsMileage'] > 0){
                        ...
                        //상품 쿠폰적립을 하지 않았다면
                        if(gd_count($orderProductCouponData) > 0) {
                ...
                // 교환추가 시 송장정보 초기화
                $updateOrderGoodsData['invoiceNo'] = '';
                $updateOrderGoodsData['invoiceCompanySno'] = '';
        ...
        if((int)$originalOrderData['totalMileage'] > 0 && ($postData['supplyMileage'] === 'n' || $postData['supplyCouponMileage'] === 'n')){
            if(gd_count($minusMileageData) > 0){
                $totalMinusMileage = [];
                $updateOrderData['totalMileage'] = $originalOrderData['totalMileage'];

                $totalMinusMileage['goodsMileage'] = (int)gd_array_sum(gd_array_column($minusMileageData, 'goodsMileage'));
                $totalMinusMileage['memberMileage'] = (int)gd_array_sum(gd_array_column($minusMileageData, 'memberMileage'));
                $totalMinusMileage['couponGoodsMileage'] = (int)gd_array_sum(gd_array_column($minusMileageData, 'couponGoodsMileage'));

                $orderData['totalMileage'] -= (int)gd_array_sum($totalMinusMileage);
        ...
        //마일리지 지급
        if (gd_in_array('p', $currentStatusPolicy['mplus'])) {
        ...
        // 주문 상품 상태 변경 WebHook 발송
        if (is_array($orderGoodsSnoArr)){
            $webhookOrderGoodsSnos = array_merge(array_values($orderGoodsSnoArr) , $addOrderGoodsSnoArr ?? []);
    ...
    public function setUserSameExchangeOrderGoods($postData)
    {
        ...
        foreach ($postData['statusCheck'] as $val) {
            $orderGoodsCnt = 0;
            $tmpOrderGoodsData = $orderGoodsData = [];
            ...
            if (gd_count($tmpOrderGoodsData) > 0) {
                foreach ($tmpOrderGoodsData as $scmNo => $dataVal) {
                    foreach ($dataVal as $goodsData) {
                        ...
                        if($goodsData['userHandleSno'] == $userHandleSno && substr($goodsData['orderStatus'], 0, 1) == 'e'){
                            //user handle 업데이트
                            $userHandleUpdate = [
                                'userHandleFl' => 'y',
            ...
            // 재고차감
            if (gd_in_array('p', $currentStatusPolicy['sminus'])) {
                $order->setGoodsStockCutback($orderNo, [$addOrderGoodsSno]);
            }
        ...
        // 주문 상품 상태 변경 WebHook 발송
        if (is_array($returnOrderGoodsSno)){
            $webhookOrderGoodsSnos = array_merge($returnOrderGoodsSno, $addOrderGoodsSnoArr ?? []);
            $this->orderWebHookService->sendOrderGoodsWebHook($webhookOrderGoodsSnos, $claimStatus, $changeStatus);
        }
    ...
    public function setAnotherExchangeOrderGoods($postData)
    {
        ...
        if (gd_count($tmpBeforeOrderGoodsSnoArr) > 0) {
        ...
        //복수배송지가 사용된 주문건인지 체크
        if(gd_in_array('y', gd_array_column($orderGoodsData, 'multiShippingFl'))){
            $this->multiShippingOrderFl = true;
            $multiShippingOrderInfoSno = $multiShippingOrderInfoSnoArr[0];
            if(gd_count(gd_array_unique($multiShippingOrderInfoSnoArr)) > 1){
        ...
        if($this->addPaymentDivisionFl === true){
                ...
                foreach ($sVal as $dKey => $dVal) {
                    ...
                    foreach ($dVal as $gKey => $gVal) {
                        ...
                        if ($onlyOneDelivery === true) {
                            ...
                            // 마일리지 배송비 미포함 설정시 배송비에 안분될 부가결제 금액은 0원
                            if($postData['mileageUseDeliveryFl'] === 'n'){
                                $deliveryPrice = 0;
                            }
            ...
            foreach($orderGoodsData as $orderGoodsNo => $data){
                ...
                if(gd_in_array($orderGoodsNo, $orderGoodsSnoArr)){
                    if((int)$data['goodsCnt'] !== (int)$orderGoodsCntArr[$orderGoodsNo]){
                        ...
                        $totalCancelOrderGoodsSettlePrice = (int)gd_array_sum($cancelOrderGoodsSettlePrice);
                    ...
                    if(!gd_in_array(substr($data['orderStatus'], 0, 1), ['e', 'f', 'r', 'c'])){
            ...
            if(gd_count($this->onlyAddOrderGoodsRateArr['price']) > 0){
                foreach($this->onlyAddOrderGoodsRateArr['price'] as $dKey => $values){
                    $addTotalOrderGoodsRealPrice = gd_array_sum($values);
                    $addOrderGoodsRealPriceCnt = gd_count($values);

                    if($addOrderGoodsRealPriceCnt > 0){
                        $index = 1;
                        foreach($values as $snoKey => $price){
                            if((int)$addOrderGoodsRealPriceCnt === (int)$index){
                                $this->onlyAddOrderGoodsRateArr['rate'][$dKey][$snoKey] = 1 - gd_array_sum($this->onlyAddOrderGoodsRateArr['rate'][$dKey]);
                            }
                            else {
                                $addTotalOrderGoodsRealPriceRate = 0;
                                if ($addTotalOrderGoodsRealPrice > 0) {
                                    $addTotalOrderGoodsRealPriceRate = $price / $addTotalOrderGoodsRealPrice;
                                }
                                $this->onlyAddOrderGoodsRateArr['rate'][$dKey][$snoKey] = gd_number_figure($addTotalOrderGoodsRealPriceRate, '0.00001', 'round');
                            }
            ...
            if(gd_count($this->onlyCancelOrderGoodsRateArr['price']) > 0){
                foreach($this->onlyCancelOrderGoodsRateArr['price'] as $dKey => $values){
                    $cancelTotalOrderGoodsRealPrice = gd_array_sum($values);
                    $cancelOrderGoodsRealPriceCnt = gd_count($values);

                    if($cancelOrderGoodsRealPriceCnt > 0){
                        $index = 1;
                        foreach($values as $snoKey => $price){
                            if((int)$cancelOrderGoodsRealPriceCnt === (int)$index){
                                $this->onlyCancelOrderGoodsRateArr['rate'][$dKey][$snoKey] = 1 - gd_array_sum($this->onlyCancelOrderGoodsRateArr['rate'][$dKey]);
                            }
                            else {
                                $cancelTotalOrderGoodsRealPriceRate = 0;
                                if ($cancelTotalOrderGoodsRealPrice > 0) {
                                    $cancelTotalOrderGoodsRealPriceRate = $price / $cancelTotalOrderGoodsRealPrice;
                                }
                                $this->onlyCancelOrderGoodsRateArr['rate'][$dKey][$snoKey] = gd_number_figure($cancelTotalOrderGoodsRealPriceRate, '0.00001', 'round');
                            }
            ...
            // 교환추가상품+배송비의 비율 구하기!
            $addTotalOrderRealPrice = gd_array_sum($addPaymentDivisionArr['add']['price']);
            $addOrderRealPriceCnt = gd_count($addPaymentDivisionArr['add']['price']);
            if($addOrderRealPriceCnt > 0){
                ...
                foreach($addPaymentDivisionArr['add']['price'] as $key => $realTaxPrice){
                    if((int)$addOrderRealPriceCnt === (int)$index){
                        $addPaymentDivisionArr['add']['rate'][$key] = 1 - gd_array_sum($addPaymentDivisionArr['add']['rate']);
                    }
                    else {
                        $addTotalOrderRealPriceRate = 0;
                        if ($addTotalOrderRealPrice > 0) {
                            $addTotalOrderRealPriceRate = $realTaxPrice/$addTotalOrderRealPrice;
                        }
                        $addPaymentDivisionArr['add']['rate'][$key] = gd_number_figure($addTotalOrderRealPriceRate, '0.00001', 'round');
                    }
            ...
            // 교환취소상품+배송비의 비율 구하기!
            $cencelTotalOrderRealPrice = gd_array_sum($addPaymentDivisionArr['cancel']['price']);
            $cencelOrderRealPriceCnt = gd_count($addPaymentDivisionArr['cancel']['price']);
            if($cencelOrderRealPriceCnt > 0){
                ...
                foreach($addPaymentDivisionArr['cancel']['price'] as $key => $realTaxPrice){
                    if((int)$cencelOrderRealPriceCnt === (int)$index){
                        $addPaymentDivisionArr['cancel']['rate'][$key] = 1 - gd_array_sum($addPaymentDivisionArr['cancel']['rate']);
                    }
                    else {
                        $cancelRate = NumberUtils::divisionExceptZero($realTaxPrice, $cencelTotalOrderRealPrice);
                        $addPaymentDivisionArr['cancel']['rate'][$key] = gd_number_figure($cancelRate, '0.00001', 'round');
                    }
            ...
            $addOrderRateCnt = gd_count($addPaymentDivisionArr['add']['rate']);
            //교환추가상품 남은 마일리지 재분배
            ...
            if($addOrderRateCnt > 0){
                ...
                foreach($addPaymentDivisionArr['add']['rate'] as $key => $rate){
                    if($addOrderRateCnt === $index){
                        $this->reDivisionAddPaymentArr['add']['mileage'][$key] = $restUseMileage - gd_array_sum($this->reDivisionAddPaymentArr['add']['mileage']);
                    }
                    else {
                    ...
                    if (preg_match('/d/', $key)) {
                        if(gd_count($this->onlyAddOrderGoodsRateArr['rate']) > 0) {
                            foreach ($this->onlyAddOrderGoodsRateArr['rate'] as $dKey => $values) {
                                if($dKeyArr[$key] === $dKey){
                                    ...
                                    foreach($values as $snoKey => $rate){
                                        if((int)gd_count($this->onlyAddOrderGoodsRateArr['rate'][$dKey]) === (int)$dindex){
                                            $this->reDivisionAddPaymentArr['add']['divisionDeliveryGoodsMileage'][$snoKey] = $sumMileage - gd_array_sum($this->reDivisionAddPaymentArr['add']['divisionDeliveryGoodsMileage']);
                                        }
                                        else {
            ...
            if($addOrderRateCnt > 0){
                ...
                foreach($addPaymentDivisionArr['add']['rate'] as $key => $rate){
                    if($addOrderRateCnt === $index){
                        $this->reDivisionAddPaymentArr['add']['deposit'][$key] = $restUseDeposit - gd_array_sum($this->reDivisionAddPaymentArr['add']['deposit']);
                    }
                    else {
                        ...
                        if(gd_count($this->onlyAddOrderGoodsRateArr['rate']) > 0) {
                            foreach ($this->onlyAddOrderGoodsRateArr['rate'] as $dKey => $values) {
                                if($dKeyArr[$key] === $dKey){
                                    ...
                                    foreach($values as $snoKey => $rate){
                                        if((int)gd_count($this->onlyAddOrderGoodsRateArr['rate'][$dKey]) === (int)$dindex){
                                            $this->reDivisionAddPaymentArr['add']['divisionDeliveryGoodsDeposit'][$snoKey] = $sumDeposit - gd_array_sum($this->reDivisionAddPaymentArr['add']['divisionDeliveryGoodsDeposit']);
                                        }
                                        else {
            ...
            $cancelOrderRateCnt = gd_count($addPaymentDivisionArr['cancel']['rate']);
            //교환취소상품에 남을 마일리지 재분배
            if($cancelOrderRateCnt > 0){
                $index = 1;
                foreach($addPaymentDivisionArr['cancel']['rate'] as $key => $rate){
                    if($cancelOrderRateCnt === $index){
                        $this->reDivisionAddPaymentArr['cancel']['mileage'][$key] = (int)$postData['cancelDivisionUseMileage'] - gd_array_sum($this->reDivisionAddPaymentArr['cancel']['mileage']);
                    }
                    else {
                    ...
                    // 배송비에 관한 마일리지 재분배시 주문상품에 들어갈 배송비마일리지 계산
                    if (preg_match('/d/', $key)) {
                        if(gd_count($this->onlyCancelOrderGoodsRateArr['rate']) > 0) {
                            foreach ($this->onlyCancelOrderGoodsRateArr['rate'] as $dKey => $values) {
                                if($key === 'd'.$dKey){
                                    $sumMileage = $this->reDivisionAddPaymentArr['cancel']['mileage'][$key];
                                    $dindex = 1;
                                    foreach($values as $snoKey => $rate){
                                        if((int)gd_count($this->onlyCancelOrderGoodsRateArr['rate'][$dKey]) === (int)$dindex){
                                            $this->reDivisionAddPaymentArr['cancel']['divisionDeliveryGoodsMileage'][$snoKey] = $sumMileage - gd_array_sum($this->reDivisionAddPaymentArr['cancel']['divisionDeliveryGoodsMileage']);
                                        }
                                        else {
            ...
            if($cancelOrderRateCnt > 0){
                ...
                foreach($addPaymentDivisionArr['cancel']['rate'] as $key => $rate){
                    if($cancelOrderRateCnt === $index){
                        $this->reDivisionAddPaymentArr['cancel']['deposit'][$key] = (int)$postData['cancelDivisionUseDeposit'] - gd_array_sum($this->reDivisionAddPaymentArr['cancel']['deposit']);
                    }
                    else {
                        ...
                        if(gd_count($this->onlyCancelOrderGoodsRateArr['rate']) > 0) {
                            foreach ($this->onlyCancelOrderGoodsRateArr['rate'] as $dKey => $values) {
                                if($key === 'd'.$dKey){
                                    ...
                                    foreach($values as $snoKey => $rate){
                                        if((int)gd_count($this->onlyCancelOrderGoodsRateArr['rate'][$dKey]) === (int)$dindex){
                                            $this->reDivisionAddPaymentArr['cancel']['divisionDeliveryGoodsDeposit'][$snoKey] = $sumDeposit - gd_array_sum($this->reDivisionAddPaymentArr['cancel']['divisionDeliveryGoodsDeposit']);
                                        }
                                        else {
            ...
            if(gd_count($this->etcOrderGoodsData) > 0){
                $etcOrderGoodsRateData = [];
                foreach($this->etcOrderGoodsData as $orderDeliverySno => $orderGoodsArr){
                    if(gd_count($orderGoodsArr) > 0){
                        $etcTotalRealTaxPrice = gd_array_sum($orderGoodsArr);
                        $etcTotalCount = gd_count($orderGoodsArr);
                        $index = 1;
                        foreach($orderGoodsArr as $orderGoodsNo => $realTaxPrice){
                            if((int)$etcTotalCount === (int)$index){
                                $etcOrderGoodsRateData[$orderDeliverySno][$orderGoodsNo] = 1 - gd_array_sum($etcOrderGoodsRateData[$orderDeliverySno]);
                            }
                            else {
                                $etcTotalRealTaxPriceRate = 0;
                                if ($etcTotalRealTaxPrice > 0) {
                                    $etcTotalRealTaxPriceRate = $realTaxPrice/$etcTotalRealTaxPrice;
                                }
                                $etcOrderGoodsRateData[$orderDeliverySno][$orderGoodsNo] = gd_number_figure($etcTotalRealTaxPriceRate, '0.00001', 'round');
                            }
                            $index++;
                        }
                    }
        ...
        // 나머지상품의 부가결제금액 조정
        if(gd_count($this->cancelDeliveryFl)){
            foreach($this->cancelDeliveryFl as $orderDeliverySno => $val){
                if($val === 'y'){
                    ...
                    $totalEtcOrderGoodsRateCnt = gd_count($etcOrderGoodsRateData[$orderDeliverySno]);
        ...
        if(gd_count($updateEtcOrderGoodsData['add']) > 0){
        ...
        if(gd_count($updateEtcOrderGoodsData['reset']) > 0){
        ...
        // 주문 상품 상태 변경 WebHook 발송
        if (is_array($orderGoodsSnoArr)){
            $webhookOrderGoodsSnos = array_merge(array_values($orderGoodsSnoArr), $addGroupData['insertOrderGoodsSnoArr'] ?? []);
            $this->orderWebHookService->sendOrderGoodsWebHook($webhookOrderGoodsSnos, $claimStatus, $changeStatus);
        }
    ...
    */
    public function getOrderGoodsRecalculateData($originalOrderGoodsData, $differentGoodsCnt)
    {
        ...
        if(gd_count($this->etcOrderGoodsData[$originalOrderGoodsData['orderDeliverySno']]) > 0){
    ...
    public function reCalculateOrderInfo($postData, $updateOrderGoodsSnoArr, $addGroupData)
    {
        ...
        if(gd_count($orderHandleData) > 0){
            $refundUseDeposit = gd_array_sum(gd_array_column($orderHandleData, 'refundUseDeposit'));
            $refundUseMileage = gd_array_sum(gd_array_column($orderHandleData, 'refundUseMileage'));
        ...
        //취소 배송비 처리
        if(gd_count($postData['cancelDeliveryFl']) > 0){
        ...
        //배송비추가
        if(gd_count($addGroupData['insertOrderDeliverySnoArr']) > 0){
    ...
    */
    public function setOrderExchangeHandle($postData, $handleGroupCd)
    {
        ...
        $updateData = [
            ...
            'ehEnuri' => gd_array_sum($postData['enuri']), //에누리
        ];
    ...
    */
    public function getOrderExchangeHandle($orderNo, $handleGroupCd=null, $handleSno=null)
    {
        ...
        //최초 주문데이터
        $query = "SELECT * FROM " . DB_ORDER_EXCHANGE_HANDLE . " WHERE " . gd_implode(" AND ", $where);
    ...
    */
            ...
            foreach ($sVal as $dKey => $dVal) {
                ...
                foreach ($dVal as $gKey => $gVal) {
                    ...
                    if($gVal['orderStatus'] === 'p1'){
                        $gVal['paymentDt'] = $date;
                        if($gVal['goodsType'] == 'goods') {
                            $goods->setOrderGoodsCount($gVal['sno'], false, $gVal['goodsNo'], $gVal['goodsCnt']);
                        }
                    }
                    $gVal['minusMileageFl'] = 'y';
                    $gVal['minusDepositFl'] = 'y';
                    ...
                    if ($onlyOneDelivery === true) {
                        ...
                        if($data['addDeliveryFl'][$dKey] !== 'y'){
                            ...
                            $deliveryInfo = gd_array_merge((array)$deliveryInfo, (array)$resetOrderDeliveryData);
                        }
    ...
    public function copyOrderGoodsData($orderNo, $orderGoodsSno, $orderStatus, $handleSno = null, $goodsCnt = null, $userHandleSno = null, $orderCd=null, $updateOrderGoodsData=[])
    {
        ...
        if(gd_count($updateOrderGoodsData)){
            $orderGoodsData = gd_array_merge((array)$orderGoodsData, (array)$updateOrderGoodsData);
        }
        ...
        // 추가상품의경우(추가든 교환이든) inflow값은 비워야함
        $orderGoodsData['inflow'] = '';

        // 교환처리시 추가상품의경우 송장번호초기화
        $orderGoodsData['invoiceCompanySno'] = '';
        $orderGoodsData['invoiceNo'] = '';
        ...
        if ($orderGoodsData['sendSmsFl'] !== null) {
            $orderGoodsData['sendSmsFl'] = null;
        }
    ...
    */
    ...
    {
        // 주문상품 DB insert 처리
        $compareField = gd_array_keys(DBTableField::getFieldNames('tableOrderGoods'));
    ...
    */
    ...
    {
        // 주문상품 DB insert 처리
        $compareField = gd_array_keys(DBTableField::getFieldNames('tableOrderDelivery'));
    ...
    public function updateOrderGoods($updateData, $orderNo, $orderGoodsSno=null, $orderDeliverySno=null)
    {
        ...
        $res = $this->db->set_update_db(DB_ORDER_GOODS, $arrBind['param'], gd_implode(" AND ", $arrWhere), $arrBind['bind']);
    ...
    */
    public function getSelectOrderBeforeData($postData)
    {
        ...
        if(gd_count($tmpRefundDeliveryData) > 0){
        ...
        foreach ($beforeOrderGoodsData as $scmNo => $dataVal) {
            foreach ($dataVal as $goodsData) {
                ...
                if ($this->myappUseFl) {
                    //TODO:php82 이게 맞?? $myappDcPrice 값 없음;;
//                    $tmpExchangeData['totalGoodsSettlePrice'][] -= $myappDcPrice;
        ...
        //교환될 상품의 차감될 상품할인 금액
        $exchangeData['totalGoodsDcPrice'] = gd_array_sum($tmpExchangeData['goodsDcPrice']);
        //교환될 상품의 차감될 회원 추가 할인 금액
        $exchangeData['totalMemberDcPrice'] = gd_array_sum($tmpExchangeData['memberDcPrice']);
        //교환될 상품의 차감될 회원 중복 할인 금액
        $exchangeData['totalMemberOverlapDcPrice'] = gd_array_sum($tmpExchangeData['memberOverlapDcPrice']);
        //교환될 상품의 차감될 회원 쿠폰 할인 금액
        $exchangeData['totalCouponGoodsDcPrice'] = gd_array_sum($tmpExchangeData['couponGoodsDcPrice']);
        //교환될 상품의 차감될 사용된 예치금
        $exchangeData['divisionUseDeposit'] = gd_array_sum($tmpExchangeData['divisionUseDeposit']);
        //교환될 상품의 차감될 사용된 마일리지
        $exchangeData['divisionUseMileage'] = gd_array_sum($tmpExchangeData['divisionUseMileage']);
        //교환될 상품의 차감될 주문 쿠폰 할인 금액
        $exchangeData['divisionCouponOrderDcPrice'] = gd_array_sum($tmpExchangeData['divisionCouponOrderDcPrice']);
        //교환될 상품의 에누리 금액 (운영자 추가 할인)
        $exchangeData['enuri'] = gd_array_sum($tmpExchangeData['enuri']);
        //교환될 상품의 마이앱 할인 금액
        if ($this->myappUseFl) {
            $exchangeData['totalMyappDcPrice'] = gd_array_sum($tmpExchangeData['myappDcPrice']);
        }

        //교환될 상품의 전체 할인 금액
        $exchangeData['totalGoodsDcPriceSum'] = gd_array_sum([
        ...
        //취소 상품 결제금액
        $exchangeData['totalGoodsSettlePrice'] = gd_array_sum($tmpExchangeData['totalGoodsSettlePrice']);
        //교환될 상품의 조정가능 배송비 금액
    ...
    */
    public function getSelectOrderAfterData($postData)
    {
        ...
        foreach ($cartData as $sKey => $sVal) {
            foreach ($sVal as $dKey => $dVal) {
                if($this->multiShippingOrderFl === true){
                    $subject = gd_array_filter(gd_array_column($dVal, 'goodsDeliveryMethod'))[0];
                    $subject = ((int)$orderInfoData['orderInfoCd'] === 1) ? '(메인) ' . $subject : '(추가'.((int)$orderInfoData['orderInfoCd']-1).') ' . $subject;
                }
                else {
                    $subject = gd_array_filter(gd_array_column($dVal, 'goodsNm'))[0];
                }
        ...
        // 추가 할인금액
        $exchangeData['addGoodsDcPrice'] = $cartAdmin->totalGoodsDcPrice + (int)gd_array_sum(explode(INT_DIVISION, $postData['enuri']));
        // 추가 상품 결제금액 - 에누리는 반영이 되어 있지 않으므로 에누리를 -처리 해준다
        $exchangeData['addGoodsSettlePrice'] = $cartAdmin->totalSettlePrice - $cartAdmin->totalDeliveryCharge - (int)gd_array_sum(explode(INT_DIVISION, $postData['enuri']));
    ...
    public function getSelectOrderGoodsExchangeData($postData)
    {
        ...
        if ($postData['mileageUseDeliveryFl'] === 'n') {
            // 마일리지 배송비 패치여부 및 설정값
            $exchangeData['mileageUseDeliveryFl'] = $postData['mileageUseDeliveryFl'];
            $totalAddSettlePrice = $exchangeData['after']['addGoodsSettlePrice'];
        } else {
    ...
    public function restoreExchangeCancel($postData)
    {
        ...
        if (gd_count($orderGoodsSnoArr) > 0) {
            $order = App::load(\Component\Order\Order::class);
    ...
    public function getOrderViewPriceInfo($data)
    {
        ...
        if ($originalData['orderDelivery']['sno']) {
            ...
            $totalDeliveryPolicyCharge = gd_array_sum(gd_array_column($originalData['orderDelivery'], 'deliveryPolicyCharge'));
            $totalDeliveryAreaCharge = gd_array_sum(gd_array_column($originalData['orderDelivery'], 'deliveryAreaCharge'));
        }
    ...
    public function getOrderViewPriceRefundAdjust($data)
    {
        ...
        if(gd_count($data['goods']) > 0){
            $deliveryUniqueKey = '';
            foreach ($data['goods'] as $sKey => $sVal) {
                foreach ($sVal as $dKey => $dVal) {
                    foreach ($dVal as $key => $val) {
                        if(!gd_in_array($val['handleMode'], $exceptHandleMode)){
                            ...
                            $refundExcludeOrderData['totalCouponOrderMileage'] += $val['divisionCouponOrderMileage'];
    ...
    public function getOrderOriginalCount($orderNo, $claimStatus = '', $refundExclude = false)
    {
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_ORDER_ORIGINAL . gd_implode(' ', $query);
    ...
    public function setBackupOrderOriginalData($orderNo, $claimStatus, $giftAction=false, $receiptAction=false, $orderStatisticsAction=false)
    {
        ...
        if(gd_count($orderCommonColumns) > 0){
            $orderData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderCommonColumns) . " FROM " . DB_ORDER . " WHERE orderNo = '" . $orderNo . "'");

            $fieldsToClean = ['orderGoodsNm', 'orderGoodsNmStandard'];
        ...
        if(gd_count($orderGoodsCommonColumns) > 0){
            $orderGoodsData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderGoodsCommonColumns) . " FROM " . DB_ORDER_GOODS . " WHERE orderNo = '" . $orderNo . "'");

            $fieldsToClean = ['goodsNm', 'goodsNmStandard'];
            if (gd_count($orderGoodsData) > 0) {
        ...
        if(gd_count($orderDeliveryCommonColumns) > 0){
            $orderDeliveryData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderDeliveryCommonColumns) . " FROM " . DB_ORDER_DELIVERY . " WHERE orderNo = '" . $orderNo . "'");

            if(gd_count($orderDeliveryData) > 0){
        ...
        if($giftAction === true){
            ...
            if(gd_count($orderGiftCommonColumns) > 0){
                $orderGiftData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderGiftCommonColumns) . " FROM " . DB_ORDER_GIFT . " WHERE orderNo = '" . $orderNo . "'");

                if(gd_count($orderGiftData) > 0){
        ...
        if($receiptAction === true){
            ...
            if(gd_count($orderCashReceiptCommonColumns) > 0){
                $orderCashReceiptData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderCashReceiptCommonColumns) . " FROM " . DB_ORDER_CASH_RECEIPT . " WHERE orderNo = '" . $orderNo . "'");

                if(gd_count($orderCashReceiptData) > 0){
        ...
        if($orderStatisticsAction === true){
            ...
            if(gd_count($orderStatisticsCommonColumns) > 0){
                $orderStatisticsData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderStatisticsCommonColumns) . " FROM " . DB_ORDER_SALES_STATISTICS . " WHERE orderNo = '" . $orderNo . "'");

                if(gd_count($orderStatisticsData) > 0){
                    $fieldTypes = [
        ...
        if(gd_in_array(false, $resultArray)){
            return false;
        }
    ...
    public function restoreOrderOriginalData($orderNo, $claimStatus, $giftAction=false, $receiptAction=false, $orderStatisticsAction=false)
    {
        ...
        $orderCommonColumns = $this->getCommonField(DB_ORDER, DB_ORDER_ORIGINAL);
        if(gd_count($orderCommonColumns) > 0){
            $orderOriginalData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderCommonColumns) . " FROM " . DB_ORDER_ORIGINAL . $where);

            $fieldsToClean = ['orderGoodsNm', 'orderGoodsNmStandard'];
            if (gd_count($orderOriginalData) > 0) {
                foreach ($orderOriginalData as &$value) {
                    foreach ($fieldsToClean as $field) {
                        if (isset($value[$field]) && is_string($value[$field])) {
                            $value[$field] = str_replace("\\'", "'", $value[$field]);
                        }
        ...
        // 주문상품복원
        $restoreOrderGoodsSno = [];
        $orderGoodsCommonColumns = $this->getCommonField(DB_ORDER_GOODS, DB_ORDER_GOODS_ORIGINAL);
        if(gd_count($orderGoodsCommonColumns) > 0){
            $orderGoodsOriginalData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderGoodsCommonColumns) . " FROM " . DB_ORDER_GOODS_ORIGINAL . $where);

            $fieldsToClean = ['goodsNm', 'goodsNmStandard'];
            if (gd_count($orderGoodsOriginalData) > 0) {
                foreach($orderGoodsOriginalData as &$value){
                    ...
                    try {
                        ...
                        $restoreOrderGoodsSno[] = $this->db->insert_id();
                        $resultArray[] = true;
                    } catch (Exception $e) {
                        $resultArray[] = false;
                    }
        ...
        $orderDeliveryCommonColumns = $this->getCommonField(DB_ORDER_DELIVERY, DB_ORDER_DELIVERY_ORIGINAL);
        if(gd_count($orderDeliveryCommonColumns) > 0){
            $orderDeliveryOriginalData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderDeliveryCommonColumns) . " FROM " . DB_ORDER_DELIVERY_ORIGINAL . $where);

            if(gd_count($orderDeliveryOriginalData) > 0){
        ...
        if($giftAction === true){
            $orderGiftCommonColumns = $this->getCommonField(DB_ORDER_GIFT, DB_ORDER_GIFT_ORIGINAL);
            if(gd_count($orderGiftCommonColumns) > 0){
                $orderGiftOriginalData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderGiftCommonColumns) . " FROM " . DB_ORDER_GIFT_ORIGINAL . $where);

                if(gd_count($orderGiftOriginalData) > 0){
        ...
        if($receiptAction === true){
            $orderCashReceiptCommonColumns = $this->getCommonField(DB_ORDER_CASH_RECEIPT, DB_ORDER_CASH_RECEIPT_ORIGINAL);
            if(gd_count($orderCashReceiptCommonColumns) > 0){
                $orderCashReceiptOriginalData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderCashReceiptCommonColumns) . " FROM " . DB_ORDER_CASH_RECEIPT_ORIGINAL . $where);

                if(gd_count($orderCashReceiptOriginalData) > 0){
        ...
        if($orderStatisticsAction === true){
            $orderStatisticsCommonColumns = $this->getCommonField(DB_ORDER_SALES_STATISTICS, DB_ORDER_SALES_STATISTICS_ORIGINAL);
            if(gd_count($orderStatisticsCommonColumns) > 0){
                $orderStatisticsOriginalData = $this->db->query_fetch("SELECT " . gd_implode(", ", $orderStatisticsCommonColumns) . " FROM " . DB_ORDER_SALES_STATISTICS_ORIGINAL . $where);

                if(gd_count($orderStatisticsOriginalData) > 0){
                    $fieldTypes = [
                        ['val' => 'orderYMD', 'typ' => 'i', 'def' => null],
                        ['val' => 'mallSno', 'typ' => 'i', 'def' => 1],
                        ['val' => 'kind', 'typ' => 's', 'def' => 'order'],
        ...
        // 주문 상품 상태 변경 WebHook 발송
        if (is_array($restoreOrderGoodsSno)) {
            $this->orderWebHookService->sendOrderGoodsWebHook($restoreOrderGoodsSno, 'restore');
        }
    ...
    public function setOrderProcessLog($data)
    {
        ...
        if (!gd_in_array($data['type'], $payHistoryType)) {
            return false;
        }
    ...
    {
        if(gd_count($postData['enuri']) > 0) {
            //주문건 업데이트 여부 flag 값
            $orderUpdateCheck = false;

            //기존 주문건 정보
            $orderData = $this->getOrderData($postData['orderNo']);
            if(gd_count($orderData) < 1){
            ...
            if ($orderUpdateCheck === true) {
                //TODO:php82 안쓰는 로직이 아닐까,,
                $beforeOrderGoodsData = $beforeOrderGoodsData ?? [];
                $afterOrderGoodsData = $afterOrderGoodsData ?? [];
                $beforeTaxSupplyGoodsPrice = gd_array_sum(gd_array_column($beforeOrderGoodsData ?? [], 'taxSupplyGoodsPrice'));
                $afterTaxSupplyGoodsPrice = gd_array_sum(gd_array_column($afterOrderGoodsData, 'taxSupplyGoodsPrice'));
                $taxSupplyPrice = $orderData['taxSupplyPrice'] - $beforeTaxSupplyGoodsPrice + $afterTaxSupplyGoodsPrice;

                $beforeTaxVatGoodsPrice = gd_array_sum(gd_array_column($beforeOrderGoodsData, 'taxVatGoodsPrice'));
                $afterTaxVatGoodsPrice = gd_array_sum(gd_array_column($afterOrderGoodsData, 'taxVatGoodsPrice'));
                $taxVatPrice = $orderData['taxVatPrice'] - $beforeTaxVatGoodsPrice + $afterTaxVatGoodsPrice;

                $beforeTaxFreeGoodsPrice = gd_array_sum(gd_array_column($beforeOrderGoodsData, 'taxFreeGoodsPrice'));
                $afterTaxFreeGoodsPrice = gd_array_sum(gd_array_column($afterOrderGoodsData, 'taxFreeGoodsPrice'));
                $taxFreePrice = $orderData['taxFreePrice'] - $beforeTaxFreeGoodsPrice + $afterTaxFreeGoodsPrice;

                $beforeEnuri = gd_array_sum(gd_array_column($beforeOrderGoodsData, 'enuri'));
                $afterEnuri = gd_array_sum(gd_array_column($afterOrderGoodsData, 'enuri'));
                $totalEnuriDcPrice = $orderData['totalEnuriDcPrice'] - $beforeEnuri + $afterEnuri;

                $beforeSettlePrice = gd_array_sum(gd_array_column($beforeOrderGoodsData, 'goodsSettlePrice'));
                $afterSettlePrice = gd_array_sum(gd_array_column($afterOrderGoodsData, 'goodsSettlePrice'));
    ...
    public function setPacket($postData)
    {
        ...
        $orderNoArr = gd_array_values(gd_array_unique(gd_array_filter(explode(INT_DIVISION, $postData['orderNoStr']))));
        ...
        $this->db->set_update_db_query(DB_ORDER_INFO, 'packetCode = ?', "orderNo IN ('".gd_implode("','", $orderNoArr)."')", $arrBind);

        $logger = \App::getInstance('logger');
        $logger->channel('order')->info('묶음배송 처리 ; 처리자 - ' . \Session::get('manager.managerId') . ' ; [' . gd_implode(",", $orderNoArr) . ']');
    ...
    public function unsetPacket($postData)
    {
        ...
        $strSQL = 'SELECT packetCode FROM ' . DB_ORDER_INFO . ' WHERE orderNo IN (' . gd_implode(',',$arrBind['param']) . ')';
        ...
        $arrBind = null;
        unset($arrBind);
        $packetCodeArr = gd_array_column($getData, 'packetCode');
        $packetCodeArr = gd_array_values(gd_array_unique(gd_array_filter($packetCodeArr)));

        //공급사일 경우 다른 공급사의 주문상품이 있는지 확인 후 exception 처리
        if(Manager::isProvider()){
            $scmNo = \Session::get('manager.scmNo');
            $checkUnsetAble = true;
            if(gd_count($packetCodeArr) > 0){
                foreach($packetCodeArr as $key => $packetCode){
                    $tmpPacketOrderNoArr[] = $this->getPacketOrderList($packetCode);
                }
                $packetOrderNoArr = gd_array_unique(array_reduce($tmpPacketOrderNoArr, function ($a, $p) { return gd_array_merge((array)$a, (array)$p); }, array()));

                //주문건의 주문상품을 확인
                if(gd_count($packetOrderNoArr) > 0) {
                    foreach ($packetOrderNoArr as $pKey => $packetOrderNo) {
                        $scmNoArr = $orderGoodsData = [];
                        $orderGoodsData = $this->getOrderGoodsData($packetOrderNo);
                        $scmNoArr = gd_array_unique(gd_array_filter(gd_array_column($orderGoodsData, 'scmNo')));
                        if (gd_count($scmNoArr) > 0) {
        ...
        if(gd_count($packetCodeArr) > 0){
            ...
            $this->db->set_update_db_query(DB_ORDER_INFO, 'packetCode = ?', "packetCode IN ('".gd_implode("','", $packetCodeArr)."')", $arrBind);
            unset($arrBind);
        }
        else {
        ...
        $logger = \App::getInstance('logger');
        $logger->channel('order')->info('묶음배송 해제 ; 처리자 - ' . \Session::get('manager.managerId') . ' ; [' . gd_implode("','", $packetCodeArr) . ']');
    ...
    public function getPacketOrderList($packetCode)
    {
        ...
        $orderList = gd_array_values(gd_array_column($data, 'orderNo'));
    ...
    public function getOrderNameChange($orderNo, $standardType = false)
    {
        ...
        $countOrderGoods = gd_count($getOrderGoodsData);
    ...
    public function changeProductCouponOrderCd($orderNo, $beforeOrderCd, $afterOrderCd)
    {
        ...
        if(gd_count($getData) > 0){
            foreach($getData as $key => $val){
                ...
                $this->db->set_update_db(DB_ORDER_COUPON, $arrBind['param'], gd_implode(" AND ", $arrWhere), $arrBind['bind']);
            }
        }
    ...
    public function restoreProductCoupon($orderNo)
    {
        ...
        if(gd_count($getData) > 0) {
            foreach ($getData as $key => $val) {
                ...
                $query = $this->db->query_complete();
                $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_ORDER_COUPON_ORIGINAL . gd_implode(' ', $query);
                $originalCouponData = $this->db->query_fetch($strSQL, $arrBind)[0];

                //교환전 orderCd 및 적립금액 교체
                if(gd_count($originalCouponData) > 0){
                    $arrBind = [];
                    $arrWhere = [];
                    $updateData = [
    ...
    */
    ...
    public function deleteOrderHandleData($orderNo, $handleMode)
    {
        ...
        if($handleMode === 'e'){
            ...
            $result = $this->db->set_delete_db(DB_ORDER_HANDLE, gd_implode(" AND ", $where), $arrBind);
            ...
            $where[] = 'ehOrderNo = ?';
            $this->db->bind_param_push($arrBind, 's', $orderNo);
            $this->db->set_delete_db(DB_ORDER_EXCHANGE_HANDLE, gd_implode(" AND ", $where), $arrBind);
        }
        else {
            $where[] = 'handleMode = ?';
            $this->db->bind_param_push($arrBind, 's', $handleMode);
    ...
    public function getOrderDepositHandleData($handleData)
    {
        ...
        if(gd_count($goodsNm)> 1){
            $data['goodsNm'] = $goodsNm[0].'외 '.(gd_count($goodsNm)-1).'건';
        }else{
    ...
    public function getOrderCashReceiptData($orderNo)
    {
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_ORDER_CASH_RECEIPT . ' as ocr ' . gd_implode(' ', $query);
        $getData = $this->db->query_fetch($strSQL, $arrBind, false);
        unset($arrBind);
    ...
    public function getCommonField($tableName1, $tableName2)
    {
        ...
        $table1Columns = gd_array_column($table1ColumnsData, 'Field');
        $table2Columns = gd_array_column($table2ColumnsData, 'Field');
        $commonField = gd_array_intersect($table1Columns, $table2Columns);
    ...
    public function getAddExchangeOrderGoodsEtcData($updateOrderGoodsData, $changeStatus)
    {
        ...
        $updateOrderGoodsData['scmAdjustNo'] = ''; // 공급사 정산요청한 상품을 동일한 상품으로 교환처리시 정산데이터값이 입력되지 않도록 조치
        if($changeStatus === 'p1'){
    ...
    public function getChangeOptionData($changeOptionSno, $goodsNo, $goodsObj)
    {
        ...
        $tmpOptionNameCnt = gd_count($tmpOptionName);
        $tmpOptionData = $goodsObj->getGoodsOptionInfo($changeOptionSno);

        if(gd_count($tmpOptionName) > 0){
            $optIdx = 1;
        ...
        if(gd_count($changeOptionData) > 0){
     ...
     *
     * @param string $orderNo 주문 번호
     * @param array $handleSno 환불 처리 코드
     *
     * @return array 이미 환불 완료된 환불 처리 코드
     */
    private function confirmRefundCompletion(string $orderNo, array $handleSno): array
    {
        $arrBind = $arrWhere = [];
        ...
        // 환불 고유 번호
        $tmpAddWhere = [];
        foreach ($handleSno as $sno) {
            $tmpAddWhere[] = '?';
            $this->db->bind_param_push($arrBind, 'i', $sno);
        }
        $arrWhere[] = 'oh.sno IN (' . gd_implode(' , ', $tmpAddWhere) . ')';
        unset($tmpAddWhere);

        // 처리 모드
        $arrWhere[] = 'oh.handleMode = ?';
        $this->db->bind_param_push($arrBind, 's', 'r');

        // 환불 처리 완료 여부
        $arrWhere[] = 'oh.handleCompleteFl = ?';
        $this->db->bind_param_push($arrBind, 's', 'y');

        $this->db->strField = 'oh.sno';
        $this->db->strWhere = gd_implode(' AND ', $arrWhere);
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM ' . DB_ORDER_HANDLE . ' AS oh ' . gd_implode(' ', $query);
        $getData = $this->db->query_fetch($strSQL, $arrBind);
        ...
        return array_column($getData, 'sno');
    }
}
