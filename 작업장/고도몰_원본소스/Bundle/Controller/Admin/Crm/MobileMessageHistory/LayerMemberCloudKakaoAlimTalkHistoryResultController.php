...
use Origin\DTO\ApiClient\Commerce\Notification\MemberCloudKakaoAlimTalkHistoryDTO;
use Origin\DTO\ApiClient\Commerce\Notification\MemberCloudKakaoAlimTalkHistoryListRequestDTO;
use Origin\DTO\ApiClient\Commerce\Notification\MemberCloudKakaoAlimTalkHistoryListResponseDTO;
use Origin\DTO\ApiClient\Commerce\Notification\MemberCloudKakaoAlimTalkHistorySearchFormDTO;
...
class LayerMemberCloudKakaoAlimTalkHistoryResultController extends \Controller\Admin\Controller
{
    public function index()
    {
        try {
            ...
            $searchFormDTO = new MemberCloudKakaoAlimTalkHistorySearchFormDTO($postData);
            $searchFromArray = $searchFormDTO->toRecursiveArray();
            $preparedData = $this->prepareRequestDataForDto($searchFromArray);
            $requestDTO = new MemberCloudKakaoAlimTalkHistoryListRequestDTO($preparedData);
            ...
            try {
                ...
                $cloudKakaoAlimTalkHistoryListResponseDTO = $kakaoAlimTalkHistoryService->getMemberCloudKakaoAlimTalkHistoryList($requestDTO);
            } catch (\Throwable $e) {
                \Logger::channel('mobileMessage')->error($e->getMessage(), ['requestDTO' => $requestDTO->toArray()]);
                $cloudKakaoAlimTalkHistoryListResponseDTO = new MemberCloudKakaoAlimTalkHistoryListResponseDTO(['totalCount' => 0, 'contents' => []], $requestDTO->getPage(), $requestDTO->getPageSize());
            }
            ...
            $this->setData('cloudKakaoAlimTalkHistoryList', $this->sanitizeHistoryList($cloudKakaoAlimTalkHistoryListResponseDTO->getMemberCloudKakaoAlimTalkHistoryList()));
     ...
     *
     * @param MemberCloudKakaoAlimTalkHistoryDTO[] $historyList
     * @return MemberCloudKakaoAlimTalkHistoryDTO[]
     */
