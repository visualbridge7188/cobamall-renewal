 ...
 */
class OrderListBackController extends \Controller\Admin\Controller
{
    /**
     * @var 기본 주문상태
     */
    private $_currentStatusCode = 'b';
    ...
    public function index()
    {
        try {
            // --- 메뉴 설정
            $this->callMenu('order', 'cancel', 'back');
            $this->addScript(
                [
            ...
            $searchConf->setGetData();$isOrderSearchMultiGrid = gd_isset(\Session::get('manager.isOrderSearchMultiGrid'), 'n');
            ...
            foreach ($order->getOrderStatusAdmin() as $key => $val) {
                if (gd_in_array(substr($key, 0, 1), $order->statusStandardCode[$this->_currentStatusCode]) === true) {
                    // 환불접수 처리
            ...
            // 공급사와 동일한 페이지 사용
            $this->getView()->setPageName('order/order_list_back.php');
