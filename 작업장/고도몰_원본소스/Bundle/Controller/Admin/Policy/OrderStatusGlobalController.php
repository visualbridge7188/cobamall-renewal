 ...
 */
class OrderStatusGlobalController extends \Controller\Admin\Controller
{
    /**
     * @inheritdoc
     */
    public function index()
    {
        try {
            ...
            $cancelStep[Order::ORDER_STATUS_EXCHANGE_ADD] = [
                'code'    => 'z',
                'add'     => 'n',
                'title'   => __('교환 추가'),
                'mrestore'    => 'n',
                ...
                'sub'     =>
                    [
                        'z1' => ['title'         => __('추가입금대기'),
                            'indispensable' => 'y',
                            'mode'          => 'oi',
                            'desc'          => __('교환추가된 상품의 접수 단계입니다.'),
                        ],
                        'z2' => ['title'         => __('추가결제완료'),
                            'indispensable' => 'n',
                            'mode'          => 'oi',
                            'desc'          => __('교환추가된 상품이 결제완료된 상태입니다.'),
                        ],
                        'z3' => ['title'         => __('추가배송중'),
                            'indispensable' => 'n',
                            'mode'          => 'oi',
                            'desc'          => __('교환추가된 상품이 배송중인 상태입니다.'),
                        ],
                        'z4' => ['title'         => __('추가배송완료'),
                            'indispensable' => 'n',
                            'mode'          => 'oi',
                            'desc'          => __('교환추가된 상품이 배송완료된 상태입니다.'),
                        ],
                        'z5' => ['title'         => __('교환추가완료'),
                            'indispensable' => 'y',
                            'mode'          => 'oi',
                            'desc'          => __('교환추가가 완료된 상태입니다.'),
                        ],
                    ],
            ];
            ...
            $orderStepCnt = $cancelStepCnt = 0;
            foreach ($orderStep as $key => $value) {
                foreach ($policy[$key] as $val) {
                    if (is_array($val) === true) $orderStepCnt++;
                }
            }
            foreach ($cancelStep as $key => $value) {
                foreach ($policy[$key] as $val) {
                    if (is_array($val) === true) $cancelStepCnt++;
                }
            }

            $mall = new Mall();
            $mallList = $mall->getListByUseMall();
            foreach (gd_array_keys($mallList) as $mallSno) {
                if ($mallSno == DEFAULT_MALL_NUMBER) continue;

                $policyGlobal[$mallSno] = gd_policy('order.status', $mallSno);
            }

            // --- 관리자 디자인 템플릿
            $this->setData('policy', $policy);
            $this->setData('policyGlobal', $policyGlobal);
            ...
            $this->setData('orderStepCnt', $orderStepCnt);
            $this->setData('cancelStepCnt', $cancelStepCnt);
            $this->setData('useFl', ['y' => '사용', 'n' => '-']);
            $this->setData('defaultMallNumber', DEFAULT_MALL_NUMBER);
