...
/**
 * Class 통계-상품분석-카테고리분석
 * @package Bundle\Controller\Admin\Statistics
 * @author  su
 */
class GoodsCategoryController extends \Controller\Admin\Controller
{
    public function index()
    {
        /** @var \Bundle\Controller\Admin\Controller $this */
        try {
            $this->callMenu('statistics', 'goods', 'category');
            ...
            $getDataArr = $goodsStatistics->getCategoryStatistics($searchData);
            ...
            foreach ($getDataArr as $key => $val) {
                if ($val['cateCd']) {
                    $returnGoodsStatistics[$i]['cateCd'] = $val['cateCd'];
                    $returnGoodsStatistics[$i]['cateNm'] = $val['cateNm'];
