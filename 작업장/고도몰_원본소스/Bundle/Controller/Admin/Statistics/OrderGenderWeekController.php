 ...
 * @author    su
 */
class OrderGenderWeekController extends \Controller\Admin\Controller
{
    public function index()
    {
        try {
            ...
            $getDataArr = $orderSalesStatistics->getOrderGenderWeek($order);
            ...
            foreach ($getDataArr as $key => $val) {
                ...
                foreach ($val as $deviceKey => $deviceVal) {
                    ...
                    foreach ($genderArr as $genderKey => $genderVal) {
                        ...
                        $ageTotal['orderCnt' . ucfirst($genderKey)][$deviceKey] = gd_count(gd_array_unique($deviceVal[$genderKey]['orderNo']));
                        $ageTotal['memberCnt' . ucfirst($genderKey)][$deviceKey] = gd_count(gd_array_unique($deviceVal[$genderKey]['memNo']));
                    }
                ...
                foreach ($genderArr as $genderKey => $genderVal) {
                    $returnOrderStatistics[$i]['goodsPrice' . ucfirst($genderKey)] = NumberUtils::moneyFormat(gd_array_sum($ageTotal['goodsPrice' . ucfirst($genderKey)]));
                    $returnOrderStatistics[$i]['goodsCnt' . ucfirst($genderKey)] = number_format(gd_array_sum($ageTotal['goodsCnt' . ucfirst($genderKey)]));
                    $returnOrderStatistics[$i]['orderCnt' . ucfirst($genderKey)] = number_format(gd_array_sum($ageTotal['orderCnt' . ucfirst($genderKey)]));
                    $returnOrderStatistics[$i]['memberCnt' . ucfirst($genderKey)] = number_format(gd_array_sum($ageTotal['memberCnt' . ucfirst($genderKey)]));
                }
                $i++;
            }
