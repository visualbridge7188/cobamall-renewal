 ...
 * @author  su
 */
class SalesWeekController extends \Controller\Admin\Controller
{
    public function index()
    {
        try {
            ...
            $getDataArr = $orderSalesStatistics->getOrderSalesWeek($order);

            // 요일별 매출 통계 데이터 테이블 생성
            ...
            foreach ($daySales as $key => $val) {
                ...
                if ($daySalesTotal['min']['price'] >= $salesTotal) {
                    ...
                    $daySalesTotal['min']['date'] = $weekKorArr[$key];
                }
                if ($daySalesTotal['max']['price'] <= $salesTotal) {
                    $daySalesTotal['max']['price'] = $salesTotal;
                    $daySalesTotal['max']['date'] = $weekKorArr[$key];
                }
