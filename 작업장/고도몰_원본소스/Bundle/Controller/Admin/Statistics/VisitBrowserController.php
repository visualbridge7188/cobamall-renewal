 ...
 *
 * @copyright Copyright (c) 2015 GodBrowseroft.
 ...
 */
class VisitBrowserController extends \Controller\Admin\Controller
{
    /**
     * index
     *
    ...
    public function index()
    {
        ...
        try {
            ...
            $getDataArr = $visitStatistics->getVisitBrowserDay($searchDate, $mallSno);
            ksort($getDataArr);
            ...
            foreach ($getDataArr as $key => $val) {
                foreach ($val['visitBrowser'] as $inflowKey => $inflowVal) {
                    if ($inflowKey != '기타') {
                ...
                foreach ($val['pc']['visitBrowser'] as $inflowPcKey => $inflowPcVal) {
                    if ($inflowPcKey != '기타') {
                ...
                foreach ($val['mobile']['visitBrowser'] as $inflowMobileKey => $inflowMobileVal) {
        ...
        $this->setData('visitChart', $visitChart);
        $this->setData('visitBrowserTitle', $visitBrowserTitle);
