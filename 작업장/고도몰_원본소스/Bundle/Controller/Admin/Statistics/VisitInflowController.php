 ...
 */
class VisitInflowController extends \Controller\Admin\Controller
{
    /**
     * index
     *
    ...
    public function index()
    {
        ...
        // @todo 직접입력, 즐겨찾기 등 검색엔진이 아닌 경우의 처리 필요 direct link
        try {
            ...
            $getDataArr = $visitStatistics->getVisitInflowDay($searchDate, $mallSno);
            ksort($getDataArr);
            ...
            foreach ($getDataArr as $key => $val) {
                ...
                foreach ($val['pc']['visitInflow'] as $inflowPcKey => $inflowPcVal) {
                    if ($inflowPcKey != '기타') {
                ...
                foreach ($val['mobile']['visitInflow'] as $inflowMobileKey => $inflowMobileVal) {
            ...
            if ($totalVisit['search'] > 0 && $totalVisit['searchAll'] > 0) {
                $totalVisit['percent'] = round($totalVisit['search'] / $totalVisit['searchAll'] * 100, 2);
            } else {
                $totalVisit['percent'] = 0;
            }

            if ($totalVisit['pc']['search'] > 0 && $totalVisit['pc']['searchAll'] > 0) {
                $totalVisit['pc']['percent'] = round($totalVisit['pc']['search'] / $totalVisit['pc']['searchAll'] * 100, 2);
            } else {
                $totalVisit['pc']['percent'] = 0;
            }

            if ($totalVisit['mobile']['search'] > 0 && $totalVisit['mobile']['searchAll'] > 0) {
                $totalVisit['mobile']['percent'] = round($totalVisit['mobile']['search'] / $totalVisit['mobile']['searchAll'] * 100, 2);
            } else {
        ...
        $this->setData('visitChart', $visitChart);
        $this->setData('visitInflowTitle', $visitInflowTitle);
