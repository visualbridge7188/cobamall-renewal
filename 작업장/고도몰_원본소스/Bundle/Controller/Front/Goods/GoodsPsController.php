        ...
        switch ($post['mode']) {
            ...
            case 'get_benefit':
                try {
                    if (!empty($post['goodsNo'])) {
                        if ($post['deliveryType'] === 'regular') {
                            $regularOrderCart = \App::getInstance(\Component\Cart\RegularOrderCart::class);
                            $setData = $regularOrderCart->regularGoodsViewBenefit($post);
                        } else {
            ...
            case 'save_restock' :
                try {
                    ...
                    //옵션이 있을시
                    if(gd_count($post['restock_option']) > 0){
                        //옵션정보가 변경되지 않았는지 체크
                        ...
                        if(gd_count($duplicationFl) > 0){
                            if(gd_count($duplicationFl) === gd_count($post['restock_option'])){
