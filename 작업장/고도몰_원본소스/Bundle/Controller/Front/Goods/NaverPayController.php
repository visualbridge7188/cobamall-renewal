...
class NaverPayController extends \Controller\Front\Controller
{
    ...
    public function index()
    {
        try {
            ...
            // 주문 테이블 checkoutData 컬럼에 cartSno 정보 저장 (결제 완료 후 Cart 제거용)
            if (empty($arrCartSno) === false) {
                $checkoutData['cartSno'] = $arrCartSno;
                \Logger::channel('naverPay')->info(
                    __METHOD__ . '[' . __LINE__ . '], ' . ' Save shopping cart number : ',
                    [
                        $orderNo,
                        [$checkoutData['cartSno']]
                    ]
                );
            }
            ...
            foreach ($orderGoodsList as $orderGoodsData) {
                ...
                $arr_data[] = '<name>' . $this->cdata(urldecode(str_replace(self::NOT_STRING, '', urlencode($orderGoodsData['goodsNm'])))) . '</name>';
            ...
            $arr_data[] = '</order>';
            $result = gd_implode(chr(10), $arr_data);
