 ...
 */
class CancelListController extends \Controller\Front\Controller
{
    /**
     * @inheritdoc
     *
    ...
    public function index()
    {
        try {
            ...
            // 날짜 형식 체크 추가 (보안)
            if (!Validator::date($wDate[0])) {
                $wDate[0] = $startDate;
            }

            if (!Validator::date($wDate[1])) {
                $wDate[1] = date('Y-m-d');
            }
            ...
            if ($orderBasic['userHandleFl'] === 'y' && (empty($mode) === true || $mode == 'cancelRequest')) {
                $mode = 'cancelRequest';
