...
class ShippingController extends \Controller\Front\Controller
{
    ...
    public function index()
    {
        ...
        if ($isUseRegularDelivery) {
            ...
            $regularShippingList = $regularOrderShippingAddress->getRegularOrderShippingAddress($session, (int) $pageNo);


            // 데이터 뷰에 전달
            $this->setData('regularShippingList', $regularShippingList['list']);
            $this->setData('totalShippingAddressList', $regularShippingList['pager']->recode['total'] ?? 0);

                    // 정기배송지 페이징 처리
            $this->setData('regularPage', $regularShippingList['pager']);
        }
