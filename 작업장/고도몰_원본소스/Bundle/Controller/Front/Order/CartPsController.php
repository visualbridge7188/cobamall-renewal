        ...
        switch (Request::request()->get('mode')) {
            ...
            case 'cartIn':
                try {
                    ...
                    if (empty($postValue['couponApplyNo']) === false && gd_count($postValue['couponApplyNo']) > 0) {
                    ...
                    if ($request->isAjax()) {
                        ...
                        switch ($cartMode) {
                            ...
                            case 'present':
                                ...
                                if (count($addedGoods) > 1) {
                                    throw new Exception('선물하기는 한 번에 하나의 상품만 가능합니다.');
                                }
                                
                                // 선물하기 버튼으로 넘어온 상품에 대한 장바구니 처리(바로구매와 동일)
                                $presentCart = \App::getInstance(PresentCart::class);
                                $returnUrl = $presentCart->updateAndBuildUrl($addedGoods[0]);
            ...
            case 'cartInMulti':
                ...
                foreach($postValue['chkRelatedGoods'] as $value){
                    ...
                    if($postValue['mainGoods'] == $value){
                        ...
                        //배송 방법
                        if(gd_in_array($postValue['deliveryMethodFl'], $relateDeliveryConfig['deliveryMethodFlEtc'])){
                            $cartValue['deliveryMethodFl'] = $postValue['deliveryMethodFl'];
                        }else{
            ...
            case 'cartToWish':
                try {
                    ...
                    $message = gd_count($postValue['cartSno']) . __('개의 상품이 찜리스트에 저장되었습니다.');
            ...
            case 'check_area_delivery':
                try {
                    ...
                    $this->json([
                        'areaDelivery' => gd_array_sum($orderPrice['totalGoodsDeliveryAreaCharge']),
            ...
            case 'multi_shipping_delivery':
                try {
                    ...
                    $this->json([
                        'deliveryCharge' => gd_array_sum($cart->totalGoodsDeliveryPolicyCharge),
                        'deliveryAreaPrice' => gd_array_sum($cart->totalGoodsDeliveryAreaPrice),
            ...
            case 'check_multi_area_delivery':
                ...
                foreach ($data['selectGoods'] as $key => $val) {
                    if (empty($val) === false) {
                        ...
                        foreach ($selectGoods as $tKey => $tVal) {
                            ...
                            if (empty($tVal['addGoodsNo']) === false) {
                                foreach ($tVal['addGoodsNo'] as $aKey => $aVal) {
                                    // 자연수가 아닐 경우 예외 발생
                                    NumberUtils::checkNaturalNumber($tVal['addGoodsCnt'][$aKey]);
                        ...
                        $policyCharge += gd_array_sum($cart->totalGoodsDeliveryPolicyCharge);
                        $areaDelivery += gd_array_sum($cart->totalGoodsDeliveryAreaPrice);
                ...
                try {
                    if($postValue['cartIdx']) {
                        ...
                        foreach ($postValue['cartIdx'] as $delKey => $delVal) {
                            if (gd_array_key_exists($delVal, $postValue['cart']) == false) {
            ...
            case 'checkCouponType':
                ...
                // 모듈 호출
                $coupon = \App::load('\\Component\\Coupon\\Coupon');
                $return = $coupon->checkCouponTypeArr($getValue['couponNo']);
                    ...
                    foreach ($memberCouponNoArr as $memberCouponNo) {
                        if (gd_isset($memberCouponNo)) {
                            ...
                            if($coupon->checkCouponType($memberCouponData['couponNo'], 'y', $memberCouponNo)) {
                                ...
                                $setCouponApplyOrderNo = gd_implode(INT_DIVISION, $setOrderCouponArr);
                            } else {
                                $reSetMemberCouponApply = true;
                                $resetOrderCouponArr[] = $memberCouponNo;
                                $resetCouponApplyOrderNo = gd_implode(INT_DIVISION, $resetOrderCouponArr);
                            }
                        }
                    }
                }
                                ...
                                foreach ($value1 as $key2 => $value2) {
                                    if ($value2['memberCouponNo']) {
                                        ...
                                        foreach ($memberCouponNoArr as $memberCouponNo) {
                                            $memberCouponData = $coupon->getMemberCouponInfo($memberCouponNo, 'c.couponNo, mc.memberCouponNo, c.couponKindType');
                                            if($coupon->checkCouponType($memberCouponData['couponNo'], 'y', $memberCouponNo)) {
                                                if($memberCouponData['couponKindType'] == 'add' && $mileageGive['giveFl'] == 'n') {
                                                    $reSetMemberCouponApply = true;
