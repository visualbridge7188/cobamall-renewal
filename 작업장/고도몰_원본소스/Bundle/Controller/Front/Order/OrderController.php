...
class OrderController extends \Controller\Front\Controller
{
    ...
    public function index()
    {
        try {
            ...
            // 장바구니 리스트에서 쿠폰이 적용되어 있을 경우 중복으로 적용되어 있는지 체크
            ...
            $this->setData('isPayLimit', gd_count($cart->payLimit)); // 결제가능PG
            ...
            if (gd_is_plus_shop(PLUSSHOP_CODE_GIFT) === true && $giftConf['giftFl'] == 'y') {
                ...
                $giftInfo = $gift->getGiftPresentOrder($cart->giftForData);
                if ($giftInfo) {
                    ...
                    $presentInfoNo = gd_implode('|', $presentInfoNoArray);
                }
                $this->setData('giftInfo', $giftInfo);
                $this->setData('presentInfoNo', $presentInfoNo);
            }
            ...
            if (!empty($cart->payLimit) && ((gd_in_array('gb', $cart->payLimit) && !gd_in_array('pg', $cart->payLimit)) || (!gd_in_array('gb', $cart->payLimit) && !gd_in_array('pg', $cart->payLimit)))) {
                $onlyBankFl = 'y';
            } else {
                $onlyBankFl = 'n';
            }
            ...
            if (gd_isset($cart->payLimit) && gd_in_array('gm', $cart->payLimit) == false) {
            ...
            if (gd_isset($cart->payLimit) && gd_in_array('gd', $cart->payLimit) == false) {
            ...
            $pgConf = gd_pgs();
            if ((empty($pgConf['pgId']) === false && $pgConf['cashReceiptFl'] == 'y') && (empty($cart->payLimit) === true || (empty($cart->payLimit) === false && gd_in_array('gb', $cart->payLimit)))) {
                $receipt['cashFl'] = 'y';
            ...
            $paycoEasypayMobilebuttonImage = $payco->getButtonHtmlCode('EASYPAY', true, '', $arrGoodsNo);
            ...
            if (($paycoEasypaybuttonImage !== false || $paycoEasypayMobilebuttonImage !== false) && $onlyBankFl == 'n') {
                $this->setData('payco', $paycoEasypaybuttonImage);
                $this->setData('paycoPc', $paycoEasypaybuttonImage);
                $this->setData('paycoMobile', $paycoEasypayMobilebuttonImage);
            }
            $this->setData('paycoDefaultUseFl', gd_isset($payco->paycoCfg['defaultUseFl'], 'y'));

            // 결제가 가능한 결제 수단 표기
            $settle = $order->useSettleKind($settleSelect, $pgConf, $cart->payLimit);

            // 페이코 일반 결제에 텍스트로 결제 수단 노출 여부
            if ($payco->isPaycoViewSettleGeneral($settle) === false) {
                // 일반 결제 off (=페이코 간편 결제 단독 영역 노출)
                unset($settle['general']['pp']);
            }
                ...
                foreach ($cartInfo as $cIKey1 => $cIval1){
                    foreach($cIval1 as $cIKey2 => $cIval2){
                        foreach($cIval2 as $cIKey3){
                            if(gd_in_array('goods', $kakaopayConfig['presentExceptFl']) === true && gd_in_array($cIKey3['goodsNo'], $kakaopayConfig['exceptGoods']) === true){
                                unset($settle['general']['pk']);
                            }
                            if(gd_in_array('brand', $kakaopayConfig['presentExceptFl']) === true && gd_in_array($cIKey3['brandCd'], $kakaopayConfig['exceptBrand']) === true){
                                unset($settle['general']['pk']);
                            }
                            foreach($cIKey3['cateAllCd'] as $cateKey1 => $cateVal1){
                                if(gd_in_array('category', $kakaopayConfig['presentExceptFl']) === true && gd_in_array($cateVal1['cateCd'], $kakaopayConfig['exceptCategory']) === true){
                                    unset($settle['general']['pk']);
                                }
                            }
                        }
            ...
            // 네이버페이설정에 예외상품에 등록된 상품이라면 미노출
            if ($settle['general']['pn']['useFl'] == 'y') {
                $naverPayConfig = gd_policy('naverEasyPay.config');
                foreach ($cartInfo as $cIval1) {
                    foreach ($cIval1 as $cIval2) {
                        foreach ($cIval2 as $cIKey3) {
                            if (gd_in_array('goods', $naverPayConfig['presentExceptFl']) && gd_in_array($cIKey3['goodsNo'], $naverPayConfig['exceptGoods'])) {
                                unset($settle['general']['pn']);
                            }
                            if (gd_in_array('brand', $naverPayConfig['presentExceptFl']) && gd_in_array($cIKey3['brandCd'], $naverPayConfig['exceptBrand'])) {
                                unset($settle['general']['pn']);
                            }
                            foreach ($cIKey3['cateAllCd'] as $cateVal1) {
                                if (gd_in_array('category', $naverPayConfig['presentExceptFl']) && gd_in_array($cateVal1['cateCd'], $naverPayConfig['exceptCategory'])) {
                                    unset($settle['general']['pn']);
                                }
                            }
                        }
            ...
            // 공급사 상품이 포함되었는지 여부
            $scmNoList = gd_array_keys($cartInfo);
            $hasScmGoods = false;
            $tmpScmList = [];
            ...
            if ($hasScmGoods) {
                $scmNmList = [];
                $ScmData = $scmAdmin->getScmSelectList(gd_implode(INT_DIVISION,$tmpScmList));
                foreach($ScmData as $val) {
                    $scmNmList[$val['scmNo']] = $val['companyNm'];
                }
                ksort($scmNmList);
                $scmNm['scmNm'] = gd_implode(', ',$scmNmList);
            ...
            if(empty($fbConfig)===false && $fbConfig['fbUseFl'] == 'y' ) {
                ...
                $this->setData('fbInitiateCheckoutScript', $fbScript['script']);
                ...
                $fbExtensionV2->fbPixelDefault("InitiateCheckout", $requestUrl, $goodsNo, $cart->totalSettlePrice, $fbScript['event_id']);
            }
            ...
            $presentCartIdx = (int)Request::request()->get('cartIdx');
            $this->setData('isPresentCartGoods', $presentCart->isPresentCartGoods($presentCartIdx));

            // 선물하기 수령자 정보 조회
            $presentCartReceiverInfo = $presentCart->getPresentCartReceiverInfo($presentCartIdx);
            $this->setData('presentReceiverName', $presentCartReceiverInfo['receiverName']);
            $this->setData('presentReceiverCellPhone', $presentCartReceiverInfo['cellPhone']);
        } catch (Exception|AlertRedirectException $e) {
