 ...
 */
class GoodsPsController extends \Controller\Mobile\Controller
{
    /**
     * {@inheritdoc}
     *
    ...
    public function index()
    {
        ...
        switch ($postValue['mode']) {
            ...
            case 'get_benefit':
                try {
                    // --- 상품 class
                    if ($postValue['deliveryType'] === 'regular') {
                        $regularOrderCart = \App::getInstance(\Component\Cart\RegularOrderCart::class);
                        $setData = $regularOrderCart->regularGoodsViewBenefit($postValue);
                    } else {
            ...
            case 'save_restock' :
                try {
                    ...
                    //옵션이 있을시
                    if(gd_count($postValue['restock_option']) > 0){
                        //옵션정보가 변경되지 않았는지 체크
                        foreach($postValue['restock_option'] as $key => $value){
                            ...
                            if(gd_count($checkOptionData) < 1 || (int)$checkOptionData['sno'] < 1){
                        ...
                        if(gd_count($duplicationFl) > 0){
                            if(gd_count($duplicationFl) === gd_count($postValue['restock_option'])){
