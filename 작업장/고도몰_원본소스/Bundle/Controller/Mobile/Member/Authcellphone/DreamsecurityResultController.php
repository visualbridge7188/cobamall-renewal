...
namespace Bundle\Controller\Mobile\Member\Authcellphone;
...
use Framework\Utility\MemberUtils;
 ...
 * @author  yjwee
 */
class DreamsecurityResultController extends \Controller\Mobile\Controller
{
    use YearTrait;
    ...
    public function index()
    {
        ...
        try {
            ...
            // 마이앱 관련 처리
            if ($request->isMyapp() && strpos($ssReturnUrl, 'returnUrl=')) {
                $ssReturnUrl = explode('returnUrl=', $ssReturnUrl);
                $ssReturnUrl = $ssReturnUrl[1];
            }
            ...
            if ($strResult == 'success' && ($ssCallType == 'joinmember' || $ssCallType == 'joinmembermobile')) {
                ...
                if ($strDupInfo) {
                    ...
                    if (MemberUtil::isExistsDupeInfo($strDupInfo, $strPhoneNum, $strUserName) == true) {
                        throw new AlertRedirectCloseException('이미 가입이 되어 있습니다.', null, null, '/member/join_agreement.php', 'parent');
                    }
                }
            ...
            // 마이앱 성인인증 처리
            if ($request->isMyapp() === true && $session->get('certAdult.adultFl') == 'y') {
        ...
        //아이폰+페이스북 앱으로 본인인증 시도하는 경우
        $userAgentInfo = $request->getUserAgent();
        if((strpos($userAgentInfo,"iPhone") !== false &&(strpos($userAgentInfo,"FBAN")!== false || Strpos($userAgentInfo,"FBAV") !== false || Strpos($userAgentInfo,"Instagram") !== false)) || (strpos($userAgentInfo,"Android") !== false && Strpos($userAgentInfo,"Instagram") !== false)) {
