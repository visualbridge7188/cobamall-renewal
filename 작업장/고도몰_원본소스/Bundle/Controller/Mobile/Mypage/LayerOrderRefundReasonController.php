 ...
 */
class LayerOrderRefundReasonController extends \Controller\Mobile\Controller
{
    /**
     * {@inheritdoc}
     */
    public function index()
    {
        try {
            ...
            // 반품/환불/교환 접수 사유
            $data = $order->getOrderListForClaim($getValue['orderNo'], $getValue['orderGoodsNo']);
            if ($data !== false) {
                if (is_array($data[0])) {
                    foreach ($data as $claimList) {
                        if ($getValue['userHandleSno'] === $claimList['userHandleNo']) {
