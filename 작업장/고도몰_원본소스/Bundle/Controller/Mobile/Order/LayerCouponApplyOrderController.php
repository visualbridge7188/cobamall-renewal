        ...
        try {
            ...
            if(Session::has('member')) {
                ...
                if($post['couponApplyOrderNo']) {
                    ...
                    unset($cartCouponArrData, $convertCartCouponArrData, $convertCartCouponPriceArrData);
                }
                ...
                unset($memberCouponArrData, $convertMemberCouponArrData, $convertMemberCouponPriceArrData);
                ...
                if($couponConfig['productCouponChangeLimitType'] == 'n') { // 상품쿠폰 주문서페이지 변경 제한안함일 때
                    ...
                    $this->setData('cartCouponNoDivisionArr', gd_implode(STR_DIVISION, $goodsCouponSnoString)); // cart 쿠폰 데이터 매칭 배열 string 변환
                                ...
                                if ($value2['memberCouponNo']) {
                                    foreach ($goodsCouponData['goodsMemberCouponNoArr'] as $goodsNo => $goodsCouponArrData) {
                                        foreach ($goodsCouponArrData as $key => $memberCouponNo) {
                                            ...
                                            if (gd_in_array($memberCouponNo, $cartCouponNoArr)) {
