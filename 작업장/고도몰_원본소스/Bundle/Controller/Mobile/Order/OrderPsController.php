...
class OrderPsController extends \Controller\Mobile\Controller
{
    ...
    public function index()
    {
        ...
        $checkMode = array('check_area_delivery', 'set_recalculation', 'shipping_list', 'check_memberOrderGoodsCount', 'getGiftPresntInfo', 'giftApplyOrder');
        ...
        \Logger::channel('order')->info(__METHOD__ . ' $postValue[mode] : ', [$postValue['mode']]);
        
        // 선물하기 주문 여부 확인
        $isPresentOrder = false;
        if (!empty($postValue['cartSno'])) {
            $cartSno = is_array($postValue['cartSno']) ? $postValue['cartSno'][0] : $postValue['cartSno'];
            $presentCart = \App::getInstance(\Component\Present\Cart\PresentCart::class);
            $isPresentOrder = $presentCart->isPresentCartGoods((int)$cartSno);
        }
        ...
        switch ($postValue['mode']) {
            ...
            case 'orderNew':
                try {
                    ...
                    // 선물하기 주문인 경우(isPresentCartGoods로 확인) tmpDeliverTab 매핑 로직을 건너뛰고 직접 전송된 receiver* 필드 사용
                    if (Request::post()->has('tmpDeliverTab') && !$isPresentOrder) {
                        \Logger::channel('order')->info(__METHOD__ . ' tmpDeliverTab has ', [Request::post()->get('tmpDeliverTab')]);
...
//                    }
                    ...
                    // 주문로그 저장
                    \Logger::channel('order')->info(__METHOD__ . ' ORDER NO : ' . $order->orderNo, $postValue);
