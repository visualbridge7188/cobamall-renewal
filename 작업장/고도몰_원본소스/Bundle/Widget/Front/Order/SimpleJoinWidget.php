...
namespace Bundle\Widget\Front\Order;
 ...
 * @package Bundle\Widget\Front\Order
 * @author  mj2
 */
class SimpleJoinWidget extends \Widget\Front\Widget
{
    /**
     * @inheritdoc
     */
    public function index()
    {
        ...
        if($joinEventOrder['useFl'] == 'y' && gd_is_plus_shop(PLUSSHOP_CODE_SIMPLEJOIN) === true) {
            if ($joinEventOrder['bannerImageType'] == 'self') {
                $joinEventOrder['joinEventOrderImage'] = UserFilePath::data('join_event')->www() . '/' . $joinEventOrder['bannerImagePc'];
            }
